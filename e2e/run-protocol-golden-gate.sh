...
artifact_dir="${NEREUS_DELAY__PROTOCOL_GOLDEN_ARTIFACT_DIR:-$(mktemp -d -t nereus-delay-protocol-golden.XXXXXX)}"
gradle_home="${NEREUS_DELAY__PROTOCOL_GOLDEN_GRADLE_USER_HOME:-${artifact_dir}/gradle-user-home}"
kafka_gradle_home="${NEREUS_DELAY__PROTOCOL_GOLDEN_KAFKA_GRADLE_USER_HOME:-${artifact_dir}/kafka-gradle-user-home}"
pulsar_gradle_home="${NEREUS_DELAY__PROTOCOL_GOLDEN_PULSAR_GRADLE_USER_HOME:-${artifact_dir}/pulsar-gradle-user-home}"
...
fail() { echo " protocol golden gate: $*" >&2; exit 1; }
count_xml_matches() {
...
test_patterns=(
  ...
  'com.nereusstream.delay.runtime.ProfileCatalogScheduleResolverTest'
)
...
jq -n \
  --arg schema "nereus-delay-full-gate-input" --arg gate "protocol-golden" \
  ...
  '{
    ...
    coverage:{complete:($status == "PASS_CERTIFIED"),required:$required,observed:(if $status == "PASS_CERTIFIED" then $required else [] end),exclusions:[]},
...
echo " protocol golden artifact: ${artifact}"
