...
# Full-v1 distribution gate.  The Kafka and Pulsar worktrees are treated as
...
artifact_dir="${NEREUS_DELAY_V1_PATCH_ARTIFACT_DIR:-}"
candidate_lock_file="${NEREUS_DELAY_V1_PATCH_CANDIDATE_SOURCE_LOCK:-}"
profile_id="${NEREUS_DELAY_V1_PATCH_PROFILE_ID:-nereus-delay-v1-patch-distribution-full-r1}"
run_cluster="${NEREUS_DELAY_V1_PATCH_RUN_CLUSTER:-0}"
started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

fail() {
  echo "full-v1 patch-distribution: $*" >&2
  exit 1
}
...
[[ -n "${artifact_dir}" ]] || fail "NEREUS_DELAY_V1_PATCH_ARTIFACT_DIR is required"
[[ -n "${candidate_lock_file}" && -s "${candidate_lock_file}" ]] \
  || fail "NEREUS_DELAY_V1_PATCH_CANDIDATE_SOURCE_LOCK must name a non-empty JSON file"
[[ "${run_cluster}" == "0" || "${run_cluster}" == "1" ]] \
  || fail "NEREUS_DELAY_V1_PATCH_RUN_CLUSTER must be 0 or 1"
...
jq -n \
  ...
  --arg gate "patch-distribution" --arg scope "full-v1" \
  ...
  '{
    schema:$schema,status:$status,scope:$scope,complete_v1:($status == "PASS_CERTIFIED"),
...
jq -e --arg status "${status}" '.schema == "nereus-delay-v1-full-gate-input-v1" and .gate == "patch-distribution" and .status == $status' "${artifact}" >/dev/null
echo "full-v1 patch-distribution artifact=${artifact} status=${status} kafka=${kafka_exit} pulsar=${pulsar_exit} cluster=${cluster_exit}"
