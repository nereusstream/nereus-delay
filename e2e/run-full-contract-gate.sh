...
# This runner is the source-locked producer for the full contract gates
...
gate="${1:-${NEREUS_DELAY__FULL_GATE_NAME:-}}"
artifact_dir="${NEREUS_DELAY__FULL_GATE_ARTIFACT_DIR:-}"
candidate_lock_file="${NEREUS_DELAY__FULL_GATE_CANDIDATE_SOURCE_LOCK:-}"
profile_id="${NEREUS_DELAY__FULL_GATE_PROFILE_ID:-nereus-delay-${gate}-full-r1}"
run_real="${NEREUS_DELAY__FULL_GATE_RUN_REAL:-0}"
gradle_home_override="${NEREUS_DELAY__FULL_GATE_GRADLE_USER_HOME:-}"
real_gradle_home_override="${NEREUS_DELAY__FULL_GATE_REAL_GRADLE_USER_HOME:-}"
started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

fail() {
  echo "full ${gate:-gate}: $*" >&2
  exit 1
}
...
[[ -n "${artifact_dir}" ]] || fail "NEREUS_DELAY__FULL_GATE_ARTIFACT_DIR is required"
[[ -n "${candidate_lock_file}" && -s "${candidate_lock_file}" ]] \
  || fail "NEREUS_DELAY__FULL_GATE_CANDIDATE_SOURCE_LOCK must name a non-empty JSON file"
[[ "${run_real}" == "0" || "${run_real}" == "1" ]] \
  || fail "NEREUS_DELAY__FULL_GATE_RUN_REAL must be 0 or 1"
[[ "${profile_id}" =~ ^[A-Za-z0-9][A-Za-z0-9._:-]{1,127}$ ]] \
  || fail "profile id is not canonical: ${profile_id}"
if [[ "${gate}" == "soak" ]]; then
  [[ "${NEREUS_DELAY__FULL_GATE_LONGEST_CONFIGURED_PERIOD_SECONDS:-}" =~ ^[1-9][0-9]*$ ]] \
    || fail "NEREUS_DELAY__FULL_GATE_LONGEST_CONFIGURED_PERIOD_SECONDS must be positive for soak"
fi
...
case "${gate}" in
  ...
  upgrade-downgrade)
    test_names=(
      ...
      com.nereusstream.delay.protocol.ProtocolVersionActivatePayloadTest
...
(
  ...
  NEREUS_DELAY_CAPACITY_PAYLOAD_RECORDS="${NEREUS_DELAY__FULL_GATE_CAPACITY_PAYLOAD_RECORDS:-256}" \
  NEREUS_DELAY_CAPACITY_SLO_SAMPLES="${NEREUS_DELAY__FULL_GATE_CAPACITY_SLO_SAMPLES:-256}" \
...
if [[ "${run_real}" == "1" ]]; then
  case "${gate}" in
    soak)
      ...
      NEREUS_DELAY_CERTIFIED_SOAK_REQUIRED_CYCLES="${NEREUS_DELAY__FULL_GATE_REQUIRED_CYCLES:-2}" \
      NEREUS_DELAY_CERTIFIED_SOAK_CYCLES="${NEREUS_DELAY__FULL_GATE_CYCLES:-2}" \
      NEREUS_DELAY_CERTIFIED_SOAK_REQUIRED_DURATION_SECONDS="${NEREUS_DELAY__FULL_GATE_REQUIRED_DURATION_SECONDS:-600}" \
      NEREUS_DELAY_CERTIFIED_SOAK_LONGEST_CONFIGURED_PERIOD_SECONDS="${NEREUS_DELAY__FULL_GATE_LONGEST_CONFIGURED_PERIOD_SECONDS}" \
      NEREUS_DELAY_CERTIFIED_SOAK_MAX_PROCESS_RSS_KIB="${NEREUS_DELAY__FULL_GATE_MAX_PROCESS_RSS_KIB:-4194304}" \
      NEREUS_DELAY_CERTIFIED_SOAK_MAX_PROCESS_FDS="${NEREUS_DELAY__FULL_GATE_MAX_PROCESS_FDS:-20000}" \
      NEREUS_DELAY_CERTIFIED_SOAK_MAX_ARTIFACT_BYTES="${NEREUS_DELAY__FULL_GATE_MAX_ARTIFACT_BYTES:-1073741824}" \
      NEREUS_DELAY_CERTIFIED_SOAK_RESOURCE_SAMPLE_INTERVAL_SECONDS="${NEREUS_DELAY__FULL_GATE_RESOURCE_SAMPLE_INTERVAL_SECONDS:-5}" \
      NEREUS_DELAY_CERTIFIED_SOAK_MAX_SAMPLE_GAP_SECONDS="${NEREUS_DELAY__FULL_GATE_MAX_SAMPLE_GAP_SECONDS:-30}" \
      NEREUS_DELAY_CERTIFIED_SOAK_BASE_PORT="${NEREUS_DELAY__FULL_GATE_BASE_PORT:-36100}" \
    ...
    operations)
      ...
      NEREUS_DELAY_CERTIFIED_OPERATIONS_SOAK_ARTIFACT="${NEREUS_DELAY__FULL_GATE_SOAK_ARTIFACT:-}" \
      NEREUS_DELAY_KAFKA_CHECKOUT="${kafka_dir}" \
      NEREUS_DELAY_PULSAR_CHECKOUT="${pulsar_dir}" \
      NEREUS_DELAY_OXIA_CHECKOUT="${oxia_dir}" \
...
echo "full ${gate} artifact=${artifact} status=${status} tests=${test_exit_code} real=${real_status}"
