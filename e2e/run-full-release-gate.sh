...
# Full  release gate.  Bounded RC1 receipts are intentionally not accepted:
# every input must use the full contract and the frozen candidate source
# lock.  A later Delay documentation overlay is allowed only on the six
# evidence-ledger paths enforced by verify-evidence-manifest.sh.
...
artifact_dir="${NEREUS_DELAY_RELEASE_GATE_ARTIFACT_DIR:-$(mktemp -d -t nereus-delay-release-gate.XXXXXX)}"
...
if ! command -v jq >/dev/null 2>&1; then
  echo " full release gate requires jq" >&2
  exit 1
fi
if ! command -v git >/dev/null 2>&1; then
  echo " full release gate requires git" >&2
  exit 1
fi
...
[[ -n "${candidate_lock_file}" ]] \
  || { echo "NEREUS_DELAY_RELEASE_GATE_CANDIDATE_SOURCE_LOCK is required for full " >&2; exit 1; }
...
check_checkout delay "${delay_dir}" nereus/delay-full-implementation "${candidate_delay}"
check_checkout kafka "${kafka_dir}" nereus/delay-guarded-producer "${candidate_kafka}"
check_checkout pulsar "${pulsar_dir}" nereus/delay-resource-guard "${candidate_pulsar}"
...
check_full_gate_artifact() {
  local gate="$1" path="$2"
  if [[ -z "${path}" ]]; then
    add_check "gate-${gate}" BLOCKED "no full artifact supplied; PASS_CERTIFIED is required"; return
  fi
  ...
  filter='(.schema == "nereus-delay-full-gate-input")
    and (.status == "PASS_CERTIFIED") and (.scope == "full")
    and (.complete == true) and (.gate == $gate)
    and (.execution == "strict-sequential") and (.source_locks == $locks)
    and (.coverage | type == "object") and (.coverage.complete == true)
  ...
  case "${gate}" in
    ...
    benchmark) filter="${filter}
      ...
      and (.observations.full_matrix_status == \"PASS\")";;
  ...
  if jq -e --arg gate "${gate}" --argjson locks "${candidate_locks_json}" \
    ...
    add_check "gate-${gate}" PASS "full PASS_CERTIFIED artifact is exact-source and complete" "${path}"
  else
    add_check "gate-${gate}" BLOCKED "artifact is not a complete full PASS_CERTIFIED input for ${gate}" "${path}"
  fi
}
...
release_artifact="${artifact_dir}/release-candidate-gate.json"
...
jq -n --arg schema nereus-delay-release-gate --arg scope full \
  ...
  '{schema:$schema,scope:$scope,release_status:$status,artifact_dir:$artifact,
    ...
    checks:$checks[0],
    boundaries:["Only the ten main-design full gates can produce PASS.",
...
echo " full release gate artifact: ${release_artifact}"
...
if [[ "${release_status}" != PASS && "${allow_not_ready}" != 1 ]]; then
  echo " full release gate is NOT_READY; set NEREUS_DELAY_RELEASE_GATE_ALLOW_NOT_READY=1 only to retain the audit artifact" >&2
  exit 1
fi
