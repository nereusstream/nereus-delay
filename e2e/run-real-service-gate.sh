...
# Full real-service gate. Every child is a real, source-locked run. The
...
candidate_lock_file="${NEREUS_DELAY__CANDIDATE_SOURCE_LOCK:-${NEREUS_DELAY_RELEASE_GATE_CANDIDATE_SOURCE_LOCK:-}}"
artifact_dir="${NEREUS_DELAY__REAL_SERVICE_ARTIFACT_DIR:-$(mktemp -d -t nereus-delay-real-service.XXXXXX)}"
gradle_home="${NEREUS_DELAY__REAL_SERVICE_GRADLE_USER_HOME:-${artifact_dir}/gradle-user-home}"
...
kafka_large_gradle_home="${NEREUS_DELAY__REAL_SERVICE_KAFKA_LARGE_GRADLE_USER_HOME:-${gradle_home}/kafka-large}"
pulsar_large_gradle_home="${NEREUS_DELAY__REAL_SERVICE_PULSAR_LARGE_GRADLE_USER_HOME:-${gradle_home}/pulsar-large}"
cross_gradle_home="${NEREUS_DELAY__REAL_SERVICE_CROSS_GRADLE_USER_HOME:-${gradle_home}/cross}"
kafka_client_gradle_home="${NEREUS_DELAY__REAL_SERVICE_KAFKA_CLIENT_GRADLE_USER_HOME:-${gradle_home}/kafka-client}"
pulsar_client_gradle_home="${NEREUS_DELAY__REAL_SERVICE_PULSAR_CLIENT_GRADLE_HOME:-${gradle_home}/pulsar-client}"
activation_gradle_home="${NEREUS_DELAY__REAL_SERVICE_ACTIVATION_GRADLE_HOME:-${gradle_home}/activation}"
...
kafka_destination_topic="nereus-delay-kafka-destination-$(date +%s)-$$"

fail() {
  echo " real-service gate: $*" >&2
  exit 1
}
...
[[ -n "${candidate_lock_file}" && -s "${candidate_lock_file}" ]] \
  || fail "NEREUS_DELAY__CANDIDATE_SOURCE_LOCK is required"
...
[[ "$(git -C "${delay_dir}" branch --show-current)" == "nereus/delay-full-implementation" ]] || source_status="FAIL"
[[ "$(git -C "${kafka_dir}" branch --show-current)" == "nereus/delay-guarded-producer" ]] || source_status="FAIL"
[[ "$(git -C "${pulsar_dir}" branch --show-current)" == "nereus/delay-resource-guard" ]] || source_status="FAIL"
...
jq -n \
  ...
  ' {
      schema:$schema,status:$status,scope:"full",complete:($status == "PASS_CERTIFIED"),
...
echo " real-service artifact: ${artifact_dir}/real-service.json"
