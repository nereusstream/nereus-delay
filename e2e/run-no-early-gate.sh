...
artifact_dir="${NEREUS_DELAY__NO_EARLY_ARTIFACT_DIR:-$(mktemp -d -t nereus-delay-no-early.XXXXXX)}"
gradle_home="${NEREUS_DELAY__NO_EARLY_GRADLE_USER_HOME:-${artifact_dir}/gradle-user-home}"
artifact="${artifact_dir}/no-early.json"
log_file="${artifact_dir}/no-early-gradle.log"

fail() { echo " no-early gate: $*" >&2; exit 1; }
count_xml_matches() {
...
source_audit_files=(
  ...
  src/main/java/com/nereusstream/delay/semantic/NativePreparationEligibility.java
  src/main/java/com/nereusstream/delay/protocol/DestinationProfileSemantic.java
...
rg -Fq 'candidate.brokerDeliverAtEpochMs() < intent.deliverAtEpochMs()' \
  "${delay_dir}/src/main/java/com/nereusstream/delay/semantic/NativePreparationEligibility.java" || source_audit_status=FAIL
rg -Fq 'targetClockAheadBoundMs' "${delay_dir}/src/main/java/com/nereusstream/delay/semantic/NativePreparationEligibility.java" || source_audit_status=FAIL
...
echo " no-early artifact: ${artifact}"
