...
def verify_managed_handoff_evidence(path: Path, policy_snapshot_digest: str, p1_source_lock: str) -> dict[str, Any]:
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(read_bytes(manifest_path, "DataResetManifest"), "DataResetManifest is empty")
    ...
    require(len({row.get("identity") for row in resources}) == 13, "manifest resource set is not unique")
    ...
    g0_path = within_run(str(gate_value.get("g0SnapshotPath", "")), run_dir, "G0 snapshot")
    ...
    observations_path = run_dir / "g0/resource-observations.json"
    ...
    managed_path = within_run(str(canary_value.get("evidence", {}).get("managedHandoff", "")), run_dir,
                              "Managed Handoff evidence")
    verify_managed_handoff_evidence(
        managed_path, str(policies["enabled"].payload.get("policySnapshotDigest")), args.expected_p1_source_lock
    )
