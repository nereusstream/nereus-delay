...
# Produces the physical §23.4 matrix consumed by validate-capacity-matrix.sh.
...
artifact_dir="${NEREUS_DELAY__CAPACITY_MATRIX_ARTIFACT_DIR:-}"
candidate_lock_file="${NEREUS_DELAY__CAPACITY_MATRIX_CANDIDATE_SOURCE_LOCK:-}"
gradle_home="${NEREUS_DELAY__CAPACITY_MATRIX_GRADLE_USER_HOME:-${artifact_dir}/gradle-user-home}"
profile_id="${NEREUS_DELAY__CAPACITY_MATRIX_PROFILE_ID:-nereus-delay-physical-capacity-r1}"
fast_mode="${NEREUS_DELAY__CAPACITY_MATRIX_FAST:-0}"
...
[[ -n "${artifact_dir}" ]] || fail "NEREUS_DELAY__CAPACITY_MATRIX_ARTIFACT_DIR is required"
...
oxia_image="${NEREUS_DELAY__CAPACITY_OXIA_IMAGE:-nereus/oxia-o1:${candidate_oxia:0:12}}"
...
require_checkout Delay "${delay_dir}" nereus/delay-full-implementation "${candidate_delay}"
require_checkout Kafka "${kafka_dir}" nereus/delay-guarded-producer "${candidate_kafka}"
require_checkout Pulsar "${pulsar_dir}" nereus/delay-resource-guard "${candidate_pulsar}"
...
kafka_project="nereus-delay-capacity-kafka-$(date +%s)-$$"
pulsar_project="nereus-delay-capacity-pulsar-$(date +%s)-$$"
kafka_image="nereus-delay-capacity-k1:${kafka_project}"
pulsar_image="nereus-delay-capacity-p1:${pulsar_project}"
kafka_context="$(mktemp -d -t nereus-delay-capacity-k1.XXXXXX)"
pulsar_context="$(mktemp -d -t nereus-delay-capacity-p1.XXXXXX)"
pulsar_runtime="$(mktemp -d -t nereus-delay-capacity-runtime.XXXXXX)"
...
upload_object_evidence() {
  ...
  jq -n --arg id "${id}" --arg key "${object_key}" --arg file "${payload_file}" \
    ...
    '{schema:"nereus-delay-capacity-object-store-observation",status:(if $put == 0 and $head == 0 then "PASS" else "FAIL" end),id:$id,object_key:$key,payload_file:$file,payload_bytes:$bytes,payload_sha256:$sha,put_exit_code:$put,head_exit_code:$head}' \
    >"${receipt}"
...
run_kafka_case() {
  ...
  local topic="nereus-delay-${profile_id}-${id//_/-}"
...
run_pulsar_case() {
  ...
  local topic_base="nereus-delay-${profile_id}-${id//_/-}"
...
export KAFKA_BROKER_1_PORT="${kafka_broker_1_port}" KAFKA_BROKER_2_PORT="${kafka_broker_2_port}" \
  ...
  NEREUS_DELAY__CAPACITY_OXIA_IMAGE="${oxia_image}"
...
jq -n --arg kc "${remaining_kafka_containers}" --arg pc "${remaining_pulsar_containers}" \
  ...
  '{schema:"nereus-delay-capacity-docker-cleanup",status:(if $kc == "" and $pc == "" and $kn == "" and $pn == "" and $kr != 0 and $pr != 0 then "PASS" else "FAIL" end),kafka_project:$ki,pulsar_project:$pi,remaining_kafka_containers:$kc,remaining_pulsar_containers:$pc,remaining_kafka_networks:$kn,remaining_pulsar_networks:$pn,kafka_image_inspect_exit:$kr,pulsar_image_inspect_exit:$pr}' \
  >"${post_cleanup}"
...
jq -n --arg schema "nereus-delay-capacity-envelope-config" --arg profile "${profile_id}" \
...
jq -n --arg schema "nereus-delay-capacity-matrix" --arg status "${matrix_status}" \
