...
gate="${1:-${NEREUS_DELAY__CAPACITY_MEASUREMENT_GATE:-capacity}}"
artifact_dir="${NEREUS_DELAY__CAPACITY_MEASUREMENT_ARTIFACT_DIR:-}"
candidate_lock_file="${NEREUS_DELAY__CAPACITY_MEASUREMENT_CANDIDATE_SOURCE_LOCK:-}"
profile_id="${NEREUS_DELAY__CAPACITY_MEASUREMENT_PROFILE_ID:-nereus-delay-${gate}-physical-envelope-r1}"
run_real="${NEREUS_DELAY__CAPACITY_MEASUREMENT_RUN_REAL:-1}"
payload_bytes="${NEREUS_DELAY__CAPACITY_PAYLOAD_BYTES:-1052672}"
matrix_image="${NEREUS_DELAY__CAPACITY_MEASUREMENT_IMAGE:-eclipse-temurin@sha256:57865c22b954cf920cb05a610af81d577e89783282514ba071e99c7357f6c769}"
seeded_gradle_home="${NEREUS_DELAY__CAPACITY_MEASUREMENT_GRADLE_USER_HOME:-}"
full_matrix_input="${NEREUS_DELAY__CAPACITY_MEASUREMENT_FULL_MATRIX_ARTIFACT:-}"
full_matrix_runner="${NEREUS_DELAY__CAPACITY_MEASUREMENT_FULL_MATRIX_RUNNER:-${script_dir}/run-physical-capacity-matrix.sh}"
run_full_matrix="${NEREUS_DELAY__CAPACITY_MEASUREMENT_RUN_FULL_MATRIX:-0}"
...
[[ -n "${artifact_dir}" ]] || fail "NEREUS_DELAY__CAPACITY_MEASUREMENT_ARTIFACT_DIR is required"
[[ -n "${candidate_lock_file}" && -s "${candidate_lock_file}" ]] \
  || fail "NEREUS_DELAY__CAPACITY_MEASUREMENT_CANDIDATE_SOURCE_LOCK must name a JSON file"
...
if [[ -z "${full_matrix_input}" && "${run_full_matrix}" == "1" ]]; then
  ...
  set +e
  NEREUS_DELAY__CAPACITY_MATRIX_ARTIFACT_DIR="${full_matrix_dir}" \
  NEREUS_DELAY__CAPACITY_MATRIX_CANDIDATE_SOURCE_LOCK="${candidate_lock_file}" \
  NEREUS_DELAY__CAPACITY_MATRIX_PROFILE_ID="${profile_id}-broker-matrix" \
  NEREUS_DELAY__CAPACITY_MATRIX_GRADLE_USER_HOME="${seeded_gradle_home:-${artifact_dir}/matrix-gradle-user-home}" \
...
full_matrix_json="$(jq -cn \
  ...
  '{schema:"nereus-delay-capacity-matrix",status:"MISSING",source_locks:$locks,
...
if [[ -n "${full_matrix_input}" ]]; then
  set +e
  bash "${script_dir}/validate-capacity-matrix.sh" \
  ...
  echo "NEREUS_DELAY__CAPACITY_MEASUREMENT_FULL_MATRIX_ARTIFACT is required" \
...
NEREUS_DELAY_CAPACITY_MATRIX_PROJECT="nereus-delay-${gate}-measurement-$(date +%s)-$$" \
NEREUS_DELAY_CAPACITY_MATRIX_PULL_IMAGE="${NEREUS_DELAY__CAPACITY_MEASUREMENT_PULL_IMAGE:-0}" \
...
if [[ "${run_real}" == "1" ]]; then
  ...
  (
    ...
    NEREUS_DELAY_KAFKA_LARGE_PAYLOAD_DESTINATION_TOPIC="nereus-delay-${gate}-measurement-kafka-${profile_id}" \
  ...
  (
    ...
    NEREUS_DELAY_PULSAR_LARGE_PAYLOAD_DESTINATION_TOPIC="persistent://public/default/nereus-delay-${gate}-measurement-pulsar-${profile_id}" \
...
jq -n \
  --arg schema "nereus-delay-capacity-observation" \
  ...
  '{
    ...
    boundaries:[
      ...
      "The full  capacity matrix is independently supplied and hash-validated; without it this artifact remains FAIL even when the bounded probe and real E2E pass.",
...
jq -e --arg status "${overall_status}" --argjson required "${required_json}" \
  '.schema == "nereus-delay-capacity-observation" and .status == $status
   ...
   and (.campaign.full_matrix.status == "MISSING" or .campaign.full_matrix.status == "FAIL" or .campaign.full_matrix.status == "PASS")' "${measurement_artifact}" >/dev/null
