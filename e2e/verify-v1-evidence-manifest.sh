...
[[ "$(jq -r '.schema // empty' "${manifest}")" == "nereus-delay-v1-evidence-manifest-v1" ]] \
...
assert_clean_checkout delay "${delay_dir}" nereus/delay-full-implementation-v1
assert_clean_checkout kafka "${kafka_dir}" nereus/delay-guarded-producer-v1
assert_clean_checkout pulsar "${pulsar_dir}" nereus/delay-resource-guard-v1
