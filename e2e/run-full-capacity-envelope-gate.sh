...
# Full benchmark/capacity producer.  The Delay tests and the optional real
...
gate="${1:-${NEREUS_DELAY__CAPACITY_GATE_NAME:-}}"
artifact_dir="${NEREUS_DELAY__CAPACITY_ARTIFACT_DIR:-}"
candidate_lock_file="${NEREUS_DELAY__CAPACITY_CANDIDATE_SOURCE_LOCK:-}"
measurement_artifact="${NEREUS_DELAY__CAPACITY_MEASUREMENT_ARTIFACT:-}"
measurement_runner="${NEREUS_DELAY__CAPACITY_MEASUREMENT_RUNNER:-${script_dir}/run-physical-capacity-observation.sh}"
run_measurement="${NEREUS_DELAY__CAPACITY_RUN_MEASUREMENT:-0}"
run_full_matrix="${NEREUS_DELAY__CAPACITY_RUN_FULL_MATRIX:-1}"
profile_id="${NEREUS_DELAY__CAPACITY_PROFILE_ID:-nereus-delay-${gate}-physical-envelope-r1}"
run_real="${NEREUS_DELAY__CAPACITY_RUN_REAL:-0}"
measurement_gradle_home="${NEREUS_DELAY__CAPACITY_MEASUREMENT_GRADLE_USER_HOME:-}"
started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

fail() {
  echo "full ${gate:-capacity}: $*" >&2
  exit 1
}
...
[[ -n "${artifact_dir}" ]] || fail "NEREUS_DELAY__CAPACITY_ARTIFACT_DIR is required"
[[ -n "${candidate_lock_file}" && -s "${candidate_lock_file}" ]] \
  || fail "NEREUS_DELAY__CAPACITY_CANDIDATE_SOURCE_LOCK must name a non-empty JSON file"
[[ "${run_real}" == "0" || "${run_real}" == "1" ]] \
  || fail "NEREUS_DELAY__CAPACITY_RUN_REAL must be 0 or 1"
[[ "${run_measurement}" == "0" || "${run_measurement}" == "1" ]] \
  || fail "NEREUS_DELAY__CAPACITY_RUN_MEASUREMENT must be 0 or 1"
[[ "${run_full_matrix}" == "0" || "${run_full_matrix}" == "1" ]] \
  || fail "NEREUS_DELAY__CAPACITY_RUN_FULL_MATRIX must be 0 or 1"
...
if [[ "${gate}" == "benchmark" ]]; then
  ...
  test_names=(
    com.nereusstream.delay.protocol.CapacityVectorTest
    com.nereusstream.delay.protocol.ShardCapacityEnvelopeTest
    com.nereusstream.delay.protocol.SloObjectiveTest
    com.nereusstream.delay.protocol.SloObservationOutboxTest
...
if [[ "${run_measurement}" == "1" ]]; then
  ...
  [[ -z "${measurement_artifact}" ]] \
    || fail "do not combine NEREUS_DELAY__CAPACITY_RUN_MEASUREMENT=1 with a supplied measurement artifact"
  ...
  set +e
  NEREUS_DELAY__CAPACITY_MEASUREMENT_ARTIFACT_DIR="${measurement_run_dir}" \
  NEREUS_DELAY__CAPACITY_MEASUREMENT_CANDIDATE_SOURCE_LOCK="${candidate_lock_file}" \
  NEREUS_DELAY__CAPACITY_MEASUREMENT_PROFILE_ID="${profile_id}" \
  NEREUS_DELAY__CAPACITY_MEASUREMENT_GATE="${gate}" \
  NEREUS_DELAY__CAPACITY_MEASUREMENT_RUN_REAL="${run_real}" \
  NEREUS_DELAY__CAPACITY_MEASUREMENT_RUN_FULL_MATRIX="${run_full_matrix}" \
  NEREUS_DELAY__CAPACITY_MEASUREMENT_GRADLE_USER_HOME="${measurement_gradle_home}" \
...
gradle_home="${NEREUS_DELAY__CAPACITY_GRADLE_USER_HOME:-${artifact_dir}/gradle-user-home}"
real_gradle_home="${NEREUS_DELAY__CAPACITY_REAL_GRADLE_USER_HOME:-${gradle_home}}"
mkdir -p "${gradle_home}"
...
delay_source="$(require_checkout Delay "${delay_dir}" nereus/delay-full-implementation "${candidate_delay}")"
...
(
  ...
  NEREUS_DELAY_CAPACITY_PAYLOAD_RECORDS="${NEREUS_DELAY__CAPACITY_PAYLOAD_RECORDS:-256}" \
  NEREUS_DELAY_CAPACITY_SLO_SAMPLES="${NEREUS_DELAY__CAPACITY_SLO_SAMPLES:-256}" \
...
if [[ "${run_real}" == "1" ]]; then
  ...
  (
    ...
    NEREUS_DELAY_KAFKA_LARGE_PAYLOAD_DESTINATION_TOPIC="nereus-delay-capacity-kafka-destination-${profile_id}" \
  ...
  (
    ...
    NEREUS_DELAY_PULSAR_LARGE_PAYLOAD_DESTINATION_TOPIC="nereus-delay-capacity-pulsar-destination-${profile_id}" \
...
if [[ -n "${measurement_artifact}" && -s "${measurement_artifact}" ]] \
  ...
  if jq -e --argjson required "${required_json}" \
      ...
      '.schema == "nereus-delay-capacity-observation"
       and .status == "PASS"
       ...
       and (.campaign.full_matrix.status == "PASS")
       and ((.campaign.full_matrix.record_cardinalities | sort) == [1000000,10000000,100000000])
       and ((.campaign.full_matrix.arrival_patterns | sort | unique) == ["burst","uniform","zipf"])
       and ((.campaign.full_matrix.ordering_modes | sort | unique) == ["ordered","unordered"])
       and ((.campaign.full_matrix.consistency_modes | sort | unique) == ["baseline","strong"])
       and ((.campaign.full_matrix.target_health | sort | unique) == ["bad","healthy"])
       and ((.campaign.full_matrix.placement_modes | sort | unique) == ["multi-shard","single-shard"])
       and ((.campaign.full_matrix.payload_modes | sort | unique) == ["inline","object"])
       and (.campaign.full_matrix.observations | type == "array" and length >= 8)
       and all(.campaign.full_matrix.observations[];
         .status == "PASS"
...
observations="$(jq -n \
  ...
  '{required_configurations_status:$required,throughput_status:$throughput,slo_status:$slo,
    ...
    full_matrix_status:$full_matrix}')"

artifact="${artifact_dir}/full-gate-input.json"
jq -n \
  --arg schema "nereus-delay-full-gate-input" \
  --arg status "${status}" --arg scope "full" --arg profile_id "${profile_id}" \
  ...
  '{schema:$schema,status:$status,scope:$scope,profile_id:$profile_id,gate:$gate,
    complete:($status == "PASS_CERTIFIED"),execution:$execution,
    started_at:$started_at,finished_at:$finished_at,
    source_locks:{delay:$delay,kafka:$kafka,pulsar:$pulsar,oxia:$oxia},
    coverage:{complete:($status == "PASS_CERTIFIED"),required:$required,observed:$required,exclusions:[]},
...
jq -e --arg expected_gate "${gate}" --arg expected_status "${status}" \
  '.schema == "nereus-delay-full-gate-input" and .gate == $expected_gate and .status == $expected_status' \
  "${artifact}" >/dev/null

echo "full ${gate} artifact=${artifact} status=${status} tests=${test_exit_code} real=${real_status} measurement=${measurement_status}"
