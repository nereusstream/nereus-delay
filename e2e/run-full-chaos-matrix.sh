...
# Full  chaos matrix.  The older certified wrapper is a useful source of
...
artifact_dir="${NEREUS_DELAY_FULL_CHAOS_ARTIFACT_DIR:-$(mktemp -d -t nereus-delay-full-chaos.XXXXXX)}"
...
profile_id="${NEREUS_DELAY_FULL_CHAOS_PROFILE_ID:-nereus-delay-full-chaos-r1}"
...
source_status="PASS"
require_checkout "${delay_dir}" "nereus/delay-full-implementation" "${candidate_delay}" \
  || source_status="BLOCKED"
require_checkout "${kafka_dir}" "nereus/delay-guarded-producer" "${candidate_kafka}" \
  || source_status="BLOCKED"
require_checkout "${pulsar_dir}" "nereus/delay-resource-guard" "${candidate_pulsar}" \
  || source_status="BLOCKED"
...
if [[ "${leader_exit}" == "0" && -s "${leader_before}" && -s "${leader_after}" ]] \
    && jq -n --slurpfile before "${leader_before}" --slurpfile after "${leader_after}" \
      '($before | length) == 1 and ($after | length) == 1
       and $before[0].schema == "nereus-delay-chaos-durable-state-dump"
       and $after[0].schema == "nereus-delay-chaos-durable-state-dump"
...
if [[ "${half_open_exit}" == "0" && -s "${half_open_transport}" \
    ...
    && jq -e --arg delay "${candidate_delay}" \
      '.schema == "nereus-delay-half-open-transport-e2e"
       and .status == "PASS"
    ...
    && jq -n --slurpfile before "${half_open_before}" --slurpfile after "${half_open_after}" \
      ...
      '($before | length) == 1 and ($after | length) == 1 and ($worker | length) == 1
       and $before[0].schema == "nereus-delay-chaos-durable-state-dump"
       and $after[0].schema == "nereus-delay-chaos-durable-state-dump"
       and $worker[0].schema == "nereus-delay-chaos-durable-state-dump"
...
if [[ "${target_isolation_before_exit}" == "0" && "${target_isolation_after_exit}" == "0" \
    ...
    && jq -n --slurpfile before "${target_isolation_dir}/before.json" \
      ...
      '($before | length) == 1 and ($after | length) == 1
       and $before[0].schema == "nereus-delay-target-isolation-durable-state-dump"
       and $after[0].schema == "nereus-delay-target-isolation-durable-state-dump"
    ...
    case "${name}" in
      fsync-error)
        if jq -n --slurpfile before "${before_dump}" --slurpfile after "${after_dump}" \
          '($before | length) == 1 and ($after | length) == 1
           and $before[0].schema == "nereus-delay-storage-chaos-durable-state-dump"
           and $after[0].schema == "nereus-delay-storage-chaos-durable-state-dump"
        ...
        if [[ -s "${kill_receipt}" ]] \
          && jq -n --slurpfile before "${before_dump}" --slurpfile after "${after_dump}" \
            ...
            '($before | length) == 1 and ($after | length) == 1 and ($kill | length) == 1
             and $before[0].schema == "nereus-delay-storage-chaos-durable-state-dump"
             and $after[0].schema == "nereus-delay-storage-chaos-durable-state-dump"
             and $kill[0].schema == "nereus-delay-storage-chaos-kill-receipt"
...
jq -n \
  ...
  '{
    ...
    profile_id:$profile_id,scope:"full",complete:($status == "PASS"),
    ...
    coverage:{complete:($status == "PASS"),required:$required,observed:$observed,exclusions:[]},
...
echo " full chaos matrix artifact: ${artifact_dir}/full-chaos-matrix.json"
