...
fresh_process_reaping=${NEREUS_DELAY_CHECKPOINT_REAPING_FRESH_PROCESS:-0}
reaping_state_dir=${NEREUS_DELAY_CHECKPOINT_REAPING_STATE_DUMP_DIR:-}
reaping_prefix=${NEREUS_DELAY_CHECKPOINT_REAPING_PREFIX:-nereus-delay-real-reaping-fresh-process/$(date +%s)-$$}
...
if [[ "${fresh_process_reaping}" != "0" && "${fresh_process_reaping}" != "1" ]]; then
    echo "NEREUS_DELAY_CHECKPOINT_REAPING_FRESH_PROCESS must be 0 or 1" >&2
    exit 1
fi
if [[ "${fresh_process_reaping}" == "1" && -z "${reaping_state_dir}" ]]; then
    echo "NEREUS_DELAY_CHECKPOINT_REAPING_STATE_DUMP_DIR is required for fresh-process checkpoint reaping" >&2
    exit 1
fi
...
if [[ "${fresh_process_reaping}" == "1" && "${minio_fault_mode}" != "NONE" ]]; then
    echo "fresh-process checkpoint reaping evidence requires NEREUS_DELAY_CHECKPOINT_MINIO_FAULT_MODE=NONE" >&2
    exit 1
fi
...
run_fresh_process_reaping() {
    local phase="$1"
    env "${smoke_environment[@]}" \
        "NEREUS_DELAY_CHECKPOINT_REAPING_PHASE=${phase}" \
...
if [[ "${minio_fault_mode}" == "NONE" ]]; then
    ...
    if [[ "${fresh_process_reaping}" == "1" ]]; then
        run_fresh_process_reaping WRITE
        run_fresh_process_reaping READ
