...
# caller cannot turn a small local Store run into a  capacity envelope by
...
jq -e --argjson locks "${locks_json}" \
  '.schema == "nereus-delay-capacity-matrix"
   and .status == "PASS"
...
echo " capacity matrix validated: ${artifact}"
