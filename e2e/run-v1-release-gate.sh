  ...
  if [[ "${status}" == "PASS_CERTIFIED" ]]; then
    ...
    if [[ "${name}" == "certified-soak" ]]; then
      artifact_check='(.schema == "nereus-delay-certified-production-chain-soak-v1")
        ...
        and (.policy.max_sample_gap_seconds | type == "number" and . >= 1)
        and (.observations | type == "object")
        and (.observations.child_exit_code == 0)
        and (.observations.monitor_exit_code == 0)
        and (.child_elapsed_seconds | type == "number" and . >= 1)
        ...
        and (.observations.resource_coverage_status == "PASS")
        and (.observations.resource_first_epoch | type == "number")
        and (.observations.resource_last_epoch | type == "number")
        and (.observations.resource_last_epoch >= .observations.resource_first_epoch)
        and (.observations.resource_max_gap_seconds | type == "number" and . >= 0)
        and (.observations.resource_max_gap_seconds <= .policy.max_sample_gap_seconds)
      ...
      artifact_check='(.schema == "nereus-delay-certified-protocol-activation-cutover-v1")
        and (.status == "PASS_CERTIFIED")
        and (.profile_id == $release_activation_profile_id)
        and (.execution == "strict-sequential")
        and (.source_checks_pass == true)
        and (.local_projection.status == "PASS")
        and (.local_projection.exit_code == 0)
        and (.external_authority.status == "PASS")
        and (.external_authority.exit_code == 0)
        and (.external_authority.cleanup_status == "PASS")
        and (.external_authority.capability_before_marker == "PASS")
