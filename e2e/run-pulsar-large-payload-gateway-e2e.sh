...
minio_request_timeout_ms="${NEREUS_DELAY_PULSAR_LARGE_PAYLOAD_MINIO_REQUEST_TIMEOUT_MS:-60000}"
...
if [[ ! "${minio_request_timeout_ms}" =~ ^[1-9][0-9]*$ ]]; then
  echo "NEREUS_DELAY_PULSAR_LARGE_PAYLOAD_MINIO_REQUEST_TIMEOUT_MS must be a positive integer" >&2
  exit 1
fi
