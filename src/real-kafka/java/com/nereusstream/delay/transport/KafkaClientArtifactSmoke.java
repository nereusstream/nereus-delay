...
public final class KafkaClientArtifactSmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        if (arguments.length < 2 || arguments.length > 3 || arguments.length == 3 && !"preserve".equals(arguments[2])) {
        ...
        try (Admin admin = Admin.create(adminConfiguration)) {
            ...
            if (preserveTopic) {
                try (KafkaProducer<byte[], byte[]> producer = newProducer(producerConfiguration)) {
                    ...
                    requirePersisted(
                            send(transport, clusterId, topic, originalTopicId, 0, 1), "original topic incarnation");
            ...
            try (KafkaProducer<byte[], byte[]> producer = newProducer(producerConfiguration)) {
                ...
                requirePersisted(
                        send(transport, clusterId, topic, replacementTopicId, 0, 3), "replacement topic incarnation");
            }
    ...
    private static void printSuccess(
            ...
            final String topic,
            final Uuid originalTopicId,
            final Uuid replacementTopicId,
            final boolean preserveTopic)
            throws Exception {
        final TopicPartitionInfo partitionInfo =
                describe(admin, topic).partitions().get(0);
        System.out.println("K1 Delay binding smoke passed: cluster=" + clusterId
                ...
                + ", leader=" + partitionInfo.leader().id() + ", replicas="
                + partitionInfo.replicas().size()
                + ", preserveTopic=" + preserveTopic);
    }

    private static KafkaProduceResult send(
            ...
            final int partition,
            final int sequence) {
        final java.util.UUID nativeTopicId =
                new java.util.UUID(topicId.getMostSignificantBits(), topicId.getLeastSignificantBits());
        final ShardId shard = new ShardId(RouteIncarnation.random(), partition);
        final KafkaProduceRequest request = new KafkaProduceRequest(
                clusterId,
                topic,
                nativeTopicId,
                partition,
                CommandId.random(shard),
                Bytes.utf8("nereus-delay-k1-smoke-" + sequence));
    ...
    private static void requirePersisted(final KafkaProduceResult result, final String label) {
        if (result.disposition() != KafkaProduceResult.Disposition.PERSISTED) {
            throw new IllegalStateException(
                    label + " was not persisted: " + result.disposition() + "/" + result.stableCode());
        }
    }
    ...
    private static void requireThreeReplicas(final TopicDescription description, final String topic) {
        if (description.partitions().size() != 1
                || description.partitions().get(0).replicas().size() != 3) {
