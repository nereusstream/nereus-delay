...
public final class KafkaClientArtifactSourceConsumerFactory {
     ...
     */
    public static GuardedConsumer<byte[], byte[]> create(final Map<String, Object> configuration,
                                                          final String authenticatedClusterId,
                                                          final String physicalTopic,
                                                          final UUID nativeTopicUuid,
                                                          final int partition) {
        final Map<String, Object> sourceConfiguration = new HashMap<>(
                Objects.requireNonNull(configuration, "configuration"));
        sourceConfiguration.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false);
        final ConsumerResourceGuard guard = new ConsumerResourceGuard(authenticatedClusterId, physicalTopic,
                toKafkaUuid(nativeTopicUuid), partition);
