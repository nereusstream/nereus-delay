...
public final class KafkaClientArtifactProcessCrashRecoverySmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        ...
        try (Admin admin = Admin.create(adminConfiguration)) {
            ...
            try (KafkaClientArtifactSourceRecordConsumer source = source(
                ...
                requireAcked(replayedFirst.acknowledgement().acknowledge(replayedFirst.entry(), null),
                ...
                requireAcked(replayedSecond.acknowledgement().acknowledge(replayedSecond.entry(), null),
    ...
    private static void crashAfterFetch(final String bootstrap, final String groupId, final String clusterId,
                                        final String topic, final Uuid topicId, final PreparedCommand first,
        ...
        if (evidence == null || evidence.requestVersion() < 13
            ...
            throw new IllegalStateException("Kafka process-crash Fetch lacks an LSO covering the fetched records: "
                    + evidence);
        }
        if (fetched.size() < 2 || fetched.get(0).offset() != 0 || fetched.get(1).offset() != 1
    ...
    private static void requireOffset(final SourceRecordConsumer.PolledSourceRecord record, final long expected,
                                      final String label) {
        if (position(record).offset() != expected) {
            throw new IllegalStateException(label + " offset mismatch: expected=" + expected
                    + ", actual=" + position(record).offset());
        }
    }
    ...
    private static void produce(final String bootstrap, final String clusterId, final String topic,
                                final Uuid topicId, final PreparedCommand first, final PreparedCommand second)
            throws Exception {
        final Map<String, Object> configuration = new HashMap<>();
    ...
    private static PreparedCommand command(final ShardId shard, final String identity) {
        ...
        final ScheduleIntentV1 intent = ScheduleIntentV1.create(destination, retryPolicy, FIXED_DELIVER_AT,
                FIXED_DELIVER_AT + 10_000, DeliveryMode.MANAGED, OrderingMode.BEST_EFFORT, new byte[0],
                Bytes.utf8("source-" + identity), null,
                AdapterMetadataV1.kafka(new KafkaMetadataV1(null, List.of())), null, null);
        return PreparedCommand.scheduleV1(shard, uuidV7(identity + "-message"),
                uuidV7(identity + "-command"), intent, FIXED_DELIVER_AT + 20_000);
    }
