...
final class KafkaClientArtifactFetchEvidence {
    ...
    static <K, V> GuardedFetchEvidence requireBatch(final GuardedConsumerRecords<K, V> records,
        ...
        if (evidence == null || evidence.requestVersion() < 13
                ...
                || evidence.firstRecordOffset() < 0 || evidence.lastRecordOffset() < evidence.firstRecordOffset()) {
            throw new IllegalStateException("Kafka guarded Fetch evidence does not match the source guard");
        }
        return evidence;
    }

    static void requireRecord(final ConsumerRecord<?, ?> record, final GuardedFetchEvidence evidence,
                              final ConsumerResourceGuard guard) {
