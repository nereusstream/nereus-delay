...
public final class KafkaClientArtifactWorkerSmoke {
    ...
    private static void runSourceAppliedPhysicalPublish(
        ...
        final AdapterMetadataV1 metadata = AdapterMetadataV1.kafka(new KafkaMetadataV1(null, List.of()));
        ...
        final TrustedUtcIntervalEvidence decisionTime = evidence(decisionNow, decisionNow + 500,
                "kafka-worker-admission-decision");
        ...
        if (append.disposition() != ShardLogMutationAppender.AppendDisposition.PERSISTED
                || !(append.sourcePosition() instanceof KafkaSourcePosition admissionPosition)
                || admissionPosition.compareTo(physicalSchedulePosition) <= 0) {
            throw new IllegalStateException(
                    "Kafka Worker physical Admission did not receive a later guarded source position");
        }
        ...
        publishEvidence.requireBusinessMutation(descriptor.publishAttemptId(), true);
