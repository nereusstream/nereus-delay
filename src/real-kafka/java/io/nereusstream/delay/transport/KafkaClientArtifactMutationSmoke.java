...
public final class KafkaClientArtifactMutationSmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        ...
        try (Admin admin = Admin.create(adminConfiguration)) {
            ...
            try (KafkaProducer<byte[], byte[]> producer = producer(bootstrap);
                 KafkaClientArtifactShardLogMutationAppender appender =
                         new KafkaClientArtifactShardLogMutationAppender((GuardedProducer<byte[], byte[]>) producer,
                                 shard, clusterId, topic, toUuid(topicId), Duration.ofSeconds(15))) {
                ...
                if (outcome.disposition() != io.nereusstream.delay.ownership.ShardLogMutationAppender
                    ...
                    throw new IllegalStateException("Kafka mutation append was not persisted: "
                            + outcome.disposition());
                }
            ...
            final SourceAssignment assignment = new SourceAssignment(shard,
                    Bytes.sha256(Bytes.utf8("kafka-mutation-assignment")), 1,
            ...
            try (KafkaClientArtifactSourceRecordConsumer source = source(
                    bootstrap, group, clusterId, topic, toUuid(topicId), shard)) {
                ...
                if (ack.disposition() != SourceAcknowledgement.Disposition.ACKED) {
                    throw new IllegalStateException("Kafka mutation ACK was not durable: " + ack.disposition(),
                            ack.failure());
                }
            }
    ...
    private static void requireReplayPosition(final KafkaSourcePosition appended,
        ...
        if (!(replayedPosition instanceof KafkaSourcePosition replayed)
                ...
                || (appended.leaderEpoch() != null
                && !appended.leaderEpoch().equals(replayed.leaderEpoch()))) {
            throw new IllegalStateException("Kafka " + phase + " position changed: appended="
                    + appended + ", replayed=" + replayedPosition);
        }
    }
    ...
    private static SystemMutation timeFence(final ShardId shard) throws Exception {
        final TrustedUtcIntervalEvidence evidence = new TrustedUtcIntervalEvidence(2_000, 2_001,
                TrustedUtcIntervalEvidence.Source.CERTIFIED_HOST_CLOCK, Bytes.utf8("kafka-clock"), 1, 2, 3,
                Bytes.sha256(Bytes.utf8("kafka-mutation-evidence")), 0, null);
        ...
        return SystemMutation.signed(shard, SystemMutationType.TIME_FENCE, 9_000, proofId, body,
                AuthorIdentity.fence(Bytes.utf8("kafka-mutation-fence"), keyVersion).canonicalBytes(), keyVersion,
                keyPair.getPrivate());
    }
    ...
    private static KafkaClientArtifactSourceRecordConsumer source(final String bootstrap, final String group,
    ...
    private static GuardedConsumer<byte[], byte[]> recoveryConsumer(final String bootstrap, final String group,
                                                                     final String clusterId, final String topic,
                                                                     final UUID topicId, final ShardId shard) {
        final Map<String, Object> configuration = new HashMap<>();
        configuration.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrap);
