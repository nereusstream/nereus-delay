...
import io.nereusstream.delay.adapter.KafkaTransactionalPublishEvidence;
...
public final class KafkaClientArtifactTransactionalReceiptEvidenceProvider
    ...
    public KafkaClientArtifactTransactionalReceiptEvidenceProvider(final Map<String, Object> consumerConfiguration,
    ...
    @Override
    public Optional<byte[]> resolve(final KafkaTransactionalDestinationRequest request,
        ...
        if (receiptOffset < 0 || receiptMetadata.responseEvidence().logAppendTimeMs() < 0
                || !request.receiptPhysicalTopic().equals(receiptMetadata.recordMetadata().topic())
                || request.receiptResource().receiptPartition() != receiptMetadata.recordMetadata().partition()
                || !request.receiptPhysicalTopic().equals(receiptMetadata.responseEvidence().canonicalTopic())
                || request.receiptResource().receiptPartition() != receiptMetadata.responseEvidence().partition()
                || !request.receiptResource().nativeTopicUuid().equals(toJavaUuid(
                        receiptMetadata.responseEvidence().expectedTopicId()))) {
        ...
        final GuardedConsumer<byte[], byte[]> consumer = KafkaClientArtifactSourceConsumerFactory.create(
                configuration, request.receiptResource().authenticatedClusterId(),
                request.receiptPhysicalTopic(), request.receiptResource().nativeTopicUuid(),
                request.receiptResource().receiptPartition());
        final TopicPartition topicPartition = new TopicPartition(request.receiptPhysicalTopic(),
                request.receiptResource().receiptPartition());
        try {
            ...
            while (System.nanoTime() < deadline) {
                ...
                final GuardedFetchEvidence fetchEvidence = KafkaClientArtifactFetchEvidence.requireBatch(records, guard);
                ...
                for (ConsumerRecord<byte[], byte[]> record : records.records(topicPartition)) {
                    ...
                    final EvidenceCursorV1 cursor = EvidenceCursorV1.kafka(
                            ...
                            request.receiptResource().receiptPartition(), evidenceGeneration,
                            receiptMetadata.responseEvidence().logAppendTimeMs(), successor(receiptOffset),
                            fetchEvidence.lastStableOffset());
                    return Optional.of(KafkaTransactionalPublishEvidence.published(request, cursor, receiptOffset,
                            KafkaTransactionalDestinationRequest.canonicalReceiptRecordHash(record.key(),
                                    record.value())).canonicalBytes());
                }
            }
