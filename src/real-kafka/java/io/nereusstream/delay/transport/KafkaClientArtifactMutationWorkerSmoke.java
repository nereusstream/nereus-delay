...
public final class KafkaClientArtifactMutationWorkerSmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        ...
        try (Admin admin = Admin.create(adminConfiguration)) {
            ...
            final KafkaSourcePositionPair positions = appendMutations(bootstrap, clusterId, topic, nativeTopicId,
                    shard, recoveryMutation.mutation(), activeMutation.mutation());
            ...
            final SourceAssignment sourceAssignment = new SourceAssignment(shard,
                    Bytes.sha256(Bytes.utf8("kafka-mutation-worker-assignment")), 1,
                    new KafkaActivationBarrier(shard, clusterId, nativeTopicId, positions.active().offset()));
            ...
            try {
                final WorkerAssignmentAuthority assignmentAuthority = oxia == null
                        ...
                        : new OxiaSyncWorkerAssignmentBackend(oxia,
                                "nereus-delay/kafka-mutation-worker-placement/" + UUID.randomUUID());
                ...
                final OwnerLease lease = authority.acquire(assignment, "kafka-mutation-worker",
                        oxia == null ? Bytes.sha256(Bytes.utf8("kafka-mutation-worker-session"))
                                : oxia.sessionIdentity(), System.currentTimeMillis(), LEASE_DURATION_MS).orElseThrow();
                ...
                try {
                    ...
                    try (SharedRocksDbResources resources = new SharedRocksDbResources(storeConfig);
                        ...
                        recoverFirstMutation(bootstrap, clusterId, topic, nativeTopicId, shard, assignment,
                                authority, ownedShard, verificationKey, controlSnapshot, workClasses,
                                recoveryMutation.mutation(), positions.recovery());
                        ...
                        try {
                            ...
                            if (result.status()
                                ...
                                throw new IllegalStateException("Kafka mutation Worker source turn did not ACK: "
                            ...
                            if (!samePosition(applied, positions.active())) {
                                throw new IllegalStateException("Kafka mutation Worker Store position is not offset one");
                            }
                            requireCommittedOffset(admin, workerGroup, topic, 0, positions.active().offset() + 1);
                            ...
                            if (drain.pendingCheckpointTask() != null || drain.finalCheckpointPath() == null
                                ...
                                throw new IllegalStateException(
                                        "Kafka mutation Worker drain did not publish the final checkpoint and release the lease");
                            }
                            runtime.close();
                            drained = true;
    ...
    private static KafkaSourcePositionPair appendMutations(final String bootstrap, final String clusterId,
                                                            final String topic, final UUID topicId,
                                                            final ShardId shard, final SystemMutation recovery,
        ...
        try (KafkaProducer<byte[], byte[]> producer = producer(bootstrap);
             KafkaClientArtifactShardLogMutationAppender appender =
                     new KafkaClientArtifactShardLogMutationAppender((GuardedProducer<byte[], byte[]>) producer,
                             shard, clusterId, topic, topicId, Duration.ofSeconds(15))) {
            ...
            return new KafkaSourcePositionPair(requirePersisted(first, "recovery"),
                    requirePersisted(second, "active"));
        }
    }

    private static KafkaSourcePosition requirePersisted(final ShardLogMutationAppender.AppendOutcome outcome,
                                                         final String phase) {
        if (outcome.disposition() != ShardLogMutationAppender.AppendDisposition.PERSISTED
            ...
            throw new IllegalStateException("Kafka " + phase + " mutation append was not persisted: "
                    + outcome.disposition());
        }
        return position;
    }

    private static void recoverFirstMutation(final String bootstrap, final String clusterId, final String topic,
        ...
        try (KafkaClientArtifactRecoverySourceCursor nativeCursor =
            ...
            final io.nereusstream.delay.ownership.SourceReplayOutcome outcome = turn.outcomes().get(0);
            final SystemMutationResult result = outcome.systemMutationResult();
            if (result == null || !java.util.Arrays.equals(result.mutationId(), expectedMutation.systemMutationId())
    ...
    private static io.nereusstream.delay.ownership.SourceApplyCoordinator.TurnResult runUntilApplied(
        ...
        do {
            ...
            if (result.status() != io.nereusstream.delay.ownership.SourceApplyCoordinator.TurnStatus.WAITING_FOR_SOURCE
                ...
                throw new IllegalStateException("Kafka mutation Worker source turn failed: " + result.status(),
                        result.failure());
            }
    ...
    private static void requireSystemMutation(final DelayShard shard, final SystemMutation mutation,
                                              final KafkaSourcePosition expectedPosition, final String phase) {
        final SystemMutationResult result = shard.getSystemMutationResult(mutation.systemMutationId());
        if (result == null || !java.util.Arrays.equals(result.mutationHash(), mutation.mutationHash())
                || result.mutationType() != mutation.type() || result.applyStatus() != ApplyStatus.APPLIED
                || result.stableCode() != StableCode.OK || !samePosition(result.appliedSourcePosition(),
                expectedPosition)) {
            throw new IllegalStateException("Kafka " + phase + " mutation Store result was not APPLIED/OK: "
                    + result);
        }
    }
    ...
    private static MutationFixture timeFence(final ShardId shard, final String identity, final KeyPair keyPair) {
        ...
        final TrustedUtcIntervalEvidence evidence = new TrustedUtcIntervalEvidence(now, now + 1_000,
                ...
                Bytes.utf8("kafka-mutation-worker-clock-" + identity), 1, 2, 3,
                Bytes.sha256(Bytes.utf8("kafka-mutation-worker-evidence-" + identity)), 0, null);
        ...
        return new MutationFixture(SystemMutation.signed(shard, SystemMutationType.TIME_FENCE, retryUntil, proofId,
                body, io.nereusstream.delay.protocol.AuthorIdentity.fence(
                        Bytes.utf8("kafka-mutation-worker-fence"), keyVersion).canonicalBytes(), keyVersion,
                keyPair.getPrivate()));
    }
    ...
    private static OxiaSyncOwnerLeaseBackend.ClientHandle connectOxiaIfConfigured() throws Exception {
        ...
        return OxiaSyncOwnerLeaseBackend.connectUnchecked(endpoint, namespace,
                "nereus-delay-kafka-mutation-worker-" + UUID.randomUUID(), Duration.ofSeconds(15),
    ...
    private static WorkerAssignment publishAssignment(final WorkerAssignmentAuthority authority,
        ...
        final WorkerPlacementPolicy.WorkerCandidate candidate = new WorkerPlacementPolicy.WorkerCandidate(
                "kafka-mutation-worker", capacity(1), CapacityVectorV1.empty(), 0, 16, 0, 16,
        ...
        final WorkerAssignmentCoordinator.PlacementResult result = coordinator.place(sourceAssignment,
                Bytes.sha256(Bytes.utf8("kafka-mutation-worker-capacity-envelope")), 1,
                List.of(candidate), capacity(1), CapacityVectorV1.empty(), CapacityVectorV1.empty(), null, now, 0, 0);
    ...
    private static io.nereusstream.delay.runtime.V1ScheduleResolver scheduleResolver() {
        ...
        return new io.nereusstream.delay.runtime.V1ScheduleResolver() {
            @Override
