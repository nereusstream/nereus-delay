...
public final class KafkaClientArtifactProduceTransport implements PinnedKafkaCommandIngress.KafkaProduceTransport {
    ...
    public CompletionStage<KafkaProduceResult> produce(final KafkaProduceRequest request) {
        ...
        try {
            guard = new ProducerResourceGuard(request.authenticatedClusterId(), request.canonicalPhysicalTopic(),
                    new Uuid(request.nativeTopicUuid().getMostSignificantBits(),
                            request.nativeTopicUuid().getLeastSignificantBits()), request.partition());
            final ProducerRecord<byte[], byte[]> record = new ProducerRecord<>(request.canonicalPhysicalTopic(),
                    request.partition(), null, request.frame());
    ...
    private static KafkaProduceResult success(final KafkaProduceRequest request,
                                              final GuardedRecordMetadata metadata) {
        if (metadata == null || metadata.recordMetadata() == null || metadata.responseEvidence() == null) {
            return KafkaProduceResult.unknown(io.nereusstream.delay.protocol.StableCode.ENQUEUE_RESULT_UNCERTAIN.wireValue(),
                    null);
        }
        ...
        if (evidence.logAppendTimeMs() < 0) {
            return KafkaProduceResult.unknown(
                    io.nereusstream.delay.protocol.StableCode.ENQUEUE_RESULT_UNCERTAIN.wireValue(),
                    encodeResponseEvidence(evidence));
        }
        final Integer leaderEpoch = evidence.responseLeaderEpoch().isPresent()
                ? evidence.responseLeaderEpoch().getAsInt() : null;
        return new KafkaProduceResult(KafkaProduceResult.Disposition.PERSISTED,
                request.authenticatedClusterId(), request.nativeTopicUuid(), request.partition(),
                metadata.recordMetadata().offset(), leaderEpoch, evidence.logAppendTimeMs(),
                io.nereusstream.delay.protocol.StableCode.OK.wireValue(), encodeRequestEvidence(evidence),
                encodeResponseEvidence(evidence));
    }
    ...
    private static KafkaProduceResult failure(final KafkaProduceRequest request, final Exception failure) {
        ...
        final byte[] evidence = guardFailure == null || guardFailure.responseEvidence().isEmpty()
                ? null : encodeResponseEvidence(guardFailure.responseEvidence().get());
        return KafkaProduceResult.unknown(
