...
class DlqExportApplyTest {
    ...
    void preservesUnknownStableCodeWhenApplyingExportOutcome() throws Exception {
        ...
        final MessageRecord dead = new MessageRecord(MessageStatus.DEAD_LETTER, 0, 7, 1_000, 5_000, lane,
                OrderingMode.BEST_EFFORT, Bytes.utf8("payload"), source.canonicalBytes())
                .withRuntimeIndex(GenerationRuntimeIndex.none(GenerationAggregateState.DEAD_LETTER,
                        java.util.List.of(), 0, 0, false, 7));
        final TerminalGenerationRecord terminal = new TerminalGenerationRecord(messageId, 0,
                MessageStatus.DEAD_LETTER, StableCode.CLAIM_PERMANENT_FAILURE, 7, source.canonicalBytes(), false);
        final DlqExportRecord pending = DlqExportRecord.pending(messageId, 0, 7, envelopeHash,
                source.canonicalBytes());
        final KeyPairGenerator generator = KeyPairGenerator.getInstance("Ed25519");
        final KeyPair keyPair = generator.generateKeyPair();
        final AuthorIdentity service = AuthorIdentity.service(Bytes.sha256(Bytes.utf8("dlq-unknown-service")),
                Bytes.sha256(Bytes.utf8("dlq-unknown-run")), 1);
        ...
        try (SharedRocksDbResources resources = new SharedRocksDbResources(config);
            ...
            final SystemMutationResult result = delayShard.applySystemMutation(mutation, appliedPosition,
                    keyPair.getPublic());
            assertEquals(StableCode.DLQ_EXPORT_OUTCOME_UNKNOWN, result.stableCode());
            assertEquals(DlqExportStateV1.UNCERTAIN, delayShard.getDlqExportRecord(messageId, 0).state());
            assertEquals(result, delayShard.applySystemMutation(mutation, appliedPosition,
                    keyPair.getPublic()));
        }
    }
    ...
    private static byte[] unknownResultBody(final ShardId shardId, final DelayMessageId messageId,
                                             final DlqExportRecord pending, final byte[] envelopeHash) {
        final TrustedUtcIntervalEvidence observed = new TrustedUtcIntervalEvidence(2_001, 2_001,
                TrustedUtcIntervalEvidence.Source.CERTIFIED_HOST_CLOCK, Bytes.utf8("unknown-clock"), 1, 1, 1,
                Bytes.sha256(Bytes.utf8("unknown-clock-proof")), 0, null);
        final byte[] subject = CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.bytes(output, 1, shardId.routeIncarnation().bytes());
            CanonicalProtobuf.uint32(output, 2, shardId.partition());
        });
