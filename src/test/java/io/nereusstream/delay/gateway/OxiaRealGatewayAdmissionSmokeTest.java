...
class OxiaRealGatewayAdmissionSmokeTest {
    ...
    void admissionPoolsAndExpiryWorkAgainstRealService() throws Exception {
        ...
        try (OxiaSyncOwnerLeaseBackend.ClientHandle client = OxiaSyncOwnerLeaseBackend.connect(
            ...
            clock.value = 110;
            final GatewayAdmissionLease replacement = reserve(controller, tenant,
                    GatewayIngressOperationV1.SCHEDULE, 100);
            first.close();
            replacement.close();
            ...
            assertEquals(0, GatewayAdmissionRecordV1.decode(records.get(0).value()).leases().size());
        }
    }
