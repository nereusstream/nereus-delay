...
class OxiaRealControlAuthoritySmokeTest {
    ...
    void controlTargetRegistrationCasAndReopenWorkAgainstRealService() throws Exception {
        ...
        try (OxiaSyncOwnerLeaseBackend.ClientHandle client = connect(endpoint, prefix + "/client")) {
            final OxiaSyncControlTargetRegistrationBackend backend =
                    new OxiaSyncControlTargetRegistrationBackend(client.client(), prefix + "/target");
            assertEquals(ControlTargetRegistrationAuthority.RegistrationResult.RECORDED,
                    backend.register(prepared));
    ...
    private static ControlOperationReceiptV1 receipt(final int seed, final long queryUntil) {
        final var registered = new io.nereusstream.delay.protocol.TrustedUtcIntervalEvidence(1_000, 1_100,
                io.nereusstream.delay.protocol.TrustedUtcIntervalEvidence.Source.CERTIFIED_HOST_CLOCK,
                Bytes.utf8("control-clock" + seed), 1, 1, 1, bytes(32, seed + 10), 0, null);
