...
class ControlSystemMutationFactoryV1Test {
    @Test
    void signsAndBindsPreparedLaneMutation() throws Exception {
        ...
        final LaneControlTargetV1 lane = new LaneControlTargetV1(bytes(32, 2), bytes(16, 3), 5);
        ...
        final PreparedControlOperationV1 prepared = PreparedControlOperationV1.prepare(operationId, request.kind(),
                new ControlAuthorV1(bytes(32, 6), bytes(32, 7), bytes(32, 8)), request, List.of(target),
                1, 2, 1, keyPair.getPrivate());

        ControlSystemMutationFactoryV1.sign(prepared, target, shardId, retryUntil, body,
                AuthorIdentity.control(bytes(32, 9), bytes(32, 10), bytes(32, 11)).canonicalBytes(), 1,
                keyPair.getPrivate());
    }
    ...
    void rejectsBodyDriftAfterTargetIdentityWasPrepared() throws Exception {
        ...
        final LaneControlTargetV1 lane = new LaneControlTargetV1(bytes(32, 13), bytes(16, 14), 5);
        ...
        final byte[] mutationHash = SystemMutation.computeMutationHash(shardId,
                SystemMutationType.APPLY_SHARD_CONTROL, 9_001, body);
        final byte[] mutationId = SystemMutation.computeSystemMutationId(shardId,
                SystemMutationType.APPLY_SHARD_CONTROL, controlRef.logicalOperationIdentity(8), mutationHash);
        final ControlTargetRefV1 target = new ControlTargetRefV1(0, ControlTargetKindV1.LANE, lane,
                mutationId, mutationHash);
        final PreparedControlOperationV1 prepared = PreparedControlOperationV1.prepare(operationId, request.kind(),
                new ControlAuthorV1(bytes(32, 17), bytes(32, 18), bytes(32, 19)), request, List.of(target),
                1, 2, 1, keyPair.getPrivate());
        final byte[] changed = body(shardId, controlRef, payload, 9_002, bytes(32, 20));
        assertThrows(IllegalArgumentException.class, () -> ControlSystemMutationFactoryV1.sign(prepared, target,
                shardId, 9_002, changed,
                AuthorIdentity.control(bytes(32, 21), bytes(32, 22), bytes(32, 23)).canonicalBytes(), 1,
                keyPair.getPrivate()));
    }

    private static byte[] body(final ShardId shardId, final ControlRef controlRef, final byte[] payload,
                               final long retryUntil, final byte[] semanticHash) {
