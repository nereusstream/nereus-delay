...
class RsaSha256GatewayJwtVerifierTest {
    ...
    void verifiesSignatureClaimsAudienceArrayAndMtlsConfirmation() throws Exception {
        ...
        final String token = token(
                keyPair,
                header(),
                claims(tenantScope, routingScope, certificateFingerprint(), NOW - 10, NOW - 5, NOW + 300));
    ...
    void rejectsSignatureMutationExpiryAndCertificateMismatch() throws Exception {
        final KeyPair keyPair = rsaKeyPair();
        final RsaSha256GatewayJwtVerifier verifier = verifier(keyPair);
        final String valid = token(
                keyPair,
                header(),
                claims(digest(21), digest(22), certificateFingerprint(), NOW - 10, NOW - 5, NOW + 300));

        assertThrows(
                IllegalArgumentException.class,
                () -> verifier.verify(mutateSignature(valid), session(CERTIFICATE_BYTES)));
        final String expired = token(
                keyPair,
                header(),
                claims(digest(21), digest(22), certificateFingerprint(), NOW - 500, NOW - 400, NOW - 100));
    ...
    void rejectsWrongPolicyDuplicateMembersAndNonCanonicalFixedBytes() throws Exception {
        ...
        final String duplicateHeader = token(
                keyPair,
                "{\"alg\":\"RS256\",\"alg\":\"none\",\"typ\":\"JWT\"}",
                claims(digest(41), digest(42), certificateFingerprint(), NOW - 10, NOW - 5, NOW + 300));
        assertThrows(
                IllegalArgumentException.class, () -> verifier.verify(duplicateHeader, session(CERTIFICATE_BYTES)));

        final String nonCanonical = claims(
                        digest(51), digest(52), certificateFingerprint(), NOW - 10, NOW - 5, NOW + 300)
                .replace("tenant_routing_scope\":\"", "tenant_routing_scope\":\"==");
        assertThrows(
                IllegalArgumentException.class,
    ...
    private static SSLSession session(final byte[] certificateBytes) {
        ...
        return (SSLSession) Proxy.newProxyInstance(
                SSLSession.class.getClassLoader(), new Class<?>[] {SSLSession.class}, (proxy, method, arguments) -> {
