...
class ControlOperationAuthorizationV1Test {
    ...
    void bindsActorRoleScopeAndTargetCoverageBeforeRegistration() throws Exception {
        ...
        final ControlTargetRefV1 target = new ControlTargetRefV1(
                ...
                new LaneControlTargetV1(bytes(32, 3), bytes(16, 4), 1),
                bytes(32, 5),
                bytes(32, 6));
        final PreparedControlOperationV1 prepared = PreparedControlOperationV1.prepare(
                bytes(32, 7),
                request.kind(),
                new ControlAuthorV1(actor, roles.digest(), scope),
                request,
                List.of(target),
                1,
                2,
        ...
        assertThrows(
                IllegalArgumentException.class,
                () -> ControlOperationAuthorizationV1.authorize(
                        prepared,
                        new ControlAuthorizationContextV1(
                                actor, ControlRoleSetV1.of(ControlRoleV1.DEAD_LETTER_OPERATOR), scope),
                        ignored -> true));
    }
    ...
    void enforcesPlatformAndDualRoleControlMatrices() throws Exception {
        ...
        final ControlTargetRefV1 shard = new ControlTargetRefV1(
                0,
                ControlTargetKindV1.SHARD,
                new ShardSubjectV1(new ShardId(new RouteIncarnation(bytes(16, 12)), 0)),
                null,
                null);
        ...
        final PreparedControlOperationV1 prepared = PreparedControlOperationV1.prepare(
                bytes(32, 13),
                force.kind(),
                new ControlAuthorV1(actor, platform.digest(), scope),
                force,
                List.of(shard),
                1,
                2,
                1,
                keyPair.getPrivate());
        assertDoesNotThrow(() -> ControlOperationAuthorizationV1.authorize(
                prepared, new ControlAuthorizationContextV1(actor, platform, scope), ignored -> true));

        final ControlOperationRequestV1 resolve =
        ...
        final ControlTargetRefV1 message = new ControlTargetRefV1(
                0,
                ControlTargetKindV1.MESSAGE,
                new ControlMessageTargetV1(DelayMessageId.random(shard.shard().shardId()), 0, 1, bytes(32, 16)),
                bytes(32, 17),
                bytes(32, 18));
        final ControlRoleSetV1 operatorAndAdmin =
                ControlRoleSetV1.of(ControlRoleV1.DEAD_LETTER_OPERATOR, ControlRoleV1.TENANT_POLICY_ADMINISTRATOR);
        final PreparedControlOperationV1 resolvePrepared = PreparedControlOperationV1.prepare(
                bytes(32, 19),
                resolve.kind(),
                new ControlAuthorV1(actor, operatorAndAdmin.digest(), scope),
                resolve,
                List.of(message),
                1,
                2,
                ...
                keyPair.getPrivate());
        assertDoesNotThrow(() -> ControlOperationAuthorizationV1.authorize(
                resolvePrepared, new ControlAuthorizationContextV1(actor, operatorAndAdmin, scope), ignored -> true));
        assertThrows(
                IllegalArgumentException.class,
                () -> ControlOperationAuthorizationV1.authorize(
                        resolvePrepared,
