...
class ClaimResultBodyTest {
    ...
    void claimMaterializationRejectsNonCanonicalBrokerResourceIdentity() {
        ...
        final byte[] materialization =
                materialization(messageId, destination, capability, broker, payload(), adapterMetadata());
    ...
    void claimMaterializationRequiresDestinationAndCapabilityProfileSlots() {
        ...
        final byte[] wrongDestinationMaterialization =
        ...
        assertThrows(
                IllegalArgumentException.class,
                () -> ClaimResultBody.decodePrecondition(precondition(messageId, wrongDestinationMaterialization)));
        assertThrows(
                IllegalArgumentException.class,
                () -> ClaimResultBody.decodePrecondition(precondition(messageId, wrongCapabilityMaterialization)));
    }

    private static byte[] materialization(
            final DelayMessageId messageId,
            final byte[] destination,
            final byte[] capability,
            final byte[] broker,
            final byte[] payload,
            final byte[] metadata) {
    ...
    private static byte[] adapterMetadata() {
        return CanonicalProtobuf.message(output -> CanonicalProtobuf.bytes(
                output, 1, CanonicalProtobuf.message(inner -> CanonicalProtobuf.bytes(inner, 1, new byte[0]))));
    }
