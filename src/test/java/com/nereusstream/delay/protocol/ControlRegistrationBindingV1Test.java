...
class ControlRegistrationBindingV1Test {
    ...
    void bindsRecordedDefinitiveAndUncertainOutcomes() throws Exception {
        ...
        final PreparedControlOperationV1 prepared = PreparedControlOperationV1.prepare(
                bytes(32, 3),
                request.kind(),
                new ControlAuthorV1(bytes(32, 4), bytes(32, 5), scope),
                request,
                List.of(target),
                1,
                2,
        ...
        assertDoesNotThrow(() ->
                ControlRegistrationBindingV1.validate(prepared, ControlRegistrationOutcomeMessageV1.recorded(receipt)));
        ...
        final ControlNonPersistenceProofV1 proof = ControlNonPersistenceProofV1.create(
                ...
                null,
                null);
        assertDoesNotThrow(() -> ControlRegistrationBindingV1.validate(
                prepared,
        ...
        final ControlOperationReceiptV1 wrongReceipt = ControlOperationReceiptV1.create(
                prepared.operationId(), bytes(32, 7), scope, prepared.targetSnapshotHash(), 1, time, 200);
        assertThrows(
                IllegalArgumentException.class,
                () -> ControlRegistrationBindingV1.validate(
                        prepared, ControlRegistrationOutcomeMessageV1.recorded(wrongReceipt)));
    }
