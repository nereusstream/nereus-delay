...
public final class ProfileCatalogV1ScheduleResolver implements V1ScheduleResolver {
    ...
    public ProfileCatalogV1ScheduleResolver(final V1ScheduleResolver delegate, final ProfileCatalog profileCatalog) {
    ...
    private ResolvedProfileSemantics requireDestinationProfile(final ProfileRefV1 reference) {
        ...
        if (semantic == null
                || semantic.profileKind() != ProfileKindV1.DESTINATION
    ...
    private static void requireIntentProfileLimits(
            final ScheduleIntentV1 intent, final long payloadLength, final DestinationProfileSemanticV1 destination) {
        final boolean adapterMatches =
                switch (destination.adapterKind()) {
                    case KAFKA ->
                        intent.adapterMetadata().kind() == com.nereusstream.delay.protocol.AdapterMetadataV1.Kind.KAFKA;
                    case PULSAR ->
                        intent.adapterMetadata().kind()
                                == com.nereusstream.delay.protocol.AdapterMetadataV1.Kind.PULSAR;
                };
        if (!adapterMatches) {
            throw new V1CommandResolutionException(
    ...
    private record ResolvedProfileSemantics(
            DestinationProfileSemanticV1 destination, DeliveryCapabilitySemanticV1 capability) {}
    ...
    private static long expectedActionAt(
        ...
        if (destination.adapterKind() == com.nereusstream.delay.protocol.AdapterKindV1.PULSAR
                && destination.handoffLeadMs() > 0
                && capability != null
                && TimingCapabilityV1.includes(
                        capability.timingCapabilityBits(), TimingCapabilityV1.PULSAR_GUARDED_HANDOFF)) {
            try {
