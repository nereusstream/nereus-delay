...
public final class ResourceGcGuard {
    ...
    public static Decision evaluate(
        ...
        final long requiredSequence =
                maxUnsigned(intent.appliedMutationSequence(), confirmation.appliedMutationSequence());
     ...
     */
    private static Decision recoveryPinDecision(
            final ResourceRetireIntentRecord intent, final RecoveryCatalogAuthority catalog) {
    ...
    private static boolean checkpointIdentityMatchesPin(final byte[] encodedIdentity, final RecoveryPin pin) {
        try {
            ...
            if (outerReader.hasRemaining()
                    || outer.number() != ResourceKind.CHECKPOINT.wireValue()
                    || outer.wireType() != 2) {
                return false;
            }
    ...
    private static boolean matches(
            ...
            final byte[] pinnedCheckpointId,
            final byte[] pinnedManifestHash) {
