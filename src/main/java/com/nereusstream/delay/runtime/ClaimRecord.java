...
public final class ClaimRecord {
    ...
    public ClaimRecord(
        ...
        if (ownerEpoch == 0
                || claimSequence == 0
                || laneControlVersion <= 0
                || runtimeLaneVersion < 0
                || runtimeRevision <= 0) {
        ...
        final ClaimResultBody.ClaimPrecondition precondition =
                ClaimResultBody.decodePrecondition(this.preconditionBytes);
        ...
        if (!Arrays.equals(
                this.instanceDigest,
                computeInstanceDigest(this.preconditionBytes, this.timelineKey, runtimeRevision))) {
            throw new IllegalArgumentException("Claim instance digest mismatch");
        }
    }

    public static ClaimRecord claimed(
            final DelayMessageId delayMessageId,
            final int generation,
            final byte[] claimId,
            final long ownerEpoch,
    ...
    public byte[] encode() {
        ...
        final byte[] base = Bytes.concat(
                ...
                Bytes.u64beBits(claimSequence),
                laneId.bytes(),
                laneIncarnation,
                Bytes.u64be(laneControlVersion),
                Bytes.u64be(runtimeLaneVersion),
                Bytes.lp32(ownerIdentity),
                storeIncarnation,
                Bytes.lp32(preconditionBytes),
    ...
    public static ClaimRecord decode(final byte[] encoded) {
        ...
        final ClaimRecord result = new ClaimRecord(
    ...
    public static byte[] computeInstanceDigest(
            final byte[] preconditionBytes, final byte[] timelineKey, final long runtimeRevision) {
    ...
    private static TimelineKeyIdentity decodeTimelineKey(final byte[] encoded) {
        ...
        final DelayMessageId message =
                new DelayMessageId(Arrays.copyOfRange(encoded, messageOffset, messageOffset + DelayMessageId.LENGTH));
        final int generation = ByteBuffer.wrap(encoded, messageOffset + DelayMessageId.LENGTH, Integer.BYTES)
                .getInt();
    ...
    public boolean equals(final Object other) {
        ...
        return delayMessageId.equals(that.delayMessageId)
                ...
                && claimSequence == that.claimSequence
                && laneControlVersion == that.laneControlVersion
                && runtimeLaneVersion == that.runtimeLaneVersion
                && runtimeRevision == that.runtimeRevision
                ...
                && Arrays.equals(timelineKey, that.timelineKey)
                && Arrays.equals(instanceDigest, that.instanceDigest)
