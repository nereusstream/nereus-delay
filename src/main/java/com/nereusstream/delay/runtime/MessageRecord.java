...
public record MessageRecord(
    ...
    public MessageRecord {
        ...
        scheduleSourcePosition =
                SourcePositionCodec.decode(scheduleSourcePosition).canonicalBytes();
    }
    ...
    public static MessageRecord decode(final byte[] encoded) {
        ...
        if (version == 1) {
            ...
            if (referenceLength < 0
                    || referenceLength > input.remaining()
    ...
    private static GenerationRuntimeIndex legacyRuntimeIndex(final MessageStatus status, final long stateVersion) {
        return GenerationRuntimeIndex.legacyNone(
                GenerationAggregateState.fromMessageStatus(status), Math.max(1, stateVersion));
    }

    private static void validateRuntimeProjection(
            final MessageStatus status, final GenerationRuntimeIndex runtimeIndex) {
