...
public record ShardQuota(
    ...
    public ShardQuota addSchedule(final long bytes, final boolean newLane) {
        ...
        return new ShardQuota(
                Math.addExact(pendingMessages, 1),
                Math.addExact(pendingBytes, bytes),
                reservationMessages,
                reservationBytes,
                Math.addExact(laneCount, newLane ? 1 : 0),
    ...
    public ShardQuota addReservation(final long bytes, final boolean newLane) {
        ...
        return new ShardQuota(
                pendingMessages,
                pendingBytes,
                Math.addExact(reservationMessages, 1),
                Math.addExact(reservationBytes, bytes),
                Math.addExact(laneCount, newLane ? 1 : 0),
                Math.addExact(usageRevision, 1));
    }
    ...
    public byte[] encode() {
        return Bytes.concat(
                Bytes.u32be(2),
                Bytes.u64be(pendingMessages),
                Bytes.u64be(pendingBytes),
                Bytes.u64be(reservationMessages),
                Bytes.u64be(reservationBytes),
                Bytes.u64be(laneCount),
