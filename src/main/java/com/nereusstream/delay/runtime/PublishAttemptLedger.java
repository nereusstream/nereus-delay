...
public final class PublishAttemptLedger {
    ...
    private PublishAttemptLedger(
        ...
        if (firstAttemptAtEpochMs < ABSENT_RETRY_WINDOW
                || retryDeadlineEpochMs < ABSENT_RETRY_WINDOW
        ...
        if (state == AttemptLedgerState.PUBLISHING
                && (this.outcomeBytes.length != 0 || this.evidenceBytes.length != 0)) {
    ...
    public static PublishAttemptLedger publishing(
            final DelayMessageId delayMessageId,
            final int generation,
            final byte[] publishAttemptId,
            final byte[] claimId,
            ...
            final byte[] admissionBytes,
            final byte[] sourcePosition) {
        return new PublishAttemptLedger(
                delayMessageId,
                generation,
                publishAttemptId,
                claimId,
    ...
    public PublishAttemptLedger withUnknownOutcome(
            final byte[] outcome, final byte[] evidence, final byte[] outcomeSourcePosition) {
    ...
    public PublishAttemptLedger withDurableJournalMapping(
            final long mappedSequenceId, final byte[] mappedJournalPosition) {
        if (state != AttemptLedgerState.PUBLISHING
                || mappedSequenceId < 0
                || mappedJournalPosition == null
                || mappedJournalPosition.length == 0) {
    ...
    public PublishAttemptLedger withDurableRetirement(final byte[] retirementJournalPosition) {
        if (state != AttemptLedgerState.PUBLISHING
                || !mappingDurable
                || !retirementPending
                || retirementJournalPosition == null
                || retirementJournalPosition.length == 0) {
    ...
    public byte[] encodedKey() {
        return KeyCodec.inflight(
                state == AttemptLedgerState.PUBLISHING ? (byte) 2 : (byte) 3, ownerEpoch, publishAttemptId);
    }
    ...
    public byte[] encode() {
        ...
        return Bytes.concat(
                ...
                Bytes.u32beBits(attemptNo),
                laneId.bytes(),
                laneIncarnation,
                retryWindow,
                Bytes.lp32(ownerIdentity),
                storeIncarnation,
                preparedPublishHash,
                Bytes.lp32(admissionBytes),
                new byte[] {(byte) state.wireValue()},
                Bytes.lp32(outcomeBytes),
    ...
    public static PublishAttemptLedger decode(final byte[] encoded) {
        ...
        final PublishAttemptLedger result = new PublishAttemptLedger(
    ...
    private PublishAttemptLedger copyWithJournal(
            final long nextSequenceId,
            final boolean nextMappingDurable,
    ...
    public boolean equals(final Object other) {
        ...
        return delayMessageId.equals(that.delayMessageId)
                ...
                && Arrays.equals(claimId, that.claimId)
                ...
                && mappingDurable == that.mappingDurable
                && retirementPending == that.retirementPending
                && Arrays.equals(journalPosition, that.journalPosition)
                && firstAttemptAtEpochMs == that.firstAttemptAtEpochMs
                && retryDeadlineEpochMs == that.retryDeadlineEpochMs
                && Arrays.equals(admissionBytes, that.admissionBytes)
                && Arrays.equals(outcomeBytes, that.outcomeBytes)
                && Arrays.equals(evidenceBytes, that.evidenceBytes)
