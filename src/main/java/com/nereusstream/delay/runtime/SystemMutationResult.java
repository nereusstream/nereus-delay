...
public record SystemMutationResult(
    ...
    public SystemMutationResult {
        ...
        appliedSourcePosition =
                SourcePositionCodec.decode(appliedSourcePosition).canonicalBytes();
    }

    public static SystemMutationResult from(
            final SystemMutation mutation,
            final ApplyStatus status,
            final StableCode code,
            final byte[] sourcePosition) {
        Objects.requireNonNull(mutation, "mutation");
        final SourcePosition decodedPosition =
                SourcePositionCodec.decode(Objects.requireNonNull(sourcePosition, "sourcePosition"));
        ...
        return new SystemMutationResult(
                ...
                mutation.type(),
                mutation.retryUntilEpochMs(),
                mutation.authorIdentity(),
                status,
                code,
    ...
    public byte[] encode() {
        return Bytes.concat(
                ...
                mutationId,
                mutationHash,
                Bytes.u32be(mutationType.wireValue()),
                Bytes.u64be(retryUntilEpochMs),
                new byte[] {(byte) applyStatus.wireValue()},
                Bytes.u32be(stableCode.wireValue()),
    ...
    public static SystemMutationResult decode(final byte[] encoded) {
        ...
        final SystemMutationResult result = new SystemMutationResult(
                id, hash, SystemMutationType.fromWire(type), retryUntil, author, applyStatus, code, source);
    ...
    public boolean equals(final Object other) {
        ...
        return retryUntilEpochMs == that.retryUntilEpochMs
                ...
                && Arrays.equals(mutationHash, that.mutationHash)
