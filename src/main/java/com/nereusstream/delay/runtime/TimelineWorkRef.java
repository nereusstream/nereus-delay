...
public final class TimelineWorkRef {
    ...
    public TimelineWorkRef(
            final TimelineWorkKind workKind,
            final byte[] encodedTimelineKey,
            ...
            final byte[] uncertainRetryControl,
            final byte[] uncertainRetryControlPosition) {
        ...
        if (actionAtEpochMs < 0 || retryEligibilityAtEpochMs < 0 || candidateAttemptNo <= 0 || runtimeRevision <= 0) {
        ...
        this.semanticWorkDigest = computeSemanticDigest(
                this.workKind,
                this.encodedTimelineKey,
                this.actionAtEpochMs,
                this.retryEligibilityAtEpochMs,
                this.candidateAttemptNo,
                this.orderedHeadBlocking,
                this.uncertainRetryAuthority,
                this.uncertainRetryControl,
                this.uncertainRetryControlPosition);
        this.workInstanceDigest = computeInstanceDigest(
                this.semanticWorkDigest,
                this.workKind,
                this.encodedTimelineKey,
                this.actionAtEpochMs,
                this.retryEligibilityAtEpochMs,
    ...
    private TimelineWorkRef(
            ...
            final byte[] uncertainRetryControl,
            final byte[] uncertainRetryControlPosition,
            final byte[] semanticWorkDigest,
            final byte[] workInstanceDigest) {
        this(
                workKind,
                encodedTimelineKey,
                actionAtEpochMs,
    ...
    public static TimelineWorkRef initial(
            final byte[] encodedTimelineKey, final long actionAtEpochMs, final long runtimeRevision) {
        return new TimelineWorkRef(
    ...
    public static TimelineWorkRef decode(final byte[] encoded) {
        ...
        final TimelineWorkRef result = new TimelineWorkRef(
                workKind,
                key,
                keyHash,
                actionAt,
    ...
    private byte[] canonicalFields(final byte[] semanticDigest, final byte[] instanceDigest, final long revision) {
        return canonicalFields(semanticDigest, instanceDigest, revision, this);
    }

    private static byte[] canonicalFields(
            final byte[] semanticDigest, final byte[] instanceDigest, final long revision, final TimelineWorkRef ref) {
    ...
    private static byte[] computeSemanticDigest(
            final TimelineWorkKind kind,
            final byte[] key,
            final long actionAt,
            final long retryAt,
            final int candidateAttempt,
    ...
    private DelayMessageId timelineMessageId() {
        ...
        return new DelayMessageId(
                Arrays.copyOfRange(encodedTimelineKey, messageOffset, messageOffset + DelayMessageId.LENGTH));
    }
