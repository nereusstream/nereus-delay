...
public record ResourceDeleteConfirmedRecord(
    ...
    public ResourceDeleteConfirmedRecord {
        ...
        final TrustedUtcIntervalEvidence observedEvidence =
                TrustedUtcIntervalEvidence.decode(Objects.requireNonNull(observedAt, "observedAt"));
        final TrustedUtcIntervalEvidence confirmedEvidence =
        ...
        ResourceRetireIntentBody.validateExternalDeleteIdentity(
                retireIntent.resourceKind(),
                retireIntent.resourceIdentity(),
                observedImmutableVersion,
                observedEtag,
                outcome);
        if (appliedSourcePosition.length == 0) {
            throw new IllegalArgumentException("appliedSourcePosition must not be empty");
        }
        final SourcePosition intentSourcePosition = SourcePositionCodec.decode(retireIntent.appliedSourcePosition());
    ...
    public byte[] encode() {
        return Bytes.concat(
                Bytes.u32be(2),
                confirmationMutationId,
                confirmationMutationHash,
                Bytes.lp32(retireIntent.encode()),
                Bytes.u8(outcome.wireValue()),
                providerRequestIdHash,
                Bytes.u64beBits(appliedMutationSequence),
                Bytes.lp32(observedImmutableVersion),
                Bytes.lp32(observedEtag),
                responseHash,
                Bytes.lp32(observedAt),
                Bytes.lp32(confirmedAt),
    ...
    public static ResourceDeleteConfirmedRecord decode(final byte[] encoded) {
        ...
        final ResourceRetireIntentRecord intent = ResourceRetireIntentRecord.decode(readLp32(input, "retireIntent"));
        final ResourceDeleteConfirmedBody.DeleteOutcome outcome =
                ResourceDeleteConfirmedBody.DeleteOutcome.fromWire(input.get() & 0xff);
        ...
        final ResourceDeleteConfirmedRecord result = new ResourceDeleteConfirmedRecord(
                mutationId,
                mutationHash,
                intent,
                outcome,
