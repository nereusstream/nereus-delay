...
public record LaneRecord(
    ...
    public static LaneRecord initial(final DestinationLaneId laneId, final SourcePosition sourcePosition) {
        ...
        return new LaneRecord(
                laneId,
                Arrays.copyOf(incarnationDigest, 16),
                1,
                0,
                AdmissionGate.OPEN,
    ...
    public LaneRecord withReadiness(final RuntimeReadiness next) {
        ...
        if (!allowed) {
            throw new IllegalStateException(
                    "invalid runtime readiness transition: " + runtimeReadiness + " -> " + next);
        }
        return new LaneRecord(
                laneId,
                laneIncarnation,
    ...
    public LaneRecord withGate(final AdmissionGate nextGate) {
        ...
        switch (nextGate) {
            ...
            case CLOSED -> {
                if (admissionGate != AdmissionGate.OPEN
                        && admissionGate != AdmissionGate.ADMIN_PAUSED
        ...
        final long nextControlVersion = nextGate == AdmissionGate.RETIRED
                ? laneControlVersion
                : nextVersion(laneControlVersion, "laneControlVersion");
        return new LaneRecord(
                laneId,
                laneIncarnation,
                nextControlVersion,
                nextVersion(laneVersion, "laneVersion"),
                nextGate,
                nextGate == AdmissionGate.OPEN ? runtimeReadiness : RuntimeReadiness.BLOCKED,
                weight,
                nextEligibleAtEpochMs);
    }
    ...
    public byte[] encode() {
        return ByteBuffer.allocate(4 + 32 + 16 + 8 + 8 + 1 + 1 + 4 + 8)
                ...
                .put(laneId.bytes())
                .put(laneIncarnation)
                .putLong(laneControlVersion)
                .putLong(laneVersion)
                .put((byte) admissionGate.wireValue())
                .put((byte) runtimeReadiness.wireValue())
                .putInt(weight)
    ...
    public static LaneRecord decode(final byte[] encoded) {
        ...
        final LaneRecord result = new LaneRecord(
                new DestinationLaneId(id),
                incarnation,
                input.getLong(),
                input.getLong(),
                AdmissionGate.fromWire(input.get() & 0xff),
                RuntimeReadiness.fromWire(input.get() & 0xff),
                input.getInt(),
                input.getLong());
