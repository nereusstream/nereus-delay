...
public final class CheckpointDeleteConfirmationComposer {
     ...
     */
    public static SystemMutation compose(
        ...
        final ResourceDeleteConfirmedBody.ExternalDeleteEvidence evidence =
                deleteResult.externalEvidence(retireIntent.resourceIdentityHash(), observedAt);
        ResourceRetireIntentBody.validateExternalDeleteIdentity(
                ResourceKind.CHECKPOINT,
                retireIntent.resourceIdentity(),
                evidence.observedImmutableVersion(),
                evidence.observedEtag(),
                evidence.outcome());

        final ShardId shard =
                SourcePositionCodec.decode(retireIntent.appliedSourcePosition()).shardId();
        final ResourceDeleteConfirmedBody.RetireIntentRef intent = new ResourceDeleteConfirmedBody.RetireIntentRef(
                retireIntent.mutationId(),
                retireIntent.mutationHash(),
        ...
        final byte[] canonicalAuthor = AuthorIdentity.decode(Objects.requireNonNull(authorIdentity, "authorIdentity"))
                .canonicalBytes();
