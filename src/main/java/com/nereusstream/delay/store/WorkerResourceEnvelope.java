...
public record WorkerResourceEnvelope(
    ...
    public WorkerResourceEnvelope {
        if (certifiedJvmHeapBytes <= 0
                || maxDirectMemoryBytes <= 0
                || maxRocksDbNativeBytes <= 0
                || maxOtherNativeBytes <= 0
                || minInProcessControlHeadroomBytes < 0
                || maxProcessRssBytes < 0
                || minContainerHeadroomBytes < 0
                || effectiveCgroupMemoryLimitBytes < 0
                || maxProcessOpenFiles < 0
                || fdHeadroom < 0
                || maxFilesystemBytes < 0
                || physicalDiskSafetyWatermarkBytes < 0
                || checkpointRestoreTempHeadroomBytes < 0
                || compactionTempHeadroomBytes < 0
                || controlReserveBytes <= 0
                || controlReserveRecords <= 0) {
    ...
    public void validate(final ShardStoreConfig config) {
        ...
        try {
            processMemory = Math.addExact(
                    Math.addExact(
                            Math.addExact(certifiedJvmHeapBytes, maxDirectMemoryBytes),
                            Math.addExact(maxRocksDbNativeBytes, maxOtherNativeBytes)),
    ...
    public byte[] canonicalBytes() {
        return ByteBuffer.allocate(4 + 16 * 8)
                .putInt(1)
                .putLong(certifiedJvmHeapBytes)
                .putLong(maxDirectMemoryBytes)
                .putLong(maxRocksDbNativeBytes)
                .putLong(maxOtherNativeBytes)
                .putLong(minInProcessControlHeadroomBytes)
                .putLong(maxProcessRssBytes)
                .putLong(minContainerHeadroomBytes)
                .putLong(effectiveCgroupMemoryLimitBytes)
                .putLong(maxProcessOpenFiles)
                .putLong(fdHeadroom)
                .putLong(maxFilesystemBytes)
                .putLong(physicalDiskSafetyWatermarkBytes)
                .putLong(checkpointRestoreTempHeadroomBytes)
                .putLong(compactionTempHeadroomBytes)
                .putLong(controlReserveBytes)
                .putLong(controlReserveRecords)
                .array();
    }
}
