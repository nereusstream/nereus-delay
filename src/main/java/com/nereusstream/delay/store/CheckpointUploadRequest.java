...
public record CheckpointUploadRequest(
        CheckpointUploadIntent intent, CheckpointManifest manifest, Path checkpointDirectory, byte[] manifestBytes) {
    public CheckpointUploadRequest {
        ...
        if (intent.state() != com.nereusstream.delay.protocol.CheckpointUploadState.PENDING_UPLOAD) {
