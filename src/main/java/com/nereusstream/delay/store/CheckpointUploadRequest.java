...
public record CheckpointUploadRequest(
        CheckpointUploadIntentV1 intent, CheckpointManifest manifest, Path checkpointDirectory, byte[] manifestBytes) {
    public CheckpointUploadRequest {
        ...
        if (intent.state() != com.nereusstream.delay.protocol.CheckpointUploadStateV1.PENDING_UPLOAD) {
