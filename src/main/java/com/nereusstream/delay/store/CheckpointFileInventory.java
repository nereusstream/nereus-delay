...
public record CheckpointFileInventory(String name, long length, byte[] checksum) {
    ...
    public static List<CheckpointFileInventory> collect(
            final Path checkpointRoot, final CheckpointManifestLimits limits) {
        if (Files.isSymbolicLink(checkpointRoot) || !Files.isDirectory(checkpointRoot, LinkOption.NOFOLLOW_LINKS)) {
        ...
        try (var paths = Files.walk(checkpointRoot)) {
            ...
            while (iterator.hasNext()) {
                ...
                final String name = canonicalName(checkpointRoot
                        .relativize(path)
                        .toString()
            ...
            return result.stream()
                    .sorted((left, right) -> compareCanonicalNames(left.name(), right.name()))
                    .toList();
