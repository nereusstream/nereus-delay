...
public record ShardStoreConfig(
    ...
    public ShardStoreConfig {
        ...
        if (maxOwnedShards <= 0
                || maxOpenShardDbs < maxOwnedShards
                || maxOpenFilesPerDb <= 0
                || maxTotalOpenFiles <= 0
                || (long) maxTotalOpenFiles < (long) maxOpenFilesPerDb * maxOpenShardDbs
                || maxBackgroundJobs <= 0
                || sharedBlockCacheBytes <= 0
                || sharedWriteBufferBudgetBytes <= 0
                || maxConcurrentCheckpointCreatesPerWorker <= 0
                || maxConcurrentCheckpointUploadsPerWorker <= 0
                || maxConcurrentCheckpointDownloadsPerWorker <= 0
                || checkpointIoBytesPerSecond <= 0
                || maxConcurrentDrainsPerWorker <= 0
                || maxWriteBufferBytesPerDb <= 0
                || reservedFlushJobs <= 0
                || maxCompactionJobs <= 0
                || maxBackgroundJobsPerDb <= 0
    ...
    public ShardStoreConfig(
            final Path rootPath,
            final int maxOwnedShards,
            final int maxOpenShardDbs,
            final int maxOpenFilesPerDb,
            final int maxTotalOpenFiles,
            final int maxBackgroundJobs,
            final long sharedBlockCacheBytes,
            final long sharedWriteBufferBudgetBytes,
            ...
            final int maxConcurrentDrainsPerWorker,
            final long maxWriteBufferBytesPerDb,
            final int reservedFlushJobs,
            final int maxCompactionJobs,
            final int maxBackgroundJobsPerDb) {
        this(
                rootPath,
                maxOwnedShards,
                maxOpenShardDbs,
                ...
                maxConcurrentCheckpointCreatesPerWorker,
                maxConcurrentCheckpointUploadsPerWorker,
                maxConcurrentCheckpointDownloadsPerWorker,
                checkpointIoBytesPerSecond,
                maxConcurrentDrainsPerWorker,
                maxWriteBufferBytesPerDb,
                reservedFlushJobs,
                maxCompactionJobs,
                maxBackgroundJobsPerDb,
                1);
    }
    ...
    public ShardStoreConfig(
            ...
            final int maxConcurrentDrainsPerWorker,
            final long maxWriteBufferBytesPerDb,
            final int maxConcurrentAcquiresPerWorker) {
        this(
                rootPath,
                maxOwnedShards,
                maxOpenShardDbs,
                ...
                1,
                defaultPerDbBackgroundJobs(maxBackgroundJobs),
                maxConcurrentAcquiresPerWorker);
    }
