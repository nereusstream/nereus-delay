...
public final class SharedRocksDbResources implements AutoCloseable {
    ...
    public SharedRocksDbResources(
        ...
        runtimeSafetyGate = envelope == null ? null : new WorkerRuntimeSafetyGate(this.config, envelope, observation);
        nativeResourceLedger = envelope == null
                ? null
        ...
        if (nativeResourceLedger == null) {
            ...
            sharedBlockCacheReservation = nativeResourceLedger.reserve(
                    "shared-block-cache", NativeResourceUsage.blockCache(this.config.sharedBlockCacheBytes()), 0);
            sharedWriteBufferReservation = nativeResourceLedger.reserve(
                    "shared-write-buffer", NativeResourceUsage.memtable(this.config.sharedWriteBufferBudgetBytes()), 0);
        }
        env = Env.getDefault();
    ...
    public static SharedRocksDbResources withRuntimeProbe(
            final ShardStoreConfig config, final WorkerResourceEnvelope envelope) {
        return new SharedRocksDbResources(config, envelope, WorkerRuntimeResourceProbe.observe(config.rootPath()));
    }
    ...
    void requireConfig(final ShardStoreConfig expected) {
        if (!config.equals(Objects.requireNonNull(expected, "expected config"))) {
            throw new IllegalArgumentException("ShardStore config does not match the shared Worker resource envelope");
        }
    }
    ...
    public synchronized void bindWorkClassExecutionRegistry(final WorkClassExecutionRegistry registry) {
        ...
        requested.bindWorkerSingleton(WorkClassExecutionRegistry.WorkerSingleton.STORE_RESOURCE_ENVELOPE, this);
     ...
     */
    public synchronized WorkerRocksDbUsageMonitor startRocksDbUsageMonitor(
            final RocksDbUsageLimits limits, final Duration interval) {
        ensureOpen();
        ...
        if (rocksDbUsageMonitor == null || rocksDbUsageMonitor.isClosed()) {
            rocksDbUsageMonitor = WorkerRocksDbUsageMonitor.start(
                    interval, () -> collectPhysicalUsage(limits), this::recordRuntimeProbeFailure);
        }
    ...
    public List<RocksDbUsageSnapshot> collectPhysicalUsage(final RocksDbUsageLimits limits) {
        ...
        for (PhysicalUsageSource source : sources) {
            try {
                final RocksDbUsageSnapshot snapshot =
                        Objects.requireNonNull(source.snapshot().get(), "physical usage source returned null");
    ...
    public synchronized void stageRuntimeEnvelope(
            final WorkerResourceEnvelope envelope, final WorkerRuntimeResourceObservation observation) {
        ensureOpen();
        if (runtimeSafetyGate == null) {
            throw new IllegalStateException("runtime safety gate requires a Worker resource envelope");
        }
    ...
    public synchronized void activateRuntimeAfterDrain(
    ...
    public synchronized void close() {
        ...
        if (!closeStarted
                && (shardAcquireCount != 0
                        ...
                        || checkpointCreateCount != 0
                        || checkpointUploadCount != 0
                        || checkpointDownloadCount != 0
                        || drainCount != 0
                        || (nativeResourceLedger != null
                                && nativeResourceLedger.snapshot().activeAllocations()
                                        != SHARED_NATIVE_RESERVATION_COUNT))) {
        ...
        if (blockCacheClosed && sharedBlockCacheReservation != null && !sharedBlockCacheReservation.isReleased()) {
            try {
        ...
        final boolean sharedReservationsReleased =
                (sharedWriteBufferReservation == null || sharedWriteBufferReservation.isReleased())
                        && (sharedBlockCacheReservation == null || sharedBlockCacheReservation.isReleased());
        if (closeFailure == null
                && rateLimiterClosed
                && writeBufferManagerClosed
                && blockCacheClosed
