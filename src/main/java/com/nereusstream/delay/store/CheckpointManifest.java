...
public record CheckpointManifest(
    ...
    public CheckpointManifest {
        ...
        files = files.stream()
                .sorted((left, right) -> CheckpointFileInventory.compareCanonicalNames(left.name(), right.name()))
                .toList();
    ...
    public String canonicalJson() {
        ...
        field(
                json,
                "restoredFromCheckpointId",
                restoredFromCheckpointId == null ? "null" : quote(b64(restoredFromCheckpointId)));
    ...
    public static CheckpointManifest decodeCanonicalJson(final byte[] encoded, final CheckpointManifestLimits limits) {
    ...
    private static String sourcePositionJson(final SourcePosition position) {
        if (position instanceof KafkaSourcePosition kafka) {
            return "{\"brokerLogAppendTime\":" + quote(u64(kafka.brokerLogAppendTimeEpochMs()))
                    + ",\"clusterId\":"
                    + quote(b64(kafka.authenticatedClusterId().getBytes(StandardCharsets.UTF_8)))
    ...
    private static void requireNonZero(final byte[] value, final String name) {
        if (value == null
                || value.length == 0
                || java.util.Arrays.stream(toIntArray(value)).allMatch(item -> item == 0)) {
    ...
    public record ParentCheckpoint(byte[] checkpointId, String manifestSha256) {
        ...
        private String toJson() {
            return "{\"checkpointId\":" + quote(b64(checkpointId)) + ",\"manifestSha256\":" + quote(manifestSha256)
                    + "}";
        }
    }
    ...
    public record CreatedBy(byte[] deploymentId, byte[] workerRunId, long ownerEpoch) {
        ...
        private String toJson() {
            return "{\"deploymentId\":" + quote(b64(deploymentId)) + ",\"ownerEpoch\":" + quote(u64Bits(ownerEpoch))
                    + ",\"workerRunId\":" + quote(b64(workerRunId)) + "}";
        }
    }

    public record CreatedAt(
            long earliestEpochMs,
            long latestEpochMs,
            String source,
            byte[] sourceId,
            long sourceConfigGeneration,
            long sampleSequence,
            long monotonicAnchorNs,
            byte[] sourceEvidenceSha256,
            int sourceKeyVersion,
    ...
    public record FileEntry(
            String name, long length, byte[] checksum, byte[] objectKey, byte[] objectVersion, byte[] etag) {
        public FileEntry {
