...
public final class OxiaRecoveryCatalog implements RecoveryCatalogAuthority {
    ...
    public RecoveryFloor advanceFloor(
        ...
        validateScalarFloorIdentity(result, publishedManifest(requestedCheckpointId));
        return result;
    }
    ...
    public RecoveryFloorRefV1 advanceFloor(
        ...
        validateTypedFloorIdentity(result, publishedManifest(requestedCheckpointId));
        return result;
    }
    ...
    public Optional<CheckpointManifest> manifest(final byte[] checkpointId) {
        ...
        final byte[] requestedCheckpointId = Bytes.copy(checkpointId);
        final byte[] backendCheckpointId = Bytes.copy(requestedCheckpointId);
        final Optional<CheckpointManifest> result =
                Objects.requireNonNull(backend.manifest(backendCheckpointId), "Oxia manifest result");
    ...
    public Optional<RecoveryFloor> currentFloor() {
        final Optional<RecoveryFloor> result =
                Objects.requireNonNull(backend.currentFloor(), "Oxia current floor result");
    ...
    public void validatePublishedRestoreCandidate(final CheckpointManifest candidate) {
        ...
        validateManifestIdentity(requested, published, "Oxia restore candidate is not the exact published manifest");
    ...
    public Optional<RecoveryCatalog.FloorCoverage> proveFloorCoverage(
        ...
        result.ifPresent(coverage -> {
            if (!Bytes.constantTimeEquals(
                    requestedCandidateCheckpointId, coverage.candidate().checkpointId())) {
                throw new IllegalStateException("Oxia floor coverage returned another candidate");
            }
            final CheckpointManifest candidate = publishedManifest(requestedCandidateCheckpointId);
            validateManifestIdentity(coverage.candidate(), candidate, "Oxia floor coverage changed candidate manifest");
            final CheckpointManifest floorManifest =
                    publishedManifest(coverage.floor().checkpointId());
            validateScalarFloorIdentity(coverage.floor(), floorManifest);
            if (!candidate.shardId().equals(floorManifest.shardId())
                    || Long.compareUnsigned(coverage.floor().includedMutationSequence(), requiredMutationSequence)
                            < 0) {
            ...
            if (ancestry.isEmpty()
                    || !Bytes.constantTimeEquals(
                            ancestry.get(ancestry.size() - 1).checkpointId(), candidate.checkpointId())
                    || !Bytes.constantTimeEquals(
                            ancestry.get(ancestry.size() - 1).manifestSha256(), candidate.manifestSha256())) {
    ...
    private static void validatePublicationIdentity(
            final CheckpointManifest requested, final RecoveryCatalog.Publication result) {
        if (!requested.shardId().equals(result.manifest().shardId())
                || !Bytes.constantTimeEquals(
                        requested.checkpointId(), result.manifest().checkpointId())
                || !Bytes.constantTimeEquals(
                        requested.manifestSha256(), result.manifest().manifestSha256())) {
    ...
    private static void validateUploadedPublicationRequest(
        ...
        if (!Bytes.constantTimeEquals(
                        intent.owner().deploymentId(), manifest.createdBy().deploymentId())
                || !Bytes.constantTimeEquals(
                        intent.owner().workerRunId(), manifest.createdBy().workerRunId())
    ...
    private void validatePublicationFloor(
            final CheckpointManifest requested, final RecoveryCatalog.Publication result) {
    ...
    private static void validateManifestIdentity(
            final CheckpointManifest actual, final CheckpointManifest expected, final String message) {
    ...
    private void validateFloorCoverageAncestry(
            final List<CheckpointManifest> ancestry, final CheckpointManifest candidate, final RecoveryFloor floor) {
        ...
        for (CheckpointManifest ancestor : ancestry) {
            ...
            validateManifestIdentity(ancestor, published, "Oxia floor coverage ancestry changed a published manifest");
    ...
    private static void validateAncestryLink(final CheckpointManifest parent, final CheckpointManifest child) {
    ...
    private static void validateScalarFloorIdentity(final RecoveryFloor result, final CheckpointManifest manifest) {
