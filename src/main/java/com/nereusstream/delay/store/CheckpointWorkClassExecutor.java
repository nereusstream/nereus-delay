...
public final class CheckpointWorkClassExecutor {
    ...
    public CheckpointWorkClassExecutor(
            final WorkClassExecutionRegistry workClasses, final CheckpointExecutionCoordinator checkpointExecutor) {
    ...
    public Submission submit(final ExecutionRequest request) {
        ...
        final WorkClassTask task = new WorkClassTask(
                ...
                "checkpoint/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, identity)),
                identity.length);
    ...
    void completeWithoutExecution(
            final CheckpointScheduler.ScheduledCheckpoint claim, final LongSupplier completionClock) {
    ...
    private void execute(final ExecutionRequest request, final Submission submission) {
        ...
        try {
            ...
            submission.complete(AttemptOutcome.succeeded(checkpointExecutor.execute(
                    ...
                    request.pending(),
                    request.manifestFactory(),
                    request.uploadNowEpochMs(),
    ...
    private static byte[] canonicalIdentity(final ExecutionRequest request) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.bytes(
                    output, 1, request.claim().shardId().routeIncarnation().bytes());
            ...
            CanonicalProtobuf.bytes(
                    output, 4, Bytes.utf8(request.checkpointDirectory().toString()));
