...
public final class CheckpointRestoreWorkClassExecutor {
    ...
    public CheckpointRestoreWorkClassExecutor(
            final WorkClassExecutionRegistry workClasses, final CheckpointRestoreCoordinator restoreCoordinator) {
    ...
    private void execute(final RestoreRequest request, final Submission submission) {
        try {
            ...
            submission.complete(RestoreOutcome.succeeded(restoreCoordinator.restore(request.request(), request.pin())));
    ...
    private static byte[] canonicalIdentity(final RestoreRequest request) {
        return com.nereusstream.delay.protocol.CanonicalProtobuf.message(output -> {
            com.nereusstream.delay.protocol.CanonicalProtobuf.bytes(
                    output, 1, request.request().manifest().canonicalJsonBytes());
            com.nereusstream.delay.protocol.CanonicalProtobuf.bytes(
                    output, 2, request.request().resource().canonicalBytes());
            if (request.pin() != null) {
                com.nereusstream.delay.protocol.CanonicalProtobuf.bytes(
                        output, 3, request.pin().canonicalBytes());
            }
        });
    }
