...
public final class KeyCodec {
    ...
    public static byte[] dedupePosition(final byte[] canonicalSourcePosition) {
        ...
        final byte[] canonical =
                SourcePositionCodec.decode(canonicalSourcePosition).canonicalBytes();
    ...
    public static byte[] timelineDue(
        ...
        return Bytes.concat(
                new byte[] {1, 1},
                laneId.bytes(),
                Bytes.u64be(eligibleAtEpochMs),
                sourceOrderToken,
                messageId.bytes(),
                Bytes.u32beBits(generation));
    }

    public static byte[] timelineOrdered(
            final DestinationLaneId laneId,
    ...
    public static byte[] timelineReady(
        ...
        return Bytes.concat(
                new byte[] {3, 1}, Bytes.u64be(nextEligibleAtEpochMs), laneId.bytes(), Bytes.u64be(laneVersion));
    }

    public static byte[] timelineExpiry(
    ...
    public static byte[] timelineSystem(
            final byte systemWorkKind, final long nextEligibleAtEpochMs, final byte[] workId, final long workVersion) {
        ...
        return Bytes.concat(
                new byte[] {6, 1, systemWorkKind},
                Bytes.u64be(nextEligibleAtEpochMs),
                Bytes.lp32(workId),
                Bytes.u64be(workVersion));
    }
    ...
    public static byte[] gcTask(
            final long notBeforeEpochMs, final byte kind, final byte[] resourceId, final long expectedVersion) {
        ...
        return Bytes.concat(
                new byte[] {1, 1},
                Bytes.u64be(notBeforeEpochMs),
                new byte[] {kind},
                Bytes.lp32(resourceId),
                Bytes.u64beBits(expectedVersion));
    }

    /** Stable gc_cf locator for a resource identity/version retire intent. */
    public static byte[] gcRetireIntent(
    ...
    public static byte[] gcProtection(
            final int protectionKind, final byte[] resourceId, final long protectionGeneration) {
        Objects.requireNonNull(resourceId, "resourceId");
        ...
        return Bytes.concat(
                new byte[] {2, 1, (byte) protectionKind},
                Bytes.lp32(resourceId),
    ...
    public static byte[] metaProducer(
            final DestinationLaneId laneId, final long physicalPartition, final long channelSlot) {
        Objects.requireNonNull(laneId, "laneId");
        if (physicalPartition < 0
                || physicalPartition > 0xffff_ffffL
                || channelSlot < 0
                || channelSlot > 0xffff_ffffL) {
            throw new IllegalArgumentException("invalid producer key values");
        }
        return Bytes.concat(
                new byte[] {4, 1}, laneId.bytes(), Bytes.u32be(physicalPartition), Bytes.u32be(channelSlot));
    }
    ...
    private static void validateSourceOrderToken(final byte[] token) {
        ...
        final int expectedLength =
                token.length == 9 && token[0] == 1 ? 9 : token.length == 21 && token[0] == 2 ? 21 : -1;
        if (expectedLength < 0) {
