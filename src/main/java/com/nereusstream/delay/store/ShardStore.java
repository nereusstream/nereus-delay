...
public final class ShardStore implements AutoCloseable {
     ...
     * method.</p>
     */
    public static ShardStore openForLocalRecoveryReuse(
            final ShardStoreConfig config,
            final ShardId shardId,
            final SharedRocksDbResources resources,
        ...
        try {
            ...
            throw new IllegalStateException("cannot locate ACTIVE shard DB for recovery reuse: " + shardId, failure);
        }
        ...
        try {
            ...
            throw new IllegalStateException("cannot open ACTIVE shard DB for recovery reuse: " + shardId, failure);
        }
        try {
    ...
    private static ShardStore restoreWithRecoveryGuard(
            final ShardStoreConfig config,
            final ShardId shardId,
            final SharedRocksDbResources resources,
            final Path checkpointPath,
    ...
    private static ShardStore restoreWithRecoveryGuard(
        ...
        final Path activeDb =
                shardRoot.resolve("incarnations").resolve(storeUuid.toString()).resolve("db");
        ...
        try {
            ...
            if (Files.isSymbolicLink(checkpointPath)
                    ...
                    || !Files.isRegularFile(
                            checkpointPath.resolve("CURRENT"), java.nio.file.LinkOption.NOFOLLOW_LINKS)) {
            ...
            if (manifest != null
                    && (!java.util.Arrays.equals(
                                    manifest.dbIdentity(), staged.metadata().dbIdentity())
                            || !manifest.sourceStoreIncarnation()
                                    .equals(staged.metadata().storeIncarnationUuid()))) {
    ...
    private static void cleanupFailedRestore(
            final Path restoreRoot,
            final Path activeDb,
            final Path shardRoot,
            final UUID storeUuid,
            final boolean activeDbMoved,
            final ShardStore staged,
            final ShardStore prepared,
    ...
    static ShardStore restoreFromCheckpoint(
            ...
            final SharedRocksDbResources resources,
            final Path checkpointPath,
            final byte[] manifestJson,
            final RecoveryCatalogAuthority catalog) {
        final CheckpointManifest manifest =
                CheckpointManifest.decodeCanonicalJson(Objects.requireNonNull(manifestJson, "manifestJson"));
    ...
    private static void validateRecoveryPin(
            final ShardId shardId,
            final CheckpointManifest manifest,
            final RecoveryCatalogAuthority catalog,
            final RecoveryPinV1 pin) {
    ...
    private static void validateCheckpointManifest(
            final ShardId shardId,
            final Path checkpointPath,
            final CheckpointManifest manifest,
            final CheckpointManifestLimits limits)
    ...
    private static void validateRestoredRuntimeState(final ShardStore staged, final CheckpointManifest manifest)
            throws IOException {
        final byte[] restoredCheckpointId = staged.runtimeMetadata().lastCheckpointId();
        if (restoredCheckpointId == null || !Bytes.constantTimeEquals(restoredCheckpointId, manifest.checkpointId())) {
        ...
        if (restoredPosition == null
                || !Bytes.constantTimeEquals(
                        restoredPosition.canonicalBytes(),
    ...
    private static void validateRestoredRecoveryState(final ShardStore staged, final CheckpointManifest manifest)
            throws IOException {
        ...
        if (recovery.lastObservedFloor() != null
                && !Bytes.constantTimeEquals(
                        recovery.lastObservedFloor().recoveryLineageId(), manifest.recoveryLineageId())) {
            throw new IOException("restored Recovery Floor lineage does not match checkpoint manifest");
        }
        if (recovery.installState() != null
                && recovery.installState().checkpointId() != null
        ...
        if (recovery.installState() != null
                && recovery.lineageBase() != null
                && !java.util.Arrays.equals(
                        recovery.installState().checkpointId(),
    ...
    private static Path locateOrCreateDbPath(final Path shardRoot) throws IOException {
        ...
        if (Files.exists(activePointer, java.nio.file.LinkOption.NOFOLLOW_LINKS)) {
            ...
            if (Files.isSymbolicLink(incarnations)
                    || Files.isSymbolicLink(activeIncarnation)
        ...
        try (var stream = Files.list(incarnations)) {
            for (Path path : stream.sorted(
                            Comparator.comparing(item -> item.getFileName().toString()))
                    .toList()) {
    ...
    private static Path prepareShardRoot(final ShardStoreConfig config, final ShardId shardId) throws IOException {
        ...
        final Path routeRoot =
                shardsRoot.resolve(shardId.routeIncarnation().uuid().toString());
    ...
    private static void ensureRealDirectoryPath(final Path path) throws IOException {
        ...
        if (Files.isSymbolicLink(cursor) || !Files.isDirectory(cursor, java.nio.file.LinkOption.NOFOLLOW_LINKS)) {
    ...
    private static UUID readActivePointer(final Path pointer) throws IOException {
        final byte[] encoded = LocalStatePathGuard.readRegularFileNoFollow(pointer, 4L + 16 + 4, "ACTIVE pointer");
        if (encoded == null) {
    ...
    private static ShardStore openAtPathWithSlot(
        ...
        try {
            ...
            final byte[] encodedIdentity =
                    db.get(handles.get(ColumnFamily.META), KeyCodec.metaFixed(META_SHARD_IDENTITY));
            final byte[] identityBytes = encodedIdentity == null
                    ? null
                    : ValueEnvelope.decode(encodedIdentity, META_FIXED_VALUE_TYPE)
                            .payload();
            ...
            if (identityBytes == null) {
                ...
                try {
                    ...
                    throw new IllegalStateException(
                            "DB path does not carry a valid store incarnation: " + dbPath, exception);
                }
                final byte[] storeIncarnation = java.nio.ByteBuffer.allocate(16)
                        .putLong(storeUuid.getMostSignificantBits())
                ...
                if (!metadata.shardId().equals(shardId)) {
                    throw new IllegalStateException(
                            "shard identity mismatch: expected " + shardId + " got " + metadata.shardId());
                }
            ...
            if (restoreStoreIncarnation != null && identityBytes != null) {
                ...
                final StoreMetadata restored = new StoreMetadata(
                        metadata.storeFormatVersion(), shardId, storeIncarnation, metadata.dbIdentity());
                try (WriteBatch batch = new WriteBatch();
                        WriteOptions writeOptions = new WriteOptions().setSync(true)) {
                    batch.put(
                            handles.get(ColumnFamily.META),
            ...
            if (publishOpenMarkers) {
                ...
                try (WriteBatch batch = new WriteBatch();
                        WriteOptions writeOptions = new WriteOptions().setSync(true)) {
                    putRuntimeMetadata(
                            batch, handles.get(ColumnFamily.META), runtimeMetadata, closedIngressDeadlineThrough);
                    putRecoveryMetadata(batch, handles.get(ColumnFamily.META), recoveryMetadata);
                    db.write(writeOptions, batch);
                }
            }
            final ShardStore result = new ShardStore(
                    config,
                    shardId,
                    dbPath,
                    resources,
    ...
    private static RuntimeMetadataRead readRuntimeMetadata(final RocksDB db, final ColumnFamilyHandle metaHandle)
        ...
        return new RuntimeMetadataRead(
                ...
                ingressFence.closedThroughEpochMs());
    }

    private static StoreRecoveryMetadata readRecoveryMetadata(
            final RocksDB db,
    ...
    private static byte[] optionalRecoveryValue(
            final RocksDB db, final ColumnFamilyHandle metaHandle, final int recoveryKeyKind) throws RocksDBException {
        final byte[] encoded = db.get(metaHandle, KeyCodec.metaRecovery(recoveryKeyKind));
        return encoded == null
                ? null
                : ValueEnvelope.decode(encoded, META_RECOVERY_VALUE_TYPE).payload();
    }

    private static void validateFixedMetadata(
            final RocksDB db, final ColumnFamilyHandle metaHandle, final ShardId shardId) throws RocksDBException {
        ...
        validateUnsignedSequence(
                optionalFixedValue(db, metaHandle, META_MUTATION_SEQUENCE), "persisted shard mutation sequence");
        validateUnsignedSequence(optionalFixedValue(db, metaHandle, META_CLAIM_SEQUENCE), "persisted Claim sequence");
        validateOptionalFixedEnvelope(db, metaHandle, META_PAYLOAD_PROOF_CONTROL_STATE, META_PAYLOAD_PROOF_VALUE_TYPE);
        validateOptionalFixedEnvelope(db, metaHandle, META_PROFILE_CONTROL_STATE, META_PROFILE_VALUE_TYPE);
        validateOptionalFixedEnvelope(
                db, metaHandle, META_PROTOCOL_ACTIVATION_STATE, META_PROTOCOL_ACTIVATION_VALUE_TYPE);
    }

    private static void validateOptionalFixedEnvelope(
            final RocksDB db, final ColumnFamilyHandle metaHandle, final int fixedKeyKind, final int valueType)
            throws RocksDBException {
    ...
    private static byte[] optionalFixedValue(
            final RocksDB db, final ColumnFamilyHandle metaHandle, final int fixedKeyKind) throws RocksDBException {
        final byte[] encoded = db.get(metaHandle, KeyCodec.metaFixed(fixedKeyKind));
        if (encoded == null) {
            return null;
        }
    ...
    private static void putRuntimeMetadata(
        ...
        final IngressFenceState ingressFence =
                new IngressFenceState(closedIngressDeadlineThrough, next.lastIngressFenceProofId());
        ...
        batch.put(
                metaHandle,
                KeyCodec.metaFixed(META_EVIDENCE_CURSORS),
        ...
        batch.put(
                metaHandle,
                KeyCodec.metaFixed(META_OWNER_EPOCH),
                ValueEnvelope.encode(META_FIXED_VALUE_TYPE, Bytes.u64beBits(next.lastOpenedOwnerEpoch())));
        batch.put(
                metaHandle,
                KeyCodec.metaFixed(META_CLEAN_CLOSE_MARKER),
                ValueEnvelope.encode(META_FIXED_VALUE_TYPE, Bytes.u8(next.cleanCloseMarker() ? 1 : 0)));
    }

    private static void putRecoveryMetadata(
            final WriteBatch batch, final ColumnFamilyHandle metaHandle, final StoreRecoveryMetadata next)
            throws RocksDBException {
        putOptionalRecoveryValue(
                batch,
                metaHandle,
                META_RECOVERY_LINEAGE_BASE,
                next.lineageBase() == null ? null : next.lineageBase().canonicalBytes());
        putOptionalRecoveryValue(
                batch,
                metaHandle,
                META_RECOVERY_LAST_OBSERVED_FLOOR,
                next.lastObservedFloor() == null
                        ? null
                        : next.lastObservedFloor().canonicalBytes());
        putOptionalRecoveryValue(
                batch,
                metaHandle,
                META_RECOVERY_CATALOG_GENERATION,
                next.catalogGeneration() == 0 ? null : Bytes.u64beBits(next.catalogGeneration()));
        putOptionalRecoveryValue(
                batch,
                metaHandle,
                META_RECOVERY_INSTALL_STATE,
                next.installState() == null ? null : next.installState().canonicalBytes());
    }

    private static void putOptionalRecoveryValue(
            final WriteBatch batch,
            final ColumnFamilyHandle metaHandle,
            final int recoveryKeyKind,
            final byte[] payload)
            throws RocksDBException {
    ...
    private static void putOptionalFixedValue(
            final WriteBatch batch, final ColumnFamilyHandle metaHandle, final int fixedKeyKind, final byte[] payload)
            throws RocksDBException {
    ...
    private static IngressFenceState readIngressFenceState(final RocksDB db, final ColumnFamilyHandle metaHandle)
            throws RocksDBException {
        ...
        return IngressFenceState.decode(
    ...
    private static List<ColumnFamilyDescriptor> descriptors(
            ...
            final List<ColumnFamilyOptions> options,
            final Path dbPath)
            throws IOException, RocksDBException {
        ...
        result.add(new ColumnFamilyDescriptor(
                RocksDB.DEFAULT_COLUMN_FAMILY, familyOptions(resources, config.maxWriteBufferBytesPerDb())));
    ...
    private static ColumnFamilyOptions familyOptions(
            final SharedRocksDbResources resources, final long maxWriteBufferBytesPerDb) {
        final BlockBasedTableConfig table = new BlockBasedTableConfig().setBlockCache(resources.blockCache());
        return new ColumnFamilyOptions().setTableFormatConfig(table).setWriteBufferSize(maxWriteBufferBytesPerDb);
    }
    ...
    private static Throwable closeHandles(
            final RocksDB db,
    ...
    public synchronized void recordRecoveryMetadata(
        ...
        final StoreRecoveryMetadata next = new StoreRecoveryMetadata(
                lineageBase,
                lastObservedFloor,
                lastObservedFloor == null ? 0 : lastObservedFloor.catalogGeneration(),
                state);
    ...
    public synchronized SourcePosition appliedShardLogPosition() {
        final ValueEnvelope.Decoded value =
                getValue(ColumnFamily.META, KeyCodec.metaFixed(META_APPLIED_SOURCE_POSITION), 1);
    ...
    public synchronized long shardMutationSequence() {
        final ValueEnvelope.Decoded value = getValue(ColumnFamily.META, KeyCodec.metaFixed(META_MUTATION_SEQUENCE), 1);
        if (value == null) {
            return 0;
        }
    ...
    public synchronized void recordLastIngressFenceProofId(final byte[] proofId) {
        ...
        persistRuntimeMetadata(new StoreRuntimeMetadata(
                ...
                runtimeMetadata.lastOpenedOwnerEpoch(),
                false,
                runtimeMetadata.evidenceCursors()));
    }
    ...
    synchronized Path createCheckpoint(final Path checkpointPath, final byte[] checkpointId) {
        ...
        try {
            ...
            primaryFailure = exception instanceof Error
                    ? exception
                    : new IllegalStateException("cannot create RocksDB checkpoint", exception);
    ...
    public synchronized void close() {
        ...
        if (closeFailure == null
                && defaultColumnFamilyClosed
                && dbClosed
                && dbOptionsClosed
                && dbSlotReleased
                && (!ownsShardSlot || ownedShardSlotReleased)
    ...
    private boolean nativeTeardownComplete() {
        return defaultColumnFamilyClosed
                && dbClosed
                && dbOptionsClosed
                && closedColumnFamilyHandles.size() == ColumnFamily.values().length
                && allOptionsClosed();
    ...
    public static final class Batch {
        ...
        public void putControlSnapshot(final CompatibleControlSnapshotV1 next) throws RocksDBException {
            ...
            batch.put(
                    ...
                    KeyCodec.metaFixed(META_CONTROL_SNAPSHOT),
        ...
        public void putIngressFenceDeadline(final long closeThroughEpochMs) throws RocksDBException {
            ...
            final byte[] proofId = runtimeMetadata == null
                    ? currentRuntimeMetadata.lastIngressFenceProofId()
                    : runtimeMetadata.lastIngressFenceProofId();
