...
public final class CheckpointScheduler {
    ...
    public CheckpointScheduler(final long intervalMs, final int jitterPercent, final int maxScheduledShards) {
    ...
    public synchronized List<ScheduledCheckpoint> claimDueForShard(
            final ShardId shardId, final long nowEpochMs, final int limit) {
        Objects.requireNonNull(shardId, "shardId");
        requireState(shardId);
        return claimDueInternal(shardId, nowEpochMs, limit);
    }

    private List<ScheduledCheckpoint> claimDueInternal(
            final ShardId shardFilter, final long nowEpochMs, final int limit) {
        requireTime(nowEpochMs, "nowEpochMs");
        if (limit <= 0) {
    ...
    private long jitterOffsetMs(final ShardId shardId) {
        ...
        final byte[] digest =
                Bytes.sha256(JITTER_DOMAIN, shardId.routeIncarnation().bytes(), Bytes.u32beBits(shardId.partition()));
