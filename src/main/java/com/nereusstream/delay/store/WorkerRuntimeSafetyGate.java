...
public final class WorkerRuntimeSafetyGate {
    ...
    public WorkerRuntimeSafetyGate(final ShardStoreConfig config, final WorkerResourceEnvelope certifiedEnvelope) {
        this(config, certifiedEnvelope, null);
    }

    public WorkerRuntimeSafetyGate(
    ...
    public synchronized void stage(
            final WorkerResourceEnvelope nextEnvelope, final WorkerRuntimeResourceObservation observation) {
    ...
    private static String boundedFailureMessage(final Throwable failure) {
        ...
        final String value =
                failure.getClass().getSimpleName() + (message == null || message.isBlank() ? "" : ": " + message);
