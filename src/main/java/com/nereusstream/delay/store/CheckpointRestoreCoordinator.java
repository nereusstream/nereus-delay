...
public final class CheckpointRestoreCoordinator {
    ...
    public CheckpointRestoreCoordinator(
    ...
    ShardStore restore(final CheckpointDownloadRequest request, final RecoveryPin pin) {
        ...
        final Path downloadRoot = config.rootPath()
                .toAbsolutePath()
                .normalize()
                .resolve("checkpoint-download-tmp")
                .normalize();
        final Path attemptRoot =
                downloadRoot.resolve(UUID.randomUUID().toString()).normalize();
        ...
        try {
            ...
            if (materialized == null
                    || !materialized.toAbsolutePath().normalize().equals(target)
            ...
            restored = ShardStore.restoreFromCheckpointWithDownloadPermit(
                    config, shardId, resources, materialized, request.manifest(), catalog, pin, limits, downloadPermit);
    ...
    private void validateDownloadedInventory(final Path directory, final CheckpointManifest manifest) {
        ...
        for (int index = 0; index < actual.size(); index++) {
            ...
            if (!left.name().equals(right.name())
                    ...
                    || !com.nereusstream.delay.protocol.Bytes.constantTimeEquals(left.checksum(), right.checksum())) {
