...
public final class KafkaReceiptJournal {
     ...
     */
    KafkaReceiptJournal(final ShardId shard, final KafkaReceiptResource receiptResource, final long initialOffset) {
        this(shard, new LocalAppender(initialOffset), receiptResource);
    }

    /** Creates a receipt journal seam with an injected durable append. */
    public KafkaReceiptJournal(
            final ShardId shard, final DurableAppender appender, final KafkaReceiptResource receiptResource) {
    ...
    public synchronized Optional<EvidenceCursor> evidenceCursor(
        ...
        for (JournalRecord record : records) {
            ...
            maxBrokerPersistedAt =
                    Math.max(maxBrokerPersistedAt, record.position().brokerLogAppendTimeEpochMs());
        }
        if (latest == null) {
            return Optional.empty();
        }
     ...
     */
    public synchronized Resolution resolve(final ProducerKey producer, final ReceiptObservation observation) {
        Objects.requireNonNull(producer, "producer");
        ...
        if (observation.receipt() != null) {
            if (state.retired) {
                return Resolution.divergence(
                        state.mapping, "a retired mapping cannot be classified from a published receipt");
            }
        ...
        if (!state.retired
                || !observation.fenceAndLsoBarrierValid()
                || !observation.receiptRangeRetained()
                || !cursorLsoIncludesOffset(observation.cursor(), mappedPosition.offset())) {
            return Resolution.divergence(
                    state.mapping, "receipt absence lacks retirement, LSO barrier or retention proof");
        }
    ...
    private void validateFencedReceiptChannel(
        ...
        if (!Arrays.equals(channel.destinationLaneId(), producer.laneId().bytes())
                ...
                || !Arrays.equals(
                        channel.producerOrTransactionalIdentitySha256(), producer.transactionalIdentitySha256())) {
    ...
    public <T> CompletionStage<T> sendAfterMapped(final Mapping mapping, final TargetSender<T> sender) {
        ...
        if (result == null) {
            ...
            throw new JournalException(
                    StableCode.INTEGRITY_ERROR, "target transaction sender returned no CompletionStage");
        }
        return result;
    }
    ...
    private ReceiptPosition append(final RecordKind kind, final Mapping mapping) {
        ...
        if (position == null) {
            throw new JournalException(
                    StableCode.INTEGRITY_ERROR, "Kafka receipt appender returned no durable position");
        }
    ...
    private void validatePosition(final ReceiptPosition position) {
        ...
        if (lastPosition != null && position.compareTo(lastPosition) <= 0) {
            throw new JournalException(StableCode.INTEGRITY_ERROR, "Kafka receipt position is not strictly increasing");
        }
    }
    ...
    public record ReceiptPosition(
            long offset, long brokerLogAppendTimeEpochMs, long lastStableOffsetExclusive, byte[] receiptRecordHash)
            implements Comparable<ReceiptPosition> {
        public ReceiptPosition {
            if (brokerLogAppendTimeEpochMs < 0 || Long.compareUnsigned(lastStableOffsetExclusive, offset) <= 0) {
        ...
        public byte[] canonicalBytes() {
            return Bytes.concat(
                    Bytes.i64be(offset),
                    Bytes.i64be(brokerLogAppendTimeEpochMs),
                    Bytes.i64be(lastStableOffsetExclusive),
                    receiptRecordHash);
        }
    }
    ...
    public record ReceiptMatch(
            long offset, byte[] publishAttemptId, byte[] preparedPublishHash, byte[] receiptRecordHash) {
        public ReceiptMatch {
    ...
    public record ProducerKey(
            ...
            byte[] transactionalIdentitySha256,
            KafkaTargetResource target) {
        public ProducerKey {
        ...
        private byte[] canonicalBytes() {
            return Bytes.concat(
                    ...
                    uuidBytes(target.nativeTopicUuid()),
    ...
    private static final class LocalAppender implements DurableAppender {
        ...
        public ReceiptPosition append(final AppendRequest request) {
            ...
            final ReceiptPosition position = new ReceiptPosition(
                    offset,
                    request.mapping().guardedBrokerTimestampEpochMs(),
                    lastStableOffsetExclusive,
