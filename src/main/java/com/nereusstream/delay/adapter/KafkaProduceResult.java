...
public record KafkaProduceResult(
    ...
    public KafkaProduceResult {
        ...
        if (disposition == Disposition.PERSISTED) {
            ...
            if (stableCode <= 0
                    || authenticatedClusterId != null
                    || nativeTopicUuid != null
                    || partition != -1
                    || offset != -1
                    || leaderEpoch != null
                    || brokerLogAppendTimeEpochMs != -1) {
    ...
    public KafkaProduceResult(
            ...
            final long brokerLogAppendTimeEpochMs,
            final int stableCode,
            final byte[] requestEvidenceBytes,
    ...
    public static KafkaProduceResult persisted(
    ...
    public static KafkaProduceResult definitelyNotPersisted(final int stableCode, final byte[] evidence) {
        return new KafkaProduceResult(
                Disposition.DEFINITIVELY_NOT_PERSISTED,
                null,
                null,
                -1,
                -1,
    ...
    public static KafkaProduceResult definitelyNotPersisted(
            final PhysicalEnqueueAttemptId physicalAttemptId, final int stableCode, final byte[] evidence) {
        return new KafkaProduceResult(
                Disposition.DEFINITIVELY_NOT_PERSISTED,
                null,
                null,
    ...
    public static KafkaProduceResult unknown(final int stableCode, final byte[] evidence) {
        return new KafkaProduceResult(
                Disposition.UNKNOWN, null, null, -1, -1, null, -1, stableCode, null, evidence, null);
    }

    public static KafkaProduceResult unknown(
            final PhysicalEnqueueAttemptId physicalAttemptId, final int stableCode, final byte[] evidence) {
        return new KafkaProduceResult(
                Disposition.UNKNOWN,
                null,
    ...
    private static byte[] brokerEvidence(final int stableCode, final byte[] evidence) {
        return stableCode == com.nereusstream.delay.protocol.StableCode.BROKER_DEFINITIVE_NOT_PERSISTED.wireValue()
                        || stableCode
                                == com.nereusstream.delay.protocol.StableCode.NATIVE_GUARD_DEFINITIVE_NOT_PERSISTED
                                        .wireValue()
                ? evidence
                : null;
    }
