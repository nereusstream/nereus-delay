...
public final class KafkaTransactionalDestinationAdapter implements DestinationPublishAdapter {
    ...
    public KafkaTransactionalDestinationAdapter(
    ...
    public CompletionStage<DestinationPublishResult> publish(
            final DestinationPublishRequest request, final SourcePosition sourcePosition) {
        Objects.requireNonNull(request, "request");
        Objects.requireNonNull(sourcePosition, "sourcePosition");
        return closeGuard.invokeIfOpen(
                () -> publishOpen(request, sourcePosition),
                () -> completed(DestinationPublishResult.unknown(StableCode.CAPABILITY_UNAVAILABLE, null)));
    }
    ...
    private CompletionStage<DestinationPublishResult> publishOpen(
        ...
        try {
            ...
            producerKey = new KafkaReceiptJournal.ProducerKey(
                    laneId, laneIncarnation, transactionalIdentitySha256, targetResource);
            identity = new KafkaReceiptJournal.AttemptIdentity(
        ...
        try {
            result = journal.sendAfterMapped(
                    producerKey,
                    identity,
                    mapping -> transport.publish(KafkaTransactionalDestinationRequest.create(
                            targetPhysicalTopic,
                            KafkaDestinationRequest.from(targetResource, request),
                            receiptPhysicalTopic,
                            receiptResource,
                            mapping)));
        } catch (KafkaReceiptJournal.JournalException | IllegalArgumentException failure) {
            return completed(DestinationPublishResult.definitelyNotPublished(
                    failure instanceof KafkaReceiptJournal.JournalException
                            ? ((KafkaReceiptJournal.JournalException) failure).stableCode()
    ...
    private DestinationPublishResult validate(
            final DestinationPublishResult result, final DestinationPublishRequest request) {
