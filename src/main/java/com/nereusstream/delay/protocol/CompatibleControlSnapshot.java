 ...
 */
public final class CompatibleControlSnapshot {
    ...
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-compatible-control-snapshot\0");
    ...
    public CompatibleControlSnapshot(
    ...
    private CompatibleControlSnapshot(
            final ShardSubject shard,
            final List<ProtocolTuple> protocolTuples,
            final List<ProfileRef> profiles,
            final QuotaGrantRef initialQuotaGrant,
    ...
    public static CompatibleControlSnapshot decode(final byte[] encoded) {
        ...
        while (index < fields.size() && fields.get(index).number() == 3) {
            tuples.add(ProtocolTuple.decode(QueryCodecSupport.nested(fields.get(index++), 3)));
        }
        while (index < fields.size() && fields.get(index).number() == 4) {
            profiles.add(ProfileRef.decode(QueryCodecSupport.nested(fields.get(index++), 4)));
        }
        ...
        final QuotaGrantRef grant = QuotaGrantRef.decode(QueryCodecSupport.nested(fields.get(index), 5));
        final byte[] digest = QueryCodecSupport.fixed(fields.get(index + 1), 6, DIGEST_LENGTH);
        final CompatibleControlSnapshot result = new CompatibleControlSnapshot(shard, tuples, profiles, grant, digest);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CompatibleControlSnapshot");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof CompatibleControlSnapshot that
    ...
    private static List<ProtocolTuple> sortedTuples(final List<ProtocolTuple> values) {
        ...
        result.sort(Comparator.comparing(ProtocolTuple::canonicalBytes, CompatibleControlSnapshot::compareBytes));
