...
public final class BreakOrderingRequestV1 implements ControlOperationRequestBranchV1 {
    ...
    public static BreakOrderingRequestV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "BreakOrderingRequestV1");
        QueryCodecSupport.requireNumbers(fields, new int[] {1}, "BreakOrderingRequestV1");
        final BreakOrderingRequestV1 result =
                new BreakOrderingRequestV1(AcknowledgementSetV1.decode(QueryCodecSupport.nested(fields.get(0), 1)));
