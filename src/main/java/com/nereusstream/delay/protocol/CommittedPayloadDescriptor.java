...
public final class CommittedPayloadDescriptor {
    ...
    public CommittedPayloadDescriptor(
    ...
    public static CommittedPayloadDescriptor decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "CommittedPayloadDescriptor");
        if (fields.size() != 8 && fields.size() != 9) {
            throw new IllegalArgumentException("CommittedPayloadDescriptor has invalid field count");
        }
        int index = 0;
        ...
        final CommittedPayloadDescriptor result = new CommittedPayloadDescriptor(
                profile, container, objectKey, objectVersion, etag, length, hash, reservation, proof);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CommittedPayloadDescriptor");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof CommittedPayloadDescriptor that
                && length == that.length
