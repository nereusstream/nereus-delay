...
public final class CurrentControlOperation {
    private static final int HASH_LENGTH = 32;

    private final byte[] operationId;
    private final byte[] requestHash;
    private final byte[] authenticatedScopeHash;
    private final ControlOperationState state;
    private final long operationRevision;
    private final List<ControlTargetStateView> targetStates;
    private final ControlTypedResult typedResult;

    public CurrentControlOperation(
            final byte[] operationId,
            final byte[] requestHash,
            final byte[] authenticatedScopeHash,
            final ControlOperationState state,
            final long operationRevision,
            final List<ControlTargetStateView> targetStates,
            final ControlTypedResult typedResult) {
        ...
        if ((state == ControlOperationState.SUCCEEDED || state == ControlOperationState.SUCCEEDED_WITH_OUTSTANDING)
                && typedResult == null) {
    ...
    public ControlOperationState state() {
        return state;
    }
    ...
    public List<ControlTargetStateView> targetStates() {
        return targetStates;
    }

    public ControlTypedResult typedResult() {
        return typedResult;
    }
    ...
    public byte[] canonicalBytes() {
        return CanonicalProtobuf.message(output -> {
            ...
            for (ControlTargetStateView target : targetStates) {
    ...
    public static CurrentControlOperation decode(final byte[] encoded) {
        ...
        if (fields.size() < 5
            ...
            throw new IllegalArgumentException("invalid CurrentControlOperation fields");
        }
        final List<ControlTargetStateView> targets = new ArrayList<>();
        int index = 5;
        while (index < fields.size() && fields.get(index).number() == 6) {
            targets.add(ControlTargetStateView.decode(fields.get(index).rawValue()));
            index++;
        }
        final ControlTypedResult typed;
        if (index < fields.size() && fields.get(index).number() == 7) {
            typed = ControlTypedResult.decode(fields.get(index).rawValue());
            index++;
        } else {
            typed = null;
        }
        if (index != fields.size()) {
            throw new IllegalArgumentException("invalid CurrentControlOperation target/result order");
        }
        final CurrentControlOperation result = new CurrentControlOperation(
                ...
                ControlOperationState.fromWire(uint32(fields.get(3), 4)),
                uint(fields.get(4), 5),
                targets,
                typed);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CurrentControlOperation");
        return result;
    }

    private static List<ControlTargetStateView> sortedTargets(final List<ControlTargetStateView> values) {
        Objects.requireNonNull(values, "targetStates");
        final List<ControlTargetStateView> copy = new ArrayList<>(values);
        copy.sort(java.util.Comparator.comparingLong(ControlTargetStateView::targetIndex));
    ...
    public boolean equals(final Object other) {
        return other instanceof CurrentControlOperation that
                && operationRevision == that.operationRevision
                && state == that.state
                ...
                && targetStates.equals(that.targetStates)
                && Objects.equals(typedResult, that.typedResult);
    }
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                Arrays.hashCode(authenticatedScopeHash),
                state,
                operationRevision,
                targetStates,
                typedResult);
