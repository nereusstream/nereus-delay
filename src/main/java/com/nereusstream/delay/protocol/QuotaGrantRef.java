...
public final class QuotaGrantRef {
    public static final int HASH_LENGTH = 32;
    private static final byte[] SEMANTIC_DOMAIN = Bytes.utf8("nereus-delay-quota-grant-semantic\0");
    ...
    public QuotaGrantRef(final byte[] grantId, final long grantVersion, final PublishAdmissionBody.ChargeVector limit) {
    ...
    private QuotaGrantRef(
            final byte[] grantId,
            final long grantVersion,
            final byte[] grantSemanticHash,
    ...
    public static QuotaGrantRef decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "QuotaGrantRef");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4}, "QuotaGrantRef");
        final byte[] grantId = QueryCodecSupport.fixed(fields.get(0), 1, HASH_LENGTH);
        if (isZero(grantId)) {
            throw new IllegalArgumentException("QuotaGrantRef grantId must be non-zero");
        }
        final long grantVersion = QueryCodecSupport.uint64Bits(fields.get(1), 2);
        if (grantVersion == 0) {
            throw new IllegalArgumentException("QuotaGrantRef grantVersion must be nonzero");
        }
        final byte[] semanticHash = QueryCodecSupport.fixed(fields.get(2), 3, HASH_LENGTH);
        final PublishAdmissionBody.ChargeVector limit =
                PublishAdmissionBody.ChargeVector.decodeCanonical(QueryCodecSupport.nested(fields.get(3), 4));
        final QuotaGrantRef result = new QuotaGrantRef(grantId, grantVersion, semanticHash, limit);
        if (!Arrays.equals(semanticHash, semanticHash(grantId, grantVersion, limit))) {
            throw new IllegalArgumentException("QuotaGrantRef semantic hash mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "QuotaGrantRef");
        return result;
    }

    private static byte[] semanticHash(
            final byte[] grantId, final long version, final PublishAdmissionBody.ChargeVector limit) {
    ...
    public boolean equals(final Object other) {
        return other instanceof QuotaGrantRef that
                && grantVersion == that.grantVersion
