 ...
 * consulted.</p>
 */
public final class ControlTargetMutationBinding {
    private static final int HASH_LENGTH = 32;

    private ControlTargetMutationBinding() {}
    ...
    public static void validate(
        ...
        final ControlTargetRef preparedTarget = prepared.targets().stream()
        ...
        final ControlRef expectedControlRef =
        ...
        switch (mutation.type()) {
            case APPLY_SHARD_CONTROL -> {
                if (prepared.request().branch() instanceof TargetCloseRequest expected) {
                    ...
                    if (!expected.equals(body.request())) {
                        throw new IllegalArgumentException("Target close body differs from its registered request");
                    }
    ...
    static SystemMutationType expectedMutationType(
        ...
        return switch (operationKind) {
            ...
            case RESOLVE_UNCERTAIN ->
                requireTargetKind(targetKind, ControlTargetKind.MESSAGE, SystemMutationType.RESOLVE_UNCERTAIN);
            case STOP_NEW_SCHEDULES,
                    ...
                    PUBLISH_QUOTA_GRANT -> {
                if (targetKind != ControlTargetKind.LANE && targetKind != ControlTargetKind.SHARD) {
    ...
    private static SystemMutationType requireTargetKind(
            final ControlTargetKind actual, final ControlTargetKind expected, final SystemMutationType type) {
        if (actual != expected) {
    ...
    private static void validateShardTarget(final ControlTargetRef target, final SystemMutation mutation) {
    ...
    private static void validateApplyTarget(
            final ControlOperationKind operationKind, final ControlTargetRef target, final ApplyShardControlBody body) {
        final int expectedControlKind =
        ...
        if (target.targetKind() != ControlTargetKind.LANE) {
            return;
        }
        final ApplyShardControlBody.LaneTarget actual = body.laneTarget();
        final LaneControlTarget expected = target.lane();
    ...
    private static void validateReplayTarget(
            final ControlOperationRequest request, final ControlTargetRef target, final ReplayDeadLetterBody body) {
        final ReplayDeadLetterRequest expected = branch(request, ReplayDeadLetterRequest.class);
        final ControlMessageTarget message = target.message();
        if (!body.messageId().equals(message.messageId())
                ...
                || body.deliverAtEpochMs() != expected.deliverAt()
                || body.expireAtEpochMs() != expected.expireAt()
    ...
    private static void validateResolveTarget(
            final ControlOperationRequest request, final ControlTargetRef target, final ResolveUncertainBody body) {
        final ResolveUncertainRequest expected = branch(request, ResolveUncertainRequest.class);
        final ControlMessageTarget message = target.message();
        if (!body.messageId().equals(message.messageId())
        ...
        final byte[] expectedEvidence =
                expected.evidence() == null ? new byte[0] : expected.evidence().canonicalBytes();
        ...
        final boolean acknowledgementPresent = body.acknowledgementHash().length == HASH_LENGTH;
        final boolean expectedAcknowledgement =
                expected.allowPossibleDuplicate() || expected.allowPossibleDeliveryTerminal();
