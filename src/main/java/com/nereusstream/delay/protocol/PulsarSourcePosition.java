...
public record PulsarSourcePosition(
    ...
    public byte[] canonicalBytes() {
        ...
        final ByteBuffer result = ByteBuffer.allocate(
                1 + 16 + 4 + brokerResourceIncarnation.length + 4 + topic.length + 4 + 8 + 8 + 4 + 4 + 1 + 8);
    ...
    public byte[] sourceOrderToken() {
        return ByteBuffer.allocate(1 + 8 + 8 + 4)
                .put((byte) 2)
                .putLong(ledgerId)
                .putLong(entryId)
                .putInt(normalizedBatchIndex)
                .array();
    }
