...
public final class SourcePositionCodec {
    ...
    private static SourcePosition decodeInternal(final byte[] bytes) {
        ...
        if (kind == SourcePositionKind.KAFKA.wireValue()) {
            ...
            final long appendTime = requireNonNegative(readLong(input, "brokerLogAppendTime"), "brokerLogAppendTime");
            requireEnd(input);
        ...
        if (kind == SourcePositionKind.PULSAR.wireValue()) {
            ...
            final long timestamp = requireNonNegative(readLong(input, "brokerEntryTimestamp"), "brokerEntryTimestamp");
            requireEnd(input);
            final PulsarSourcePosition.EntryKind decodedKind =
                    switch (entryKind) {
