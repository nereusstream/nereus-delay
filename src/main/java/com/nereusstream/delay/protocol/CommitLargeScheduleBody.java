 ...
 *
 * <p>The body/common-field boundary and nested proof are canonical . The
 * decoder never accepts a proof whose reservation or message identity
 * disagrees with the body.</p>
 */
public final class CommitLargeScheduleBody {
    ...
    public CommitLargeScheduleBody(
    ...
    public static CommitLargeScheduleBody decode(final byte[] encoded) {
        ...
        if (QueryCodecSupport.uint(fields.get(1), 2) != COMMAND_TYPE) {
            throw new IllegalArgumentException("CommitLargeScheduleBody has the wrong command type");
        }
        final CommitLargeScheduleBody result = new CommitLargeScheduleBody(
                ...
                CanonicalPayloadCommitProof.decode(QueryCodecSupport.nested(fields.get(4), 11)));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CommitLargeScheduleBody");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof CommitLargeScheduleBody that
