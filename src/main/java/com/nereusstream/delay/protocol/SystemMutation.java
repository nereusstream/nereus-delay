...
public final class SystemMutation {
    ...
    private SystemMutation(
        ...
        if (!Bytes.constantTimeEquals(
                this.mutationHash, computeMutationHash(shardId, type, retryUntilEpochMs, this.canonicalBody))) {
            throw new IllegalArgumentException("mutationHash does not match the canonical mutation");
        }
        if (!Bytes.constantTimeEquals(
                this.systemMutationId,
                computeSystemMutationId(shardId, type, this.logicalOperationIdentity, this.mutationHash))) {
    ...
    public static SystemMutation signed(
            final ShardId shardId,
            final SystemMutationType type,
            final long retryUntilEpochMs,
            final byte[] logicalOperationIdentity,
        ...
        final byte[] signature = sign(
                signatureDigest(shardId, type, retryUntilEpochMs, id, body, hash, author, signingKeyVersion),
                privateKey);
        return new SystemMutation(
                shardId, type, retryUntilEpochMs, logical, body, author, signingKeyVersion, id, hash, signature);
    }
    ...
    public static SystemMutation decodeEnvelope(final byte[] envelope, final byte[] logicalOperationIdentity) {
        final EnvelopeFields fields = readEnvelope(envelope);
        final SystemMutation decoded = new SystemMutation(
                fields.shardId(),
                fields.type(),
                fields.retryUntilEpochMs(),
                logicalOperationIdentity,
                fields.body(),
                fields.authorIdentity(),
                fields.signingKeyVersion(),
                fields.systemMutationId(),
                fields.mutationHash(),
                fields.signature());
    ...
    private static EnvelopeFields readEnvelope(final byte[] envelope) {
        final List<CanonicalProtobuf.Reader.Field> outer =
                readAll(new CanonicalProtobuf.Reader(Objects.requireNonNull(envelope, "envelope")));
        if (outer.size() != 2) {
        ...
        final List<CanonicalProtobuf.Reader.Field> fields =
                readAll(new CanonicalProtobuf.Reader(outer.get(1).rawValue()));
        if (fields.size() != 11) {
    ...
    public static byte[] computeMutationHash(
            final ShardId shardId,
            final SystemMutationType type,
            final long retryUntilEpochMs,
            final byte[] canonicalBody) {
    ...
    public static byte[] computeSystemMutationId(
            final ShardId shardId,
            final SystemMutationType type,
            final byte[] logicalOperationIdentity,
            final byte[] mutationHash) {
    ...
    public static byte[] computePublishAttemptLogicalIdentity(
            final byte[] claimId, final byte[] messageId, final long generation, final long attemptNo) {
        ...
        return Bytes.sha256(
                ...
                Bytes.u32be(attemptNo));
    }

    /** Computes the registered logical identity for one Publish Admission. */
    public static byte[] computePublishAttemptLogicalIdentity(
            final byte[] claimId, final DelayMessageId messageId, final long generation, final long attemptNo) {
        return computePublishAttemptLogicalIdentity(
    ...
    public static byte[] computeResourceRetireLogicalIdentity(
        ...
        return computeResourceRetireLogicalIdentity(
                resourceKind.wireValue(), resourceIdentityHash, expectedResourceStateVersion);
    }

    /** Computes the same identity from the closed numeric registry value. */
    public static byte[] computeResourceRetireLogicalIdentity(
            final int resourceKind, final byte[] resourceIdentityHash, final long expectedResourceStateVersion) {
    ...
    public static byte[] computeDlqExportAttemptLogicalIdentity(final byte[] dlqExportId, final int physicalAttemptNo) {
    ...
    public static byte[] computeExpiryLogicalIdentity(
            final byte[] messageId, final int generation, final long expireAtEpochMs) {
    ...
    /** Computes the logical identity for one generation-expiry decision. */
    public static byte[] computeExpiryLogicalIdentity(
            final DelayMessageId messageId, final int generation, final long expireAtEpochMs) {
        return computeExpiryLogicalIdentity(
                Objects.requireNonNull(messageId, "messageId").bytes(), generation, expireAtEpochMs);
    }
    ...
    public static void validateBodyPrefix(
            final ShardId shardId,
            final SystemMutationType type,
            final long retryUntilEpochMs,
            final byte[] canonicalBody) {
    ...
    private static byte[] signatureDigest(
            final ShardId shardId,
            final SystemMutationType type,
            final long retryUntilEpochMs,
            final byte[] systemMutationId,
    ...
    private static void requireVarint(
            final CanonicalProtobuf.Reader.Field field, final int number, final long expected) {
    ...
    private static byte[] requireFixed(final CanonicalProtobuf.Reader.Field field, final int number, final int length) {
    ...
    private record EnvelopeFields(
            ShardId shardId,
            SystemMutationType type,
            long retryUntilEpochMs,
            byte[] body,
            byte[] authorIdentity,
            int signingKeyVersion,
            byte[] systemMutationId,
            byte[] mutationHash,
            byte[] signature) {
    ...
    public boolean equals(final Object other) {
        ...
        return shardId.equals(mutation.shardId)
                && type == mutation.type
