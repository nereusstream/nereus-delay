...
public final class RouteControlResult {
    private final byte[] routeUuid;
    private final RouteLifecycle lifecycle;
    private final long controlVersion;

    public RouteControlResult(final byte[] routeUuid, final RouteLifecycle lifecycle, final long controlVersion) {
    ...
    public static RouteControlResult decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "RouteControlResult");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3}, "RouteControlResult");
        final RouteControlResult result = new RouteControlResult(
                QueryCodecSupport.fixed(fields.get(0), 1, RouteIncarnation.LENGTH),
                RouteLifecycle.fromWire(QueryCodecSupport.uint(fields.get(1), 2)),
                QueryCodecSupport.uint(fields.get(2), 3));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "RouteControlResult");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof RouteControlResult that
