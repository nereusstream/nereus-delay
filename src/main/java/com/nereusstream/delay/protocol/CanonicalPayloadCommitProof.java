...
public final class CanonicalPayloadCommitProof implements PayloadCommitProofView {
    ...
    public CanonicalPayloadCommitProof(
        ...
        if (!Arrays.equals(
                        this.routeIncarnationUuid,
                        delayMessageId.routingId().shardId().routeIncarnation().bytes())
    ...
    public byte[] canonicalBytes() {
        return CanonicalProtobuf.message(output -> {
            ...
            CanonicalProtobuf.uint64Bits(output, 8, trustSetVersion);
    ...
    public static CanonicalPayloadCommitProof signed(
            final byte[] reservationId,
            final byte[] tenantRoutingScope,
            final byte[] routeIncarnationUuid,
            final int partition,
        ...
        final CanonicalPayloadCommitProof unsigned = new CanonicalPayloadCommitProof(
                reservationId,
                tenantRoutingScope,
                routeIncarnationUuid,
                partition,
                delayMessageId,
                objectStoreProfile,
                trustSetVersion,
        ...
        return new CanonicalPayloadCommitProof(
                reservationId,
                tenantRoutingScope,
                routeIncarnationUuid,
                partition,
    ...
    public static CanonicalPayloadCommitProof decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields =
                QueryCodecSupport.read(encoded, "CanonicalPayloadCommitProof");
        if (fields.size() != 17 && fields.size() != 18) {
            throw new IllegalArgumentException("CanonicalPayloadCommitProof has an invalid field count");
        }
        int index = 0;
        if (QueryCodecSupport.uint32(fields.get(index++), 1) != PROOF_VERSION) {
            throw new IllegalArgumentException("unsupported CanonicalPayloadCommitProof version");
        }
        ...
        final DelayMessageId messageId =
                new DelayMessageId(QueryCodecSupport.fixed(fields.get(index++), 6, DelayMessageId.LENGTH));
        final ProfileRef profile = ProfileRef.decode(QueryCodecSupport.nested(fields.get(index++), 7));
        final long trustSetVersion = QueryCodecSupport.uint64Bits(fields.get(index++), 8);
        ...
        final CanonicalPayloadCommitProof result = new CanonicalPayloadCommitProof(
                reservationId,
                tenantRoutingScope,
                route,
                partition,
                ...
                proofId,
                signature);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CanonicalPayloadCommitProof");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        if (!(other instanceof CanonicalPayloadCommitProof that)) {
            return false;
        }
        return trustSetVersion == that.trustSetVersion
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                Arrays.hashCode(routeIncarnationUuid),
                partition,
                delayMessageId,
                objectStoreProfile,
                trustSetVersion,
    ...
    private static byte[] computeProofId(
            final byte[] reservationId,
            final byte[] tenantRoutingScope,
            final byte[] routeIncarnationUuid,
            final int partition,
    ...
    private static byte[] proofIdFields(
            final byte[] reservationId,
            final byte[] tenantRoutingScope,
            final byte[] routeIncarnationUuid,
            final int partition,
    ...
    private byte[] signatureDigest() {
        return Bytes.sha256(Bytes.utf8("nereus-delay-payload-proof-signature\0"), signatureFields());
    }
