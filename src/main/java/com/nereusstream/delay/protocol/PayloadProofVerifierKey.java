...
public final class PayloadProofVerifierKey {
    ...
    public PayloadProofVerifierKey(
            final int keyVersion,
            final byte[] publicKey,
            final long verifyNotBeforeEpochMs,
            final long verifyNotAfterEpochMs) {
        if (keyVersion == 0 || verifyNotBeforeEpochMs < 0 || verifyNotAfterEpochMs <= verifyNotBeforeEpochMs) {
    ...
    public PublicKey toPublicKey() {
        try {
            return KeyFactory.getInstance("Ed25519")
                    .generatePublic(new X509EncodedKeySpec(Bytes.concat(ED25519_X509_PREFIX, publicKey)));
        } catch (GeneralSecurityException exception) {
            throw new IllegalArgumentException("invalid Ed25519 public key", exception);
        }
    }

    public static PayloadProofVerifierKey fromPublicKey(
            final int keyVersion,
        ...
        if (encoded == null
                || encoded.length != ED25519_X509_PREFIX.length + PUBLIC_KEY_LENGTH
        ...
        return new PayloadProofVerifierKey(
                keyVersion,
                Arrays.copyOfRange(encoded, ED25519_X509_PREFIX.length, encoded.length),
    ...
    public static PayloadProofVerifierKey decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "PayloadProofVerifierKey");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4}, "PayloadProofVerifierKey");
        final PayloadProofVerifierKey result = new PayloadProofVerifierKey(
                QueryCodecSupport.uint32Bits(fields.get(0), 1),
                QueryCodecSupport.fixed(fields.get(1), 2, PUBLIC_KEY_LENGTH),
                QueryCodecSupport.uint(fields.get(2), 3),
                QueryCodecSupport.uint(fields.get(3), 4));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "PayloadProofVerifierKey");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof PayloadProofVerifierKey that
    ...
    @Override
    public int hashCode() {
        return Objects.hash(keyVersion, Arrays.hashCode(publicKey), verifyNotBeforeEpochMs, verifyNotAfterEpochMs);
    }
}
