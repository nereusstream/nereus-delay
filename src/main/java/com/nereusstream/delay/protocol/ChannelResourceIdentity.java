...
public final class ChannelResourceIdentity {
    ...
    private final BrokerResourceIdentity evidenceResource;
    ...
    private final CredentialUseLease credentialUseLease;

    public ChannelResourceIdentity(
            ...
            final CredentialUseLease credentialUseLease) {
        ...
        this.producerOrTransactionalIdentity =
                nonEmpty(producerOrTransactionalIdentity, "producerOrTransactionalIdentity");
        this.producerOrTransactionalIdentitySha256 =
                fixed(producerOrTransactionalIdentitySha256, HASH_LENGTH, "producerOrTransactionalIdentitySha256");
        if (!Arrays.equals(
                this.producerOrTransactionalIdentitySha256, Bytes.sha256(this.producerOrTransactionalIdentity))) {
        ...
        this.resourceGuardAttestationDigest =
                fixed(resourceGuardAttestationDigest, HASH_LENGTH, "resourceGuardAttestationDigest");
        ...
        this.resolvedCredentialVersionFingerprintDigest = fixed(
                resolvedCredentialVersionFingerprintDigest, HASH_LENGTH, "resolvedCredentialVersionFingerprintDigest");
        this.credentialUseLease = Objects.requireNonNull(credentialUseLease, "credentialUseLease");
        if (credentialUseLease.kind() != CredentialUseKind.DESTINATION_CHANNEL
                || credentialUseLease.profile().profileKind() != ProfileKind.DESTINATION
                ...
                || !Arrays.equals(
                        credentialUseLease.resolvedCredentialFingerprintDigest(),
        ...
        if (!Arrays.equals(
                credentialUseLease.holderScopeDigest(),
                CredentialUseLease.destinationChannelHolderScope(canonicalFieldsThrough13()))) {
    ...
    public BrokerResourceIdentity evidenceResource() {
    ...
    public CredentialUseLease credentialUseLease() {
    ...
    public static ChannelResourceIdentity decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "ChannelResourceIdentity");
        if (fields.size() != 15 && fields.size() != 17) {
            throw new IllegalArgumentException("ChannelResourceIdentity has an unexpected field count");
        }
        ...
        QueryCodecSupport.requireNumbers(fields, expected, "ChannelResourceIdentity");
        final ChannelResourceIdentity result = new ChannelResourceIdentity(
                AdapterKind.fromWire(QueryCodecSupport.uint(fields.get(0), 1)),
                ChannelKind.fromWire(QueryCodecSupport.uint(fields.get(1), 2)),
                QueryCodecSupport.fixed(fields.get(2), 3, HASH_LENGTH),
                QueryCodecSupport.fixed(fields.get(3), 4, LANE_INCARNATION_LENGTH),
                BrokerResourceIdentity.decode(QueryCodecSupport.nested(fields.get(4), 5)),
                ...
                hasEvidence ? BrokerResourceIdentity.decode(QueryCodecSupport.nested(fields.get(10), 11)) : null,
                ...
                nonZero(QueryCodecSupport.uint(fields.get(hasEvidence ? 13 : 11), 14), "credentialBindingGeneration"),
                ...
                CredentialUseLease.decode(QueryCodecSupport.nested(fields.get(hasEvidence ? 16 : 14), 17)));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ChannelResourceIdentity");
        return result;
    }
    ...
    private void validateAdapterAndChannel() {
        final boolean targetMatches = (adapterKind == AdapterKind.KAFKA
                        && targetResource.kind() == BrokerResourceIdentity.Kind.KAFKA)
        ...
        if (channelKind == ChannelKind.KAFKA_TRANSACTIONAL_RECEIPT && adapterKind != AdapterKind.KAFKA) {
            throw new IllegalArgumentException("Kafka transactional channel requires Kafka adapter");
        }
        if ((channelKind == ChannelKind.PULSAR_DEDUP_PRODUCER || channelKind == ChannelKind.PULSAR_NATIVE_DELAYED)
                && adapterKind != AdapterKind.PULSAR) {
    ...
    public boolean equals(final Object other) {
        return other instanceof ChannelResourceIdentity that
                ...
                && Arrays.equals(producerOrTransactionalIdentitySha256, that.producerOrTransactionalIdentitySha256)
                ...
                && Arrays.equals(
                        resolvedCredentialVersionFingerprintDigest, that.resolvedCredentialVersionFingerprintDigest)
    ...
    public int hashCode() {
        return Objects.hash(
                adapterKind,
                channelKind,
                Arrays.hashCode(destinationLaneId),
                Arrays.hashCode(laneIncarnation),
                targetResource,
                physicalPartition,
                channelGeneration,
                channelSlot,
                Arrays.hashCode(producerOrTransactionalIdentity),
                Arrays.hashCode(producerOrTransactionalIdentitySha256),
                evidenceResource,
                evidenceGeneration,
                Arrays.hashCode(resourceGuardAttestationDigest),
                credentialBindingGeneration,
                Arrays.hashCode(credentialBindingDigest),
                Arrays.hashCode(resolvedCredentialVersionFingerprintDigest),
                credentialUseLease);
    }
}
