...
public final class CommandQueuedReceiptV1 {
    ...
    private CommandQueuedReceiptV1(
    ...
    public static CommandQueuedReceiptV1 create(
        ...
        return new CommandQueuedReceiptV1(
                command,
                sourcePosition,
                brokerAck,
                receiptQueryUntilEpochMs,
                physicalEnqueueAttemptId,
                Bytes.sha256(Bytes.utf8("nereus-delay-command-queued-receipt-v1\0"), fields));
    }
    ...
    public static CommandQueuedReceiptV1 decodePayload(final byte[] encoded) {
        ...
        if (!Bytes.constantTimeEquals(
                digest, Bytes.sha256(Bytes.utf8("nereus-delay-command-queued-receipt-v1\0"), canonical))) {
            throw new IllegalArgumentException("CommandQueuedReceipt payload digest mismatch");
        }
        final CommandQueuedReceiptV1 result =
                new CommandQueuedReceiptV1(command, sourcePosition, brokerAck, queryUntil, attemptId, digest);
    ...
    public record PreparedCommandRef(
        ...
        public static PreparedCommandRef from(final PreparedCommand command) {
            ...
            return new PreparedCommandRef(
                    ...
                    command.type(),
                    ProtocolTuple.managedCommandV1(),
                    command.commandHash(),
                    command.retryUntilEpochMs(),
                    Bytes.sha256(CommandCodec.encodeFrameV1(command)));
        }
    ...
    public record ProtocolTuple(
            int framingVersion, int logEnvelopeVersion, int recordKind, int envelopeVersion, int bodyVersion) {
        public ProtocolTuple {
            if (framingVersion != 1
                    || logEnvelopeVersion != 1
                    || recordKind != ShardLogFrame.CLIENT_COMMAND_KIND
                    || envelopeVersion != 1
                    || bodyVersion != 1) {
        ...
        private static ProtocolTuple decode(final byte[] encoded) {
            ...
            return new ProtocolTuple(
                    uint32Int(fields.get(0), 1),
                    uint32Int(fields.get(1), 2),
                    uint32Int(fields.get(2), 3),
                    uint32Int(fields.get(3), 4),
                    uint32Int(fields.get(4), 5));
        }
    }
    ...
    public record KafkaQueuedAck(
        ...
        private static KafkaQueuedAck decode(final byte[] encoded) {
            ...
            final List<CanonicalProtobuf.Reader.Field> resource =
                    read(nested(fields.get(0), 1), "KafkaResourceIdentityV1");
    ...
    public record PulsarQueuedAck(
        ...
        private static PulsarQueuedAck decode(final byte[] encoded) {
            ...
            requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8});
            final List<CanonicalProtobuf.Reader.Field> resource =
                    read(nested(fields.get(0), 1), "PulsarResourceIdentityV1");
