 ...
 */
public final class ProtocolDowngradePackage {
    public static final int VERSION = 1;
    private static final int HASH_LENGTH = 32;
    private static final int MAX_CANONICAL_BYTES = 1 << 20;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-protocol-downgrade-package\0");

    private final ProtocolTuple fallbackTuple;
    private final ProtocolTuple activatedTuple;
    ...
    public ProtocolDowngradePackage(
            final ProtocolTuple fallbackTuple,
            final ProtocolTuple activatedTuple,
    ...
    public ProtocolTuple fallbackTuple() {
        return fallbackTuple;
    }

    public ProtocolTuple activatedTuple() {
     ...
     */
    public void validateAgainst(final ProtocolActivationState state) {
        Objects.requireNonNull(state, "state");
        if (state.activation(fallbackTuple) == null) {
            throw new IllegalStateException("downgrade fallback tuple is not active");
        }
        final ProtocolActivationState.Activation activation = state.activation(activatedTuple);
        if (activation == null) {
    ...
    public static ProtocolDowngradePackage decode(final byte[] encoded) {
        ...
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "ProtocolDowngradePackage");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8}, "ProtocolDowngradePackage");
        ...
        final ProtocolDowngradePackage result = new ProtocolDowngradePackage(
        ...
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ProtocolDowngradePackage");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof ProtocolDowngradePackage that
