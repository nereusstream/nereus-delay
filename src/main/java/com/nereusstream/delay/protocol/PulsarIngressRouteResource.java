...
public final class PulsarIngressRouteResource implements IngressRouteResource {
    private final String authenticatedClusterId;
    private final String physicalTopicBase;
    private final List<PulsarPhysicalPartitionIdentity> partitions;

    public PulsarIngressRouteResource(
            final String authenticatedClusterId,
            final String physicalTopicBase,
            final List<PulsarPhysicalPartitionIdentity> partitions) {
        ...
        final List<PulsarPhysicalPartitionIdentity> copy = new ArrayList<>(partitions.size());
        for (int index = 0; index < partitions.size(); index++) {
            final PulsarPhysicalPartitionIdentity partition =
    ...
    public List<PulsarPhysicalPartitionIdentity> partitions() {
        return partitions;
    }
    ...
    public byte[] canonicalBytes() {
        return CanonicalProtobuf.message(
                output -> CanonicalProtobuf.bytes(output, 2, CanonicalProtobuf.message(fields -> {
                    ...
                    for (PulsarPhysicalPartitionIdentity partition : partitions) {
                        CanonicalProtobuf.bytes(fields, 4, partition.canonicalBytes());
                    }
                })));
    }

    public static PulsarIngressRouteResource decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> outer = QueryCodecSupport.read(encoded, "IngressRouteResource");
        QueryCodecSupport.requireNumbers(outer, new int[] {2}, "IngressRouteResource");
        final List<CanonicalProtobuf.Reader.Field> fields =
                QueryCodecSupport.read(QueryCodecSupport.nested(outer.get(0), 2), "PulsarIngressRouteResource", true);
        if (fields.size() < 4
                || fields.get(0).number() != 1
                || fields.get(1).number() != 2
                || fields.get(2).number() != 3) {
            throw new IllegalArgumentException("PulsarIngressRouteResource fields are incomplete");
        }
        ...
        final List<PulsarPhysicalPartitionIdentity> partitions = new ArrayList<>(partitionCount);
        for (int index = 0; index < partitionCount; index++) {
            if (fields.get(index + 3).number() != 4) {
                throw new IllegalArgumentException("Pulsar Route partition field order is invalid");
            }
            partitions.add(PulsarPhysicalPartitionIdentity.decode(QueryCodecSupport.nested(fields.get(index + 3), 4)));
        }
        final PulsarIngressRouteResource result = new PulsarIngressRouteResource(
                utf8(QueryCodecSupport.bytes(fields.get(0), 1)),
                utf8(QueryCodecSupport.bytes(fields.get(1), 2)),
                partitions);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "IngressRouteResource");
        return result;
    }

    public PulsarPhysicalPartitionIdentity partition(final int partition) {
    ...
    public boolean equals(final Object other) {
        return other instanceof PulsarIngressRouteResource that
