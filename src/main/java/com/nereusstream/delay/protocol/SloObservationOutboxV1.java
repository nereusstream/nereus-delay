...
public final class SloObservationOutboxV1 {
    ...
    private SloObservationOutboxV1(
            final SloSampleStartV1 start, final SloSampleFinalV1 finalObservation, final byte[] suppliedDigest) {
    ...
    public SloObservationOutboxV1 mergeFinal(final SloSampleFinalV1 incoming, final SloThresholdDirectionV1 direction) {
        Objects.requireNonNull(direction, "direction");
        validateFinalAgainstStart(Objects.requireNonNull(incoming, "incoming"), start, direction);
        final SloSampleFinalV1 merged =
                finalObservation == null ? incoming : SloSampleFinalV1.merge(finalObservation, incoming, direction);
     ...
     */
    public SloObservationOutboxV1 mergeFinal(
            final SloSampleFinalV1 incoming,
    ...
    public static SloObservationOutboxV1 decode(final byte[] encoded) {
        ...
        final SloSampleFinalV1 finalObservation =
                fields.size() == 4 ? SloSampleFinalV1.decode(QueryCodecSupport.nested(fields.get(2), 3)) : null;
        final SloObservationOutboxV1 result = new SloObservationOutboxV1(
    ...
    public boolean equals(final Object other) {
        return other instanceof SloObservationOutboxV1 that && Arrays.equals(canonicalBytes(), that.canonicalBytes());
    }
