...
public final class CheckpointCatalogResult {
    ...
    private final List<CheckpointSummary> summaries;

    public CheckpointCatalogResult(
            final ShardSubject shard,
            final byte[] recoveryLineageId,
            final byte[] floorCheckpointId,
            final byte[] floorManifestSha256,
            final long catalogGeneration,
            final List<CheckpointSummary> summaries) {
        ...
        for (CheckpointSummary summary : this.summaries) {
    ...
    public List<CheckpointSummary> summaries() {
        return summaries;
    }
    ...
    public static CheckpointCatalogResult decode(final byte[] encoded) {
        ...
        if (fields.size() < 5) {
            throw new IllegalArgumentException("CheckpointCatalogResult has too few fields");
        }
        if (fields.get(0).number() != 1
            ...
            throw new IllegalArgumentException("CheckpointCatalogResult has invalid required field order");
        }
        final List<CheckpointSummary> summaries = new ArrayList<>();
        for (int index = 5; index < fields.size(); index++) {
            if (fields.get(index).number() != 6) {
                throw new IllegalArgumentException("CheckpointCatalogResult summaries must use field 6");
            }
            summaries.add(CheckpointSummary.decode(QueryCodecSupport.nested(fields.get(index), 6)));
        }
        final CheckpointCatalogResult result = new CheckpointCatalogResult(
                ...
                summaries);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CheckpointCatalogResult");
        return result;
    }

    private static List<CheckpointSummary> sortedUnique(
            final List<CheckpointSummary> values, final ShardSubject shard) {
        Objects.requireNonNull(values, "summaries");
        final List<CheckpointSummary> result = new ArrayList<>(values.size());
        for (CheckpointSummary value : values) {
            ...
            if (!result.isEmpty()) {
                final CheckpointSummary previous = result.get(result.size() - 1);
        ...
        int floorCount = 0;
        for (CheckpointSummary summary : result) {
            if (summary.recoveryFloor()) {
                floorCount++;
            }
        }
    ...
    public boolean equals(final Object other) {
        return other instanceof CheckpointCatalogResult that
    ...
    public int hashCode() {
        return Objects.hash(
                shard,
                Arrays.hashCode(recoveryLineageId),
                Arrays.hashCode(floorCheckpointId),
                Arrays.hashCode(floorManifestSha256),
                catalogGeneration,
                summaries);
    }
}
