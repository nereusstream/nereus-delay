...
public final class NativePreparedDeliveryV1 {
    ...
    private NativePreparedDeliveryV1(
    ...
    public static NativePreparedDeliveryV1 create(
            final byte[] nativeDeliveryId,
            final ProfileRefV1 destination,
            final ProfileRefV1 capability,
            final PulsarBrokerResourceIdentityV1 target,
        ...
        return new NativePreparedDeliveryV1(
                nativeDeliveryId,
                destination,
                capability,
                target,
    ...
    public static NativePreparedDeliveryV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "NativePreparedDeliveryV1");
        ...
        final PulsarBrokerResourceIdentityV1 target =
                PulsarBrokerResourceIdentityV1.decode(QueryCodecSupport.nested(fields.get(4), 5));
        ...
        final NativeCapabilitySnapshotV1 snapshot =
                NativeCapabilitySnapshotV1.decode(QueryCodecSupport.nested(fields.get(index + 2), 12));
        ...
        final NativePreparedDeliveryV1 result = new NativePreparedDeliveryV1(
                nativeDeliveryId,
                destination,
                capability,
                target,
        ...
        final byte[] expected = Bytes.sha256(
                Bytes.utf8("nereus-delay-native-submission-v1\0"),
                canonicalFields(
                        nativeDeliveryId,
                        destination,
                        capability,
    ...
    public NativePreparedRefV1 preparedRef() {
        return new NativePreparedRefV1(
                nativeDeliveryId,
                submissionHash,
                destination,
                target,
    ...
    private static byte[] canonicalFields(
            ...
            final byte[] resourceGuardAttestationSha256,
            final long capabilityExpiryEpochMs) {
        final NativePreparedDeliveryV1 fields = new NativePreparedDeliveryV1(
                nativeDeliveryId,
                destination,
                capability,
                target,
