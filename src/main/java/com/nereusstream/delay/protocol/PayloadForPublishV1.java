...
public final class PayloadForPublishV1 {
    ...
    private PayloadForPublishV1(
            final long length,
            final byte[] payloadSha256,
            final byte[] inlinePayload,
            final CommittedPayloadDescriptorV1 object) {
        if (length < 0) {
    ...
    public static PayloadForPublishV1 object(final CommittedPayloadDescriptorV1 descriptor) {
        return new PayloadForPublishV1(
    ...
    public static PayloadForPublishV1 decode(final byte[] encoded) {
        ...
        if (fields.get(2).number() == 3) {
            ...
            result = objectWithDeclaredValues(
                    length, hash, CommittedPayloadDescriptorV1.decode(QueryCodecSupport.nested(fields.get(2), 4)));
        } else {
    ...
    private static PayloadForPublishV1 objectWithDeclaredValues(
            final long length, final byte[] hash, final CommittedPayloadDescriptorV1 descriptor) {
        return new PayloadForPublishV1(length, hash, null, descriptor);
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof PayloadForPublishV1 that
                && length == that.length
