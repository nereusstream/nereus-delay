...
public final class RoutePartitionPolicy {
    ...
    public RoutePartitionPolicy(
            final int partition,
            final ActivationBarrier activationBarrier,
            final QuotaGrantRef quotaGrant,
            final long brokerGuardAttestationGeneration,
    ...
    public static RoutePartitionPolicy decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "RoutePartitionPolicy");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5}, "RoutePartitionPolicy");
        final RoutePartitionPolicy result = new RoutePartitionPolicy(
                QueryCodecSupport.uint32Bits(fields.get(0), 1),
                ActivationBarrier.decode(QueryCodecSupport.nested(fields.get(1), 2)),
                QuotaGrantRef.decode(QueryCodecSupport.nested(fields.get(2), 3)),
                QueryCodecSupport.uint64Bits(fields.get(3), 4),
                QueryCodecSupport.fixed(fields.get(4), 5, 32));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "RoutePartitionPolicy");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof RoutePartitionPolicy that
                && partition == that.partition
                && activationBarrier.equals(that.activationBarrier)
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                activationBarrier,
                quotaGrant,
                brokerGuardAttestationGeneration,
