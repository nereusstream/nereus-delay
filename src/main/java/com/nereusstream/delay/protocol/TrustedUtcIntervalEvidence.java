...
public final class TrustedUtcIntervalEvidence {
    ...
    public TrustedUtcIntervalEvidence(
            final long earliestEpochMs,
            final long latestEpochMs,
            final Source source,
            final byte[] sourceId,
            final long sourceConfigGeneration,
            final long sampleSequence,
            final long monotonicAnchorNs,
            final byte[] sourceEvidenceSha256,
    ...
    public byte[] canonicalBytes() {
        return CanonicalProtobuf.message(output -> {
            ...
            CanonicalProtobuf.uint64Bits(output, 5, sourceConfigGeneration);
    ...
    public static TrustedUtcIntervalEvidence decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields =
                readAll(new CanonicalProtobuf.Reader(Objects.requireNonNull(encoded, "encoded")));
        ...
        final TrustedUtcIntervalEvidence result = new TrustedUtcIntervalEvidence(
                signedNonNegative(fields.get(0), 1),
                signedNonNegative(fields.get(1), 2),
                Source.fromWire(unsigned(fields.get(2), 3)),
                bytes(fields.get(3), 4),
                rawUint64(fields.get(4), 5),
                rawUint64(fields.get(5), 6),
                rawUint64(fields.get(6), 7),
                fixed(bytes(fields.get(7), 8), HASH_LENGTH, "sourceEvidenceSha256"),
                uint32Bits(fields.get(8), 9),
                fields.size() == 10
                        ? fixed(bytes(fields.get(9), 10), SIGNATURE_LENGTH, "sourceSignature")
                        : new byte[0]);
    ...
    private static long rawUint64(final CanonicalProtobuf.Reader.Field field, final int number) {
        if (field.number() != number || field.wireType() != 0) {
            throw new IllegalArgumentException("invalid trusted UTC uint64 field " + number);
        }
        return field.unsignedValue();
    }
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                latestEpochMs,
                source,
                Arrays.hashCode(sourceId),
                sourceConfigGeneration,
                sampleSequence,
                monotonicAnchorNs,
                Arrays.hashCode(sourceEvidenceSha256),
                sourceKeyVersion,
                Arrays.hashCode(sourceSignature));
    }
