...
public final class LaneControlResult {
    ...
    public LaneAdmissionGate admissionGate() {
    ...
    public static LaneControlResult decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "LaneControlResult");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6}, "LaneControlResult");
        final LaneControlResult result = new LaneControlResult(
                ...
                LaneAdmissionGate.fromWire(QueryCodecSupport.uint(fields.get(3), 4)),
                QueryCodecSupport.uint(fields.get(4), 5),
                StableCode.fromWire(QueryCodecSupport.uint32(fields.get(5), 6)));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "LaneControlResult");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof LaneControlResult that
                && laneControlVersion == that.laneControlVersion
                && outstandingAttempts == that.outstandingAttempts
