...
public final class RecoveryPin {
    private static final int ID_LENGTH = 16;
    private static final int HASH_LENGTH = 32;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-recovery-pin\0");

    private final byte[] pinId;
    private final ShardSubject shard;
    private final OwnerIdentity owner;
    private final RecoveryCandidateRef candidate;
    private final RecoveryFloorRef observedFloor;
    ...
    public RecoveryPin(
            ...
            final RecoveryCandidateRef candidate,
            final RecoveryFloorRef observedFloor,
            final long observedCatalogGeneration,
            final byte[] oxiaSessionIdentityDigest) {
        ...
        if (observedCatalogGeneration == 0 || observedCatalogGeneration != observedFloor.catalogGeneration()) {
        ...
        this.oxiaSessionIdentityDigest = fixed(oxiaSessionIdentityDigest, HASH_LENGTH, "oxiaSessionIdentityDigest");
    ...
    public RecoveryCandidateRef candidate() {
        return candidate;
    }

    public RecoveryFloorRef observedFloor() {
    ...
    public static RecoveryPin decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "RecoveryPin");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8, 9}, "RecoveryPin");
        if (QueryCodecSupport.uint32(fields.get(0), 1) != 1) {
            throw new IllegalArgumentException("unsupported RecoveryPin version");
        }
        final RecoveryPin result = new RecoveryPin(
                QueryCodecSupport.fixed(fields.get(1), 2, ID_LENGTH),
                ShardSubject.decode(QueryCodecSupport.nested(fields.get(2), 3)),
                OwnerIdentity.decode(QueryCodecSupport.nested(fields.get(3), 4)),
                RecoveryCandidateRef.decode(QueryCodecSupport.nested(fields.get(4), 5)),
                RecoveryFloorRef.decode(QueryCodecSupport.nested(fields.get(5), 6)),
        ...
        if (!Bytes.constantTimeEquals(digest, result.pinDigest)) {
            throw new IllegalArgumentException("RecoveryPin digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "RecoveryPin");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof RecoveryPin that && Arrays.equals(canonicalBytes(), that.canonicalBytes());
    }
