 ...
 */
public final class SchedulerProjections {
    private static final int VERSION = 1;
    private static final int HASH_LENGTH = 32;

    private SchedulerProjections() {}
    ...
    public static final class ReadyDiscoveryCursor {
        ...
        public ReadyDiscoveryCursor(
                final byte[] lastScannedReadyKey, final long wrapGeneration, final long activeRingGeneration) {
            ...
            this.digest =
                    SchedulerProjections.digest("nereus-delay-scheduler-ready-discovery-cursor\0", fieldsOneToFive());
        }

        private ReadyDiscoveryCursor(
                final byte[] lastScannedReadyKey,
                final long wrapGeneration,
        ...
        public static ReadyDiscoveryCursor decode(final byte[] encoded) {
            final var fields = read(encoded, true, "SchedulerReadyDiscoveryCursor");
            if (fields.size() < 4
                    || fields.size() > 6
                    || number(fields, 0) != 1
                    || number(fields, fields.size() - 2) != 5
                    || number(fields, fields.size() - 1) != 6) {
                throw new IllegalArgumentException("invalid SchedulerReadyDiscoveryCursor fields");
            }
            int index = 1;
            byte[] key = null;
            ...
            if (number(fields, index) != 4 || number(fields, index + 1) != 5 || index + 2 != fields.size() - 1) {
                throw new IllegalArgumentException("invalid ready discovery cursor field order");
            }
            final ReadyDiscoveryCursor result = new ReadyDiscoveryCursor(
                    key,
                    uint(fields.get(index), 4),
                    uint(fields.get(index + 1), 5),
                    fixed(bytes(fields.get(fields.size() - 1), 6), "digest"));
            requireDigest(result.digest, result.fieldsOneToFive(), "nereus-delay-scheduler-ready-discovery-cursor\0");
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerReadyDiscoveryCursor");
            return result;
        }
    ...
    public static final class RingEntry {
        ...
        public RingEntry(final DestinationLaneId laneId, final byte[] laneIncarnation, final long observedLaneVersion) {
        ...
        public static RingEntry decode(final byte[] encoded) {
            final var fields = read(encoded, false, "SchedulerRingEntry");
            requireNumbers(fields, new int[] {1, 2, 3}, "SchedulerRingEntry");
            final RingEntry result = new RingEntry(
                    new DestinationLaneId(fixedLength(bytes(fields.get(0), 1), DestinationLaneId.LENGTH, "laneId")),
                    fixedLength(bytes(fields.get(1), 2), 16, "laneIncarnation"),
                    uint(fields.get(2), 3));
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerRingEntry");
            return result;
        }

        @Override
        public boolean equals(final Object other) {
            return other instanceof RingEntry that
                    && observedLaneVersion == that.observedLaneVersion
    ...
    public static final class ActiveRing {
        ...
        public ActiveRing(
                final long ringGeneration,
                final long roundGeneration,
                final int nextIndex,
            ...
            if (nextIndex < 0
                    || (!this.entries.isEmpty() && nextIndex >= this.entries.size())
                    || (this.entries.isEmpty() && nextIndex != 0)) {
                throw new IllegalArgumentException("invalid scheduler ring nextIndex");
            }
            this.nextIndex = nextIndex;
            this.digest = SchedulerProjections.digest("nereus-delay-scheduler-active-ring\0", fieldsOneToFive());
        }
        ...
        public static ActiveRing decode(final byte[] encoded) {
            final var fields = read(encoded, true, "SchedulerActiveRing");
            if (fields.size() < 5
                    || number(fields, 0) != 1
                    || number(fields, 1) != 2
                    || number(fields, 2) != 3
                    || number(fields, 3) != 4
                    || number(fields, fields.size() - 1) != 6) {
                throw new IllegalArgumentException("invalid SchedulerActiveRing fields");
            }
            ...
            final ActiveRing result = new ActiveRing(
                    uint(fields.get(1), 2),
                    uint(fields.get(2), 3),
                    uint32(fields.get(3), 4),
                    entries,
                    bytes(fields.get(fields.size() - 1), 6));
            requireDigest(result.digest, result.fieldsOneToFive(), "nereus-delay-scheduler-active-ring\0");
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerActiveRing");
            return result;
        }
    ...
    public static final class DeficitEntry {
        ...
        public static DeficitEntry decode(final byte[] encoded) {
            final var fields = read(encoded, false, "SchedulerDeficitEntry");
            requireNumbers(fields, new int[] {1, 2, 3, 4}, "SchedulerDeficitEntry");
            final DeficitEntry result = new DeficitEntry(
                    new DestinationLaneId(fixedLength(bytes(fields.get(0), 1), DestinationLaneId.LENGTH, "laneId")),
                    fixedLength(bytes(fields.get(1), 2), 16, "laneIncarnation"),
                    uint(fields.get(2), 3),
                    uint(fields.get(3), 4));
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerDeficitEntry");
            return result;
        }
    }
    ...
    public static final class DeficitMap {
        ...
        public DeficitMap(final List<DeficitEntry> entries) {
            ...
            this.digest = SchedulerProjections.digest("nereus-delay-scheduler-deficit-map\0", fieldsOneAndTwo());
        }
        ...
        public static DeficitMap decode(final byte[] encoded) {
            final var fields = read(encoded, true, "SchedulerDeficitMap");
            if (fields.size() < 2 || number(fields, 0) != 1 || number(fields, fields.size() - 1) != 3) {
                throw new IllegalArgumentException("invalid SchedulerDeficitMap fields");
            }
            ...
            requireDigest(result.digest, result.fieldsOneAndTwo(), "nereus-delay-scheduler-deficit-map\0");
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerDeficitMap");
            return result;
        }
    ...
    public static final class Round {
        ...
        public Round(final long roundGeneration, final OwnerIdentity owner, final boolean recoveryFirstPass) {
            this.roundGeneration = roundGeneration;
            this.owner = Objects.requireNonNull(owner, "owner");
            this.recoveryFirstPass = recoveryFirstPass;
            this.digest = SchedulerProjections.digest("nereus-delay-scheduler-round\0", fieldsOneToFour());
        }

        private Round(
                final long roundGeneration,
                final OwnerIdentity owner,
                final boolean recoveryFirstPass,
                final byte[] digest) {
            this.roundGeneration = roundGeneration;
            this.owner = Objects.requireNonNull(owner, "owner");
            this.recoveryFirstPass = recoveryFirstPass;
        ...
        public static Round decode(final byte[] encoded) {
            final var fields = read(encoded, false, "SchedulerRound");
            requireNumbers(fields, new int[] {1, 2, 3, 4, 5}, "SchedulerRound");
            final Round result = new Round(
                    uint(fields.get(1), 2),
                    OwnerIdentity.decode(bytes(fields.get(2), 3)),
                    bool(fields.get(3), 4),
                    bytes(fields.get(4), 5));
            requireDigest(result.digest, result.fieldsOneToFour(), "nereus-delay-scheduler-round\0");
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerRound");
            return result;
        }
    ...
    public static final class LastServedEntry {
        ...
        public LastServedEntry(
                final DestinationLaneId laneId,
                final byte[] laneIncarnation,
                final long lastServedRound,
                final long serviceGapGeneration) {
            this.laneId = Objects.requireNonNull(laneId, "laneId");
            this.laneIncarnation = fixedLength(laneIncarnation, 16, "laneIncarnation");
        ...
        public static LastServedEntry decode(final byte[] encoded) {
            final var fields = read(encoded, false, "SchedulerLastServedEntry");
            requireNumbers(fields, new int[] {1, 2, 3, 4}, "SchedulerLastServedEntry");
            final LastServedEntry result = new LastServedEntry(
                    new DestinationLaneId(fixedLength(bytes(fields.get(0), 1), DestinationLaneId.LENGTH, "laneId")),
                    fixedLength(bytes(fields.get(1), 2), 16, "laneIncarnation"),
                    uint(fields.get(2), 3),
                    uint(fields.get(3), 4));
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerLastServedEntry");
            return result;
        }
    }
    ...
    public static final class LastServedMap {
        ...
        public LastServedMap(final List<LastServedEntry> entries) {
            ...
            this.digest = SchedulerProjections.digest("nereus-delay-scheduler-last-served-map\0", fieldsOneAndTwo());
        }
        ...
        public static LastServedMap decode(final byte[] encoded) {
            final var fields = read(encoded, true, "SchedulerLastServedMap");
            if (fields.size() < 2 || number(fields, 0) != 1 || number(fields, fields.size() - 1) != 3) {
                throw new IllegalArgumentException("invalid SchedulerLastServedMap fields");
            }
            ...
            requireDigest(result.digest, result.fieldsOneAndTwo(), "nereus-delay-scheduler-last-served-map\0");
            requireCanonical(encoded, result.canonicalBytes(), "SchedulerLastServedMap");
            return result;
        }
    ...
    private static List<CanonicalProtobuf.Reader.Field> read(
            final byte[] encoded, final boolean repeated, final String name) {
    ...
    private static List<RingEntry> uniqueOrdered(final List<RingEntry> values) {
        ...
        for (int left = 0; left < copy.size(); left++) {
            for (int right = left + 1; right < copy.size(); right++) {
                if (sameIdentity(
                        copy.get(left).laneId(),
                        copy.get(left).laneIncarnation(),
                        copy.get(right).laneId(),
    ...
    private static List<DeficitEntry> sortedDeficit(final List<DeficitEntry> values) {
        ...
        copy.sort((left, right) -> compareIdentity(
                identityLane(left), identityIncarnation(left), identityLane(right), identityIncarnation(right)));
        for (int index = 1; index < copy.size(); index++) {
            if (compareIdentity(
                            identityLane(copy.get(index - 1)),
                            identityIncarnation(copy.get(index - 1)),
                            identityLane(copy.get(index)),
                            identityIncarnation(copy.get(index)))
                    == 0) {
    ...
    private static boolean sameIdentity(
            final DestinationLaneId leftLane,
            final byte[] leftIncarnation,
            final DestinationLaneId rightLane,
            final byte[] rightIncarnation) {
