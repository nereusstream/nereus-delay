...
/**
 * Canonical payload for the durable ingress acknowledgement. It proves
 ...
 */
public final class CanonicalCommandQueuedReceipt {
    ...
    private CanonicalCommandQueuedReceipt(
            final PreparedCommandRef command,
    ...
    public static CanonicalCommandQueuedReceipt create(
        ...
        final byte[] fields =
                canonicalFields(command, sourcePosition, brokerAck, receiptQueryUntilEpochMs, physicalEnqueueAttemptId);
        return new CanonicalCommandQueuedReceipt(
                command,
                sourcePosition,
                brokerAck,
                receiptQueryUntilEpochMs,
                physicalEnqueueAttemptId,
                Bytes.sha256(Bytes.utf8("nereus-delay-command-queued-receipt\0"), fields));
    }

    public static CanonicalCommandQueuedReceipt decodeFrame(final byte[] frame) {
    ...
    public static CanonicalCommandQueuedReceipt decodePayload(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = read(encoded, "CanonicalCommandQueuedReceipt");
        ...
        if (nonNegative(fields.get(5), 6) != CAPABILITY_BITS) {
            throw new IllegalArgumentException("CommandQueuedReceipt capability bits are not the set");
        }
        ...
        if (!Bytes.constantTimeEquals(
                digest, Bytes.sha256(Bytes.utf8("nereus-delay-command-queued-receipt\0"), canonical))) {
            throw new IllegalArgumentException("CommandQueuedReceipt payload digest mismatch");
        }
        final CanonicalCommandQueuedReceipt result =
                new CanonicalCommandQueuedReceipt(command, sourcePosition, brokerAck, queryUntil, attemptId, digest);
    ...
    public boolean equals(final Object other) {
        if (!(other instanceof CanonicalCommandQueuedReceipt that)) {
            return false;
        }
        return command.equals(that.command)
                && sourcePosition.equals(that.sourcePosition)
                && brokerAck.equals(that.brokerAck)
                && receiptQueryUntilEpochMs == that.receiptQueryUntilEpochMs
    ...
    private static byte[] encodeSourcePosition(final SourcePosition position) {
        ...
        if (position instanceof KafkaSourcePosition kafka) {
            return CanonicalProtobuf.message(output ->
                    CanonicalProtobuf.bytes(output, KAFKA_SOURCE_BRANCH, CanonicalProtobuf.message(kafkaOutput -> {
                        CanonicalProtobuf.bytes(
                                kafkaOutput,
                                1,
                                kafka.shardId().routeIncarnation().bytes());
                        CanonicalProtobuf.bytes(
                                kafkaOutput, 2, kafka.authenticatedClusterId().getBytes(StandardCharsets.UTF_8));
                        CanonicalProtobuf.bytes(kafkaOutput, 3, uuidBytes(kafka.nativeTopicUuid()));
                        CanonicalProtobuf.uint32Bits(
                                kafkaOutput, 4, kafka.shardId().partition());
        ...
        return CanonicalProtobuf.message(output ->
                CanonicalProtobuf.bytes(output, PULSAR_SOURCE_BRANCH, CanonicalProtobuf.message(pulsarOutput -> {
                    CanonicalProtobuf.bytes(
                            pulsarOutput, 1, pulsar.shardId().routeIncarnation().bytes());
                    ...
                    CanonicalProtobuf.uint32Bits(
                            pulsarOutput, 4, pulsar.shardId().partition());
    ...
    private static SourcePosition decodeSourcePosition(final byte[] encoded) {
        ...
        if (branch.number() == KAFKA_SOURCE_BRANCH) {
            ...
            return new KafkaSourcePosition(
                    new ShardId(new RouteIncarnation(route), partition), cluster, topic, offset, leader, append);
        }
        if (branch.number() == PULSAR_SOURCE_BRANCH) {
            ...
            final PulsarSourcePosition.EntryKind entryKind =
                    switch (uint32Int(fields.get(8), 9)) {
            ...
            return new PulsarSourcePosition(
                    new ShardId(new RouteIncarnation(route), partition),
                    resource,
                    topic,
                    ledger,
                    entry,
    ...
    private static void validateSourceAndAck(final SourcePosition source, final SafeBrokerAck ack) {
        ...
        if (source instanceof PulsarSourcePosition pulsar && ack instanceof PulsarQueuedAck pulsarAck) {
            if (!Arrays.equals(pulsar.brokerResourceIncarnation(), pulsarAck.brokerResourceIncarnation())
                    ...
                    || pulsar.ledgerId() != pulsarAck.ledgerId()
                    || pulsar.entryId() != pulsarAck.entryId()
    ...
    private static Integer optionalVarint(
            final List<CanonicalProtobuf.Reader.Field> fields, final int number, final String name) {
    ...
    private static void requireNumbers(
        ...
        if (allowOptionalLeaderEpoch) {
            ...
            final int[] withOptional = fields.size() == expected.length ? expected : new int[] {1, 2, 3, 4, 5, 6, 7};
    ...
    /** The exact PreparedCommandRef projection, without command body bytes. */
    public record PreparedCommandRef(
            ShardId shardId,
            CommandId commandId,
        ...
        public static PreparedCommandRef from(final PreparedCommand command) {
            ...
            return new PreparedCommandRef(
                    ...
                    Bytes.sha256(CommandCodec.encodeManagedFrame(command)));
        }
        ...
        public boolean equals(final Object other) {
            ...
            return shardId.equals(that.shardId)
                    ...
                    && Arrays.equals(frameSha256, that.frameSha256);
        }
        ...
        static PreparedCommandRef decode(final byte[] encoded) {
            final List<CanonicalProtobuf.Reader.Field> fields = read(encoded, "PreparedCommandRef");
            requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8, 9});
            final ShardId shard =
                    new ShardId(new RouteIncarnation(fixed(fields.get(0), 1, 16)), uint32Bits(fields.get(1), 2));
            ...
            return new PreparedCommandRef(
                    ...
                    type,
                    tuple,
                    fixed(fields.get(6), 7, HASH_LENGTH),
                    nonNegative(fields.get(7), 8),
    ...
    public sealed interface SafeBrokerAck permits KafkaQueuedAck, PulsarQueuedAck {
        ...
        static SafeBrokerAck decode(final byte[] encoded) {
            final List<CanonicalProtobuf.Reader.Field> fields = read(encoded, "SafeBrokerAck");
            if (fields.size() != 1) {
                throw new IllegalArgumentException("SafeBrokerAck must select one branch");
            }
            return switch (fields.get(0).number()) {
                ...
                default -> throw new IllegalArgumentException("unknown SafeBrokerAck branch");
            };
        }
    }

    public record KafkaQueuedAck(
        ...
        public byte[] canonicalBytes() {
            return CanonicalProtobuf.message(
                    output -> CanonicalProtobuf.bytes(output, 1, CanonicalProtobuf.message(kafka -> {
                        CanonicalProtobuf.bytes(kafka, 1, CanonicalProtobuf.message(resource -> {
                            CanonicalProtobuf.bytes(
                                    resource, 1, authenticatedClusterId.getBytes(StandardCharsets.UTF_8));
        ...
        private static KafkaQueuedAck decode(final byte[] encoded) {
            final List<CanonicalProtobuf.Reader.Field> fields = read(encoded, "KafkaQueuedAck");
            ...
            final List<CanonicalProtobuf.Reader.Field> resource =
                    read(nested(fields.get(0), 1), "KafkaResourceIdentity");
            ...
            return new KafkaQueuedAck(
                    cluster,
                    topic,
                    uint32Bits(fields.get(1), 2),
                    uint64Bits(fields.get(2), 3),
                    leader,
                    nonNegative(fields.get(index), 5),
                    fixed(fields.get(index + 1), 6, HASH_LENGTH));
        }
        ...
        public int hashCode() {
            return Objects.hash(
                    ...
                    offset,
                    leaderEpoch,
                    brokerLogAppendTimeEpochMs,
                    Arrays.hashCode(responseSha256));
        }
    }

    public record PulsarQueuedAck(
            String authenticatedClusterId,
        ...
        public byte[] canonicalBytes() {
            return CanonicalProtobuf.message(
                    output -> CanonicalProtobuf.bytes(output, 2, CanonicalProtobuf.message(pulsar -> {
                        CanonicalProtobuf.bytes(pulsar, 1, CanonicalProtobuf.message(resource -> {
                            CanonicalProtobuf.bytes(
                                    resource, 1, authenticatedClusterId.getBytes(StandardCharsets.UTF_8));
        ...
        private static PulsarQueuedAck decode(final byte[] encoded) {
            final List<CanonicalProtobuf.Reader.Field> fields = read(encoded, "PulsarQueuedAck");
            requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8});
            final List<CanonicalProtobuf.Reader.Field> resource =
                    read(nested(fields.get(0), 1), "PulsarResourceIdentity");
            requireNumbers(resource, new int[] {1, 2, 3, 4});
            return new PulsarQueuedAck(
                    utf8(bytes(resource.get(0), 1), "authenticatedClusterId"),
                    ...
                    uint64Bits(resource.get(3), 4),
                    uint32Bits(fields.get(1), 2),
                    uint64Bits(fields.get(2), 3),
                    uint64Bits(fields.get(3), 4),
                    uint32Bits(fields.get(4), 5),
                    uint32Bits(fields.get(5), 6),
                    nonNegative(fields.get(6), 7),
                    fixed(fields.get(7), 8, HASH_LENGTH));
        }
        ...
        public boolean equals(final Object other) {
            ...
            return authenticatedClusterId.equals(that.authenticatedClusterId)
                    ...
                    && normalizedBatchIndex == that.normalizedBatchIndex
        ...
        public int hashCode() {
            return Objects.hash(
                    ...
                    batchSize,
                    brokerEntryTimestampEpochMs,
                    Arrays.hashCode(sendReceiptSha256));
        }
    }
