...
public final class DeliveryCapabilitySemanticV1 implements ProfileSemanticBodyV1 {
    ...
    public DeliveryCapabilitySemanticV1(
        ...
        if (adapterKind == AdapterKindV1.KAFKA
                && (timingCapabilityBits
                                & (TimingCapabilityV1.PULSAR_GUARDED_HANDOFF | TimingCapabilityV1.PULSAR_AUTO_FAST))
                        != 0) {
            throw new IllegalArgumentException("Kafka capability cannot carry Pulsar timing bits");
        }
        if (adapterKind == AdapterKindV1.PULSAR
                && outcomeCapability == OutcomeCapabilityV1.KAFKA_TRANSACTIONAL_RECEIPT) {
    ...
    public static DeliveryCapabilitySemanticV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields =
                QueryCodecSupport.read(encoded, "DeliveryCapabilitySemanticV1");
