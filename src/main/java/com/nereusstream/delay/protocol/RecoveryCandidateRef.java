...
public final class RecoveryCandidateRef {
    private static final int ID_LENGTH = 16;
    private static final int HASH_LENGTH = 32;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-recovery-candidate-ref\0");

    private final RecoveryCandidateKind kind;
    ...
    public RecoveryCandidateRef(
        ...
        if ((kind == RecoveryCandidateKind.LOCAL_STORE) != (storeIncarnation != null)) {
            throw new IllegalArgumentException("LOCAL_STORE requires store incarnation and catalog forbids it");
        }
        this.storeIncarnation =
                storeIncarnation == null ? null : nonZeroFixed(storeIncarnation, ID_LENGTH, "storeIncarnation");
        this.candidateDigest = Bytes.sha256(DIGEST_DOMAIN, fieldsOneToFive());
    }

    public RecoveryCandidateKind kind() {
        return kind;
    }
    ...
    public static RecoveryCandidateRef decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "RecoveryCandidateRef");
        if (fields.size() != 5 && fields.size() != 6) {
            throw new IllegalArgumentException("invalid RecoveryCandidateRef field count");
        }
        QueryCodecSupport.requireNumbers(
                fields,
                fields.size() == 5 ? new int[] {1, 2, 3, 4, 6} : new int[] {1, 2, 3, 4, 5, 6},
                "RecoveryCandidateRef");
        final RecoveryCandidateKind kind = RecoveryCandidateKind.fromWire(QueryCodecSupport.uint(fields.get(0), 1));
        final RecoveryCandidateRef result = new RecoveryCandidateRef(
                kind,
        ...
        if (!Bytes.constantTimeEquals(digest, result.candidateDigest)) {
            throw new IllegalArgumentException("RecoveryCandidateRef digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "RecoveryCandidateRef");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof RecoveryCandidateRef that && Arrays.equals(canonicalBytes(), that.canonicalBytes());
    }
