...
public record PayloadReference(
    ...
    public byte[] encode() {
        final byte[] base = Bytes.concat(
                Bytes.u32be(hasCommitIdentity() ? VERSION : LEGACY_VERSION),
                objectStoreProfileHash,
                Bytes.lp32(container),
                Bytes.lp32(objectKey),
                Bytes.lp32(immutableObjectVersion),
                Bytes.lp32(etag == null ? new byte[0] : etag),
                Bytes.u64be(length),
                payloadSha256);
    ...
    public static PayloadReference decode(final byte[] encoded) {
        ...
        final PayloadReference result = new PayloadReference(
                profile, container, key, version, etag.length == 0 ? null : etag, length, sha, reservationId, proofId);
