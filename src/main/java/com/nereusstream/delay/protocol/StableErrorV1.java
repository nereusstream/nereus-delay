...
public final class StableErrorV1 {
    ...
    public StableErrorV1(
            final FailureStageV1 stage,
            final StableCode code,
            final RetryabilityV1 retryability,
            final Long retryAtEpochMs,
            final CommandQueuedReceiptV1.PreparedCommandRef command,
            final NativePreparedRefV1 nativePrepared,
    ...
    public static StableErrorV1 of(
            final FailureStageV1 stage,
            final StableCode code,
            final Long retryAtEpochMs,
            final CommandQueuedReceiptV1.PreparedCommandRef command,
            final NativePreparedRefV1 nativePrepared,
            final Integer diagnosticCode) {
        return new StableErrorV1(
                stage, code, RetryabilityV1.forCode(code), retryAtEpochMs, command, nativePrepared, diagnosticCode);
    }
    ...
    public static StableErrorV1 decode(final byte[] encoded) {
        ...
        final StableErrorV1 result =
                new StableErrorV1(stage, code, retryability, retryAt, command, nativePrepared, diagnostic);
