...
public final class LaneQuotaUsageEntry {
    private static final int HASH_LENGTH = 32;
    private static final int INCARNATION_LENGTH = 16;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-lane-quota-usage-entry\0");
    ...
    public static LaneQuotaUsageEntry decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "LaneQuotaUsageEntry");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5}, "LaneQuotaUsageEntry");
        final byte[] entryDigest = QueryCodecSupport.fixed(fields.get(4), 5, HASH_LENGTH);
        final LaneQuotaUsageEntry result = new LaneQuotaUsageEntry(
        ...
        if (!Bytes.constantTimeEquals(entryDigest, result.entryDigest)) {
            throw new IllegalArgumentException("LaneQuotaUsageEntry digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "LaneQuotaUsageEntry");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof LaneQuotaUsageEntry that
                && usageRevision == that.usageRevision
                && laneId.equals(that.laneId)
                && Arrays.equals(laneIncarnation, that.laneIncarnation)
                && usage.equals(that.usage)
                && Arrays.equals(entryDigest, that.entryDigest);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
                laneId, Arrays.hashCode(laneIncarnation), usage, usageRevision, Arrays.hashCode(entryDigest));
    }
}
