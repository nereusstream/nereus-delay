...
public final class ClaimResultBody {
    ...
    private ClaimResultBody(
            final byte[] claimId,
            final byte[] messageId,
            final int generation,
            final byte[] laneId,
            final byte[] laneIncarnation,
            final ClaimPrecondition precondition,
            final int resultKind,
    ...
    public static ClaimResultBody decode(final byte[] canonicalBody) {
        ...
        final TrustedUtcIntervalEvidence observedAt = TrustedUtcIntervalEvidence.decode(nested(field(fields, 18), 18));
    ...
    public static ClaimPrecondition decodePrecondition(final byte[] encoded) {
        ...
        final ClaimPrecondition result = new ClaimPrecondition(
                encoded,
                fixed(field(fields, 1), 1, HASH_LENGTH),
                fixed(field(fields, 2), 2, MESSAGE_ID_LENGTH),
                uint32Bits(field(fields, 3), 3),
                bodyUnsigned(field(fields, 4), 4),
                fixed(field(fields, 5), 5, HASH_LENGTH),
                fixed(field(fields, 6), 6, INCARNATION_LENGTH),
                bodyUnsigned(field(fields, 7), 7),
                bodyUnsigned(field(fields, 8), 8),
                fixed(field(fields, 9), 9, HASH_LENGTH),
                materialization,
                nested(field(fields, 12), 12),
                bodyUnsigned(field(fields, 13), 13),
                owner,
                fixed(field(fields, 15), 15, INCARNATION_LENGTH),
                sourceWorkKind,
                intValue(field(fields, 17), 17),
                intValue(field(fields, 18), 18),
                fixed(field(fields, 19), 19, HASH_LENGTH),
                fixed(field(fields, 20), 20, HASH_LENGTH));
    ...
    public static final class ClaimPrecondition {
        ...
        private ClaimPrecondition(
            ...
            this.originalTimelineKeySha256 = fixed(originalTimelineKeySha256, HASH_LENGTH, "originalTimelineKeySha256");
            ...
            this.expectedObligationSetDigest =
                    fixed(expectedObligationSetDigest, HASH_LENGTH, "expectedObligationSetDigest");
            this.sourceTimelineSemanticDigest =
                    fixed(sourceTimelineSemanticDigest, HASH_LENGTH, "sourceTimelineSemanticDigest");
        }
        ...
        public byte[] originalTimelineKeySha256() {
            return copy(originalTimelineKeySha256);
        }

        public boolean hasMaterialization() {
            return materialization.length != 0;
        }
        ...
        public byte[] claimedCharge() {
            return copy(claimedCharge);
        }

        public long claimDeadline() {
        ...
        public int sourceWorkKind() {
            return sourceWorkKind;
        }

        public int expectedAdmissionsUsed() {
            return expectedAdmissionsUsed;
        }

        public int expectedUncertainRetryAdmissionsUsed() {
            return expectedUncertainRetryAdmissionsUsed;
        }

        public byte[] expectedObligationSetDigest() {
            return copy(expectedObligationSetDigest);
        }

        public byte[] sourceTimelineSemanticDigest() {
            return copy(sourceTimelineSemanticDigest);
        }
    }
}
