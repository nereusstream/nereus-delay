...
public final class MessagePrecondition {
    private final Long expectedGeneration;
    private final Long expectedStateVersion;

    public MessagePrecondition(final Long expectedGeneration, final Long expectedStateVersion) {
    ...
    public static MessagePrecondition decode(final byte[] encoded) {
        ...
        if (fields.size() > 2
                || !fields.isEmpty() && fields.get(0).number() != 1
                || fields.size() == 2 && fields.get(1).number() != 2) {
            throw new IllegalArgumentException("MessagePrecondition has invalid field order");
        }
        ...
        final MessagePrecondition result = new MessagePrecondition(generation, stateVersion);
        if (!Arrays.equals(encoded, result.canonicalBytes())) {
            throw new IllegalArgumentException("non-canonical MessagePrecondition");
        }
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof MessagePrecondition that
