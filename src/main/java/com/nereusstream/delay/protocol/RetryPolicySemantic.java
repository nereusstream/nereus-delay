 ...
 */
public final class RetryPolicySemantic {
    public static final int ENVELOPE_VERSION = 1;
    public static final int JITTER_ALGORITHM_VERSION = 1;
    public static final int HASH_LENGTH = 32;
    private static final String HASH_DOMAIN = "nereus-delay-retry-policy-semantic\0";
    ...
    private final UncertainPolicy uncertainPolicy;
    private final int maxUncertainRetries;
    private final DlqExportMode dlqExportMode;
    ...
    public RetryPolicySemantic(
            final byte[] policyId,
            final long version,
            final long initialBackoffMs,
            final long maxBackoffMs,
            final int maxPublishAdmissions,
            final long maxRetryDurationMs,
            final UncertainPolicy uncertainPolicy,
            final int maxUncertainRetries,
            final DlqExportMode dlqExportMode,
            final long dlqInitialBackoffMs,
            final long dlqMaxBackoffMs,
            final int dlqMaxAttempts,
            final long dlqMaxRetryDurationMs,
            final boolean dlqAllowPossibleDuplicate,
            final byte[] terminalPolicyDigest) {
        ...
        if (uncertainPolicy == UncertainPolicy.BOUNDED_RETRY_POSSIBLE_DUPLICATE
        ...
        validateDlq(
                ...
                dlqMaxBackoffMs,
                dlqMaxAttempts,
                dlqMaxRetryDurationMs,
                dlqAllowPossibleDuplicate);
    ...
    public long dlqRetryBackoffCap(final long completedAttemptNo) {
        if (dlqExportMode == DlqExportMode.NOT_CONFIGURED) {
    ...
    public UncertainPolicy uncertainPolicy() {
    ...
    public DlqExportMode dlqExportMode() {
    ...
    public RetryPolicyRef ref() {
        return new RetryPolicyRef(policyId, version, semanticHash);
    }
    ...
    public void validateFor(final OrderingMode orderingMode) {
        ...
        if (orderingMode == OrderingMode.DELIVERY_TIME_FIFO
                && uncertainPolicy == UncertainPolicy.BOUNDED_RETRY_POSSIBLE_DUPLICATE) {
    ...
    public static RetryPolicySemantic decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "RetryPolicySemantic");
        ...
        if (QueryCodecSupport.uint(fields.get(0), 1) != ENVELOPE_VERSION
            ...
            throw new IllegalArgumentException("unsupported RetryPolicySemantic version");
        }
        final RetryPolicySemantic result = new RetryPolicySemantic(
                ...
                UncertainPolicy.fromWire(QueryCodecSupport.uint(fields.get(7), 8)),
                QueryCodecSupport.uint32(fields.get(8), 9),
                DlqExportMode.fromWire(QueryCodecSupport.uint(fields.get(9), 10)),
                QueryCodecSupport.uint(fields.get(10), 11),
                QueryCodecSupport.uint(fields.get(11), 12),
                QueryCodecSupport.uint32(fields.get(12), 13),
                QueryCodecSupport.uint(fields.get(13), 14),
                QueryCodecSupport.bool(fields.get(14), 15),
        ...
        if (!Bytes.constantTimeEquals(result.semanticHash, QueryCodecSupport.fixed(fields.get(17), 18, HASH_LENGTH))) {
            throw new IllegalArgumentException("RetryPolicySemantic semantic hash mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "RetryPolicySemantic");
        return result;
    }
    ...
    private static long retryBackoffCap(
            final long initialBackoff, final long maxBackoff, final long completedAttemptNo) {
    ...
    public boolean equals(final Object other) {
        return other instanceof RetryPolicySemantic that
                && version == that.version
                && initialBackoffMs == that.initialBackoffMs
                && maxBackoffMs == that.maxBackoffMs
                && maxPublishAdmissions == that.maxPublishAdmissions
                && maxRetryDurationMs == that.maxRetryDurationMs
                && uncertainPolicy == that.uncertainPolicy
                && maxUncertainRetries == that.maxUncertainRetries
                && dlqExportMode == that.dlqExportMode
                && dlqInitialBackoffMs == that.dlqInitialBackoffMs
                && dlqMaxBackoffMs == that.dlqMaxBackoffMs
                && dlqMaxAttempts == that.dlqMaxAttempts
                && dlqMaxRetryDurationMs == that.dlqMaxRetryDurationMs
                && dlqAllowPossibleDuplicate == that.dlqAllowPossibleDuplicate
                && Arrays.equals(policyId, that.policyId)
                && Arrays.equals(terminalPolicyDigest, that.terminalPolicyDigest)
    ...
    public int hashCode() {
        return Objects.hash(
                Arrays.hashCode(policyId),
                version,
                initialBackoffMs,
                maxBackoffMs,
                maxPublishAdmissions,
                maxRetryDurationMs,
                uncertainPolicy,
                maxUncertainRetries,
                dlqExportMode,
                ...
                dlqMaxRetryDurationMs,
                dlqAllowPossibleDuplicate,
                Arrays.hashCode(terminalPolicyDigest),
