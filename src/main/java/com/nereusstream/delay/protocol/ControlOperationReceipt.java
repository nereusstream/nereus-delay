...
public final class ControlOperationReceipt {
    ...
    private ControlOperationReceipt(
    ...
    public static ControlOperationReceipt create(
            final byte[] operationId,
            final byte[] requestHash,
            final byte[] authenticatedScopeHash,
            final byte[] targetSnapshotHash,
    ...
    public static ControlOperationReceipt createWithQueryWindow(
            final byte[] operationId,
            final byte[] requestHash,
            final byte[] authenticatedScopeHash,
            final byte[] targetSnapshotHash,
    ...
    public static ControlOperationReceipt decodeFrame(final byte[] frame) {
    ...
    public static ControlOperationReceipt decodePayload(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "ControlOperationReceipt");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8, 9}, "ControlOperationReceipt");
        if (QueryCodecSupport.uint(fields.get(0), 1) != RECEIPT_VERSION) {
            throw new IllegalArgumentException("unsupported ControlOperationReceipt version");
        }
        ...
        final TrustedUtcIntervalEvidence registeredAt =
                TrustedUtcIntervalEvidence.decode(QueryCodecSupport.nested(fields.get(6), 7));
        final long queryUntil = QueryCodecSupport.uint(fields.get(7), 8);
        final byte[] digest = QueryCodecSupport.fixed(fields.get(8), 9, 32);
        final ControlOperationReceipt result = new ControlOperationReceipt(
                operationId, requestHash, scopeHash, targetHash, operationRevision, registeredAt, queryUntil, digest);
        final byte[] expected = Bytes.sha256(canonicalFields(
                operationId, requestHash, scopeHash, targetHash, operationRevision, registeredAt, queryUntil));
        if (!Bytes.constantTimeEquals(digest, expected)) {
            throw new IllegalArgumentException("ControlOperationReceipt digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.payload(), "ControlOperationReceipt");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        if (!(other instanceof ControlOperationReceipt that)) {
            return false;
        }
        return operationRevision == that.operationRevision
                && queryUntilEpochMs == that.queryUntilEpochMs
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                operationRevision,
                Arrays.hashCode(registeredAt.canonicalBytes()),
                queryUntilEpochMs,
