...
/**
 * Canonical active branch of the Registry {@code LaneRecord} value.
 *
 ...
 */
public final class ActiveLaneState {
    ...
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-active-lane-state\0");
    ...
    private final LaneRuntimeBlockReason runtimeBlockReason;
    ...
    private final LaneCircuitState circuitState;
    ...
    private final LaneRetirementProgress retirement;
    private final byte[] stateDigest;

    public ActiveLaneState(
            final DestinationLaneId laneId,
            final byte[] laneIncarnation,
            final AdmissionGate admissionGate,
            final RuntimeReadiness runtimeReadiness,
            final LaneRuntimeBlockReason runtimeBlockReason,
            ...
            final long schedulerWeight,
            final PublishAdmissionBody.ChargeVector laneUsage,
            final Long earliestActionAtEpochMs,
            final Long nextEligibleAtEpochMs,
            final LaneCircuitState circuitState,
            final long circuitOpenUntilEpochMs,
            final long consecutiveFailures,
            final long laneRetryBackoffUntilEpochMs,
            final long executorRetryAtEpochMs,
            final byte[] encodedReadyKey,
            final byte[] readyCertificate,
            final LaneRetirementProgress retirement) {
        ...
        if (admissionGate == AdmissionGate.ABSENT || admissionGate == AdmissionGate.RETIRED) {
            throw new IllegalArgumentException("ActiveLaneState cannot carry ABSENT or RETIRED gate");
        }
        ...
        if (circuitState == LaneCircuitState.OPEN && circuitOpenUntilEpochMs <= 0) {
            throw new IllegalArgumentException("OPEN circuit must have a positive open-until time");
        }
        if (circuitState == LaneCircuitState.CLOSED && circuitOpenUntilEpochMs != 0) {
        ...
        this.laneRetryBackoffUntilEpochMs = nonNegative(laneRetryBackoffUntilEpochMs, "laneRetryBackoffUntilEpochMs");
        this.executorRetryAtEpochMs = nonNegative(executorRetryAtEpochMs, "executorRetryAtEpochMs");
        final long minimumNextEligibleAt = Math.max(
                earliestActionAtEpochMs == null ? 0 : earliestActionAtEpochMs,
                Math.max(
                        circuitState == LaneCircuitState.OPEN ? this.circuitOpenUntilEpochMs : 0,
        ...
        final boolean hasCompleteTimingProjection = earliestActionAtEpochMs != null && nextEligibleAtEpochMs != null;
        ...
        if (readyCertificate != null) {
            ...
            if (!Arrays.equals(certificate.destinationLaneId(), laneId.bytes())
                ...
                throw new IllegalArgumentException("ReadyCertificate identity does not match ActiveLaneState");
            }
        }
    ...
    public LaneRuntimeBlockReason runtimeBlockReason() {
    ...
    public LaneCircuitState circuitState() {
        return circuitState;
    }
    ...
    public LaneRetirementProgress retirement() {
        return retirement;
    }
     ...
     */
    public ActiveLaneState withLocalProjection(
            final AdmissionGate nextAdmissionGate,
            final RuntimeReadiness nextRuntimeReadiness,
            final LaneRuntimeBlockReason nextRuntimeBlockReason,
        ...
        return withLocalProjection(
                nextAdmissionGate,
                nextRuntimeReadiness,
                nextRuntimeBlockReason,
                nextLaneControlVersion,
                nextLaneVersion,
                nextSchedulerWeight,
                nextLaneUsage,
                nextEarliestActionAtEpochMs,
    ...
    public ActiveLaneState withLocalProjection(
        ...
        return new ActiveLaneState(
                ...
                nextEligibleAtEpochMs,
                circuitState,
                circuitOpenUntilEpochMs,
                consecutiveFailures,
                laneRetryBackoffUntilEpochMs,
                executorRetryAtEpochMs,
                nextEncodedReadyKey,
                nextReadyCertificate,
                retirement);
    }
    ...
    public static ActiveLaneState decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "ActiveLaneState");
        if (fields.size() < 19
                || fields.get(0).number() != 1
                || fields.get(fields.size() - 1).number() != 26) {
            throw new IllegalArgumentException("ActiveLaneState has invalid field count/order");
        }
        int index = 0;
        final long version = QueryCodecSupport.uint(fields.get(index), 1);
        if (version != VERSION) {
            throw new IllegalArgumentException("unsupported ActiveLaneState version");
        }
        index++;
        final DestinationLaneId laneId =
                new DestinationLaneId(QueryCodecSupport.fixed(fields.get(index++), 2, DestinationLaneId.LENGTH));
        ...
        LaneRuntimeBlockReason blockReason = null;
        if (fields.get(index).number() == 6) {
            blockReason = LaneRuntimeBlockReason.fromWire(QueryCodecSupport.uint(fields.get(index++), 6));
        }
        ...
        final ProfileRef destination = ProfileRef.decode(QueryCodecSupport.nested(fields.get(index++), 9));
        final ProfileRef capability = ProfileRef.decode(QueryCodecSupport.nested(fields.get(index++), 10));
        ...
        final PublishAdmissionBody.ChargeVector usage =
                PublishAdmissionBody.ChargeVector.decodeCanonical(QueryCodecSupport.nested(fields.get(index++), 14));
        ...
        final LaneCircuitState circuit = LaneCircuitState.fromWire(QueryCodecSupport.uint(fields.get(index++), 17));
        ...
        LaneRetirementProgress retirement = null;
        if (index < fields.size() - 1 && fields.get(index).number() == 25) {
            retirement = LaneRetirementProgress.decode(QueryCodecSupport.nested(fields.get(index++), 25));
        }
        if (index != fields.size() - 1) {
            throw new IllegalArgumentException("ActiveLaneState has unexpected optional fields");
        }
        final byte[] stateDigest = QueryCodecSupport.fixed(fields.get(index), 26, HASH_LENGTH);
        final ActiveLaneState result = new ActiveLaneState(
                laneId,
                laneIncarnation,
                gate,
                readiness,
        ...
        if (!Bytes.constantTimeEquals(stateDigest, result.stateDigest)) {
            throw new IllegalArgumentException("ActiveLaneState digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ActiveLaneState");
        return result;
    }
    ...
    private static byte[] optionalCanonicalBytes(final byte[] value, final String name) {
        ...
        if (result != null) {
            ...
            if ("readyCertificate".equals(name)) {
                ReadyCertificate.decode(result);
            }
        }
        return result;
    }
     ...
     */
    private static void validateReadyKeyProjection(
            final byte[] encodedReadyKey,
            final DestinationLaneId laneId,
            final long laneVersion,
            final Long nextEligibleAtEpochMs) {
        ...
        final byte[] expected = Bytes.concat(
    ...
    public boolean equals(final Object other) {
        return other instanceof ActiveLaneState that
                && laneControlVersion == that.laneControlVersion
                && laneVersion == that.laneVersion
                && schedulerWeight == that.schedulerWeight
                ...
                && executorRetryAtEpochMs == that.executorRetryAtEpochMs
                && laneId.equals(that.laneId)
                && Arrays.equals(laneIncarnation, that.laneIncarnation)
                && admissionGate == that.admissionGate
                && runtimeReadiness == that.runtimeReadiness
                && runtimeBlockReason == that.runtimeBlockReason
                ...
                && laneUsage.equals(that.laneUsage)
                ...
                && Arrays.equals(encodedReadyKey, that.encodedReadyKey)
                && Arrays.equals(readyCertificate, that.readyCertificate)
                && Objects.equals(retirement, that.retirement)
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                laneRetryBackoffUntilEpochMs,
                executorRetryAtEpochMs,
                Arrays.hashCode(encodedReadyKey),
                Arrays.hashCode(readyCertificate),
                retirement,
                Arrays.hashCode(stateDigest));
    }
}
