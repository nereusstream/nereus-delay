...
public final class ControlMessageTarget {
    ...
    public static ControlMessageTarget decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "ControlMessageTarget");
        if (fields.size() != 3 && fields.size() != 4) {
            throw new IllegalArgumentException("invalid ControlMessageTarget field count");
        }
        if (fields.get(0).number() != 1
                || fields.get(1).number() != 2
                || fields.get(2).number() != 3
                || (fields.size() == 4 && fields.get(3).number() != 4)) {
            throw new IllegalArgumentException("invalid ControlMessageTarget field order");
        }
        final ControlMessageTarget result = new ControlMessageTarget(
        ...
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ControlMessageTarget");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof ControlMessageTarget that
                && expectedGeneration == that.expectedGeneration
                && expectedStateVersion == that.expectedStateVersion
