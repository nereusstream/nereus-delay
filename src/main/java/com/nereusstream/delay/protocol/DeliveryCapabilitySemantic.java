 ...
 */
public final class DeliveryCapabilitySemantic implements ProfileSemanticBody {
    ...
    private final OutcomeCapability outcomeCapability;
    ...
    public DeliveryCapabilitySemantic(
            final AdapterKind adapterKind,
            final OutcomeCapability outcomeCapability,
        ...
        TimingCapability.requireValid(timingCapabilityBits);
        if (adapterKind == AdapterKind.KAFKA
                && (timingCapabilityBits
                                & (TimingCapability.PULSAR_GUARDED_HANDOFF | TimingCapability.PULSAR_AUTO_FAST))
                        != 0) {
            throw new IllegalArgumentException("Kafka capability cannot carry Pulsar timing bits");
        }
        if (adapterKind == AdapterKind.PULSAR && outcomeCapability == OutcomeCapability.KAFKA_TRANSACTIONAL_RECEIPT) {
            throw new IllegalArgumentException("Kafka transactional capability requires a Kafka adapter");
        }
        if (adapterKind == AdapterKind.KAFKA && outcomeCapability == OutcomeCapability.PULSAR_BROKER_DEDUP) {
        ...
        if (evidencePartitionCount < 0
                || minimumEvidenceRetentionMs < 0
                || minimumDedupHorizonMs < 0
        ...
        if (outcomeCapability == OutcomeCapability.AT_LEAST_ONCE) {
            if (evidenceResource != null
                    || evidencePartitionCount != 0
                    || minimumEvidenceRetentionMs != 0
                    || minimumDedupHorizonMs != 0
                    || maximumCertifiedProducerKeys != 0
                    || adapterConformanceVersion != 0
                    || rejectionClassifierVersion != 0) {
            ...
            final BrokerResourceIdentity.Kind expected =
                    outcomeCapability == OutcomeCapability.KAFKA_TRANSACTIONAL_RECEIPT
    ...
    public OutcomeCapability outcomeCapability() {
    ...
    public boolean requiresEvidenceResource() {
        return outcomeCapability != OutcomeCapability.AT_LEAST_ONCE;
    }

    @Override
    public ProfileKind profileKind() {
        return ProfileKind.DELIVERY_CAPABILITY;
    }
    ...
    public static DeliveryCapabilitySemantic decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields =
                QueryCodecSupport.read(encoded, "DeliveryCapabilitySemantic");
        if (fields.size() != 11 && fields.size() != 12) {
            throw new IllegalArgumentException("DeliveryCapabilitySemantic has an unexpected field count");
        }
        final boolean hasEvidence = fields.size() == 12;
        final int offset = hasEvidence ? 1 : 0;
        if (hasEvidence && fields.get(3).number() != 4) {
            throw new IllegalArgumentException("invalid DeliveryCapabilitySemantic evidence field");
        }
        final DeliveryCapabilitySemantic result = new DeliveryCapabilitySemantic(
                AdapterKind.fromWire(QueryCodecSupport.uint(fields.get(0), 1)),
                OutcomeCapability.fromWire(QueryCodecSupport.uint(fields.get(1), 2)),
                QueryCodecSupport.uint32(fields.get(2), 3),
                hasEvidence ? BrokerResourceIdentity.decode(QueryCodecSupport.nested(fields.get(3), 4)) : null,
        ...
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "DeliveryCapabilitySemantic");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof DeliveryCapabilitySemantic that
                && adapterKind == that.adapterKind
                && outcomeCapability == that.outcomeCapability
    ...
    public int hashCode() {
        return Objects.hash(
                ...
                minimumEvidenceRetentionMs,
                minimumDedupHorizonMs,
                maximumCertifiedProducerKeys,
                Arrays.hashCode(brokerPrerequisiteDigest),
                Arrays.hashCode(sourceLockDigest),
                adapterConformanceVersion,
                rejectionClassifierVersion);
    }
