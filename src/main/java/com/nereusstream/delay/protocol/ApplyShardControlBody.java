...
public final class ApplyShardControlBody {
    ...
    public static ApplyShardControlBody decode(final byte[] canonicalBody) {
        ...
        final ApplyShardControlBody result = new ApplyShardControlBody(
                controlRef, controlKind, semanticVersion, semanticHash, expectedPrior, payload);
    ...
    public LaneTarget laneTarget() {
        ...
        if (fields.size() != expectedFieldCount
                ...
                || fields.get(1).wireType() != 2) {
            throw new IllegalArgumentException("lane control payload is incomplete");
        }
        final List<CanonicalProtobuf.Reader.Field> target =
                readAll(new CanonicalProtobuf.Reader(fields.get(0).rawValue()));
        if (target.size() != 3) {
        ...
        if (controlKind == 8 || controlKind == 9) {
            ...
            if (fields.get(2).number() != 3
                    || fields.get(2).wireType() != 0
                    || fields.get(2).unsignedValue() != 1
                    || fields.get(3).number() != 4
                    || fields.get(3).wireType() != 0
                    || fields.get(3).unsignedValue() < 0
                    || fields.get(3).unsignedValue() > 1
                    || fields.get(4).number() != 5
                    || fields.get(4).wireType() != 2) {
    ...
    private void validatePayloadBranch() {
        final List<CanonicalProtobuf.Reader.Field> branches = readAll(new CanonicalProtobuf.Reader(payload));
        if (branches.size() != 1
                || branches.get(0).number() != controlKind
                || branches.get(0).wireType() != 2) {
