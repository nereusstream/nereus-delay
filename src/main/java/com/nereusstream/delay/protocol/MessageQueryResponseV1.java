...
public record MessageQueryResponseV1(
        MessageQueryResult resultKind, PublicQueryErrorV1 error, QueryResponseBranchV1 branch) {
    ...
    public static MessageQueryResponseV1 identityRetired() {
        return new MessageQueryResponseV1(MessageQueryResult.IDENTITY_RETIRED, IdentityRetiredMessageViewV1.INSTANCE);
    }
    ...
    public static MessageQueryResponseV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "MessageQueryResponseV1");
        if (fields.size() != 3
        ...
        final QueryResponseBranchV1 branch = decodeBranch(result, QueryCodecSupport.nested(fields.get(2), branchField));
