...
public final class ActiveMessageView implements QueryResponseBranch {
    ...
    public static ActiveMessageView decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "ActiveMessageView");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8}, "ActiveMessageView");
        final ActiveMessageView result = new ActiveMessageView(
                ...
                PublicDestinationBindingView.decode(QueryCodecSupport.nested(fields.get(5), 6)),
                PayloadAvailability.fromWire(QueryCodecSupport.uint(fields.get(6), 7)),
                QueryCodecSupport.bool(fields.get(7), 8));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ActiveMessageView");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        if (!(other instanceof ActiveMessageView that)) {
            return false;
        }
