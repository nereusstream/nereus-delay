...
public final class CapacityGrant {
    public static final int HASH_LENGTH = 32;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-capacity-grant\0");

    private final CapacityGrantKind kind;
    private final byte[] grantId;
    private final long reserveSourceVersion;
    private final CapacityVector vector;
    private final byte[] digest;

    public CapacityGrant(
            final CapacityGrantKind kind,
            final byte[] grantId,
            final long reserveSourceVersion,
            final CapacityVector vector) {
    ...
    public static CapacityGrant decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "CapacityGrant");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5}, "CapacityGrant");
        final CapacityGrantKind kind = CapacityGrantKind.fromWire(QueryCodecSupport.uint(fields.get(0), 1));
        final byte[] grantId = QueryCodecSupport.fixed(fields.get(1), 2, HASH_LENGTH);
        if (isZero(grantId)) {
            throw new IllegalArgumentException("CapacityGrant grantId must be non-zero");
        }
        final long sourceVersion = QueryCodecSupport.uint64Bits(fields.get(2), 3);
        if (sourceVersion == 0) {
            throw new IllegalArgumentException("CapacityGrant reserveSourceVersion must be nonzero");
        }
        final CapacityVector vector = CapacityVector.decode(QueryCodecSupport.nested(fields.get(3), 4));
        final byte[] digest = QueryCodecSupport.fixed(fields.get(4), 5, HASH_LENGTH);
        final CapacityGrant result = new CapacityGrant(kind, grantId, sourceVersion, vector, digest);
        if (!Arrays.equals(digest, Bytes.sha256(DIGEST_DOMAIN, result.fieldsOneToFour()))) {
            throw new IllegalArgumentException("CapacityGrant digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "CapacityGrant");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof CapacityGrant that
                && kind == that.kind
    ...
    @Override
    public int hashCode() {
        return Objects.hash(kind, Arrays.hashCode(grantId), reserveSourceVersion, vector, Arrays.hashCode(digest));
    }
}
