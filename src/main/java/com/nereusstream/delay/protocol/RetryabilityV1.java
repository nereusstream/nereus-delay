...
public enum RetryabilityV1 {
    ...
    RetryabilityV1(final int wireValue) {
    ...
    public static RetryabilityV1 fromWire(final long value) {
        for (RetryabilityV1 retryability : values()) {
            if (retryability.wireValue == value) {
                return retryability;
            }
        }
        throw new IllegalArgumentException("unknown RetryabilityV1: " + value);
    }

    public static RetryabilityV1 forCode(final StableCode code) {
        return switch (code) {
            case CLIENT_CLOSED,
