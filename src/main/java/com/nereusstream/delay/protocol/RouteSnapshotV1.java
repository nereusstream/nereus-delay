...
public final class RouteSnapshotV1 {
    ...
    public static RouteSnapshotV1 create(
            final RouteIncarnation routeIncarnation,
            final byte[] authenticatedTenantScopeHash,
            final byte[] tenantRoutingScope,
            final RouteLifecycleV1 lifecycle,
        ...
        final RouteSnapshotV1 unsigned = new RouteSnapshotV1(
                routeIncarnation,
    ...
    public void requireUsableForNewSchedule(
        ...
        if (lifecycle != RouteLifecycleV1.ACTIVE_FOR_NEW
    ...
    private static BrokerResourceIdentityV1 resourceIdentity(
            final IngressRouteResourceV1 ingress, final int partition) {
        if (ingress instanceof KafkaIngressRouteResourceV1 kafka) {
            return BrokerResourceIdentityV1.kafka(
                    new KafkaBrokerResourceIdentityV1(kafka.authenticatedClusterId(), kafka.nativeTopicUuid()));
        }
        final PulsarPhysicalPartitionIdentityV1 physical =
                ((PulsarIngressRouteResourceV1) ingress).partition(partition);
