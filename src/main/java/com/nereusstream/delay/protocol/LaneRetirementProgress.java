...
public final class LaneRetirementProgress {
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-lane-retirement-progress\0");
    ...
    public LaneRetirementProgress(
            final byte[] retireMutationId,
            final long appliedShardMutationSequence,
    ...
    private LaneRetirementProgress(
            final byte[] retireMutationId,
            final long appliedShardMutationSequence,
            final SourcePosition intentSourcePosition,
            final byte[] progressDigest) {
        this.retireMutationId = nonZero(retireMutationId, "retireMutationId");
        if (appliedShardMutationSequence == 0) {
            throw new IllegalArgumentException("appliedShardMutationSequence must be non-zero");
        }
    ...
    public static LaneRetirementProgress decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "LaneRetirementProgress");
        QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4}, "LaneRetirementProgress");
        final LaneRetirementProgress result = new LaneRetirementProgress(
        ...
        if (!Bytes.constantTimeEquals(result.progressDigest, Bytes.sha256(DIGEST_DOMAIN, result.fieldsOneToThree()))) {
            throw new IllegalArgumentException("LaneRetirementProgress digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "LaneRetirementProgress");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof LaneRetirementProgress that
    ...
    public int hashCode() {
        return Objects.hash(
                Arrays.hashCode(retireMutationId),
                appliedShardMutationSequence,
                intentSourcePosition,
                Arrays.hashCode(progressDigest));
    }
}
