...
public final class CommandAppliedReceiptV1 {
    ...
    private CommandAppliedReceiptV1(
    ...
    public static CommandAppliedReceiptV1 create(
        ...
        return new CommandAppliedReceiptV1(
                queuedReceipt.receiptPayloadDigest(),
                applyStatus,
                stableCode,
                appliedSourcePosition,
    ...
    public static CommandAppliedReceiptV1 decodePayload(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "CommandAppliedReceiptV1");
        ...
        while (index < fields.size() - 2) {
            switch (fields.get(index).number()) {
                ...
                case 8 ->
                    binding = PublicDestinationBindingViewV1.decode(QueryCodecSupport.nested(fields.get(index++), 8));
        ...
        final CommandAppliedReceiptV1 result = new CommandAppliedReceiptV1(
                queuedDigest,
        ...
        final byte[] expected = Bytes.sha256(
                Bytes.utf8("nereus-delay-command-applied-receipt-v1\0"),
                canonicalFields(
                        queuedDigest,
                        result.applyStatus,
                        result.stableCode,
