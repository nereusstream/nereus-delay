...
public final class CommandAppliedReceipt {
    ...
    private CommandAppliedReceipt(
            final byte[] queuedReceiptPayloadDigest,
            final CommandApplyStatus applyStatus,
        ...
        if (applyStatus == CommandApplyStatus.REJECTED
    ...
    public static CommandAppliedReceipt create(
        ...
        final byte[] fields = canonicalFields(
                queuedReceipt.receiptPayloadDigest(),
                applyStatus,
                stableCode,
        ...
        return new CommandAppliedReceipt(
                queuedReceipt.receiptPayloadDigest(),
                applyStatus,
                stableCode,
                appliedSourcePosition,
                ...
                binding,
                fullResultRetainUntilEpochMs,
                Bytes.sha256(Bytes.utf8("nereus-delay-command-applied-receipt\0"), fields));
    }

    public static CommandAppliedReceipt decodeFrame(final byte[] frame) {
    ...
    public static CommandAppliedReceipt decodePayload(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "CommandAppliedReceipt");
        if (fields.size() < 7 || fields.size() > 10) {
            throw new IllegalArgumentException("invalid CommandAppliedReceipt field count");
        }
        if (fields.get(0).number() != 1
            ...
            throw new IllegalArgumentException("invalid CommandAppliedReceipt required fields");
        }
        int index = 5;
        ...
        while (index < fields.size() - 2) {
            switch (fields.get(index).number()) {
                ...
                case 8 ->
                    binding = PublicDestinationBindingView.decode(QueryCodecSupport.nested(fields.get(index++), 8));
                default -> throw new IllegalArgumentException("invalid CommandAppliedReceipt optional field");
            }
        }
        if (index != fields.size() - 2 || fields.get(index).number() != 9) {
            throw new IllegalArgumentException("invalid CommandAppliedReceipt optional field order");
        }
        ...
        final CommandAppliedReceipt result = new CommandAppliedReceipt(
                queuedDigest,
                CommandApplyStatus.fromWire(QueryCodecSupport.uint(fields.get(2), 3)),
                ...
                stateVersion,
                binding,
                QueryCodecSupport.uint(fields.get(index), 9),
                digest);
        final byte[] expected = Bytes.sha256(
                Bytes.utf8("nereus-delay-command-applied-receipt\0"),
                canonicalFields(
                        queuedDigest,
                        result.applyStatus,
                        result.stableCode,
                        result.appliedSourcePosition,
                        generation,
                        stateVersion,
                        binding,
                        result.fullResultRetainUntilEpochMs));
        ...
        QueryCodecSupport.requireCanonical(encoded, result.payload(), "CommandAppliedReceipt");
        return result;
    }
    ...
    public CommandApplyStatus applyStatus() {
        return applyStatus;
    }
    ...
    public boolean equals(final Object other) {
        if (!(other instanceof CommandAppliedReceipt that)) {
            return false;
        }
    ...
    public int hashCode() {
        return Objects.hash(
                Arrays.hashCode(queuedReceiptPayloadDigest),
                applyStatus,
                stableCode,
                appliedSourcePosition,
                generation,
    ...
    private static void validateAppliedPosition(
            final SourcePosition queuedPosition, final SourcePosition appliedPosition) {
        if (!queuedPosition.sameSourceIdentity(appliedPosition)
                || !queuedPosition.shardId().equals(appliedPosition.shardId())) {
        ...
        if (order < 0
                || (order == 0
                        && !Bytes.constantTimeEquals(
                                appliedPosition.canonicalBytes(), queuedPosition.canonicalBytes()))) {
