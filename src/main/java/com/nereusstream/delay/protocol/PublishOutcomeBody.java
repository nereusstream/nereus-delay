...
public final class PublishOutcomeBody {
    ...
    public static byte[] encodeInitial(
        ...
        final byte[] encoded = encodeCommon(shardId, retryUntilEpochMs, SystemMutationType.PUBLISH_OUTCOME, output -> {
            ...
            CanonicalProtobuf.uint32(
                    output, 13, Objects.requireNonNull(stableCode, "stableCode").wireValue());
            ...
            CanonicalProtobuf.bytes(
                    output, 16, Objects.requireNonNull(observedAt, "observedAt").canonicalBytes());
    ...
    public static byte[] encodeEvidenceResolution(
            ...
            final byte[] retryDecision) {
        final byte[] encoded =
                encodeCommon(shardId, retryUntilEpochMs, SystemMutationType.EVIDENCE_RESOLUTION, output -> {
                    CanonicalProtobuf.bytes(output, 10, fixed(publishAttemptId, "publishAttemptId"));
                    CanonicalProtobuf.bytes(
                            output,
                            11,
                            Objects.requireNonNull(evidenceCursor, "evidenceCursor")
                                    .canonicalBytes());
                    CanonicalProtobuf.bytes(output, 12, nonEmpty(evidence, "evidence"));
                    CanonicalProtobuf.uint32(
                            output,
                            13,
                            Objects.requireNonNull(stableCode, "stableCode").wireValue());
                    ...
                    CanonicalProtobuf.bytes(
                            output,
                            17,
                            Objects.requireNonNull(observedAt, "observedAt").canonicalBytes());
    ...
    public static PublishOutcomeBody decode(final byte[] canonicalBody) {
        ...
        final TrustedUtcIntervalEvidence observedAt = TrustedUtcIntervalEvidence.decode(nested(field(fields, 16), 16));
    ...
    public static PublishOutcomeBody decodeEvidenceResolution(final byte[] canonicalBody) {
        ...
        validateChargeVector(transfer);
        final TrustedUtcIntervalEvidence observedAt = TrustedUtcIntervalEvidence.decode(nested(field(fields, 17), 17));
    ...
    private static void validateDefinitiveCombination(
            final int sideEffect,
            final int disposition,
            final StableCode stableCode,
            final byte[] evidence,
    ...
    public static final class RetryDecision {
        ...
        public boolean equals(final Object other) {
            return other instanceof RetryDecision that
                    && kind == that.kind
                    && completedAttemptNo == that.completedAttemptNo
                    && firstAttemptAt == that.firstAttemptAt
                    && retryDeadline == that.retryDeadline
