...
public final class ResourceRetireIntentBody {
    ...
    private ResourceRetireIntentBody(
            final ResourceKind resourceKind,
            final ExactResourceIdentity resource,
            final long expectedResourceStateVersion,
            final ProtectionSet protections) {
     ...
     */
    public static void validateExternalDeleteIdentity(
    ...
    public static void validateExternalDeleteIdentity(
        ...
        final CanonicalProtobuf.Reader.Field branchField =
                new CanonicalProtobuf.Reader(identity.canonicalBytes()).next();
        ...
        if (kind == ResourceKind.PAYLOAD_OBJECT) {
            ...
            if (outcome == ResourceDeleteConfirmedBody.DeleteOutcome.DELETED && observedImmutableVersion.length == 0) {
                throw new IllegalArgumentException("DELETED payload evidence must carry immutable version");
            }
            if (observedImmutableVersion.length != 0 && !Arrays.equals(exactVersion, observedImmutableVersion)) {
            ...
            if (observedEtag.length != 0 && (exactEtag.length == 0 || !Arrays.equals(exactEtag, observedEtag))) {
            ...
            if (observedImmutableVersion.length != 0 && !Arrays.equals(exactVersion, observedImmutableVersion)) {
                throw new IllegalArgumentException(
                        "delete evidence immutable version does not match checkpoint identity");
            }
    ...
    public record ProtectionSet(List<ProtectionRef> references, byte[] canonicalBytes, byte[] digest) {
        ...
        private static ProtectionSet decode(final byte[] encoded) {
            ...
            for (int index = 1; index < refs.size(); index++) {
                if (Arrays.equals(
                        refs.get(index - 1).canonicalBytes(), refs.get(index).canonicalBytes())) {
                    throw new IllegalArgumentException("ProtectionSet contains duplicate references");
                }
            }
    ...
    public record ProtectionRef(
        ...
        public ProtectionRef {
            ...
            Objects.requireNonNull(canonicalBytes, "canonicalBytes");
            if (protectionKind == 1) {
                if (minimumSourcePosition.length == 0
                        || recoveryLineageId.length != INCARNATION_LENGTH
                        || checkpointId.length != INCARNATION_LENGTH
                        || manifestHash.length != HASH_LENGTH) {
                    throw new IllegalArgumentException("Recovery Floor protection is incomplete");
                }
                minimumSourcePosition =
                        SourcePositionCodec.decode(minimumSourcePosition).canonicalBytes();
            } else if (protectionKind == 2 || protectionKind == 4) {
                if (minimumSourcePosition.length == 0
                        || recoveryLineageId.length != 0
                ...
                minimumSourcePosition =
                        SourcePositionCodec.decode(minimumSourcePosition).canonicalBytes();
            } else if (minimumSourcePosition.length != 0
                    || recoveryLineageId.length != 0
                    || checkpointId.length != 0
                    || manifestHash.length != 0) {
            ...
            final byte[] expectedCanonical = canonicalBytes(
                    protectionKind,
                    protectedResourceId,
        ...
        private static ProtectionRef decode(final byte[] encoded) {
            ...
            if (kind == 1) {
                if (source.length == 0
                        || lineage.length != INCARNATION_LENGTH
                        || checkpoint.length != INCARNATION_LENGTH
                        || manifest.length != HASH_LENGTH) {
                    throw new IllegalArgumentException("Recovery Floor protection is incomplete");
                }
            ...
            final byte[] canonical =
                    canonicalBytes(kind, resourceId, generation, source, lineage, checkpoint, manifest);
            if (!Arrays.equals(encoded, canonical)) {
    ...
    private static void requireExact(
            final List<CanonicalProtobuf.Reader.Field> fields, final int count, final Object name) {
    ...
    private static void requireNumbers(
            final List<CanonicalProtobuf.Reader.Field> fields, final int[] numbers, final Object name) {
