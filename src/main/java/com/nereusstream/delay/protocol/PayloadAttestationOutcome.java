...
public enum PayloadAttestationOutcome {
    ATTESTED(1),
    ...
    PayloadAttestationOutcome(final int wireValue) {
    ...
    public static PayloadAttestationOutcome fromWire(final long value) {
        for (PayloadAttestationOutcome outcome : values()) {
            if (outcome.wireValue == value) {
                return outcome;
            }
        }
        throw new IllegalArgumentException("unknown PayloadAttestationOutcome: " + value);
    }
}
