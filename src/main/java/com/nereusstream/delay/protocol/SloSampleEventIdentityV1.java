...
public final class SloSampleEventIdentityV1 {
    ...
    private SloSampleEventIdentityV1(
            final SloObjectiveNameV1 objective, final byte[] canonicalBytes, final boolean alreadyCanonical) {
        this.objective = Objects.requireNonNull(objective, "objective");
    ...
    public static SloSampleEventIdentityV1 decode(final byte[] encoded) {
        ...
        final SloSampleEventIdentityV1 result = new SloSampleEventIdentityV1(objective, Bytes.copy(encoded), true);
    ...
    private static void validateBranchPayload(final SloObjectiveNameV1 objective, final byte[] payload) {
        ...
        switch (objective) {
            ...
            case SOURCE_RETENTION_TIME_MARGIN, SOURCE_RETENTION_BYTE_MARGIN -> {
                QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4}, "SloSourceMarginIdentityV1");
                nestedNonEmpty(fields.get(0), 1, "shardSubject");
                QueryCodecSupport.uint(fields.get(1), 2);
                QueryCodecSupport.uint(fields.get(2), 3);
            ...
            case HEALTHY_LANE_SERVICE_GAP ->
                validateLane(fields, "SloLaneServiceGapIdentityV1", "serviceGapGeneration");
            case LANE_RECOVERY_READY_RTO -> {
                QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5}, "SloLaneRecoveryReadyIdentityV1");
                nestedNonEmpty(fields.get(0), 1, "shardSubject");
