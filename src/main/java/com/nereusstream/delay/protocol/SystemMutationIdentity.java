 ...
 */
public final class SystemMutationIdentity {
    private SystemMutationIdentity() {}

    /** Returns the exact registered logical identity for one canonical body. */
    public static byte[] resolve(final ShardId shardId, final SystemMutationType type, final byte[] canonicalBody) {
        ...
        return switch (mutationType) {
            ...
            case REPLAY_DEAD_LETTER ->
                ReplayDeadLetterBody.decode(body).controlRef().logicalOperationIdentity(mutationType);
            ...
            case EXPIRE_GENERATION -> {
                final DelayMessageId messageId =
                        new DelayMessageId(fixed(field(fields, 10), 10, DelayMessageId.LENGTH));
                yield SystemMutation.computeExpiryLogicalIdentity(
                        messageId, (int) unsigned(field(fields, 11), 11), unsigned(field(fields, 12), 12));
            }
            case EVIDENCE_RESOLUTION ->
                PublishOutcomeBody.decodeEvidenceResolution(body).evidenceResolutionLogicalOperationIdentity();
            case RESOURCE_RETIRE_INTENT -> {
                final ResourceRetireIntentBody retire = ResourceRetireIntentBody.decode(body);
                yield SystemMutation.computeResourceRetireLogicalIdentity(
                        retire.resourceKind(), retire.resource().identityHash(), retire.expectedResourceStateVersion());
            }
            case RESOURCE_DELETE_CONFIRMED ->
                ResourceDeleteConfirmedBody.decode(body).intent().mutationId();
