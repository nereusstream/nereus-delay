...
public final class KafkaMetadata {
     ...
     * presence value
     */
    public KafkaMetadata(final byte[] key, final List<Header> headers) {
    ...
    public static KafkaMetadata decode(final byte[] encoded) {
        ...
        while (index < fields.size()) {
            if (fields.get(index).number() != 2) {
                throw new IllegalArgumentException(
                        "unexpected Kafka metadata field " + fields.get(index).number());
            }
            headers.add(Header.decode(QueryCodecSupport.nested(fields.get(index++), 2)));
        }
        final KafkaMetadata result = new KafkaMetadata(key, headers);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "KafkaMetadata");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof KafkaMetadata that && Arrays.equals(key, that.key) && headers.equals(that.headers);
    }
    ...
    public static final class Header {
        ...
        private static Header decode(final byte[] encoded) {
            final var fields = QueryCodecSupport.read(encoded, "KafkaHeader");
            QueryCodecSupport.requireNumbers(fields, new int[] {1, 2}, "KafkaHeader");
            final Header result =
                    new Header(QueryCodecSupport.bytes(fields.get(0), 1), QueryCodecSupport.bytes(fields.get(1), 2));
            QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "KafkaHeader");
            return result;
        }
        ...
        public boolean equals(final Object other) {
            return other instanceof Header that
                    && Arrays.equals(nameUtf8, that.nameUtf8)
