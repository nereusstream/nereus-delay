...
public final class PayloadProofTrustSetControlState {
    ...
    private PayloadProofTrustSetControlState(
            final List<ActivationMarker> activations, final List<IssuanceClosure> closures) {
    ...
    public Optional<SourcePosition> activeTrustSetPosition() {
        return activations.isEmpty()
                ? Optional.empty()
                : Optional.of(activations.get(activations.size() - 1).sourcePosition());
    }
    ...
    public static PayloadProofTrustSetControlState decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = readRepeated(encoded, "PayloadProofTrustSetControlState");
        if (fields.isEmpty()
        ...
        final PayloadProofTrustSetControlState result = new PayloadProofTrustSetControlState(activations, closures);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "PayloadProofTrustSetControlState");
        return result;
    }
    ...
    private static Comparator<IssuanceClosure> closureComparator() {
        return Comparator.comparing(
                        (IssuanceClosure value) -> value.trustSet().canonicalBytes(),
                        PayloadProofTrustSetControlState::compareBytes)
                .thenComparing(
                        (left, right) -> Integer.compareUnsigned(left.proofKeyVersion(), right.proofKeyVersion()));
    }
