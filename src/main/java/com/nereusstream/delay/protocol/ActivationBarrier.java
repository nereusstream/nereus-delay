...
/** Canonical source activation barrier used by a ReadyCertificate. */
public final class ActivationBarrier {
    ...
    private final BrokerResourceIdentity resource;
    ...
    private ActivationBarrier(
            ...
            final long observedLsoExclusive,
            final long ledgerId,
            final long entryId,
        ...
        this.resourceGuardAttestationDigest = resourceGuardAttestationDigest == null
                ? null
        ...
        if (kind == Kind.EMPTY
                && resource.kind() == BrokerResourceIdentity.Kind.PULSAR
                && (guardedSourceConnectionGeneration == null || this.resourceGuardAttestationDigest == null)) {
    ...
    public static ActivationBarrier empty(
            final BrokerResourceIdentity resource,
            final int partition,
            final Long guardedSourceConnectionGeneration,
            final byte[] resourceGuardAttestationDigest) {
        return new ActivationBarrier(
                Kind.EMPTY,
                resource,
                partition,
    ...
    public static ActivationBarrier kafka(
            final BrokerResourceIdentity resource,
            final int partition,
            final long nextOffsetExclusive,
            final long observedLsoExclusive) {
        return new ActivationBarrier(
                Kind.KAFKA, resource, partition, null, null, nextOffsetExclusive, observedLsoExclusive, 0, 0, 0, 0);
    }

    public static ActivationBarrier pulsar(
            final BrokerResourceIdentity resource,
            final int partition,
            final long ledgerId,
            final long entryId,
    ...
    public BrokerResourceIdentity resource() {
        return resource;
    }
                ...
                switch (resource.kind()) {
                    case KAFKA ->
                        new KafkaActivationBarrier(
                                shard,
                                resource.kafka().authenticatedClusterId(),
                                resource.kafka().nativeTopicUuid(),
                                0);
                    case PULSAR ->
                        PulsarActivationBarrier.empty(
                                shard,
                                resource.pulsar().resourceIncarnation(),
                                resource.pulsar().physicalTopic(),
                                guardedSourceConnectionGeneration,
                                resourceGuardAttestationDigest);
                };
            case KAFKA ->
    ...
    public byte[] canonicalBytes() {
        return CanonicalProtobuf.message(output -> {
            switch (kind) {
                case EMPTY ->
                    CanonicalProtobuf.bytes(output, 1, CanonicalProtobuf.message(fields -> {
                ...
                case KAFKA ->
                    CanonicalProtobuf.bytes(output, 2, CanonicalProtobuf.message(fields -> {
                        CanonicalProtobuf.bytes(fields, 1, resource.canonicalBytes());
                        CanonicalProtobuf.uint32Bits(fields, 2, partition);
                ...
                case PULSAR ->
                    CanonicalProtobuf.bytes(output, 3, CanonicalProtobuf.message(fields -> {
                        CanonicalProtobuf.bytes(fields, 1, resource.canonicalBytes());
                        CanonicalProtobuf.uint32Bits(fields, 2, partition);
    ...
    public static ActivationBarrier decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> outer = QueryCodecSupport.read(encoded, "ActivationBarrier");
        if (outer.size() != 1) {
            throw new IllegalArgumentException("ActivationBarrier must select one branch");
        }
        final CanonicalProtobuf.Reader.Field branch = outer.get(0);
        final List<CanonicalProtobuf.Reader.Field> fields =
                QueryCodecSupport.read(QueryCodecSupport.nested(branch, branch.number()), "ActivationBarrier branch");
        final ActivationBarrier result;
        switch (branch.number()) {
            ...
            case 3 -> {
                QueryCodecSupport.requireNumbers(fields, new int[] {1, 2, 3, 4, 5, 6, 7, 8}, "PulsarActivationBarrier");
                result = pulsar(
                        BrokerResourceIdentity.decode(QueryCodecSupport.nested(fields.get(0), 1)),
                        QueryCodecSupport.uint32Bits(fields.get(1), 2),
                        QueryCodecSupport.uint(fields.get(2), 3),
                        ...
                        QueryCodecSupport.uint32Bits(fields.get(5), 6),
                        QueryCodecSupport.uint(fields.get(6), 7),
                        QueryCodecSupport.fixed(fields.get(7), 8, 32));
            }
            default -> throw new IllegalArgumentException("unknown ActivationBarrier branch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ActivationBarrier");
        return result;
    }
    ...
    public boolean equals(final Object other) {
        return other instanceof ActivationBarrier that
                && kind == that.kind
                ...
                && nextOffsetExclusive == that.nextOffsetExclusive
                && observedLsoExclusive == that.observedLsoExclusive
