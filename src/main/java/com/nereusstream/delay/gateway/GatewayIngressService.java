...
public final class GatewayIngressService {
    ...
    public GatewayIngressService(
            final GatewayScheduleService scheduleService,
            ...
            try {
                if (failure != null || outcome == null) {
                    ...
                    throw new CompletionException(
                            failure == null
                                    ? new IllegalStateException("Gateway domain stage returned no outcome")
                                    : failure);
                }
                try {
                    audit.record(GatewayAuditEventV1.completed(
                            ...
                            keyHash,
                            bodyHash,
                            outcome.hasSubmissionOutcome()
                                    ? outcome.submissionOutcome().canonicalBytes()
                                    : outcome.preparationError().canonicalBytes(),
                            now()));
                } catch (RuntimeException auditFailure) {
                    throw new CompletionException(
                            new GatewayIngressException(GatewayIngressException.Kind.UNAVAILABLE, auditFailure));
                }
                return outcome;
            } finally {
                lease.close();
