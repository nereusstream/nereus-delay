...
final class GatewayAdmissionRecord {
    static final int VERSION = 1;
    private static final int LEASE_ID_LENGTH = 16;
    private static final int MAX_LEASES = 4096;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-gateway-admission-record\0");
    ...
    GatewayAdmissionRecord(final Digest32 tenantScopeHash, final long revision, final List<Lease> leases) {
    ...
    static GatewayAdmissionRecord empty(final Digest32 tenantScopeHash) {
        return new GatewayAdmissionRecord(tenantScopeHash, 1, List.of());
    }
    ...
    GatewayAdmissionRecord withLeases(final List<Lease> nextLeases) {
        if (revision == Long.MAX_VALUE) {
            throw new IllegalStateException("Gateway admission revision is exhausted");
        }
        return new GatewayAdmissionRecord(tenantScopeHash, revision + 1, nextLeases);
    }
    ...
    static GatewayAdmissionRecord decode(final byte[] encoded) {
        ...
        final GatewayAdmissionRecord result = new GatewayAdmissionRecord(tenant, revision, leases);
    ...
    static final class Lease {
        ...
        private static byte[] digest(final byte[] withoutDigest) {
            return Bytes.sha256(Bytes.utf8("nereus-delay-gateway-admission-lease\0"), withoutDigest);
        }
        ...
        static Lease decode(final byte[] encoded) {
            ...
            final GatewayIngressOperation operation = operation(uint(field(fields, 2, 3), 3));
        ...
        private static long operationWire(final GatewayIngressOperation operation) {
        ...
        private static GatewayIngressOperation operation(final long wire) {
            return switch ((int) wire) {
                case 1 -> GatewayIngressOperation.SCHEDULE;
                case 2 -> GatewayIngressOperation.RETRY_UNCERTAIN;
                case 3 -> GatewayIngressOperation.CONTROL;
