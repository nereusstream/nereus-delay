...
public final class GatewayPhysicalAttempt {
    private final int attemptNo;
    private final PhysicalEnqueueAttemptId physicalAttemptId;
    private final GatewayPhysicalAttemptState state;
    ...
    private final com.nereusstream.delay.transport.Digest32 retryRequestHash;
    private final long revision;
    private final long ownershipNotAfterEpochMs;

    public GatewayPhysicalAttempt(
            final int attemptNo,
            ...
            final long uncertaintyAtEpochMs,
            final long revision,
            final long ownershipNotAfterEpochMs) {
        this(
                attemptNo,
                physicalAttemptId,
                state,
    ...
    public GatewayPhysicalAttempt(
            ...
            final com.nereusstream.delay.transport.Digest32 retryRequestHash,
            final long revision,
            final long ownershipNotAfterEpochMs) {
        if (attemptNo <= 0
                || startedAtEpochMs < 0
                || uncertaintyAtEpochMs <= startedAtEpochMs
                || revision <= 0
        ...
        if ((this.state == GatewayPhysicalAttemptState.STARTED) != (outcomeBytes == null)) {
    ...
    public GatewayPhysicalAttemptState state() {
        return state;
    }
    ...
    public com.nereusstream.delay.transport.Digest32 retryRequestHash() {
    ...
    public static GatewayPhysicalAttempt decode(final byte[] encoded) {
        ...
        final PhysicalEnqueueAttemptId physicalAttemptId =
                PhysicalEnqueueAttemptId.require(fixed(field(fields, index++, 2), 2, 16));
        final GatewayPhysicalAttemptState state =
                GatewayPhysicalAttemptState.fromWire(uint(field(fields, index++, 3), 3));
        final byte[] outcomeBytes;
        ...
        final long ownershipNotAfter = nonNegative(uint(field(fields, index++, 10), 10), "ownershipNotAfterEpochMs");
        if (index != fields.size()) {
            throw new IllegalArgumentException("Gateway physical attempt has unknown fields");
        }
        final GatewayPhysicalAttempt result = new GatewayPhysicalAttempt(
                attemptNo,
                physicalAttemptId,
                state,
                outcomeBytes,
