...
public final class WorkerAssignmentCoordinator {
    ...
    public WorkerAssignmentCoordinator(
            final WorkerPlacementPolicy placementPolicy, final WorkerAssignmentAuthority authority) {
    ...
    public PlacementResult place(
        ...
        final WorkerPlacementPolicy.Decision decision = placementPolicy.select(
                candidates,
                incomingShardCapacity,
                workerFixedCost,
                transitionDemand,
        ...
        final WorkerAssignment assignment = new WorkerAssignment(
                decision.workerId(), sourceAssignment, placementEpoch, capacityEnvelopeDigest, routeSnapshotDigest);
    ...
    public WorkerAssignment requireAccepted(
            final ShardId shardId, final long expectedRevision, final WorkerAssignment expectedAssignment) {
        ...
        final WorkerAssignmentAuthority.Publication observed = authority
                .current(shardId)
    ...
    public record PlacementResult(
            WorkerPlacementPolicy.Decision decision, Optional<WorkerAssignmentAuthority.Publication> publication) {
