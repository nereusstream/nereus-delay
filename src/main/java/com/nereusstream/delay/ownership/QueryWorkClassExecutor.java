...
public final class QueryWorkClassExecutor {
    ...
    public <T> Submission<T> submit(
            final QueryRequest request, final LongSupplier ownerClock, final QueryOperation<T> operation) {
        ...
        final WorkClassTask task = new WorkClassTask(
                WorkClass.QUERY, "query-read/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, taskBytes)), taskBytes.length);
    ...
    private static byte[] taskBytes(final QueryRequest request) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.bytes(
                    output, 1, request.shardId().routeIncarnation().bytes());
    ...
    public record QueryResult<T>(ResultKind kind, T value, Throwable failure) {
        ...
        private static <T> QueryResult<T> transitioning(final Throwable failure) {
            return new QueryResult<>(ResultKind.SHARD_TRANSITIONING, null, Objects.requireNonNull(failure, "failure"));
        }
