...
public final class ReservationExpiryDiscoveryWorkClassExecutor {
    ...
    public Submission submit(
        ...
        final WorkClassTask task = new WorkClassTask(
                WorkClass.GC, "reservation-expiry-discovery/" + Bytes.hex(Bytes.sha256(identity)), chargedBytes);
        final Submission submission = new Submission(task);
        workClasses.submit(
                task,
                () -> submission.complete(ownedShard.discoverReservationExpiryAuthoritativelyStrict(
