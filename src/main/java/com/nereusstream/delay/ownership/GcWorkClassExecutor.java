...
public final class GcWorkClassExecutor {
     ...
     */
    public Submission submitDeleteConfirmation(
    ...
    private Submission submitInternal(
            final SystemMutation exact, final SourcePosition predecessor, final LongSupplier clock) {
        ...
        final WorkClassTask task = new WorkClassTask(
                WorkClass.GC, "gc-mutation/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, frame)), frame.length);
    ...
    private void execute(
            final SystemMutation mutation,
            final SourcePosition predecessor,
            final LongSupplier ownerClock,
        ...
        try {
            ...
            switch (appended.disposition()) {
                ...
                case DEFINITIVELY_NOT_PERSISTED ->
                    submission.complete(GcHandoffResult.definitelyNotPersisted(mutation));
