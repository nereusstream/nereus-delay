...
public final class OutcomeWorkClassExecutor {
    ...
    public OutcomeWorkClassExecutor(
    ...
    public Submission submit(final SystemMutation mutation, final LongSupplier ownerClock) {
        ...
        final WorkClassTask task = new WorkClassTask(
                ...
                "outcome-mutation/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, frame)),
                frame.length);
    ...
    private void execute(final SystemMutation mutation, final LongSupplier ownerClock, final Submission submission) {
        try {
            ...
            final ShardLogMutationAppender.AppendOutcome appended =
                    Objects.requireNonNull(appender.append(mutation), "Shard Log append outcome");
            switch (appended.disposition()) {
                ...
                case DEFINITIVELY_NOT_PERSISTED ->
                    submission.complete(OutcomeHandoffResult.definitelyNotPersisted(mutation));
    ...
    public record OutcomeHandoffResult(
        ...
        private static OutcomeHandoffResult persisted(final SystemMutation mutation, final SourcePosition position) {
            return new OutcomeHandoffResult(
                    ResultKind.PERSISTED, mutation, Objects.requireNonNull(position, "position"), null);
        }
