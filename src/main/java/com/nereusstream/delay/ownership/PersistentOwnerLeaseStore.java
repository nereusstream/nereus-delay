...
public final class PersistentOwnerLeaseStore implements OwnerLeaseStore {
    ...
    public Optional<OwnerLease> acquire(
        ...
        return withExclusiveLock(() -> {
            ...
            final long expiresAt = Math.addExact(nowEpochMs, leaseDurationMs);
            final long nextEpoch = InMemoryOwnerLeaseStore.nextEpoch(current.lastEpoch());
            final OwnerLease next = new OwnerLease(
                    assignment.shardId(),
                    ownerId,
                    nextEpoch,
                    randomToken(),
    ...
    public Optional<OwnerLease> renew(final OwnerLease expected, final long nowEpochMs, final long leaseDurationMs) {
        ...
        return withExclusiveLock(() -> {
            ...
            if (!sameIdentity(current.lease(), expected)
                    || current.lease().state() != expected.state()
    ...
    private State load(final ShardId shardId) throws IOException {
        ...
        final byte[] encoded =
                LocalStatePathGuard.readRegularFileNoFollow(statePath, MAX_STATE_BYTES, "owner lease state");
        if (encoded == null) {
    ...
    private static OwnerLeaseContext decodeContext(final byte[] encoded) {
        ...
        requireWire(fields.get(2), 2);
        return new OwnerLeaseContext(
                assignmentId, assignmentEpoch, fields.get(2).rawValue());
    }
    ...
    private static String decodeOwnerId(final CanonicalProtobuf.Reader.Field field) {
        ...
        try {
            final String value = StandardCharsets.UTF_8
                    ...
                    .decode(ByteBuffer.wrap(field.rawValue()))
                    .toString();
    ...
    private static List<CanonicalProtobuf.Reader.Field> readFields(final byte[] encoded, final boolean allowRepeated) {
