...
public final class OxiaOwnerLeaseStore implements OwnerLeaseStore {
    ...
    public Optional<OwnerLease> acquire(
        ...
        return validateAcquired(
                backend.acquire(shardId, ownerId, nowEpochMs, leaseDurationMs), shardId, ownerId, nowEpochMs);
    }
    ...
    public Optional<OwnerLease> acquire(
        ...
        return validateAcquired(
                backend.acquire(assignment, ownerId, context.sessionIdentity(), nowEpochMs, leaseDurationMs),
                assignment.shardId(),
                ownerId,
                nowEpochMs,
                context);
    ...
    public Optional<OwnerLease> renew(final OwnerLease expected, final long nowEpochMs, final long leaseDurationMs) {
        ...
        if (!sameIdentity(expected, renewed)
                || renewed.state() != expected.state()
    ...
    public Optional<OwnerLease> transition(final OwnerLease expected, final ShardLifecycleState nextState) {
        ...
        if (!sameIdentity(expected, transitioned)
                || transitioned.state() != nextState
    ...
    public Optional<OwnerLease> transitionOrRead(final OwnerLease expected, final ShardLifecycleState nextState) {
        ...
        if (observed.isPresent()
                && expected.sameIdentity(observed.get())
    ...
    private static Optional<OwnerLease> validateAcquired(
            final Optional<OwnerLease> result,
    ...
    private static Optional<OwnerLease> validateAcquired(
            final Optional<OwnerLease> result, final ShardId shardId, final String ownerId, final long nowEpochMs) {
        ...
        if (!shardId.equals(lease.shardId())
                ...
                || lease.state() != ShardLifecycleState.ACQUIRING) {
    ...
    private static boolean sameIdentity(final OwnerLease expected, final OwnerLease actual) {
        return expected.shardId().equals(actual.shardId())
