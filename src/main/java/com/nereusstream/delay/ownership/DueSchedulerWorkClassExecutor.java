...
public final class DueSchedulerWorkClassExecutor {
    ...
    public DueSchedulerWorkClassExecutor(
    ...
    public Submission submit(
        ...
        final WorkClassTask task = new WorkClassTask(
                WorkClass.DUE_SCHEDULER, taskId(scheduler.shardId(), evidenceBytes, submittedBudget), chargedBytes);
        final Submission submission = new Submission(task);
        workClasses.submit(
                task,
                () -> submission.complete(ownedShard.discoverReadyAuthoritativelyStrict(
                        authority, scheduler, submittedEvidence, submittedBudget, submittedClock)));
        return submission;
    }

    private static String taskId(final ShardId shard, final byte[] evidence, final SchedulerBudget budget) {
        final byte[] digest = Bytes.sha256(
                TASK_ID_DOMAIN,
                shard.routeIncarnation().bytes(),
                Bytes.u32beBits(shard.partition()),
                Bytes.lp32(evidence),
                Bytes.u32be(budget.maxMessages()),
                Bytes.u64be(budget.maxBytes()),
                Bytes.u64be(budget.maxElapsedNanos()));
