...
public final class LaneCloseDiscoveryWorkClassExecutor {
    ...
    public LaneCloseDiscoveryWorkClassExecutor(
    ...
    public Submission submit(
            final SchedulerBudget scanBudget, final LongSupplier ownerClock, final LongSupplier scanClockNanos) {
        ...
        final WorkClassTask task = new WorkClassTask(
                WorkClass.GC, "lane-close-discovery/" + Bytes.hex(Bytes.sha256(identity)), chargedBytes);
        final Submission submission = new Submission(task);
        workClasses.submit(
                task,
                () -> submission.complete(ownedShard.discoverLaneCloseAuthoritativelyStrict(
                        authority, submittedBudget, submittedOwnerClock, submittedScanClock)));
        return submission;
    }
