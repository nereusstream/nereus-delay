...
public final class ClaimExecutionAdmission {
    ...
    public ClaimExecutionAdmission(final long maximumWorkerMessages, final long maximumWorkerBytes) {
    ...
    public synchronized void openReady(
            final ShardId shardId, final DestinationLaneId laneId, final byte[] laneIncarnation) {
    ...
    public synchronized void closeReady(
            final ShardId shardId, final DestinationLaneId laneId, final byte[] laneIncarnation) {
        final LaneState lane = lane(shardId, laneId);
        requireIncarnation(lane, laneIncarnation);
        lane.ready = false;
    ...
    public synchronized AdmissionDecision tryAcquire(
        ...
        final Reservation reservation = new Reservation(
                this, reservationId, identity, requestedShard, requestedLane, lane.laneIncarnation, accountedBytes);
    ...
    public synchronized LaneSnapshot laneSnapshot(final ShardId shardId, final DestinationLaneId laneId) {
        final LaneState lane = lane(shardId, laneId);
    ...
    public synchronized ShardSnapshot shardSnapshot(final ShardId shardId) {
        ...
        return new ShardSnapshot(
                ...
                shard.activeBytes,
                protectedShardMessages(null, shard.shardId),
                protectedShardBytes(null, shard.shardId),
                shard.maxMessages,
                shard.maxBytes);
    }
    ...
    public synchronized WorkerSnapshot workerSnapshot() {
        return new WorkerSnapshot(
                ...
                workerActiveBytes,
                protectedWorkerMessages(null, false),
                protectedWorkerBytes(null, false),
                workerMaxMessages,
                workerMaxBytes);
    }
    ...
    private long protectedWorker(
            final LaneState excluded, final boolean includeExcludedIfClosed, final boolean messages) {
        long total = 0;
        ...
        if (excluded != null && includeExcludedIfClosed && !excluded.ready) {
            total = Math.addExact(total, messages ? excluded.minimumReadyMessages : excluded.minimumReadyBytes);
        }
        return total;
    }
    ...
    private long protectedShard(
            final LaneState excluded,
            final ShardId shardId,
            final boolean includeExcludedIfClosed,
    ...
    private LaneState lane(final ShardId shardId, final DestinationLaneId laneId) {
        final LaneState result = lanes.get(
                new LaneKey(Objects.requireNonNull(shardId, "shardId"), Objects.requireNonNull(laneId, "laneId")));
        if (result == null) {
    ...
    public record LaneSpec(
        ...
        public LaneSpec {
            ...
            Bytes.requireLength(laneIncarnation, 16, "laneIncarnation");
            if (minimumReadyMessages < 0
                    || minimumReadyBytes < 0
                    || maxMessages <= 0
                    || maxBytes <= 0
                    || minimumReadyMessages > maxMessages
    ...
    public final class Reservation implements AutoCloseable {
        ...
        private Reservation(
                final ClaimExecutionAdmission owner,
                final long id,
                final ClaimIdentity identity,
                final ShardId shardId,
                final DestinationLaneId laneId,
                final byte[] laneIncarnation,
                final long accountedBytes) {
