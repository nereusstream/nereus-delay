...
public final class WorkClassEventLoop {
    ...
    public WorkClassEventLoop(final WorkClassScheduler scheduler, final WorkClassResourcePool resources) {
    ...
    public synchronized Turn poll(final SchedulerBudget budget) {
        ...
        try {
            final List<WorkClassTask> tasks =
                    scheduler.poll(budget, task -> builder.add(resources.acquire(task.workClass(), 1, task.bytes())));
            if (tasks.isEmpty()) {
                builder.release();
                return Turn.empty();
            }
    ...
    private static final class TurnBuilder {
        private final java.util.ArrayList<WorkClassResourcePool.ResourceLease> leases = new java.util.ArrayList<>();
    ...
    public static final class Turn implements AutoCloseable {
        ...
        private Turn(
                final WorkClassEventLoop owner,
                final List<WorkClassTask> tasks,
