...
public final class WorkerScheduler {
    ...
    synchronized void registerShard(final ShardId shardId, final int weight, final LaneScheduler laneScheduler) {
    ...
    public synchronized List<ScheduleWorkItem> poll(final long dueThroughEpochMs, final SchedulerBudget budget) {
        ...
        try {
            ...
            while (visits < visitLimit
                    && result.size() < budget.maxMessages()
                ...
                final Set<ShardId> eligible =
                        firstPassVisit ? eligibleShards(dueThroughEpochMs, budget.maxBytes()) : Set.of();
                if (firstPassVisit && recoveryServed.contains(shard.shardId)) {
                    continue;
                }
                shard.deficit = Math.min(
                        saturatingAdd(shard.deficit, checkedWeightIncrement(shard.weight)),
                ...
                final List<ScheduleWorkItem> visit = shard.scheduler.poll(
                        dueThroughEpochMs,
                        new SchedulerBudget(
                                visitMaxMessages, shardBudgetBytes, Math.max(1, budget.maxElapsedNanos() - elapsed)));
                if (visit.isEmpty()) {
                    continue;
                }
                long visitBytes = 0;
    ...
    private PollSnapshot pollSnapshot() {
        ...
        for (Map.Entry<ShardId, ShardQueue> entry : shards.entrySet()) {
            ...
            shardSnapshots.put(
                    entry.getKey(),
                    new ShardPollSnapshot(
                            shard.scheduler.snapshot(), shard.deficit, shard.lastServedRound, shard.blocked));
        }
        return new PollSnapshot(
                ...
                roundGeneration,
                recoveryFirstPass,
                Set.copyOf(recoveryServed),
                Map.copyOf(shardSnapshots));
    }
    ...
    public synchronized WorkerSnapshot snapshot() {
        final List<ShardSnapshot> states = shards.values().stream()
                ...
                .map(state -> new ShardSnapshot(
                        state.shardId, state.weight, state.deficit, state.lastServedRound, state.blocked))
                .toList();
    ...
    private record PollSnapshot(
            ...
            Set<ShardId> recoveryServed,
            Map<ShardId, ShardPollSnapshot> shards) {}

    private record ShardPollSnapshot(
            LaneScheduler.SchedulerSnapshot scheduler, long deficit, long lastServedRound, boolean blocked) {}
