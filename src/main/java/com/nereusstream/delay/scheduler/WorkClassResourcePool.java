...
public final class WorkClassResourcePool {
    ...
    public WorkClassResourcePool(
            final Map<WorkClass, WorkClassPolicy> policies,
    ...
    public synchronized ResourceLease acquire(final WorkClass workClass, final long records, final long bytes) {
        ...
        final long previousBorrowedRecords = Math.max(0, classUsage.records - policy.nonBorrowableMinimumRecords());
        final long previousBorrowedBytes = Math.max(0, classUsage.bytes - policy.nonBorrowableMinimumBytes());
        final long borrowedRecords =
                Math.max(0, nextClassRecords - policy.nonBorrowableMinimumRecords()) - previousBorrowedRecords;
        final long borrowedBytes =
                Math.max(0, nextClassBytes - policy.nonBorrowableMinimumBytes()) - previousBorrowedBytes;
        final long leaseId;
        try {
        ...
        final ResourceLease lease = new ResourceLease(
                leaseId, workClass, records, bytes, borrowedRecords > 0 || borrowedBytes > 0, readClock());
    ...
    private long protectedMinimumRecords(final WorkClass excluded) {
        ...
        for (WorkClass workClass : WorkClass.values()) {
            ...
            result = Math.addExact(
                    result, Math.max(0, policy.nonBorrowableMinimumRecords() - held.get(workClass).records));
        }
        return result;
    }
    ...
    private long protectedMinimumBytes(final WorkClass excluded) {
        ...
        for (WorkClass workClass : WorkClass.values()) {
            ...
            final WorkClassPolicy policy = policies.get(workClass);
            result = Math.addExact(result, Math.max(0, policy.nonBorrowableMinimumBytes() - held.get(workClass).bytes));
        }
        return result;
    }
    ...
    private void release(final ResourceLease lease) {
        synchronized (this) {
            ...
            if (classUsage.records < lease.records
    ...
    public record Snapshot(long usedRecords, long usedBytes, long totalRecords, long totalBytes, int activeLeases) {
        public Snapshot {
            if (usedRecords < 0
                    || usedBytes < 0
                    || totalRecords <= 0
