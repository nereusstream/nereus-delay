...
public final class WorkClassScheduler {
    ...
    synchronized List<WorkClassTask> poll(final SchedulerBudget budget, final Consumer<WorkClassTask> beforeRemove) {
        ...
        try {
            ...
            while (result.size() < budget.maxMessages() && bytes < budget.maxBytes()) {
                ...
                final int selected = priority >= 0 ? priority : normalIndex(usage, budget.maxBytes() - bytes, elapsed);
                if (selected < 0) {
            ...
            for (int index = tasks.size() - 1; index >= 0; index--) {
                ...
                if (state.queue.size() >= state.policy.maxQueueRecords()
                    ...
                    throw new IllegalStateException(
                            "reserved work-class requeue capacity was lost for " + task.workClass());
                }
    ...
    private int priorityIndex(
            final EnumMap<WorkClass, TurnUsage> usage, final long remainingBytes, final long elapsedNanos) {
        final boolean deferPreemptive =
                preemptiveServedSinceNormal && hasRunnableNonPreemptive(usage, remainingBytes, elapsedNanos);
        int selected = -1;
        ...
        for (int index = 0; index < order.size(); index++) {
            ...
            final boolean overdue = elapsedSince(state.lastServedNanos, readClock()) >= maxEventLoopClassDelayNanos;
    ...
    private boolean hasRunnableNonPreemptive(
            final EnumMap<WorkClass, TurnUsage> usage, final long remainingBytes, final long elapsedNanos) {
        for (WorkClass workClass : order) {
    ...
    private boolean canServe(
            final ClassState state, final TurnUsage usage, final long remainingBytes, final long elapsedNanos) {
    ...
    private void replenishCredits() {
        for (ClassState state : states.values()) {
            state.credits = state.credits > Integer.MAX_VALUE - state.policy.weight()
                    ? Integer.MAX_VALUE
                    : state.credits + state.policy.weight();
        }
    }
    ...
    private SchedulerSnapshot snapshot() {
        ...
        for (WorkClass workClass : order) {
            ...
            classSnapshots.put(
                    workClass,
                    new ClassSnapshot(
                            List.copyOf(state.queue), state.queuedBytes, state.credits, state.lastServedNanos));
        }
    ...
    private record SchedulerSnapshot(
            EnumMap<WorkClass, ClassSnapshot> classes, int cursor, boolean preemptiveServedSinceNormal) {}

    private record ClassSnapshot(List<WorkClassTask> queue, long queuedBytes, int credits, long lastServedNanos) {}
}
