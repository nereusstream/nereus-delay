...
public final class LaneScheduler {
     ...
     */
    synchronized List<ScheduleWorkItem> pollRecoveryFirstPass(
            final SchedulerBudget budget, final Set<DestinationLaneId> alreadyServed) {
        return pollRecoveryFirstPass(Long.MAX_VALUE, budget, alreadyServed);
    }

    synchronized List<ScheduleWorkItem> pollRecoveryFirstPass(
            final long dueThroughEpochMs, final SchedulerBudget budget, final Set<DestinationLaneId> alreadyServed) {
        requireDueThrough(dueThroughEpochMs);
    ...
    private List<ScheduleWorkItem> pollInternal(
        ...
        try {
            ...
            while (visits < ringVisitLimit
                    && result.size() < Math.min(maxVisitMessages, budget.maxMessages())
                ...
                lane.deficit = Math.min(
                        saturatingAdd(lane.deficit, increment), Math.max(maxDeficitBytes, head.accountedBytes()));
    ...
    synchronized void markReady(final DestinationLaneId laneId) {
        final LaneQueue lane = requireLane(laneId);
        if (lane.gate != com.nereusstream.delay.runtime.AdmissionGate.OPEN) {
    ...
    synchronized void restoreReadiness(final Map<DestinationLaneId, RuntimeReadiness> snapshot) {
        ...
        for (Map.Entry<DestinationLaneId, RuntimeReadiness> entry : snapshot.entrySet()) {
            requireLane(entry.getKey()).runtimeReadiness =
                    Objects.requireNonNull(entry.getValue(), "snapshot readiness");
        }
    }
    ...
    public synchronized SchedulerSnapshot snapshot() {
        final List<LaneSnapshot> states = lanes.values().stream()
                ...
                .map(state -> new LaneSnapshot(
                        state.laneId,
                        state.weight,
                        state.deficit,
                        state.lastServedRound,
                        state.queue.size(),
                        state.schedulable()))
                .toList();
    ...
    synchronized void unregister(final DestinationLaneId laneId, final byte[] laneIncarnation) {
        ...
        if (lane.gate != com.nereusstream.delay.runtime.AdmissionGate.RETIRED) {
    ...
    private static final class LaneQueue {
        ...
        private com.nereusstream.delay.runtime.AdmissionGate gate;
