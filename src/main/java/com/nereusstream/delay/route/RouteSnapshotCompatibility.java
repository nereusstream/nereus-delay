 ...
 */
public final class RouteSnapshotCompatibility {
    private RouteSnapshotCompatibility() {}

    public static void requireCompatibleSuccessor(final RouteSnapshot previous, final RouteSnapshot next) {
        ...
        if (!Bytes.constantTimeEquals(previous.authenticatedTenantScopeHash(), next.authenticatedTenantScopeHash())
        ...
        if (!Bytes.constantTimeEquals(previousCredential.bindingId(), nextCredential.bindingId())
                ...
                || !Bytes.constantTimeEquals(
                        previousCredential.resolvedCredentialFingerprintDigest(),
                        nextCredential.resolvedCredentialFingerprintDigest())
                || !Bytes.constantTimeEquals(
                        previousCredential.authorizationScopeDigest(), nextCredential.authorizationScopeDigest())
        ...
        for (int index = 0; index < previous.partitions().size(); index++) {
            final RoutePartitionPolicy oldPolicy = previous.partitions().get(index);
            final RoutePartitionPolicy newPolicy = next.partitions().get(index);
            if (oldPolicy.partition() != newPolicy.partition()
                    || !oldPolicy.activationBarrier().equals(newPolicy.activationBarrier())
                    || oldPolicy.brokerGuardAttestationGeneration() != newPolicy.brokerGuardAttestationGeneration()
                    || !Bytes.constantTimeEquals(
                            oldPolicy.brokerGuardAttestationDigest(), newPolicy.brokerGuardAttestationDigest())) {
