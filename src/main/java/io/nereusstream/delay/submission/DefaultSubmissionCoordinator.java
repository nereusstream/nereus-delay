...
public final class DefaultSubmissionCoordinator implements SubmissionCoordinator {
    ...
    public DefaultSubmissionCoordinator(final SubmissionTransportPlanResolver resolver,
    ...
    public CompletionStage<SubmissionOutcomeMessageV1> submit(final AuthenticatedTenantContext tenant,
        ...
        try {
            ...
            permit.close();
            return completed(permit.state() == TransportOwnershipState.LIBRARY_OWNED
        ...
        try {
            ...
            return mapped == null
                    ? completed(projector.uncertain(plan, attempt, StableCode.ENQUEUE_RESULT_UNCERTAIN)) : mapped;
    ...
    private static SubmissionOutcomeMessageV1 localFailure(final SubmissionTransportPlan plan,
                                                            final PhysicalEnqueueAttemptId attempt,
                                                            final StableCode code) {
        if (plan.submission().isManaged()) {
            final PreparedCommand command = CommandCodec.decodeFrameV1(plan.submission().managedFrame());
    ...
    private static SubmissionOutcomeMessageV1 localFailure(final PreparedSubmissionV1 submission,
                                                            final PhysicalEnqueueAttemptId attempt,
                                                            final StableCode code) {
