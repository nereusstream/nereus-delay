...
public final class OxiaSignedRouteSnapshotPublisher {
    ...
    public OxiaSignedRouteSnapshotPublisher(final SyncOxiaClient client, final String keyPrefix,
    ...
    public Publication publish(final RouteSelectionHint route, final RouteSnapshotV1 snapshot,
        ...
        final long observedRevision = currentHead == null ? 0 : currentHead.head().publishedRevision();
        ...
        final OxiaRouteSnapshotRecordV1 event = OxiaRouteSnapshotRecordV1.create(nextRevision,
                expectedPreviousRevision, route, verified);
        if (expectedPreviousRevision > 0) {
            ...
            final OxiaRouteSnapshotRecordV1 previous = OxiaRouteSnapshotRecordV1.decode(previousResult.value(),
                    verificationKey);
        ...
        try {
            ...
            if (observed != null && Bytes.constantTimeEquals(observed.head().canonicalBytes(),
                    nextHead.canonicalBytes())) {
    ...
    private void putEventExactly(final OxiaRouteSnapshotRecordV1 event) {
        ...
        if (existing != null) {
            if (existing.value() == null || !key.equals(existing.key())
        ...
        try {
            ...
            if (observed == null || observed.value() == null
    ...
    private void putHeadExactly(final OxiaRouteSnapshotHeadV1 nextHead, final HeadEntry currentHead) {
        final Set<PutOption> options = currentHead == null ? Set.of(PutOption.IfRecordDoesNotExist)
    ...
    private static final class SyncRecordClient implements OxiaRouteRecordClient {
        ...
        @Override
        public io.oxia.client.api.CloseableIterable<GetResult> rangeScan(final String startKeyInclusive,
