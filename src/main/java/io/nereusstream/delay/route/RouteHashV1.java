...
public final class RouteHashV1 {
    ...
    public static int partition(final RouteSnapshotV1 snapshot, final byte[] tenantRoutingScope,
        ...
        final byte[] digest = Bytes.sha256(Bytes.utf8("nereus-delay-routing-v1"),
                Bytes.lp32(snapshot.routeIncarnation().bytes()), Bytes.lp32(tenantRoutingScope),
                Bytes.lp32(routingKey));
        final long unsignedFirst64 = Bytes.readU64be(digest, 0);
        return (int) Long.remainderUnsigned(unsignedFirst64,
                Integer.toUnsignedLong(snapshot.ingress().partitionCount()));
    }
}
