...
public final class OxiaRouteSnapshotRecordV1 {
    public static final int VERSION = 1;
    private static final int MAX_SNAPSHOT_BYTES = 16 * 1024 * 1024;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-oxia-route-event-v1\0");
    ...
    private OxiaRouteSnapshotRecordV1(final long revision, final long previousRevision,
                                      final RouteSelectionHint route, final RouteSnapshotV1 snapshot) {
        if (revision <= 0 || previousRevision < 0 || previousRevision == Long.MAX_VALUE
    ...
    public static OxiaRouteSnapshotRecordV1 create(final long revision, final long previousRevision,
                                                    final RouteSelectionHint route,
                                                    final RouteSnapshotV1 snapshot) {
        return new OxiaRouteSnapshotRecordV1(revision, previousRevision, route, snapshot);
    }

    public static OxiaRouteSnapshotRecordV1 decode(final byte[] encoded, final PublicKey verificationKey) {
        ...
        final RouteSelectionHint route = new RouteSelectionHint(
                io.nereusstream.delay.protocol.AdapterKindV1.fromWire(uint(next(reader, 4), 4)),
        ...
        if (reader.hasRemaining() || !Bytes.constantTimeEquals(digest,
                Bytes.sha256(DIGEST_DOMAIN, canonicalWithoutDigest(revision, previousRevision, route,
                        snapshotBytes)))) {
            throw new IllegalArgumentException("Route event digest or field order is invalid");
        }
        final RouteSnapshotV1 snapshot = RouteSnapshotV1.decode(snapshotBytes, verificationKey);
        final OxiaRouteSnapshotRecordV1 result = new OxiaRouteSnapshotRecordV1(revision, previousRevision, route,
                snapshot);
    ...
    public RouteSnapshotV1 snapshot() {
        return snapshot;
    }

    public byte[] recordDigest() {
        return Bytes.sha256(DIGEST_DOMAIN, canonicalWithoutDigest(revision, previousRevision, route,
    ...
    private static byte[] canonicalWithoutDigest(final long revision, final long previousRevision,
                                                 final RouteSelectionHint route, final byte[] snapshotBytes) {
        return CanonicalProtobuf.message(output -> {
