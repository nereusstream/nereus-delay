 ...
 */
public final class ProtocolActivationStateV1 {
    public static final int VERSION = 1;
    public static final int DIGEST_LENGTH = 32;
    private static final int MAX_ACTIVATIONS = 32;
    private static final int MAX_CANONICAL_BYTES = 1 << 20;
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8("nereus-delay-protocol-activation-state-v1\0");

    private final ShardSubjectV1 shard;
    private final List<Activation> activations;
    private final byte[] stateDigest;

    public ProtocolActivationStateV1(final ShardSubjectV1 shard, final List<Activation> activations) {
        this(shard, activations, null);
    }

    private ProtocolActivationStateV1(final ShardSubjectV1 shard, final List<Activation> activations,
    ...
    public boolean isMarkedActivated(final ProtocolTupleV1 tuple) {
        Objects.requireNonNull(tuple, "tuple");
        return activation(tuple) != null;
    }

    public Activation activation(final ProtocolTupleV1 tuple) {
        Objects.requireNonNull(tuple, "tuple");
    ...
    public ProtocolActivationStateV1 activate(final ProtocolTupleV1 tuple, final byte[] canonicalSchemaHash,
                                              final byte[] compatibleReaderSetEvidenceHash,
                                              final SourcePosition sourcePosition, final byte[] mutationId) {
        Objects.requireNonNull(tuple, "tuple");
        ...
        next.add(new Activation(tuple, canonicalSchemaHash, compatibleReaderSetEvidenceHash,
                sourcePosition.canonicalBytes(), mutationId));
        return new ProtocolActivationStateV1(shard, next);
    }
    ...
    public static ProtocolActivationStateV1 decode(final byte[] encoded) {
        ...
        final ProtocolActivationStateV1 result = new ProtocolActivationStateV1(shard, activations, stateDigest);
        for (Activation activation : result.activations) {
            if (!shard.shardId().equals(activation.sourcePosition().shardId())) {
                throw new IllegalArgumentException("protocol activation source position belongs to another shard");
            }
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ProtocolActivationStateV1");
        return result;
    }
    ...
    private static List<Activation> sortedActivations(final List<Activation> values) {
        ...
        result.sort(Comparator.comparing(activation -> activation.tuple().canonicalBytes(),
                ProtocolActivationStateV1::compareBytes));
    ...
    public static final class Activation {
        ...
        public Activation(final ProtocolTupleV1 tuple, final byte[] canonicalSchemaHash,
                          final byte[] compatibleReaderSetEvidenceHash, final byte[] sourcePosition,
            ...
            this.compatibleReaderSetEvidenceHash = fixed(compatibleReaderSetEvidenceHash,
                    "compatibleReaderSetEvidenceHash");
            this.sourcePosition = SourcePositionCodec.decode(Objects.requireNonNull(sourcePosition,
        ...
        public static Activation decode(final byte[] encoded) {
            final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded,
                    "ProtocolActivationStateV1.Activation");
    ...
    @Override
    public boolean equals(final Object other) {
        return other instanceof ProtocolActivationStateV1 that
