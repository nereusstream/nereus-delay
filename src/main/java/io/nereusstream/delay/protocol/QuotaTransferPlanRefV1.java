...
public final class QuotaTransferPlanRefV1 {
    ...
    public QuotaTransferPlanRefV1(final byte[] controlOperationId, final byte[] requestHash,
    ...
    public static QuotaTransferPlanRefV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(
                encoded, "QuotaTransferPlanRefV1");
        QueryCodecSupport.requireNumbers(fields, new int[]{1, 2, 3, 4}, "QuotaTransferPlanRefV1");
    ...
    public boolean equals(final Object other) {
        return other instanceof QuotaTransferPlanRefV1 that && tenantPolicyVersion == that.tenantPolicyVersion
                && Arrays.equals(controlOperationId, that.controlOperationId)
                && Arrays.equals(requestHash, that.requestHash) && Arrays.equals(planHash, that.planHash);
    }

    @Override
    public int hashCode() {
        return Objects.hash(Arrays.hashCode(controlOperationId), Arrays.hashCode(requestHash),
                tenantPolicyVersion, Arrays.hashCode(planHash));
    }
