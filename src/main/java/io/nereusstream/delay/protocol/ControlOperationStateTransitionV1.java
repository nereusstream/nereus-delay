...
public final class ControlOperationStateTransitionV1 {
    ...
    public static void validate(final ControlOperationStateV1 current, final ControlOperationStateV1 next) {
        ...
        final boolean allowed = switch (current) {
            case PENDING -> next == ControlOperationStateV1.DISPATCHING
                    || next == ControlOperationStateV1.REJECTED
                    || next == ControlOperationStateV1.FAILED_BEFORE_EFFECT;
            case DISPATCHING -> next == ControlOperationStateV1.PARTIALLY_EFFECTIVE
                    || next == ControlOperationStateV1.IN_PROGRESS
                    || next == ControlOperationStateV1.REJECTED
                    || next == ControlOperationStateV1.FAILED_BEFORE_EFFECT;
            case PARTIALLY_EFFECTIVE -> next == ControlOperationStateV1.IN_PROGRESS
                    || next == ControlOperationStateV1.SUCCEEDED
                    || next == ControlOperationStateV1.SUCCEEDED_WITH_OUTSTANDING;
            case IN_PROGRESS -> next == ControlOperationStateV1.SUCCEEDED
                    || next == ControlOperationStateV1.SUCCEEDED_WITH_OUTSTANDING;
            case SUCCEEDED, SUCCEEDED_WITH_OUTSTANDING, REJECTED, FAILED_BEFORE_EFFECT -> false;
        };
        if (!allowed) {
            throw new IllegalArgumentException("invalid Control Operation state transition: "
                    + current + " -> " + next);
        }
    }
     ...
     */
    public static void validateTargets(final List<ControlTargetStateViewV1> current,
                                       final List<ControlTargetStateViewV1> next) {
        ...
        for (int index = 0; index < current.size(); index++) {
            ...
            if (before.markerState() != after.markerState()
                    && after.targetRevision() <= before.targetRevision()) {
    ...
    public static void validateTarget(final TargetMarkerStateV1 current, final TargetMarkerStateV1 next) {
        ...
        final boolean allowed = switch (current) {
            case PENDING -> next == TargetMarkerStateV1.ENQUEUE_UNCERTAIN
                    || next == TargetMarkerStateV1.QUEUED
                    || next == TargetMarkerStateV1.REJECTED
                    || next == TargetMarkerStateV1.FAILED_BEFORE_EFFECT;
            case ENQUEUE_UNCERTAIN -> next == TargetMarkerStateV1.QUEUED
                    || next == TargetMarkerStateV1.EFFECTIVE
                    || next == TargetMarkerStateV1.REJECTED
                    || next == TargetMarkerStateV1.FAILED_BEFORE_EFFECT;
            case QUEUED -> next == TargetMarkerStateV1.EFFECTIVE
                    || next == TargetMarkerStateV1.REJECTED
                    || next == TargetMarkerStateV1.FAILED_BEFORE_EFFECT;
            case EFFECTIVE -> next == TargetMarkerStateV1.MATERIALIZING
                    || next == TargetMarkerStateV1.COMPLETED;
            case MATERIALIZING -> next == TargetMarkerStateV1.COMPLETED;
            case COMPLETED, REJECTED, FAILED_BEFORE_EFFECT -> false;
        };
        if (!allowed) {
            throw new IllegalArgumentException("invalid Control target marker transition: "
                    + current + " -> " + next);
        }
    }
