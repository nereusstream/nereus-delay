 ...
 */
public final class ControlRegistrationBindingV1 {
    private ControlRegistrationBindingV1() {
    }

    public static void validate(final PreparedControlOperationV1 prepared,
                                final ControlRegistrationOutcomeMessageV1 outcome) {
        Objects.requireNonNull(prepared, "prepared");
        Objects.requireNonNull(outcome, "outcome");
        switch (outcome.outcome()) {
            case RECORDED -> validateReceipt(prepared, outcome.receipt());
            case DEFINITELY_NOT_RECORDED -> {
                final ControlDefinitelyNotRecordedV1 rejection = outcome.definitelyNotRecorded();
                if (!Bytes.constantTimeEquals(prepared.preparedDigest(), rejection.preparedDigest())
                        || !Bytes.constantTimeEquals(prepared.operationId(), rejection.proof().operationId())
                        || !Bytes.constantTimeEquals(prepared.preparedDigest(), rejection.proof().preparedDigest())) {
                    throw new IllegalArgumentException("Control rejection is not bound to the prepared operation");
                }
            }
            case RECORD_UNCERTAIN -> {
                final ControlRecordUncertainV1 uncertain = outcome.uncertain();
                if (!Bytes.constantTimeEquals(prepared.operationId(), uncertain.operationId())
                    ...
                    throw new IllegalArgumentException("uncertain Control outcome is not bound to the prepared operation");
                }
            }
        }
    }

    private static void validateReceipt(final PreparedControlOperationV1 prepared,
        ...
        if (!Bytes.constantTimeEquals(prepared.operationId(), receipt.operationId())
                ...
                || !Bytes.constantTimeEquals(prepared.author().tenantResourceScopeHash(),
