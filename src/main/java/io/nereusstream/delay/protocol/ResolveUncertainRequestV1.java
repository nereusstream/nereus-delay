...
public final class ResolveUncertainRequestV1 implements ControlOperationRequestBranchV1 {
    ...
    public ResolveUncertainRequestV1(final UncertainResolutionKindV1 resolutionKind,
                                     final PublishEvidenceV1 evidence, final boolean allowPossibleDuplicate,
    ...
    public static ResolveUncertainRequestV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded,
                "ResolveUncertainRequestV1");
        if (fields.size() != 4 && fields.size() != 5) {
            throw new IllegalArgumentException("ResolveUncertainRequestV1 has an unexpected field count");
        }
        if (fields.get(0).number() != 1
                || (fields.size() == 5 ? fields.get(1).number() != 2 : fields.get(1).number() != 3)
                || (fields.size() == 5 ? fields.get(2).number() != 3 : fields.get(2).number() != 4)
                || (fields.size() == 5 ? fields.get(3).number() != 4 : fields.get(3).number() != 5)
        ...
        final PublishEvidenceV1 evidence = offset == 1
                ? PublishEvidenceV1.decode(QueryCodecSupport.nested(fields.get(1), 2)) : null;
        final ResolveUncertainRequestV1 result = new ResolveUncertainRequestV1(
                UncertainResolutionKindV1.fromWire(QueryCodecSupport.uint(fields.get(0), 1)), evidence,
    ...
    private void validateMatrix() {
        switch (resolutionKind) {
            ...
            case RETRY_ALLOW_POSSIBLE_DUPLICATE -> {
                ...
                if (!allowPossibleDuplicate || allowPossibleDeliveryTerminal
    ...
    public boolean equals(final Object other) {
        return other instanceof ResolveUncertainRequestV1 that
                && resolutionKind == that.resolutionKind && Objects.equals(evidence, that.evidence)
    ...
    @Override
    public int hashCode() {
        return Objects.hash(resolutionKind, evidence, allowPossibleDuplicate,
                allowPossibleDeliveryTerminal, acknowledgements);
    }
}
