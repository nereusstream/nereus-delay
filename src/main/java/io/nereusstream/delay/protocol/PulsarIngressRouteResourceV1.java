...
public final class PulsarIngressRouteResourceV1 implements IngressRouteResourceV1 {
    ...
    public PulsarIngressRouteResourceV1(final String authenticatedClusterId, final String physicalTopicBase,
        ...
        for (int index = 0; index < partitions.size(); index++) {
            final PulsarPhysicalPartitionIdentityV1 partition = Objects.requireNonNull(partitions.get(index),
                    "partitions[" + index + "]");
    ...
    public static PulsarIngressRouteResourceV1 decode(final byte[] encoded) {
        ...
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(
                QueryCodecSupport.nested(outer.get(0), 2), "PulsarIngressRouteResourceV1", true);
        ...
        for (int index = 0; index < partitionCount; index++) {
            ...
            partitions.add(PulsarPhysicalPartitionIdentityV1.decode(
                    QueryCodecSupport.nested(fields.get(index + 3), 4)));
        }
        final PulsarIngressRouteResourceV1 result = new PulsarIngressRouteResourceV1(
                utf8(QueryCodecSupport.bytes(fields.get(0), 1)),
                utf8(QueryCodecSupport.bytes(fields.get(1), 2)), partitions);
