...
public final class PendingCommandViewV1 implements QueryResponseBranchV1 {
    ...
    public PendingCommandViewV1(final SourcePosition awaitedSourcePosition, final SourcePosition currentSourcePosition,
    ...
    public static PendingCommandViewV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded, "PendingCommandViewV1");
        if (fields.size() != 2 && fields.size() != 3) {
            throw new IllegalArgumentException("invalid PendingCommandViewV1 field count");
        }
        if (fields.get(0).number() != 1 || fields.get(fields.size() - 1).number() != 3
                || (fields.size() == 3 && fields.get(1).number() != 2)) {
            throw new IllegalArgumentException("invalid PendingCommandViewV1 fields");
        }
        final PendingCommandViewV1 result = new PendingCommandViewV1(
                QueryCodecSupport.decodeSourcePosition(QueryCodecSupport.nested(fields.get(0), 1)),
                fields.size() == 3
                        ? QueryCodecSupport.decodeSourcePosition(QueryCodecSupport.nested(fields.get(1), 2)) : null,
                QueryCodecSupport.uint(fields.get(fields.size() - 1), 3));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "PendingCommandViewV1");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        if (!(other instanceof PendingCommandViewV1 that)) {
            return false;
        }
        return retryAtEpochMs == that.retryAtEpochMs && awaitedSourcePosition.equals(that.awaitedSourcePosition)
