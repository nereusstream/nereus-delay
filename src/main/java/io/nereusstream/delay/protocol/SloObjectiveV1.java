...
public final class SloObjectiveV1 {
    ...
    public SloObjectiveV1(final SloObjectiveNameV1 name, final SloPopulationV1 population,
                          final SloThresholdDirectionV1 direction, final SloThresholdUnitV1 unit,
                          final long threshold, final long objectiveNumerator,
                          final long objectiveDenominator, final long rollingWindowMs,
                          final long minimumSamples, final List<DueExclusionReasonV1> exclusions,
                          final long healthyLoadEnvelopeVersion, final byte[] healthyLoadEnvelopeDigest) {
        this(name, population, direction, unit, threshold, objectiveNumerator, objectiveDenominator,
                rollingWindowMs, minimumSamples, SloTimeoutTreatmentV1.BAD, exclusions,
                healthyLoadEnvelopeVersion, healthyLoadEnvelopeDigest, null);
    }

    private SloObjectiveV1(final SloObjectiveNameV1 name, final SloPopulationV1 population,
                           final SloThresholdDirectionV1 direction, final SloThresholdUnitV1 unit,
                           final long threshold, final long objectiveNumerator,
                           final long objectiveDenominator, final long rollingWindowMs,
                           final long minimumSamples, final SloTimeoutTreatmentV1 timeoutTreatment,
                           final List<DueExclusionReasonV1> exclusions, final long healthyLoadEnvelopeVersion,
                           final byte[] healthyLoadEnvelopeDigest, final byte[] suppliedDigest) {
    ...
    public static SloObjectiveV1 decode(final byte[] encoded) {
        ...
        if (fields.get(0).number() != 1 || fields.get(1).number() != 2 || fields.get(2).number() != 3
                ...
                || fields.get(6).number() != 7 || fields.get(7).number() != 8 || fields.get(8).number() != 9
        ...
        if (index + 3 != fields.size() || fields.get(index).number() != 12
                || fields.get(index + 1).number() != 13 || fields.get(index + 2).number() != 14) {
