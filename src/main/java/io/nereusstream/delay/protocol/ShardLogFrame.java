...
public final class ShardLogFrame {
    ...
    public static byte[] encode(final int recordKind, final byte[] canonicalEnvelope) {
        ...
        header.putInt(MAGIC).put((byte) FRAME_VERSION).put((byte) recordKind).putShort((short) 0)
                .putInt(canonicalEnvelope.length);
        return Bytes.concat(header.array(), canonicalEnvelope,
                Bytes.crc32cbe(Bytes.concat(header.array(), canonicalEnvelope)));
    }
