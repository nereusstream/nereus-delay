...
public final class ProtocolCapabilityDeclarationV1 {
    ...
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8(
            "nereus-delay-protocol-capability-declaration-v1\0");
    ...
    public ProtocolCapabilityDeclarationV1(final String workerId, final byte[] workerIdentity,
                                           final List<ProtocolTupleV1> supportedTuples,
                                           final long capabilityEpoch, final byte[] sessionIdentity) {
        this(workerId, workerIdentity, supportedTuples, capabilityEpoch, sessionIdentity, null);
    }

    private ProtocolCapabilityDeclarationV1(final String workerId, final byte[] workerIdentity,
                                            final List<ProtocolTupleV1> supportedTuples,
                                            final long capabilityEpoch, final byte[] sessionIdentity,
    ...
    public static ProtocolCapabilityDeclarationV1 decode(final byte[] encoded) {
        ...
        if (fields.size() < 7 || fields.get(0).number() != 1 || fields.get(1).number() != 2
        ...
        if (index + 3 != fields.size() || fields.get(index).number() != 5
        ...
        final ProtocolCapabilityDeclarationV1 result = new ProtocolCapabilityDeclarationV1(
                workerId, workerIdentity, tuples, epoch, sessionIdentity, digest);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(),
                "ProtocolCapabilityDeclarationV1");
        return result;
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(workerId, Arrays.hashCode(workerIdentity), supportedTuples, capabilityEpoch,
                Arrays.hashCode(sessionIdentity), Arrays.hashCode(declarationDigest));
    }
    ...
    private static List<ProtocolTupleV1> sortedTuples(final List<ProtocolTupleV1> values) {
        ...
        result.sort(Comparator.comparing(ProtocolTupleV1::canonicalBytes,
                ProtocolCapabilityDeclarationV1::compareBytes));
