...
public final class OpaquePayloadUploadHandleV1 {
    ...
    private final UploadHandleKindV1 kind;
    private final long expiresAtEpochMs;
    private final byte[] capabilityEnvelope;
    private final byte[] capabilityEnvelopeSha256;

    private OpaquePayloadUploadHandleV1(final byte[] reservationId, final ProfileRefV1 objectStoreProfile,
                                        final UploadHandleKindV1 kind, final long expiresAtEpochMs,
                                        final byte[] capabilityEnvelope, final byte[] capabilityEnvelopeSha256) {
        ...
        if (capabilityEnvelope.length == 0 || capabilityEnvelope.length > MAX_CAPABILITY_ENVELOPE_BYTES) {
            throw new IllegalArgumentException("capability envelope is outside the bounded V1 range");
        }
    ...
    public static OpaquePayloadUploadHandleV1 create(final byte[] reservationId,
                                                     final ProfileRefV1 objectStoreProfile,
                                                     final UploadHandleKindV1 kind, final long expiresAtEpochMs,
                                                     final byte[] capabilityEnvelope) {
        return new OpaquePayloadUploadHandleV1(reservationId, objectStoreProfile, kind, expiresAtEpochMs,
                capabilityEnvelope, Bytes.sha256(capabilityEnvelope));
    }

    public static OpaquePayloadUploadHandleV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded,
                "OpaquePayloadUploadHandleV1");
        ...
        final OpaquePayloadUploadHandleV1 result = new OpaquePayloadUploadHandleV1(
                QueryCodecSupport.fixed(fields.get(1), 2, HASH_LENGTH),
                ProfileRefV1.decode(QueryCodecSupport.nested(fields.get(2), 3)),
                UploadHandleKindV1.fromWire(QueryCodecSupport.uint(fields.get(3), 4)),
                QueryCodecSupport.uint(fields.get(4), 5), QueryCodecSupport.bytes(fields.get(5), 6),
                QueryCodecSupport.fixed(fields.get(6), 7, HASH_LENGTH));
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "OpaquePayloadUploadHandleV1");
        return result;
    }
    ...
    public UploadHandleKindV1 kind() {
        return kind;
    }
    ...
    public boolean equals(final Object other) {
        if (!(other instanceof OpaquePayloadUploadHandleV1 that)) {
            return false;
        }
        return expiresAtEpochMs == that.expiresAtEpochMs && kind == that.kind
    ...
    @Override
    public int hashCode() {
        return Objects.hash(Arrays.hashCode(reservationId), objectStoreProfile, kind, expiresAtEpochMs,
                Arrays.hashCode(capabilityEnvelope), Arrays.hashCode(capabilityEnvelopeSha256));
    }
