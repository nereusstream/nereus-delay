...
public final class MessageControlResultV1 {
    ...
    public MessageControlResultV1(final DelayMessageId messageId, final int generation, final long stateVersion,
    ...
    public static MessageControlResultV1 decode(final byte[] encoded) {
        final var fields = QueryCodecSupport.read(encoded, "MessageControlResultV1");
        if (fields.size() != 5 && fields.size() != 6) {
            throw new IllegalArgumentException("invalid MessageControlResultV1 field count");
        }
        if (fields.get(0).number() != 1 || fields.get(1).number() != 2 || fields.get(2).number() != 3
            ...
            throw new IllegalArgumentException("invalid MessageControlResultV1 field order");
        }
        ...
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "MessageControlResultV1");
        return result;
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof MessageControlResultV1 that && generation == that.generation
                && stateVersion == that.stateVersion && messageId.equals(that.messageId) && state == that.state
                && stableCode == that.stableCode && Objects.equals(evidence, that.evidence);
    }
