...
public final class NativeDeliveryReceiptV1 {
    ...
    private NativeDeliveryReceiptV1(final NativePreparedRefV1 prepared,
                                    final CommandQueuedReceiptV1.PulsarQueuedAck brokerAck,
                                    final byte[] physicalEnqueueAttemptId, final byte[] payloadDigest) {
        ...
        if (brokerAck.partition() != prepared.physicalPartition()
                ...
                || !Arrays.equals(brokerAck.brokerResourceIncarnation(), prepared.target().resourceIncarnation())
                || !brokerAck.physicalTopic().equals(prepared.target().physicalTopic())
                || brokerAck.physicalTopicCreationTimestamp() != prepared.target().physicalTopicCreationTimestamp()) {
    ...
    public static NativeDeliveryReceiptV1 create(final NativePreparedRefV1 prepared,
                                                  final CommandQueuedReceiptV1.PulsarQueuedAck brokerAck,
                                                  final byte[] physicalEnqueueAttemptId) {
        final byte[] fields = canonicalFields(prepared, brokerAck, physicalEnqueueAttemptId);
        return new NativeDeliveryReceiptV1(prepared, brokerAck, physicalEnqueueAttemptId,
                Bytes.sha256(fields));
    }
    ...
    public static NativeDeliveryReceiptV1 decodePayload(final byte[] encoded) {
        ...
        final CommandQueuedReceiptV1.SafeBrokerAck safeAck = CommandQueuedReceiptV1.SafeBrokerAck.decode(
                QueryCodecSupport.nested(fields.get(2), 3));
    ...
    public boolean equals(final Object other) {
        ...
        return prepared.equals(that.prepared) && brokerAck.equals(that.brokerAck)
    ...
    @Override
    public int hashCode() {
        return Objects.hash(prepared, brokerAck, Arrays.hashCode(physicalEnqueueAttemptId),
                Arrays.hashCode(payloadDigest));
    }

    private static byte[] canonicalFields(final NativePreparedRefV1 prepared,
                                          final CommandQueuedReceiptV1.PulsarQueuedAck brokerAck,
                                          final byte[] attempt) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.uint32(output, 1, RECEIPT_VERSION);
