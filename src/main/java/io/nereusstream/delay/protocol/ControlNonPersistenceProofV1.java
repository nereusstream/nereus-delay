 ...
 */
public final class ControlNonPersistenceProofV1 {
    public static final int HASH_LENGTH = 32;

    private final ControlNonPersistenceProofKindV1 kind;
    ...
    private ControlNonPersistenceProofV1(final ControlNonPersistenceProofKindV1 kind, final byte[] operationId,
                                         final byte[] preparedDigest, final byte[] oxiaTransactionSha256,
                                         final byte[] authenticatedResponseSha256, final byte[] proofDigest) {
        ...
        this.authenticatedResponseSha256 = optionalFixed(authenticatedResponseSha256,
    ...
    public static ControlNonPersistenceProofV1 create(final ControlNonPersistenceProofKindV1 kind,
                                                      final byte[] operationId, final byte[] preparedDigest,
                                                      final byte[] oxiaTransactionSha256,
                                                      final byte[] authenticatedResponseSha256) {
        final byte[] fields = canonicalFields(kind, operationId, preparedDigest, oxiaTransactionSha256,
                authenticatedResponseSha256);
        return new ControlNonPersistenceProofV1(kind, operationId, preparedDigest, oxiaTransactionSha256,
                authenticatedResponseSha256, Bytes.sha256(fields));
    }

    public static ControlNonPersistenceProofV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded,
                "ControlNonPersistenceProofV1");
        if (fields.size() < 4 || fields.size() > 6) {
            throw new IllegalArgumentException("ControlNonPersistenceProofV1 fields are incomplete or unknown");
        }
        final ControlNonPersistenceProofKindV1 kind = ControlNonPersistenceProofKindV1.fromWire(
                QueryCodecSupport.uint(fields.get(0), 1));
        if (fields.get(1).number() != 2 || fields.get(2).number() != 3) {
            throw new IllegalArgumentException("ControlNonPersistenceProofV1 fixed field order is invalid");
        }
        int index = 3;
        ...
        if (index != fields.size() - 1 || fields.get(index).number() != 6) {
            throw new IllegalArgumentException("ControlNonPersistenceProofV1 proof digest is missing or out of order");
        }
        ...
        final ControlNonPersistenceProofV1 result = new ControlNonPersistenceProofV1(kind, operationId,
                preparedDigest, transaction, response, digest);
        if (!Bytes.constantTimeEquals(digest,
                Bytes.sha256(canonicalFields(kind, operationId, preparedDigest, transaction, response)))) {
            throw new IllegalArgumentException("ControlNonPersistenceProofV1 digest mismatch");
        }
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ControlNonPersistenceProofV1");
        return result;
    }

    public ControlNonPersistenceProofKindV1 kind() {
        return kind;
    }
    ...
    private static byte[] canonicalFields(final ControlNonPersistenceProofKindV1 kind, final byte[] operationId,
                                          final byte[] preparedDigest, final byte[] transaction,
                                          final byte[] response) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.uint32(output, 1, kind.wireValue());
            CanonicalProtobuf.bytes(output, 2, operationId);
    ...
    private static void validatePresence(final ControlNonPersistenceProofKindV1 kind, final byte[] transaction,
                                         final byte[] response) {
        if (kind == ControlNonPersistenceProofKindV1.BEFORE_OXIA_OWNERSHIP
                && (transaction != null || response != null)) {
            throw new IllegalArgumentException("BEFORE_OXIA_OWNERSHIP cannot carry Oxia evidence");
        }
        if (kind == ControlNonPersistenceProofKindV1.OXIA_CONDITIONAL_REJECTION
                && (transaction == null || response == null)) {
    ...
    public boolean equals(final Object other) {
        return other instanceof ControlNonPersistenceProofV1 that && kind == that.kind
                && Arrays.equals(operationId, that.operationId) && Arrays.equals(preparedDigest, that.preparedDigest)
    ...
    @Override
    public int hashCode() {
        return Objects.hash(kind, Arrays.hashCode(operationId), Arrays.hashCode(preparedDigest),
                Arrays.hashCode(oxiaTransactionSha256), Arrays.hashCode(authenticatedResponseSha256),
