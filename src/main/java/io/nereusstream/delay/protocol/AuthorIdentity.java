...
public final class AuthorIdentity {
    ...
    private AuthorIdentity(final Kind kind, final byte[] first, final byte[] second, final long generation,
                           final byte[] digest) {
    ...
    public static AuthorIdentity owner(final byte[] deploymentId, final byte[] workerRunId, final long ownerEpoch,
                                       final byte[] leaseFencingDigest) {
        return new AuthorIdentity(Kind.OWNER, deploymentId, workerRunId, ownerEpoch, leaseFencingDigest);
    }

    public static AuthorIdentity control(final byte[] operationActorIdHash, final byte[] roleSetHash,
                                         final byte[] tenantResourceScopeHash) {
        return new AuthorIdentity(Kind.CONTROL, fixed(operationActorIdHash, "operationActorIdHash"),
                fixed(roleSetHash, "authenticatedRoleSetHash"), 1,
    ...
    public static AuthorIdentity service(final byte[] serviceId, final byte[] serviceRunId,
    ...
    public void requireFor(final SystemMutationType mutationType) {
        final Kind expected = switch (Objects.requireNonNull(mutationType, "mutationType")) {
    ...
    public static AuthorIdentity decode(final byte[] encoded) {
        ...
        final List<CanonicalProtobuf.Reader.Field> fields = readAll(
                new CanonicalProtobuf.Reader(outer.get(0).rawValue()));
        final AuthorIdentity result = switch (branch) {
    ...
    private static AuthorIdentity decodeOwner(final List<CanonicalProtobuf.Reader.Field> fields) {
        ...
        return owner(bytes(fields.get(0), 1), bytes(fields.get(1), 2), positive(fields.get(2), 3),
    ...
    private static AuthorIdentity decodeControl(final List<CanonicalProtobuf.Reader.Field> fields) {
        ...
        return control(fixed(bytes(fields.get(0), 1), "operationActorIdHash"),
    ...
    public boolean equals(final Object other) {
        ...
        return kind == that.kind && generation == that.generation && Arrays.equals(first, that.first)
                && Arrays.equals(second, that.second) && Arrays.equals(digest, that.digest);
    }

    @Override
    public int hashCode() {
        return Objects.hash(kind, Arrays.hashCode(first), Arrays.hashCode(second), generation,
