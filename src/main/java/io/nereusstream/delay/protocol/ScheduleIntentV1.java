...
public final class ScheduleIntentV1 {
    ...
    private ScheduleIntentV1(final ProfileRefV1 profile, final RetryPolicyRefV1 retryPolicy,
                             final long deliverAtEpochMs, final long expireAtEpochMs,
                             final DeliveryMode deliveryMode, final OrderingMode orderingMode,
                             final byte[] orderingKey, final byte[] inlinePayload,
                             final CommittedPayloadDescriptorV1 committedPayload,
                             final AdapterMetadataV1 adapterMetadata, final byte[] businessKey,
                             final Long eventTimeEpochMs, final boolean payloadRequired) {
    ...
    public static ScheduleIntentV1 create(final ProfileRefV1 profile, final RetryPolicyRefV1 retryPolicy,
                                          final long deliverAtEpochMs, final long expireAtEpochMs,
                                          final DeliveryMode deliveryMode, final OrderingMode orderingMode,
                                          final byte[] orderingKey, final byte[] inlinePayload,
                                          final CommittedPayloadDescriptorV1 committedPayload,
        ...
        return new ScheduleIntentV1(profile, retryPolicy, deliverAtEpochMs, expireAtEpochMs, deliveryMode,
                orderingMode, orderingKey, inlinePayload, committedPayload, adapterMetadata, businessKey,
                eventTimeEpochMs, true);
    }

    /** Creates the PrepareLargeSchedule form, which deliberately has no payload branch. */
    public static ScheduleIntentV1 forPrepare(final ProfileRefV1 profile, final RetryPolicyRefV1 retryPolicy,
                                              final long deliverAtEpochMs, final long expireAtEpochMs,
                                              final DeliveryMode deliveryMode, final OrderingMode orderingMode,
                                              final byte[] orderingKey, final AdapterMetadataV1 adapterMetadata,
                                              final byte[] businessKey, final Long eventTimeEpochMs) {
        return new ScheduleIntentV1(profile, retryPolicy, deliverAtEpochMs, expireAtEpochMs, deliveryMode,
                orderingMode, orderingKey, null, null, adapterMetadata, businessKey, eventTimeEpochMs, false);
    }
    ...
    public static ScheduleIntentV1 decode(final byte[] encoded) {
        ...
        final AdapterMetadataV1 metadata = AdapterMetadataV1.decode(
                QueryCodecSupport.nested(fields.get(index++), 10));
        byte[] business = null;
        ...
        if (inline != null || committed != null) {
            result = create(profile, retry, deliverAt, expireAt, delivery, ordering, orderingKey, inline, committed,
                    metadata, business, eventTime);
        } else {
            result = forPrepare(profile, retry, deliverAt, expireAt, delivery, ordering, orderingKey, metadata,
                    business, eventTime);
        }
    ...
    public boolean equals(final Object other) {
        return other instanceof ScheduleIntentV1 that && deliverAtEpochMs == that.deliverAtEpochMs
                && expireAtEpochMs == that.expireAtEpochMs && deliveryMode == that.deliveryMode
                && orderingMode == that.orderingMode && profile.equals(that.profile)
                && retryPolicy.equals(that.retryPolicy) && Arrays.equals(orderingKey, that.orderingKey)
                && Arrays.equals(inlinePayload, that.inlinePayload)
                && Objects.equals(committedPayload, that.committedPayload)
                && adapterMetadata.equals(that.adapterMetadata) && Arrays.equals(businessKey, that.businessKey)
                && Objects.equals(eventTimeEpochMs, that.eventTimeEpochMs);
    }

    @Override
    public int hashCode() {
        return Objects.hash(profile, retryPolicy, deliverAtEpochMs, expireAtEpochMs, deliveryMode, orderingMode,
                Arrays.hashCode(orderingKey), Arrays.hashCode(inlinePayload), committedPayload, adapterMetadata,
                Arrays.hashCode(businessKey), eventTimeEpochMs);
    }
}
