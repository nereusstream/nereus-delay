...
public final class ControlOperationAuthorizationV1 {
    ...
    public static void authorize(final PreparedControlOperationV1 prepared,
        ...
        if (!Bytes.constantTimeEquals(author.authenticatedRoleSetHash(), context.roleSet().digest())) {
    ...
    private static ControlRoleV1[] requiredRoles(final PreparedControlOperationV1 prepared) {
        return switch (prepared.kind()) {
            case STOP_NEW_SCHEDULES, PAUSE_DESTINATION_LANE, RESUME_DESTINATION_LANE,
                    CLOSE_DESTINATION_LANE, BREAK_ORDERING_DOMAIN,
                    PUBLISH_DESTINATION_PROFILE_VERSION, DEPRECATE_DESTINATION_PROFILE_VERSION ->
            ...
            case DRAIN_SHARD, FENCE_SHARD_FOR_MAINTENANCE, FORCE_CHECKPOINT, GET_CHECKPOINT_CATALOG,
                    PUBLISH_QUOTA_GRANT, ROTATE_EQUIVALENT_SECRET_REFERENCE ->
                    new ControlRoleV1[]{ControlRoleV1.PLATFORM_OPERATOR};
        };
    }

    private static ControlRoleV1[] resolveRoles(final ControlOperationRequestV1 request) {
        final ResolveUncertainRequestV1 branch = request.branch() instanceof ResolveUncertainRequestV1 value
                ? value : null;
        if (branch == null) {
