 ...
 */
public final class ControlTargetRefV1 {
    public static final int HASH_LENGTH = 32;
    public static final int ROUTE_UUID_LENGTH = RouteIncarnation.LENGTH;

    private final long targetIndex;
    private final ControlTargetKindV1 targetKind;
    private final Object target;
    private final byte[] expectedMutationId;
    private final byte[] expectedMutationHash;
    private final byte[] targetDigest;

    public ControlTargetRefV1(final long targetIndex, final ControlTargetKindV1 targetKind,
                              final Object target, final byte[] expectedMutationId,
                              final byte[] expectedMutationHash) {
        this(targetIndex, targetKind, target, expectedMutationId, expectedMutationHash, null);
    }

    private ControlTargetRefV1(final long targetIndex, final ControlTargetKindV1 targetKind,
                               final Object target, final byte[] expectedMutationId,
                               final byte[] expectedMutationHash, final byte[] encodedDigest) {
        ...
        if (encodedDigest != null) {
            ...
            if (!Bytes.constantTimeEquals(encodedDigest, calculated)) {
                throw new IllegalArgumentException("ControlTargetRefV1 targetDigest mismatch");
            }
    ...
    public ControlTargetKindV1 targetKind() {
        return targetKind;
    }
    ...
    public ShardSubjectV1 shard() {
        return target instanceof ShardSubjectV1 value ? value : null;
    }

    public LaneControlTargetV1 lane() {
        return target instanceof LaneControlTargetV1 value ? value : null;
    }

    public ControlMessageTargetV1 message() {
        return target instanceof ControlMessageTargetV1 value ? value : null;
    }
    ...
    public ProfileControlTargetV1 profile() {
        return target instanceof ProfileControlTargetV1 value ? value : null;
    }

    public QuotaGrantRefV1 quotaGrant() {
        return target instanceof QuotaGrantRefV1 value ? value : null;
    }
    ...
    public static ControlTargetRefV1 decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded,
                "ControlTargetRefV1");
        if (fields.size() != 4 && fields.size() != 6) {
            throw new IllegalArgumentException("invalid ControlTargetRefV1 field count");
        }
        if (fields.get(0).number() != 1 || fields.get(1).number() != 2
                || fields.get(fields.size() - 1).number() != 22) {
            throw new IllegalArgumentException("invalid ControlTargetRefV1 field order");
        }
        final ControlTargetKindV1 kind = ControlTargetKindV1.fromWire(QueryCodecSupport.uint(fields.get(1), 2));
        final int branchIndex = 2;
        final int branchField = fields.get(branchIndex).number();
        if (branchField != branchField(kind)) {
            throw new IllegalArgumentException("ControlTargetRefV1 branch does not match target kind");
        }
        ...
        if (hasMutation && (fields.get(3).number() != 20 || fields.get(4).number() != 21)) {
            throw new IllegalArgumentException("invalid ControlTargetRefV1 mutation identity order");
        }
        ...
        final ControlTargetRefV1 result = new ControlTargetRefV1(QueryCodecSupport.uint(fields.get(0), 1), kind,
                target, mutationId, mutationHash, digest);
        QueryCodecSupport.requireCanonical(encoded, result.canonicalBytes(), "ControlTargetRefV1");
        return result;
    }
    ...
    private void writeWithoutDigest(final java.io.ByteArrayOutputStream output) {
        ...
        if (targetKind == ControlTargetKindV1.ROUTE) {
    ...
    private static Object decodeTarget(final ControlTargetKindV1 kind,
                                       final CanonicalProtobuf.Reader.Field field) {
        return switch (kind) {
            case SHARD -> ShardSubjectV1.decode(QueryCodecSupport.nested(field, 10));
            case LANE -> LaneControlTargetV1.decode(QueryCodecSupport.nested(field, 11));
            case MESSAGE -> ControlMessageTargetV1.decode(QueryCodecSupport.nested(field, 12));
            case ROUTE -> QueryCodecSupport.fixed(field, 13, ROUTE_UUID_LENGTH);
            case PROFILE -> ProfileControlTargetV1.decode(QueryCodecSupport.nested(field, 14));
            case QUOTA_GRANT -> QuotaGrantRefV1.decode(QueryCodecSupport.nested(field, 15));
        };
    }

    private static int branchField(final ControlTargetKindV1 kind) {
        return 9 + kind.wireValue();
    }

    private static Object requireTargetType(final ControlTargetKindV1 kind, final Object value) {
        Objects.requireNonNull(value, "target");
        final boolean valid = switch (kind) {
            case SHARD -> value instanceof ShardSubjectV1;
            case LANE -> value instanceof LaneControlTargetV1;
            case MESSAGE -> value instanceof ControlMessageTargetV1;
            case ROUTE -> value instanceof byte[] && ((byte[]) value).length == ROUTE_UUID_LENGTH;
            case PROFILE -> value instanceof ProfileControlTargetV1;
            case QUOTA_GRANT -> value instanceof QuotaGrantRefV1;
        };
        if (!valid) {
            throw new IllegalArgumentException("ControlTargetRefV1 target does not match target kind");
        }
        return value;
    }
    ...
    private static byte[] targetCanonical(final Object value) {
        return switch (value) {
            case ShardSubjectV1 shard -> shard.canonicalBytes();
            case LaneControlTargetV1 lane -> lane.canonicalBytes();
            case ControlMessageTargetV1 message -> message.canonicalBytes();
            case ProfileControlTargetV1 profile -> profile.canonicalBytes();
            case QuotaGrantRefV1 quota -> quota.canonicalBytes();
            case byte[] ignored -> throw new IllegalArgumentException("route target is not nested");
            default -> throw new IllegalArgumentException("unknown ControlTargetRefV1 target");
        };
    }

    @Override
    public boolean equals(final Object other) {
        return other instanceof ControlTargetRefV1 that && targetIndex == that.targetIndex
                && targetKind == that.targetKind && targetEquals(target, that.target)
    ...
    @Override
    public int hashCode() {
        return Objects.hash(targetIndex, targetKind, targetHash(target), Arrays.hashCode(expectedMutationId),
                Arrays.hashCode(expectedMutationHash), Arrays.hashCode(targetDigest));
    }
    ...
    private static boolean targetEquals(final Object left, final Object right) {
        return left instanceof byte[] leftBytes && right instanceof byte[] rightBytes
                ? Arrays.equals(leftBytes, rightBytes) : Objects.equals(left, right);
    }
