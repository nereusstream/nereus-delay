...
public record PublicQueryErrorV1(StableCode code, Long retryAtEpochMs) {
    ...
    public static PublicQueryErrorV1 decode(final byte[] encoded) {
        ...
        if (fields.isEmpty() || fields.size() > 2 || fields.get(0).number() != 1
                || fields.get(0).wireType() != 0 || (fields.size() == 2 && fields.get(1).number() != 2)) {
            throw new IllegalArgumentException("invalid PublicQueryErrorV1 fields");
        }
        final StableCode code = StableCode.fromWire(Math.toIntExact(fields.get(0).unsignedValue()));
    ...
    public static boolean isQueryError(final StableCode code) {
        return switch (code) {
            case INVALID_RECEIPT, RECEIPT_MISMATCH, NOT_FOUND_OR_NOT_AUTHORIZED, SHARD_TRANSITIONING,
                    SHARD_UNAVAILABLE, INTEGRITY_ERROR -> true;
            default -> false;
        };
    }
