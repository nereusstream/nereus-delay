 ...
 */
public final class ControlSystemMutationFactoryV1 {
    private ControlSystemMutationFactoryV1() {
    }

    public static SystemMutation sign(final PreparedControlOperationV1 prepared,
        ...
        final SystemMutationType type = ControlTargetMutationBindingV1.expectedMutationType(
                prepared.kind(), target.targetKind());
        final ControlRef controlRef = new ControlRef(prepared.operationId(), prepared.requestHash(),
                target.targetIndex());
        final byte[] logicalIdentity = switch (type) {
            case APPLY_SHARD_CONTROL -> controlRef.logicalOperationIdentity(
        ...
        final SystemMutation mutation = SystemMutation.signed(shardId, type, retryUntilEpochMs, logicalIdentity,
                canonicalBody, authorIdentity, signingKeyVersion, signingKey);
