...
public final class CheckpointUploadIntentV1 {
    ...
    private static final byte[] DIGEST_DOMAIN =
            Bytes.utf8("nereus-delay-checkpoint-upload-intent-v1\0");
    ...
    public CheckpointUploadIntentV1(final ShardSubjectV1 shard, final byte[] recoveryLineageId,
                                    final byte[] checkpointId, final OwnerIdentityV1 owner,
                                    final byte[] sourceStoreIncarnation, final byte[] uploadToken,
                                    final long baseCatalogGeneration, final byte[] parentCheckpointId,
                                    final byte[] parentManifestSha256, final ProfileRefV1 objectStoreProfile,
                                    final TrustedUtcIntervalEvidence checkpointCreatedAt,
                                    final long uploadDeadlineEpochMs, final CheckpointUploadStateV1 state,
                                    final long stateRevision, final CheckpointResourceV1 publishedManifest,
        ...
        this.parentCheckpointId = parentCheckpointId == null ? null
                : nonZeroFixed(parentCheckpointId, ID_LENGTH, "parentCheckpointId");
        this.parentManifestSha256 = parentManifestSha256 == null ? null
                : fixed(parentManifestSha256, HASH_LENGTH, "parentManifestSha256");
    ...
    public static CheckpointUploadIntentV1 decode(final byte[] encoded) {
        ...
        if (fields.size() < 14 || fields.get(0).number() != 1 || fields.get(fields.size() - 1).number() != 18) {
        ...
        final ProfileRefV1 profile = requireObjectStoreProfile(
                ProfileRefV1.decode(QueryCodecSupport.nested(fields.get(index++), 11)));
        final TrustedUtcIntervalEvidence created = TrustedUtcIntervalEvidence.decode(
                QueryCodecSupport.nested(fields.get(index++), 12));
        ...
        final CheckpointUploadIntentV1 result = new CheckpointUploadIntentV1(shard, lineage, checkpoint, owner,
                store, token, baseGeneration, parentCheckpoint, parentManifest, profile, created, deadline, state,
                revision, resource, reaping);
