...
public final class PayloadObjectResourceV1 {
    ...
    public PayloadObjectResourceV1(final ProfileRefV1 objectStoreProfile, final byte[] container,
                                   final byte[] objectKey, final byte[] immutableVersion, final byte[] etag,
                                   final long length, final byte[] payloadSha256) {
    ...
    public byte[] exactResourceCanonicalBytes() {
        return CanonicalProtobuf.message(output ->
                CanonicalProtobuf.bytes(output, ResourceKind.PAYLOAD_OBJECT.wireValue(), canonicalBytes()));
    }
    ...
    public static PayloadObjectResourceV1 decode(final byte[] encoded) {
        ...
        QueryCodecSupport.requireNumbers(fields, fields.size() == 6
                ? new int[]{1, 2, 3, 4, 6, 7}
                : new int[]{1, 2, 3, 4, 5, 6, 7}, "PayloadObjectResourceV1");
        int index = 0;
        ...
        final PayloadObjectResourceV1 result = new PayloadObjectResourceV1(profile, container, objectKey,
                immutableVersion, etag, length, payloadSha256);
    ...
    @Override
    public int hashCode() {
        return Objects.hash(objectStoreProfile, Arrays.hashCode(container), Arrays.hashCode(objectKey),
                Arrays.hashCode(immutableVersion), Arrays.hashCode(etag), length, Arrays.hashCode(payloadSha256));
    }
}
