...
public final class ProfileBindingControlState {
    ...
    private ProfileBindingControlState(final List<ActivationMarker> activations,
                                       final List<BindingClosure> closures) {
    ...
    public ProfileAcceptanceV1 firstBindingAcceptance(final ProfileRefV1 profile,
        ...
        return closure != null && compare(closure.sourcePosition(), sourcePosition) <= 0
                ? ProfileAcceptanceV1.CLOSED_FOR_FIRST_BINDING : ProfileAcceptanceV1.ACTIVE_FOR_FIRST_BINDING;
    }

    public ProfileBindingControlState activate(final ProfileRefV1 profile,
                                               final SourcePosition sourcePosition) {
        Objects.requireNonNull(profile, "profile");
        Objects.requireNonNull(sourcePosition, "sourcePosition");
        ...
        if (!activations.isEmpty() && compare(sourcePosition,
                activations.get(activations.size() - 1).sourcePosition()) <= 0) {
    ...
    public ProfileBindingControlState close(final ProfileNewBindingClosePayloadV1 payload,
                                            final SourcePosition sourcePosition) {
        ...
        next.sort(Comparator.comparing((BindingClosure value) -> value.profile().canonicalBytes(),
                ProfileBindingControlState::compareBytes));
    ...
    public boolean equals(final Object other) {
        return other instanceof ProfileBindingControlState that && activations.equals(that.activations)
    ...
    private ActivationMarker activationFor(final ProfileRefV1 profile) {
        return activations.stream().filter(marker -> marker.profile().equals(profile)).findFirst().orElse(null);
    }

    private BindingClosure closureFor(final ProfileRefV1 profile) {
        return closures.stream().filter(marker -> marker.profile().equals(profile)).findFirst().orElse(null);
    }
    ...
    private static List<BindingClosure> validateClosures(final List<BindingClosure> values,
                                                         final List<ActivationMarker> activations) {
        Objects.requireNonNull(values, "closures");
        final List<BindingClosure> result = new ArrayList<>(values);
        result.sort(Comparator.comparing((BindingClosure value) -> value.profile().canonicalBytes(),
                ProfileBindingControlState::compareBytes));
        ...
        for (BindingClosure value : result) {
            ...
            final ActivationMarker activation = activations.stream()
                    .filter(marker -> marker.profile().equals(value.profile())).findFirst().orElse(null);
    ...
    public record ActivationMarker(ProfileRefV1 profile, SourcePosition sourcePosition) {
        ...
        private static ActivationMarker decode(final byte[] encoded) {
            final List<CanonicalProtobuf.Reader.Field> fields = QueryCodecSupport.read(encoded,
                    "ProfileBindingActivationMarker");
