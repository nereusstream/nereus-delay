...
public final class PreparedSubmissionV1 {
    ...
    public static PreparedSubmissionV1 decode(final byte[] encoded) {
        ...
        if (fields.size() != 2 || fields.get(0).number() != 1
                || QueryCodecSupport.uint(fields.get(0), 1) != VERSION) {
            throw new IllegalArgumentException("invalid PreparedSubmissionV1 version/field set");
        }
        final PreparedSubmissionV1 result = switch (fields.get(1).number()) {
            case 2 -> managed(QueryCodecSupport.bytes(fields.get(1), 2));
            case 3 -> nativePrepared(NativePreparedDeliveryV1.decode(QueryCodecSupport.nested(fields.get(1), 3)));
    ...
    public boolean equals(final Object other) {
        return other instanceof PreparedSubmissionV1 that && Arrays.equals(managedFrame, that.managedFrame)
