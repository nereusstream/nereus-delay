...
public final class NonPersistenceProofV1 {
    ...
    private NonPersistenceProofV1(final NonPersistenceProofKindV1 kind, final byte[] physicalEnqueueAttemptId,
                                  final byte[] preparedHash, final BrokerResourceIdentityV1 brokerResource,
                                  final byte[] brokerRequestSha256, final byte[] authenticatedResponseSha256,
                                  final byte[] proofDigest) {
        this.kind = Objects.requireNonNull(kind, "kind");
        this.physicalEnqueueAttemptId = optionalFixed(physicalEnqueueAttemptId, ATTEMPT_ID_LENGTH,
        ...
        this.authenticatedResponseSha256 = optionalFixed(authenticatedResponseSha256, HASH_LENGTH,
                "authenticatedResponseSha256");
        validatePresence(kind, this.physicalEnqueueAttemptId, brokerResource, this.brokerRequestSha256,
                this.authenticatedResponseSha256);
        this.proofDigest = fixed(proofDigest, "proofDigest");
    }

    public static NonPersistenceProofV1 create(final NonPersistenceProofKindV1 kind,
                                               final byte[] physicalEnqueueAttemptId, final byte[] preparedHash,
                                               final BrokerResourceIdentityV1 brokerResource,
                                               final byte[] brokerRequestSha256,
                                               final byte[] authenticatedResponseSha256) {
        final byte[] fields = canonicalFields(kind, physicalEnqueueAttemptId, preparedHash, brokerResource,
                brokerRequestSha256, authenticatedResponseSha256);
        return new NonPersistenceProofV1(kind, physicalEnqueueAttemptId, preparedHash, brokerResource,
                brokerRequestSha256, authenticatedResponseSha256, Bytes.sha256(fields));
    }
    ...
    public static NonPersistenceProofV1 decode(final byte[] encoded) {
        ...
        final NonPersistenceProofV1 result = new NonPersistenceProofV1(kind, attempt, preparedHash, resource,
                requestHash, responseHash, digest);
    ...
    public boolean equals(final Object other) {
        ...
        return kind == that.kind && Objects.equals(brokerResource, that.brokerResource)
    ...
    @Override
    public int hashCode() {
        return Objects.hash(kind, Arrays.hashCode(physicalEnqueueAttemptId), Arrays.hashCode(preparedHash),
                brokerResource, Arrays.hashCode(brokerRequestSha256), Arrays.hashCode(authenticatedResponseSha256),
                Arrays.hashCode(proofDigest));
    }

    private static byte[] canonicalFields(final NonPersistenceProofKindV1 kind, final byte[] attempt,
                                           final byte[] preparedHash, final BrokerResourceIdentityV1 resource,
                                           final byte[] requestHash, final byte[] responseHash) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.uint32(output, 1, kind.wireValue());
            CanonicalProtobuf.uint32(output, 2, ADAPTER_PROOF_VERSION);
            if (attempt != null) {
    ...
    private static void validatePresence(final NonPersistenceProofKindV1 kind, final byte[] attempt,
                                         final BrokerResourceIdentityV1 resource, final byte[] requestHash,
        ...
        switch (kind) {
            ...
            case KAFKA_DEFINITIVE_REJECTION -> {
                requireBrokerEvidence(attempt, resource, requestHash, responseHash, BrokerResourceIdentityV1.Kind.KAFKA);
            }
            case PULSAR_GUARD_REJECTION -> {
                requireBrokerEvidence(attempt, resource, requestHash, responseHash, BrokerResourceIdentityV1.Kind.PULSAR);
            }
        }
    }

    private static void requireBrokerEvidence(final byte[] attempt, final BrokerResourceIdentityV1 resource,
                                              final byte[] requestHash, final byte[] responseHash,
                                              final BrokerResourceIdentityV1.Kind expectedKind) {
        if (attempt == null || resource == null || resource.kind() != expectedKind
