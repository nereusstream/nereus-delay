...
public final class ReservationExpiryWorkClassExecutor {
    private static final byte[] TASK_ID_DOMAIN =
            Bytes.utf8("nereus-delay-reservation-expiry-materialization-v1\0");

    private final WorkClassExecutionRegistry workClasses;
    private final OwnedDelayShard ownedShard;
    private final OxiaOwnerLeaseStore authority;

    public ReservationExpiryWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final DelayShard.ReservationExpiryWork candidate,
        ...
        final byte[] identity = Bytes.concat(TASK_ID_DOMAIN, submitted.reservationId(),
                submitted.messageId().bytes(), Bytes.u64be(submitted.reservationExpiryEpochMs()),
                Bytes.u64be(submitted.stateVersion()));
        final WorkClassTask task = new WorkClassTask(WorkClass.GC,
                "reservation-expiry/" + Bytes.hex(Bytes.sha256(identity)), identity.length);
        final Submission submission = new Submission(task);
        workClasses.submit(task, () -> execute(submitted, clock, submission));
        return submission;
    }

    private void execute(final DelayShard.ReservationExpiryWork candidate,
                         final LongSupplier clock, final Submission submission) {
        try {
    ...
    public record Outcome(DelayShard.ReservationExpiryMaterializationResult result,
                          Throwable failure) {
        public Outcome {
