...
public final class ExpiryWorkClassExecutor {
    private static final byte[] TASK_ID_DOMAIN =
            Bytes.utf8("nereus-delay-expiry-handoff-task-v1\0");
    ...
    public ExpiryWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final DelayShard.ExpiryWork candidate,
        ...
        final Request request = Request.prepare(candidate, evidence, retryUntilEpochMs, owner,
        ...
        final WorkClassTask task = new WorkClassTask(WorkClass.EXPIRY,
                "expiry-handoff/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, frame)), frame.length);
    ...
    private void execute(final Request request, final Submission submission) {
        try {
            ownedShard.requireExpiryAuthoritativelyStrict(authority, request.candidate, request.evidence,
                    request.owner, request.ownerClock);
            ...
            switch (appended.disposition()) {
                ...
                case DEFINITIVELY_NOT_PERSISTED -> submission.complete(
                        ExpiryHandoffResult.notEnqueued(request.mutation));
    ...
    private static final class Request {
        ...
        private Request(final DelayShard.ExpiryWork candidate, final TrustedUtcIntervalEvidence evidence,
                        final OwnerIdentityV1 owner, final SystemMutation mutation, final LongSupplier ownerClock) {
        ...
        private static Request prepare(final DelayShard.ExpiryWork candidate,
                                       final TrustedUtcIntervalEvidence evidence,
                                       final long retryUntilEpochMs,
                                       final OwnerIdentityV1 owner,
                                       final int signingKeyVersion,
            ...
            final byte[] author = AuthorIdentity.owner(typedOwner.deploymentId(), typedOwner.workerRunId(),
                    typedOwner.ownerEpoch(), typedOwner.leaseFencingDigest()).canonicalBytes();
            final io.nereusstream.delay.protocol.ShardId shard = work.messageId().routingId().shardId();
            final byte[] body = body(shard, work, retryUntilEpochMs, trusted);
            final SystemMutation mutation = SystemMutation.signed(shard, SystemMutationType.EXPIRE_GENERATION,
                    retryUntilEpochMs, SystemMutation.computeExpiryLogicalIdentity(work.messageId(), work.generation(),
                            work.expireAtEpochMs()), body, author, signingKeyVersion, key);
            return new Request(work, trusted, typedOwner, mutation, clock);
        }

        private static byte[] body(final io.nereusstream.delay.protocol.ShardId shard,
                                   final DelayShard.ExpiryWork work, final long retryUntilEpochMs,
                                   final TrustedUtcIntervalEvidence evidence) {
    ...
    public record ExpiryHandoffResult(ResultKind kind, SystemMutation mutation,
        ...
        private static ExpiryHandoffResult enqueued(final SystemMutation mutation, final SourcePosition position) {
            return new ExpiryHandoffResult(ResultKind.ENQUEUED, mutation, Objects.requireNonNull(position, "position"),
                    null);
        }
