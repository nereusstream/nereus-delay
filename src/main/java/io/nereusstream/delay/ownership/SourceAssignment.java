...
public record SourceAssignment(
    ...
    public static SourceAssignment decode(final byte[] encoded) {
        ...
        final SourceAssignment result = new SourceAssignment(new ShardId(route, partition), assignmentId,
                assignmentEpoch, barrier);
    ...
    private static byte[] encode(final SourceAssignment assignment) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.bytes(output, 1, assignment.shardId().routeIncarnation().bytes());
    ...
    private static byte[] encodeBarrier(final SourceActivationBarrier barrier) {
        if (barrier instanceof KafkaActivationBarrier kafka) {
            return CanonicalProtobuf.message(output -> CanonicalProtobuf.bytes(output, 1,
                    CanonicalProtobuf.message(fields -> {
                        CanonicalProtobuf.bytes(fields, 1, kafka.shardId().routeIncarnation().bytes());
                        CanonicalProtobuf.uint32Bits(fields, 2, kafka.shardId().partition());
                        CanonicalProtobuf.bytes(fields, 3, utf8(kafka.authenticatedClusterId(),
        ...
        return CanonicalProtobuf.message(output -> CanonicalProtobuf.bytes(output, 2,
                CanonicalProtobuf.message(fields -> {
                    CanonicalProtobuf.bytes(fields, 1, pulsar.shardId().routeIncarnation().bytes());
    ...
    private static SourceActivationBarrier decodeBarrier(final byte[] encoded) {
        ...
        if (branch.number() == 1) {
            ...
            return new KafkaActivationBarrier(new ShardId(route, partition), cluster, topic,
        ...
        if (branch.number() == 2) {
            ...
            return new PulsarActivationBarrier(new ShardId(route, partition), resource, topic,
                    uint64Bits(fields.get(4), 5), uint64Bits(fields.get(5), 6),
                    uint32Bits(fields.get(6), 7), uint32Bits(fields.get(7), 8), generation, attestation,
                    empty == 1);
        }
