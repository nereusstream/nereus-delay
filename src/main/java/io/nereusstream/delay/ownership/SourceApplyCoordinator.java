...
public final class SourceApplyCoordinator {
    ...
    public synchronized TurnResult runTurn(final SchedulerBudget workBudget, final LongSupplier ownerClock) {
        ...
        if (pending.appliedOutcome == null) {
            ...
            try {
                ...
                final SourceApplyWorkClassExecutor.ApplyOutcome observed = pending.submission.outcome().orElse(null);
                if (observed == null) {
                    return TurnResult.failed(TurnStatus.WORK_CLASS_FAILURE, pending.entry, failure);
                }
            }
            final SourceApplyWorkClassExecutor.ApplyOutcome applied = pending.submission.outcome().orElse(null);
            if (applied == null) {
    ...
    private static boolean sameEntry(final SourceReplayEntry first, final SourceReplayEntry second) {
        return second != null
                && java.util.Arrays.equals(first.position().canonicalBytes(), second.position().canonicalBytes())
    ...
    private static byte[] frame(final SourceReplayEntry entry) {
        if (entry instanceof SourceReplayRecord command) {
            return io.nereusstream.delay.protocol.CommandCodec.encodeFrame(command.command());
        }
    ...
    public record TurnResult(TurnStatus status, SourceReplayEntry entry, SourceReplayOutcome appliedOutcome,
        ...
        private static TurnResult rejected(final TurnStatus status, final SourceReplayEntry entry,
                                           final Throwable failure) {
            return new TurnResult(status, entry, null, null, Objects.requireNonNull(failure, "failure"));
        }

        private static TurnResult failed(final TurnStatus status, final SourceReplayEntry entry,
                                         final Throwable failure) {
            return new TurnResult(status, entry, null, null, Objects.requireNonNull(failure, "failure"));
        }

        private static TurnResult ackPending(final SourceAcknowledgement.Disposition disposition,
                                             final SourceReplayEntry entry, final Throwable failure) {
            final TurnStatus status = disposition == SourceAcknowledgement.Disposition.UNKNOWN
                    ? TurnStatus.ACK_UNKNOWN : TurnStatus.ACK_DEFINITIVELY_NOT_ACKED;
