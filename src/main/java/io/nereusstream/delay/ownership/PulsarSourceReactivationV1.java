 ...
 * immutable.</p>
 */
public record PulsarSourceReactivationV1(
        byte[] routeSnapshotDigest,
        SourceAssignment previousAssignment,
        SourceAssignment successorAssignment) {
    private static final int VERSION = 1;
    private static final byte[] DIGEST_DOMAIN =
            Bytes.utf8("nereus-delay-pulsar-source-reactivation-v1\0");

    public PulsarSourceReactivationV1 {
    ...
    public static PulsarSourceReactivationV1 decode(final byte[] encoded) {
        ...
        if (uint(fields.get(0), 1) != VERSION) {
            throw new IllegalArgumentException("unsupported PulsarSourceReactivationV1 version");
        }
        final byte[] body = CanonicalProtobuf.message(output -> {
        ...
        final PulsarSourceReactivationV1 result = new PulsarSourceReactivationV1(
    ...
    public boolean equals(final Object other) {
        return other instanceof PulsarSourceReactivationV1 that
    ...
    @Override
    public int hashCode() {
        return Objects.hash(Arrays.hashCode(routeSnapshotDigest), previousAssignment,
                successorAssignment);
    }

    private static void validateSuccessor(final SourceAssignment previous,
                                          final SourceAssignment successor) {
        ...
        final PulsarActivationBarrier oldBarrier = pulsarBarrier(previous.activationBarrier(),
                "previousAssignment");
        final PulsarActivationBarrier newBarrier = pulsarBarrier(successor.activationBarrier(),
                "successorAssignment");
        if (!Arrays.equals(oldBarrier.brokerResourceIncarnation(), newBarrier.brokerResourceIncarnation())
                ...
                || !Arrays.equals(oldBarrier.resourceGuardAttestationDigest(),
                newBarrier.resourceGuardAttestationDigest())
        ...
        if (oldBarrier.guardedSourceConnectionGeneration()
                == newBarrier.guardedSourceConnectionGeneration()) {
            throw new IllegalArgumentException("Pulsar source reactivation did not create a new connection generation");
        }
    }

    private static PulsarActivationBarrier pulsarBarrier(final SourceActivationBarrier barrier,
                                                         final String name) {
    ...
    private static void requireNumbers(final List<CanonicalProtobuf.Reader.Field> fields,
                                       final int... numbers) {
