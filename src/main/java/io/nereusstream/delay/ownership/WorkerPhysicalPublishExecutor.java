...
public final class WorkerPhysicalPublishExecutor implements AutoCloseable {
     ...
     */
    public WorkerPhysicalPublishExecutor(final DestinationPublishAdapter delegate,
        ...
        this(new BoundedDestinationPublishAdapter(Objects.requireNonNull(delegate, "delegate"),
                ...
                physicalGate, outcomeFactory, fenceOnFailure);
    }

    /** Creates the bridge around an already composed bounded physical adapter. */
    public WorkerPhysicalPublishExecutor(final BoundedDestinationPublishAdapter physicalAdapter,
                                         final OutcomeWorkClassExecutor outcomeExecutor,
                                         final PhysicalPublishGate physicalGate,
                                         final PublishOutcomeMutationFactory outcomeFactory,
                                         final Runnable fenceOnFailure) {
        this(physicalAdapter, (mutation, ownerClock) -> Objects.requireNonNull(outcomeExecutor, "outcomeExecutor")
                        .submit(mutation, ownerClock),
                physicalGate, outcomeFactory, fenceOnFailure);
    }

    WorkerPhysicalPublishExecutor(final BoundedDestinationPublishAdapter physicalAdapter,
     ...
     */
    public static DestinationPublishRequest prepareRequest(final PublishAttemptLedger attempt,
                                                            final byte[] payload) {
        ...
        return new DestinationPublishRequest(descriptor.destinationLaneId(), descriptor.laneIncarnation(),
                descriptor.messageId(), Math.toIntExact(descriptor.generation()), descriptor.publishAttemptId(),
                descriptor.actionAtEpochMs(), descriptor.deliverAtEpochMs(), exactPayload,
                descriptor.businessMetadata().canonicalBytes());
    }

    /** Starts one bounded physical attempt or returns a deferred gate result. */
    public Submission submit(final PublishAttemptLedger attempt,
    ...
    private void completeResult(final Submission submission,
    ...
    private Submission handoff(final PublishAttemptLedger attempt,
                               final DestinationPublishRequest request,
                               final DestinationPublishResult result,
                               final LongSupplier ownerClock) {
    ...
    private Decision checkGate(final PublishAttemptLedger attempt,
                               final DestinationPublishRequest request,
                               final LongSupplier ownerClock) {
        try {
            return Objects.requireNonNull(physicalGate.check(attempt, request, ownerClock),
                    "physical publish gate decision");
        } catch (RuntimeException | Error failure) {
            failClosed(failure);
            throw failure;
        }
    ...
    private DestinationPublishResult lateGateResult(final PublishAttemptLedger attempt,
                                                    final DestinationPublishRequest request,
                                                    final LongSupplier ownerClock) {
        final Decision decision;
        try {
            decision = Objects.requireNonNull(physicalGate.check(attempt, request, ownerClock),
                    "physical publish gate decision");
        } catch (RuntimeException | Error failure) {
            failClosed(failure);
    ...
    private static void registerCompletion(final CompletionStage<DestinationPublishResult> stage,
    ...
    private static void requireAttemptIdentity(final PublishAttemptLedger attempt,
    ...
    public interface PublishOutcomeMutationFactory {
        SystemMutation create(PublishAttemptLedger attempt,
                              DestinationPublishRequest request,
    ...
    public static final class Submission {
        ...
        private Submission(final PublishAttemptLedger attempt,
                           final DestinationPublishRequest request,
        ...
        private static Submission pending(final PublishAttemptLedger attempt,
                                          final DestinationPublishRequest request) {
            return new Submission(attempt, request, SubmissionState.PENDING);
        }

        private static Submission deferred(final PublishAttemptLedger attempt,
                                           final DestinationPublishRequest request,
                                           final Decision decision) {
        ...
        private synchronized void attachPhysicalCall(
                final BoundedDestinationPublishAdapter.PublishCall call) {
        ...
        private synchronized void complete(final DestinationPublishResult result,
