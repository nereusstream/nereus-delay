...
public final class OxiaSyncProtocolCapabilityBackend implements ProtocolCapabilityAuthority {
    ...
    private static final byte[] DIGEST_DOMAIN = Bytes.utf8(
            "nereus-delay-oxia-protocol-capability-record-v1\0");
    ...
    public OxiaSyncProtocolCapabilityBackend(final OxiaSyncOwnerLeaseBackend.ClientHandle handle,
                                             final String keyPrefix) {
        this(new SyncRecordClient(Objects.requireNonNull(handle, "handle").client()), keyPrefix,
                handle.backend()::assertConnectedSession, handle.backend()::connectedSessionIdentity, true);
    }
    ...
    @Override
    public Publication publish(final ProtocolCapabilityDeclarationV1 declaration,
        ...
        if (current != null && Arrays.equals(current.publication().declaration().canonicalBytes(),
        ...
        if (current == null) {
            options = ephemeral ? Set.of(PutOption.IfRecordDoesNotExist, PutOption.AsEphemeralRecord)
                    : Set.of(PutOption.IfRecordDoesNotExist);
        } else {
            options = ephemeral ? Set.of(PutOption.IfVersionIdEquals(current.versionId()),
                    PutOption.AsEphemeralRecord) : Set.of(PutOption.IfVersionIdEquals(current.versionId()));
        }
        try {
    ...
    private static boolean exact(final Stored observed, final long revision,
                                 final ProtocolCapabilityDeclarationV1 declaration, final byte[] value) {
    ...
    private static Decoded decode(final byte[] encoded) {
        ...
        final ProtocolCapabilityDeclarationV1 declaration =
                ProtocolCapabilityDeclarationV1.decode(declarationBytes);
