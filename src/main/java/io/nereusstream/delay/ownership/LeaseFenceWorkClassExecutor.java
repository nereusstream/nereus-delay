...
public final class LeaseFenceWorkClassExecutor {
    private static final byte[] TASK_ID_DOMAIN =
            Bytes.utf8("nereus-delay-lease-fence-handoff-task-v1\0");

    private final WorkClassExecutionRegistry workClasses;
    private final OwnedDelayShard ownedShard;
    private final OxiaOwnerLeaseStore authority;

    public LeaseFenceWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final OwnerLease expectedLease,
                              final LongSupplier ownerClock,
                              final FenceCallbacks callbacks) {
        ...
        final WorkClassTask task = new WorkClassTask(WorkClass.LEASE_FENCE,
                "lease-fence/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, requestBytes)), requestBytes.length);
        final Submission submission = new Submission(task, expected);
        workClasses.submit(task, () -> execute(submission, clock, stop));
        return submission;
    }

    private void execute(final Submission submission, final LongSupplier clock,
                         final FenceCallbacks callbacks) {
    ...
    private void fenceAndStop(final Submission submission, final FenceCallbacks callbacks,
    ...
    public record FenceResult(ResultKind kind, OwnerLease expectedLease,
                              OwnerLease observedLease, Throwable failure) {
        public FenceResult {
        ...
        private static FenceResult unknown(final OwnerLease expected, final OwnerLease observed,
                                           final Throwable failure) {
