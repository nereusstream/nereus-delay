...
public final class LaneCloseWorkClassExecutor {
    private static final byte[] TASK_ID_DOMAIN =
            Bytes.utf8("nereus-delay-lane-close-materialization-v1\0");

    private final WorkClassExecutionRegistry workClasses;
    private final OwnedDelayShard ownedShard;
    private final OxiaOwnerLeaseStore authority;

    public LaneCloseWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final DelayShard.LaneCloseMaterializationWork candidate,
                              final int maxRecords, final LongSupplier ownerClock) {
        ...
        final byte[] identity = Bytes.concat(TASK_ID_DOMAIN, submitted.laneId().bytes(),
                submitted.cursor().canonicalBytes(), Bytes.u32be(maxRecords));
        final WorkClassTask task = new WorkClassTask(WorkClass.GC,
                "lane-close/" + Bytes.hex(Bytes.sha256(identity)), identity.length);
        final Submission submission = new Submission(task);
        workClasses.submit(task, () -> execute(submitted, maxRecords, clock, submission));
        return submission;
    }

    private void execute(final DelayShard.LaneCloseMaterializationWork candidate,
                         final int maxRecords, final LongSupplier clock, final Submission submission) {
        try {
            submission.complete(Outcome.succeeded(ownedShard.materializeLaneCloseAuthoritativelyStrict(
                    authority, candidate, maxRecords, clock)));
    ...
    public record Outcome(DelayShard.LaneCloseMaterializationExecutionResult result,
                          Throwable failure) {
        public Outcome {
