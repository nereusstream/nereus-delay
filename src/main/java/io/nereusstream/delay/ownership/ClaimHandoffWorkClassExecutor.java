...
public final class ClaimHandoffWorkClassExecutor {
    ...
    public ClaimHandoffWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final ScheduleWorkItem item,
        ...
        final ClaimRequest request = ClaimRequest.create(item, evidence, claimDeadlineEpochMs,
                materialization, claimedCharge, ownerClock);
    ...
    private void execute(final Submission submission, final ClaimRequest request) {
        ...
        try {
            ...
            final PrerequisiteDecision prerequisite = Objects.requireNonNull(
                    prerequisiteGate.validate(new ClaimPrerequisite(candidate, request.evidence,
                            request.materialization, request.claimDeadlineEpochMs)),
            ...
            final ClaimExecutionAdmission.AdmissionDecision permit = permits.tryAcquire(scheduler.shardId(),
                    request.item.laneId(), candidate.laneIncarnation(), request.item.messageId(),
                    Integer.toUnsignedLong(request.item.generation()), request.item.accountedBytes());
            if (!permit.granted()) {
                scheduler.requeueFailedClaim(request.item);
            ...
            final ClaimRecord claim = ownedShard.claimAuthoritativelyStrict(authority, scheduler,
                    request.item, request.evidence, request.claimDeadlineEpochMs,
                    request.materialization, request.claimedCharge, request.ownerClock);
    ...
    public record ClaimPrerequisite(PersistentLaneScheduler.ClaimCandidate candidate,
    ...
    public record ClaimHandoffResult(ResultKind kind, ClaimRecord claim,
        ...
        private static ClaimHandoffResult claimed(final ClaimRecord claim,
                                                  final ClaimExecutionAdmission.Reservation reservation) {
            return new ClaimHandoffResult(ResultKind.CLAIMED,
                    Objects.requireNonNull(claim, "claim"),
                    Objects.requireNonNull(reservation, "reservation"), null, null);
        }

        private static ClaimHandoffResult prerequisiteUnavailable(final PrerequisiteRejection rejection) {
            return new ClaimHandoffResult(ResultKind.PREREQUISITE_UNAVAILABLE, null, null,
        ...
        private static ClaimHandoffResult permitUnavailable(final ClaimExecutionAdmission.Rejection rejection) {
            return new ClaimHandoffResult(ResultKind.PERMIT_UNAVAILABLE, null, null, null,
    ...
    private static final class ClaimRequest {
        ...
        private ClaimRequest(final ScheduleWorkItem item,
                             final TrustedUtcIntervalEvidence evidence,
                             final long claimDeadlineEpochMs,
                             final ClaimMaterializationV1 materialization,
                             final byte[] claimedCharge,
        ...
        private static ClaimRequest create(final ScheduleWorkItem item,
                                           final TrustedUtcIntervalEvidence evidence,
                                           final long claimDeadlineEpochMs,
                                           final ClaimMaterializationV1 materialization,
                                           final byte[] claimedCharge,
            ...
            if (claimDeadlineEpochMs <= trusted.latestEpochMs()
                    || claimDeadlineEpochMs > prepared.expireAtEpochMs()) {
            ...
            return new ClaimRequest(selected, trusted, claimDeadlineEpochMs, prepared, chargeBytes,
        ...
        private byte[] canonicalTaskBytes(final PersistentLaneScheduler scheduler) {
            return Bytes.concat(scheduler.shardId().routeIncarnation().bytes(),
                    Bytes.u32beBits(scheduler.shardId().partition()), item.laneId().bytes(),
                    item.messageId().bytes(), Bytes.u32beBits(item.generation()),
                    Bytes.i64be(item.eligibleAtEpochMs()), Bytes.u64be(item.accountedBytes()),
                    Bytes.lp32(evidence.canonicalBytes()), Bytes.i64be(claimDeadlineEpochMs),
                    Bytes.lp32(materialization.canonicalBytes()), Bytes.lp32(claimedCharge));
        }
    }
}
