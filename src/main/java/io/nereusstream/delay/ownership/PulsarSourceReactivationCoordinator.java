...
public final class PulsarSourceReactivationCoordinator {
    ...
    public PulsarSourceReactivationCoordinator(final RouteSnapshotProvider routeProvider,
    ...
    public FencedPlan fenceForReactivation(final AuthenticatedTenantContext context,
        ...
        final RouteSnapshotV1 route = routeProvider.exact(
                reactivation.previousAssignment().shardId().routeIncarnation(), context);
        requireRoute(route, context, reactivation);
        final WorkerAssignmentAuthority.Publication observedPublication = assignmentAuthority.current(
                reactivation.previousAssignment().shardId()).orElseThrow(
        ...
        final OwnerLease observedLease = ownerAuthority.current(expectedLease.shardId()).orElseThrow(
        ...
        final OwnerLease fenced = ownerAuthority.transitionOrRead(expectedLease, ShardLifecycleState.FENCED)
                .orElseThrow(() -> new IllegalStateException("Pulsar Owner Lease fence CAS was lost"));
        if (!expectedLease.sameIdentity(fenced) || fenced.state() != ShardLifecycleState.FENCED
     ...
     */
    public WorkerAssignmentAuthority.Publication publishSuccessor(final FencedPlan plan,
                                                                  final WorkerAssignment successor,
                                                                  final SourceQuiescenceProof quiescence) {
        ...
        if (currentAssignment.isEmpty()
                || !samePublication(plan.expectedPublication(), currentAssignment.get())) {
        ...
        try {
            final WorkerAssignmentAuthority.Publication published = assignmentAuthority.publish(successor,
                    plan.expectedPublication().revision());
            ...
            if (reread.isPresent() && reread.get().revision() == successorRevision
    ...
    public OwnerLease acquireSuccessor(final FencedPlan plan,
        ...
        final WorkerAssignmentAuthority.Publication observed = assignmentAuthority.current(
                successorPublication.assignment().sourceAssignment().shardId()).orElseThrow(
        ...
        if (ownerAuthority.current(successorPublication.assignment().sourceAssignment().shardId()).isPresent()) {
            throw new IllegalStateException("Pulsar successor Owner Lease already exists");
        }
        final OwnerLease acquired = ownerAuthority.acquire(successorPublication.assignment().sourceAssignment(),
                workerId, sessionIdentity, nowEpochMs, leaseDurationMs).orElseThrow(
                () -> new IllegalStateException("Pulsar successor Owner Lease acquire was rejected"));
        if (!workerId.equals(acquired.ownerId()) || acquired.state() != ShardLifecycleState.ACQUIRING
                || !acquired.validAt(nowEpochMs) || !contextMatches(acquired,
                plan.reactivation().successorAssignment())) {
    ...
    private static void requirePreviousAssignment(final WorkerAssignment previous,
                                                  final PulsarSourceReactivationV1 reactivation) {
    ...
    private static void requireRoute(final RouteSnapshotV1 route,
        ...
        try {
            ...
            throw new IllegalArgumentException("Pulsar reactivation Route does not contain the source partition",
                    failure);
        }
        if (!routeBarrier.toSourceBarrier(reactivation.previousAssignment().shardId().routeIncarnation())
    ...
    private static void requireSuccessorAssignment(final FencedPlan plan,
                                                   final WorkerAssignment successor) {
        ...
        if (!successor.routeBound()
                ...
                || !successor.sourceAssignment().sameIdentity(plan.reactivation().successorAssignment())
    ...
    private static void requireSuccessorPublication(final WorkerAssignmentAuthority.Publication publication,
                                                   final WorkerAssignment successor,
                                                   final long expectedRevision) {
        if (publication == null || publication.revision() != expectedRevision
    ...
    private static boolean samePublication(final WorkerAssignmentAuthority.Publication expected,
                                           final WorkerAssignmentAuthority.Publication actual) {
        return actual != null && expected.revision() == actual.revision()
    ...
    public record FencedPlan(
        ...
        public FencedPlan {
            ...
            Objects.requireNonNull(reactivation, "reactivation");
            if (!expectedLease.sameIdentity(fencedLease)
                    || fencedLease.state() != ShardLifecycleState.FENCED) {
