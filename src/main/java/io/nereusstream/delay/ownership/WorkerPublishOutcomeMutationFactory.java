...
public final class WorkerPublishOutcomeMutationFactory
    ...
    public WorkerPublishOutcomeMutationFactory(final OutcomeContextProvider contextProvider,
    ...
    @Override
    public SystemMutation create(final PublishAttemptLedger attempt,
        ...
        final byte[] body = PublishOutcomeBody.encodeInitial(
                exactRequest.delayMessageId().routingId().shardId(), context.retryUntilEpochMs(),
                exactAttempt.publishAttemptId(), sideEffect, context.disposition(), exactResult.stableCode(),
                evidence, context.transfer(), context.observedAt(), context.retryDecision());
        return SystemMutation.signed(exactRequest.delayMessageId().routingId().shardId(),
                SystemMutationType.PUBLISH_OUTCOME, context.retryUntilEpochMs(),
                exactAttempt.publishAttemptId(), body, authorIdentity, signingKeyVersion, signingKey);
    }
    ...
    private static DestinationPublishRequest requireRequest(final PublishAttemptLedger attempt,
                                                             final DestinationPublishRequest request) {
    ...
    private static void validateResultEvidence(final PublishAttemptLedger attempt,
                                               final DestinationPublishResult result,
    ...
    private static void validateDisposition(final DestinationPublishResult result,
        ...
        if ((sideEffect == 1 && disposition != 0)
            ...
            throw new IllegalArgumentException("Publish Outcome disposition does not match physical result"
                    + ": " + result.disposition());
        }
    }
    ...
    public interface OutcomeContextProvider {
        OutcomeContext context(PublishAttemptLedger attempt,
    ...
    public record OutcomeContext(long retryUntilEpochMs,
                                 int disposition,
                                 byte[] transfer,
