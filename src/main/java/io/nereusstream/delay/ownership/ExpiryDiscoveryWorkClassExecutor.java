...
public final class ExpiryDiscoveryWorkClassExecutor {
    private static final byte[] TASK_ID_DOMAIN =
            Bytes.utf8("nereus-delay-expiry-discovery-task-v1\0");

    private final WorkClassExecutionRegistry workClasses;
    private final OwnedDelayShard ownedShard;
    private final OxiaOwnerLeaseStore authority;

    public ExpiryDiscoveryWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final TrustedUtcIntervalEvidence evidence,
        ...
        final byte[] identity = Bytes.concat(TASK_ID_DOMAIN,
                ...
                Bytes.lp32(evidenceBytes), Bytes.u32be(submittedBudget.maxMessages()),
        ...
        final WorkClassTask task = new WorkClassTask(WorkClass.EXPIRY,
                "expiry-discovery/" + Bytes.hex(Bytes.sha256(identity)), chargedBytes);
        final Submission submission = new Submission(task);
        workClasses.submit(task, () -> submission.complete(
                ownedShard.discoverExpiryAuthoritativelyStrict(authority, submittedEvidence,
                        submittedBudget, submittedOwnerClock, submittedScanClock)));
        return submission;
    }
