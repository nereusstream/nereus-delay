...
public final class ProtocolActivationAuthorityCoordinator {
    private static final byte[] EVIDENCE_DOMAIN = Bytes.utf8(
            "nereus-delay-protocol-eligible-reader-set-v1\0");
    ...
    public EligibleReaderSet requireEligibleReaders(final ProtocolTupleV1 tuple,
        ...
        for (String workerId : workerIds) {
            final ProtocolCapabilityAuthority.Publication publication = authority.current(workerId)
            ...
            if (!declaration.supports(tuple)) {
                throw new IllegalStateException("eligible Worker does not support the requested protocol tuple: "
                        + workerId);
            }
    ...
    public EligibleReaderSet authorize(final ProtocolVersionActivatePayloadV1 payload,
                                       final List<String> eligibleWorkerIds) {
    ...
    private static byte[] evidenceHash(final ProtocolTupleV1 tuple,
        ...
        final byte[] canonical = CanonicalProtobuf.message(output -> {
            ...
            for (ProtocolCapabilityAuthority.Publication publication : publications) {
                ...
                final byte[] worker = CanonicalProtobuf.message(workerOutput -> {
                    CanonicalProtobuf.bytes(workerOutput, 1,
                            declaration.workerId().getBytes(StandardCharsets.UTF_8));
    ...
    public record EligibleReaderSet(ProtocolTupleV1 tuple, List<String> workerIds,
