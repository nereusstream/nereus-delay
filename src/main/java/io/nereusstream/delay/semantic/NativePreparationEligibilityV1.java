...
public final class NativePreparationEligibilityV1 {
    ...
    public static void require(final AuthenticatedTenantContext context, final RouteSnapshotV1 managedRoute,
        ...
        managedRoute.requireUsableForNewSchedule(context.authenticatedTenantScopeHash(),
                context.tenantRoutingScope(), trustedTime.epochMs());
        if (managedRoute.ingress().adapterKind() != AdapterKindV1.PULSAR
                ...
                || managedCommand != null && !managedCommand.shardId().routeIncarnation()
                .equals(managedRoute.routeIncarnation())) {
        ...
        if (destinationBody.adapterKind() != AdapterKindV1.PULSAR
                ...
                || !destinationBody.targetResource().equals(
                io.nereusstream.delay.protocol.BrokerResourceIdentityV1.pulsar(candidate.target()))) {
        ...
        final boolean explicitPartition = destinationBody.allowedExplicitPartitions()
                .contains(candidate.physicalPartition());
        final boolean partitionEligible = switch (destinationBody.targetPartitionPolicy()) {
            case EXPLICIT_ONLY -> explicitPartition;
            case HASH_ONLY, EXPLICIT_OR_HASH -> explicitPartition
                    || TargetPartitionHashV1.partition(destination.ref(), destinationBody.targetPartitionCount(),
                    routingBytes(intent, managedCommand, destinationBody.targetPartitionHashInput()))
                    == physicalPartition;
        };
    ...
    private static byte[] routingBytes(final ScheduleIntentV1 intent, final PreparedCommand managedCommand,
