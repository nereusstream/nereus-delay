...
public final class CheckpointDrainWorkClassExecutor {
    private static final byte[] TASK_ID_DOMAIN =
            Bytes.utf8("nereus-delay-checkpoint-drain-handoff-v1\0");

    private final WorkClassExecutionRegistry workClasses;
    private final ShardStore store;

    public CheckpointDrainWorkClassExecutor(final WorkClassExecutionRegistry workClasses,
    ...
    public Submission submit(final Request request) {
        ...
        final WorkClassTask task = new WorkClassTask(WorkClass.CHECKPOINT,
                "checkpoint-drain/" + Bytes.hex(Bytes.sha256(TASK_ID_DOMAIN, identity)), identity.length);
    ...
    private void requireAuthoritativeDrain(final Request request) {
        ...
        final Optional<OwnerLease> current = request.authority().current(request.expectedLease().shardId());
        if (current.isEmpty()) {
    ...
    private static byte[] canonicalIdentity(final Request request) {
        return CanonicalProtobuf.message(output -> {
            CanonicalProtobuf.bytes(output, 1, request.expectedLease().shardId().routeIncarnation().bytes());
            CanonicalProtobuf.uint32(output, 2, request.expectedLease().shardId().partition());
            CanonicalProtobuf.bytes(output, 3, Bytes.utf8(request.checkpointPath().toAbsolutePath().normalize().toString()));
            CanonicalProtobuf.bytes(output, 4, request.checkpointId());
            CanonicalProtobuf.bytes(output, 5, Bytes.utf8(request.expectedLease().ownerId()));
            ...
            if (request.expectedLease().context() != null) {
                CanonicalProtobuf.bytes(output, 10, CanonicalProtobuf.message(context -> {
                    CanonicalProtobuf.bytes(context, 1, request.expectedLease().context().sourceAssignmentId());
                    CanonicalProtobuf.uint64Bits(context, 2, request.expectedLease().context().assignmentEpoch());
                    CanonicalProtobuf.bytes(context, 3, request.expectedLease().context().sessionIdentity());
                }));
            }
        });
    }
    ...
    public record Request(Path checkpointPath, byte[] checkpointId, OwnerLease expectedLease,
                          OxiaOwnerLeaseStore authority, LongSupplier ownerClock,
