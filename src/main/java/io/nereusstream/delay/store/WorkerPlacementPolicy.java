...
public final class WorkerPlacementPolicy {
    ...
    public Decision select(final List<WorkerCandidate> candidates,
        ...
        final CapacityVectorV1 required = incomingShardCapacity.add(workerFixedCost).add(transitionDemand);
    ...
    private static double dominantCapacityUtilization(final CapacityVectorV1 hardCapacity,
    ...
    public record Configuration(long telemetryMaxAgeMs, long minimumResidenceMs, double hysteresisMargin,
                                double staleTelemetryPenalty, double movementCostPerByte) {
        public Configuration {
            if (telemetryMaxAgeMs < 0 || minimumResidenceMs < 0 || hysteresisMargin < 0
                    || staleTelemetryPenalty < 0 || movementCostPerByte < 0
                    || !Double.isFinite(hysteresisMargin) || !Double.isFinite(staleTelemetryPenalty)
    ...
    public record WorkerCandidate(
        ...
        public WorkerCandidate {
            ...
            if (workerId.isBlank() || ownedShardDbs < 0 || maxOwnedShardDbs <= 0 || openShardDbs < 0
                    || maxOpenShardDbs <= 0 || telemetryObservedAtEpochMs < 0 || residenceSinceEpochMs < 0) {
        ...
        public boolean eligible(final CapacityVectorV1 required) {
            return healthy && ownedShardDbs < maxOwnedShardDbs && openShardDbs < maxOpenShardDbs
