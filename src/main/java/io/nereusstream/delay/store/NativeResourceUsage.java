...
public record NativeResourceUsage(
    ...
    public NativeResourceUsage {
        if (blockCacheBytes < 0 || memtableBytes < 0 || tableReaderMetadataBytes < 0
                || pinnedBlockBytes < 0 || pinnedIteratorBytes < 0 || flushCompactionScratchBytes < 0) {
    ...
    public long rocksDbNativeBytes() {
        try {
            return Math.addExact(Math.addExact(Math.addExact(blockCacheBytes, memtableBytes),
