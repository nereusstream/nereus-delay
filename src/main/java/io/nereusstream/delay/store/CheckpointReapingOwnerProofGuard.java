...
public final class CheckpointReapingOwnerProofGuard {
    ...
    public static Decision evaluate(final CheckpointUploadIntentV1 pending,
        ...
        if (proof.observedCurrentLease() != null
                && proof.recordedLease().sameIdentity(proof.observedCurrentLease())) {
            return Decision.OWNER_PROOF_OBSERVATION_INVALID;
        }
        return Decision.OWNER_PROOF_ACCEPTED;
    }

    public static void require(final CheckpointUploadIntentV1 pending,
                               final CheckpointReapingOwnerProof proof) {
