...
public record CheckpointDeleteResult(
    ...
    public CheckpointDeleteResult {
        ...
        if (outcome == ResourceDeleteConfirmedBody.DeleteOutcome.DELETED
                && resource.immutableVersion().length == 0) {
    ...
    public ResourceDeleteConfirmedBody.ExternalDeleteEvidence externalEvidence(
        ...
        return new ResourceDeleteConfirmedBody.ExternalDeleteEvidence(resourceIdentityHash,
                providerRequestIdHash, outcome,
                outcome == ResourceDeleteConfirmedBody.DeleteOutcome.DELETED
                        ? resource.immutableVersion() : new byte[0],
                new byte[0], responseHash, observedAt);
    }
}
