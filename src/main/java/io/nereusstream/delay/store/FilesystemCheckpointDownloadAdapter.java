...
public final class FilesystemCheckpointDownloadAdapter {
    ...
    public synchronized Path download(final CheckpointDownloadRequest request, final Path targetDirectory) {
        ...
        final Path prefix = root.resolve("checkpoints").resolve(Bytes.hex(manifest.recoveryLineageId()))
                .resolve(Bytes.hex(manifest.checkpointId())).normalize();
        ensureWithinRoot(prefix);
        final Path temporary = targetParent.resolve(
                "." + target.getFileName() + ".checkpoint-download-" + UUID.randomUUID()).normalize();
            ...
            for (CheckpointManifest.FileEntry file : manifest.files()) {
                final Path source = prefix.resolve("objects")
                        .resolve(FilesystemCheckpointUploadAdapter.objectFileName(
                                file.objectKey(), file.objectVersion())).normalize();
    ...
    private void verifyManifestObject(final Path manifestObject, final CheckpointManifest expected,
                                      final CheckpointResourceV1 resource) {
        try {
            final byte[] bytes = LocalStatePathGuard.readRegularFileNoFollow(manifestObject,
                    limits.maxManifestBytes(), "checkpoint manifest object");
            if (bytes == null || bytes.length != resource.manifestLength()
    ...
    private Path resolveObjectPath(final byte[] encodedKey, final String description) {
        ...
        if (!java.util.Arrays.equals(encodedKey, Bytes.utf8(key)) || key.isEmpty()
                || key.indexOf('\0') >= 0 || key.indexOf('\\') >= 0 || key.startsWith("/")
                || key.contains("..")) {
    ...
    private static void copyAndHash(final Path boundary, final Path source, final Path destination,
    ...
    private static void moveCreateNew(final Path source, final Path target) throws IOException {
        try {
            ...
            throw new IllegalStateException("checkpoint restore target appeared during publication: " + target,
                    conflict);
        }
    }
