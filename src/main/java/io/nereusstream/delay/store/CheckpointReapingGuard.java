...
public final class CheckpointReapingGuard {
    ...
    public static Decision evaluate(final CheckpointUploadIntentV1 pending,
    ...
    private static boolean protectsPending(final RecoveryPinV1 pin,
                                           final CheckpointUploadIntentV1 pending) {
        return matches(pin.candidate().recoveryLineageId(), pin.candidate().checkpointId(),
                pending.recoveryLineageId(), pending.checkpointId())
                || matches(pin.observedFloor().recoveryLineageId(), pin.observedFloor().checkpointId(),
                pending.recoveryLineageId(), pending.checkpointId());
    }

    private static boolean matches(final byte[] lineage, final byte[] checkpoint,
                                   final byte[] pendingLineage, final byte[] pendingCheckpoint) {
