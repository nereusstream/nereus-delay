...
public final class VerifiedCredentialMaterialCache
    ...
    public synchronized void install(final ProfileSemanticEnvelopeV1 profile,
                                     final CredentialBindingV1 binding,
                                     final OxiaObjectStoreCredentialLeaseActivator.ObjectStoreCredentialMaterial
                                             material) {
        ...
        final Map<CacheKey, OxiaObjectStoreCredentialLeaseActivator.ObjectStoreCredentialMaterial> next =
                new HashMap<>(entries);
        final OxiaObjectStoreCredentialLeaseActivator.ObjectStoreCredentialMaterial previous = next.put(
                key, validated.material());
        if (previous != null && !Bytes.constantTimeEquals(previous.resolvedCredentialFingerprintDigest(),
    ...
    public synchronized void replaceAll(final Collection<Entry> values) {
        ...
        for (Entry value : values) {
            ...
            final CacheKey key = CacheKey.from(validated.profile(), validated.binding());
            final OxiaObjectStoreCredentialLeaseActivator.ObjectStoreCredentialMaterial previous = replacement.put(
                    key, validated.material());
            if (previous != null && !Bytes.constantTimeEquals(previous.resolvedCredentialFingerprintDigest(),
                    validated.material().resolvedCredentialFingerprintDigest())) {
    ...
    public synchronized void remove(final ProfileSemanticEnvelopeV1 profile,
                                    final CredentialBindingV1 binding) {
        final CacheKey key = CacheKey.from(Objects.requireNonNull(profile, "profile"),
    ...
    private Entry validate(final ProfileSemanticEnvelopeV1 profile, final CredentialBindingV1 binding,
        ...
        binding.equivalenceAttestation().requireAuthorizationScopeDigest(
    ...
    public record Entry(ProfileSemanticEnvelopeV1 profile, CredentialBindingV1 binding,
    ...
    private static final class CacheKey {
        ...
        private CacheKey(final ProfileRefV1 profile, final long generation, final byte[] bindingDigest,
        ...
        private static CacheKey from(final ProfileSemanticEnvelopeV1 profile, final CredentialBindingV1 binding) {
            return new CacheKey(profile.ref(), binding.secretGeneration(), binding.bindingDigest(),
                    binding.secretReferenceSha256());
        }

        @Override
        public boolean equals(final Object other) {
            return other instanceof CacheKey that && generation == that.generation
