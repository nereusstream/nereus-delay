...
public final class PersistentRecoveryCatalog implements RecoveryCatalogAuthority {
    ...
    private static final byte[] DIGEST_DOMAIN =
            Bytes.utf8("nereus-delay-recovery-catalog-state-v1\0");
    ...
    public RecoveryFloor advanceFloor(final byte[] checkpointId, final long expectedCatalogGeneration,
        ...
        return mutate(() -> delegate.advanceFloor(checkpointId, expectedCatalogGeneration,
    ...
    public RecoveryCatalog.Publication publishUploadedCheckpoint(
            ...
            final long expectedCatalogGeneration) {
        return mutate(() -> delegate.publishUploadedCheckpoint(publishedIntent, manifest,
    ...
    public Optional<RecoveryCatalog.FloorCoverage> proveFloorCoverage(final byte[] candidateCheckpointId,
        ...
        return read(() -> delegate.proveFloorCoverage(candidateCheckpointId, requiredMutationSequence,
                requiredPositions));
    }
    ...
    private void ensureParentDirectory() throws IOException {
        ...
        rejectSymbolicLink(parent, "Recovery Catalog state parent");
        if (!Files.exists(parent, LinkOption.NOFOLLOW_LINKS)) {
            Files.createDirectories(parent);
        }
        if (!Files.isDirectory(parent, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException("Recovery Catalog state parent is not a directory: " + parent);
        }
    }
    ...
    private static byte[] encodeSnapshot(final RecoveryCatalog.Snapshot snapshot) {
        ...
        final byte[] payload = CanonicalProtobuf.message(output -> {
            ...
            for (CheckpointManifest manifest : manifests) {
                final CheckpointResourceV1 resource = snapshot.manifestResources().get(
    ...
    private static byte[] encodeManifestEntry(final CheckpointManifest manifest,
    ...
    private static Entry decodeManifestEntry(final byte[] encoded) {
        ...
        final CheckpointResourceV1 resource = fields.size() == 2
                ? CheckpointResourceV1.decode(bytes(fields.get(1), 2)) : null;
