...
public final class FilesystemCheckpointUploadAdapter implements CheckpointUploadAdapter {
    private static final byte[] OBJECT_PATH_DOMAIN =
            Bytes.utf8("nereus-delay-filesystem-checkpoint-object-v1\0");
    ...
    public FilesystemCheckpointUploadAdapter(final Path root, final String container) {
        this(root, Bytes.utf8(Objects.requireNonNull(container, "container")),
    ...
    public FilesystemCheckpointUploadAdapter(final Path root, final byte[] container,
                                             final CheckpointManifestLimits limits) {
    ...
    public synchronized CheckpointResourceV1 upload(final CheckpointUploadRequest request) {
        ...
        final List<CheckpointFileInventory> actual = CheckpointFileInventory.collect(
                checkpointDirectory, limits);
        validateInventory(actual, manifest.files());

        final Path prefix = root.resolve("checkpoints").resolve(Bytes.hex(manifest.recoveryLineageId()))
        ...
        for (CheckpointManifest.FileEntry file : manifest.files()) {
            ...
            putImmutable(prefix.resolve("objects").resolve(objectFileName(file.objectKey(), file.objectVersion())),
                    source, file.length(), file.checksum());
        }
        ...
        return new CheckpointResourceV1(manifest.recoveryLineageId(), manifest.checkpointId(),
                request.intent().objectStoreProfile(), container, Bytes.utf8(manifestKey),
                Bytes.utf8("sha256-" + Bytes.hex(manifestHash)), manifestBytes.length, manifestHash);
    }

    private static void validateInventory(final List<CheckpointFileInventory> actual,
                                          final List<CheckpointManifest.FileEntry> expected) {
    ...
    private void putImmutable(final Path target, final Path source, final long expectedLength,
    ...
    private static void copyAndHash(final Path source, final Path temporary, final long expectedLength,
                                    final byte[] expectedChecksum) throws IOException {
    ...
    private static void moveCreateNewOrVerify(final Path temporary, final Path target,
                                              final long expectedLength, final byte[] expectedChecksum)
            throws IOException {
        try {
    ...
    private void ensureDirectoryWithinRoot(final Path directory) {
        final Path normalized = Objects.requireNonNull(directory, "directory").toAbsolutePath().normalize();
