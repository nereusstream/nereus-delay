...
public record CheckpointReapingQuiescenceProof(
    ...
    public CheckpointReapingQuiescenceProof {
        ...
        providerOwnershipClosedAt = Objects.requireNonNull(providerOwnershipClosedAt,
                "providerOwnershipClosedAt");
        if (requestQuiescenceHorizonMs <= 0 || maximumProviderOwnershipLifetimeMs < 0
                || maximumTrustedUtcIntervalWidthMs < 0) {
            throw new IllegalArgumentException("invalid checkpoint reaping quiescence bounds");
        }
        long minimumHorizon;
        try {
            minimumHorizon = Math.addExact(maximumProviderOwnershipLifetimeMs,
                    maximumTrustedUtcIntervalWidthMs);
        ...
        if (requestQuiescenceHorizonMs < minimumHorizon) {
            throw new IllegalArgumentException("checkpoint reaping horizon is shorter than provider ownership plus clock width");
        }
