...
public final class TrustedUtcClock {
    ...
    public synchronized TrustedUtcInterval current(final long monotonicNowNs) {
        ...
        final long earliest = subtractFloor(addSaturating(sample.earliestEpochMs(), elapsedMs),
                config.maxWallMonotonicDivergenceMs());
        final long latest = addSaturating(addSaturating(sample.latestEpochMs(), elapsedMs),
                config.maxWallMonotonicDivergenceMs());
        ...
        return new TrustedUtcInterval(earliest, latest, qualified && sampleHealthy && fresh && widthWithinBound,
                sample);
    }
    ...
    private boolean monotonicAndWallProgressIsConsistent(final TrustedUtcIntervalEvidence prior,
                                                          final TrustedUtcIntervalEvidence next) {
    ...
    private TrustedUtcInterval projected(final TrustedUtcIntervalEvidence evidence, final boolean isQualified,
                                         final long elapsedMs) {
        final long earliest = subtractFloor(addSaturating(evidence.earliestEpochMs(), elapsedMs),
                config.maxWallMonotonicDivergenceMs());
        final long latest = addSaturating(addSaturating(evidence.latestEpochMs(), elapsedMs),
                config.maxWallMonotonicDivergenceMs());
        return new TrustedUtcInterval(earliest, latest, isQualified, evidence);
    }

    private TrustedUtcInterval nullInterval() {
        final TrustedUtcIntervalEvidence empty = new TrustedUtcIntervalEvidence(0, 0,
                TrustedUtcIntervalEvidence.Source.CERTIFIED_HOST_CLOCK, new byte[]{1}, 0, 0, 0,
                new byte[TrustedUtcIntervalEvidence.HASH_LENGTH], 0, new byte[0]);
    ...
    public record Config(long maxUncertaintyMs, long maxSampleAgeMs, long maxWallMonotonicDivergenceMs,
