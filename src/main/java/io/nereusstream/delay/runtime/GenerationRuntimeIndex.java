...
public final class GenerationRuntimeIndex {
    ...
    private GenerationRuntimeIndex(final GenerationAggregateState aggregateState,
        ...
        if (admissionsUsed < 0 || uncertainRetryAdmissionsUsed < 0
                || uncertainRetryAdmissionsUsed > admissionsUsed || runtimeRevision <= 0) {
    ...
    private GenerationRuntimeIndex(final GenerationAggregateState aggregateState,
                                   final CurrentSendWorkKind currentWorkKind, final TimelineWorkRef timeline,
                                   final byte[] claimId, final byte[] publishAttemptId,
                                   final List<AttemptObligationRef> obligations, final int admissionsUsed,
                                   final int uncertainRetryAdmissionsUsed, final boolean duplicate,
                                   final long runtimeRevision, final byte[] runtimeDigest) {
        this(aggregateState, currentWorkKind, timeline, claimId, publishAttemptId, obligations, admissionsUsed,
                uncertainRetryAdmissionsUsed, duplicate, runtimeRevision);
    ...
    public static GenerationRuntimeIndex publishing(final byte[] publishAttemptId,
                                                     final List<AttemptObligationRef> obligations,
                                                     final int admissionsUsed, final int uncertainRetryAdmissionsUsed,
                                                     final boolean duplicate, final long runtimeRevision) {
    ...
    public static GenerationRuntimeIndex none(final GenerationAggregateState aggregateState,
                                               final List<AttemptObligationRef> obligations,
                                               final int admissionsUsed, final int uncertainRetryAdmissionsUsed,
                                               final boolean duplicate, final long runtimeRevision) {
        return new GenerationRuntimeIndex(aggregateState, CurrentSendWorkKind.NONE, null, null, null, obligations,
                admissionsUsed, uncertainRetryAdmissionsUsed, duplicate, runtimeRevision);
    }
    ...
    public static GenerationRuntimeIndex decode(final byte[] encoded) {
        final List<CanonicalProtobuf.Reader.Field> fields = readAll(
                new CanonicalProtobuf.Reader(encoded, true));
        if (fields.size() < 8) {
        ...
        final CurrentSendWorkKind currentWorkKind =
                CurrentSendWorkKind.fromWire(unsigned(fields.get(index++), 3));
        ...
        final GenerationRuntimeIndex result = new GenerationRuntimeIndex(aggregateState, currentWorkKind, timeline,
                claimId, publishAttemptId, obligations, admissions, uncertainAdmissions, duplicate, runtimeRevision,
                runtimeDigest);
    ...
    private void validateCurrentWork() {
        final boolean terminal = switch (aggregateState) {
    ...
    private static List<AttemptObligationRef> copyAndValidateObligations(
            final List<AttemptObligationRef> obligations) {
    ...
    public boolean equals(final Object other) {
        return other instanceof GenerationRuntimeIndex that
