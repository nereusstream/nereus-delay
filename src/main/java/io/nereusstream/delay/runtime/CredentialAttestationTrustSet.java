...
public final class CredentialAttestationTrustSet {
    ...
    private static final byte[] DIGEST_DOMAIN =
            Bytes.utf8("nereus-delay-credential-attestation-trust-set-v1\0");
    ...
    public static CredentialAttestationTrustSet single(final int verifierVersion,
                                                       final byte[] verifierId,
                                                       final int signingKeyVersion,
                                                       final PublicKey publicKey,
                                                       final long verifyNotBeforeEpochMs,
                                                       final long verifyNotAfterEpochMs) {
        return new CredentialAttestationTrustSet(List.of(new VerifierKey(verifierVersion, verifierId,
                signingKeyVersion, publicKey, verifyNotBeforeEpochMs, verifyNotAfterEpochMs)));
    }
    ...
    public void verify(final ProfileSemanticEnvelopeV1 profile, final CredentialBindingV1 binding) {
        ...
        final VerifierKey key = keys.stream().filter(candidate -> candidate.matches(attestation)).findFirst()
    ...
    public static final class VerifierKey {
        ...
        public VerifierKey(final int verifierVersion, final byte[] verifierId,
                           final int signingKeyVersion, final PublicKey publicKey,
