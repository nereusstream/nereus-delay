...
public record CommandResultRetentionPolicy(long policyVersion, long fullCommandResultRetentionMs) {
    ...
    public long retainUntil(final SourcePosition firstSourcePosition) {
        ...
        try {
            return Math.addExact(firstSourcePosition.brokerPersistenceTimeEpochMs(),
                    fullCommandResultRetentionMs);
        } catch (ArithmeticException overflow) {
            throw new IllegalArgumentException("full result retention boundary overflows epoch milliseconds",
                    overflow);
        }
    }
}
