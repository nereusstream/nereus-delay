...
import io.nereusstream.delay.store.ObjectStoreCredentialUseLeaseGate;
...
public final class S3CompatiblePayloadObjectStore {
    ...
    public S3CompatiblePayloadObjectStore(final ProfileSemanticEnvelopeV1 profile,
                                          final URI endpoint,
                                          final String region,
                                          final String bucket,
        ...
        this(profile, endpoint, region, bucket, accessKeyId, secretAccessKey, sessionToken,
                tenantRoutingScope, trustSet, proofKeyVersion, Long.MAX_VALUE, proofSigningKey, null,
    ...
    public S3CompatiblePayloadObjectStore(final ProfileSemanticEnvelopeV1 profile,
        ...
        if (!Bytes.constantTimeEquals(semantic.endpointConfigDigest(), endpointConfigDigest(
                this.endpoint, this.region, this.bucket))) {
        ...
        this.delegate = new InMemoryPayloadObjectStore(profile, tenantRoutingScope, trustSet, proofKeyVersion,
    ...
    public static byte[] endpointConfigDigest(final URI endpoint, final String region, final String bucket) {
        ...
        return Bytes.sha256(ENDPOINT_DOMAIN, Bytes.utf8(canonical.toString()),
    ...
    public static byte[] credentialAuthorizationScopeDigest(final String accessKeyId,
        ...
        return Bytes.sha256(CREDENTIAL_DOMAIN, Bytes.utf8(canonicalText(accessKeyId, "accessKeyId")),
                Bytes.utf8(canonicalText(region, "region")), Bytes.utf8(canonicalBucket(bucket)));
    }
    ...
    public PayloadUploadHandleResponseV1 issueUploadHandle(final byte[] reservationId,
                                                            final UploadHandleKindV1 kind,
                                                            final long nowEpochMs) {
        return delegate.issueUploadHandle(reservationId, kind, nowEpochMs);
    }

    public PayloadUploadHandleResponseV1 issueUploadHandle(final PayloadReservationReceiptV1 receipt,
                                                            final UploadHandleKindV1 kind,
                                                            final long nowEpochMs) {
        return delegate.issueUploadHandle(receipt, kind, nowEpochMs);
    }

    public void upload(final OpaquePayloadUploadHandleV1 handle, final byte[] payload,
                       final long nowEpochMs) {
        delegate.upload(handle, payload, nowEpochMs);
    }

    public void upload(final PayloadReservationReceiptV1 receipt,
                       final OpaquePayloadUploadHandleV1 handle, final byte[] payload,
                       final long nowEpochMs) {
        delegate.upload(receipt, handle, payload, nowEpochMs);
    }

    public PayloadAttestationResponseV1 attest(final OpaquePayloadUploadHandleV1 handle,
                                               final long nowEpochMs) {
        return delegate.attest(handle, nowEpochMs);
    }

    public PayloadAttestationResponseV1 attest(final PayloadReservationReceiptV1 receipt,
    ...
    public Optional<byte[]> load(final PublishAttemptLedger attempt) {
        ...
        final CommittedPayloadDescriptorV1 descriptor = admission.descriptor().value().payload().hasObject()
                ? admission.descriptor().value().payload().object() : null;
        if (descriptor == null) {
    ...
    private byte[] readVerified(final PayloadReference reference) {
        ...
        final HttpResponse<InputStream> response = send("GET", remoteObjectKey(identity), EMPTY_SHA256,
        ...
        try (InputStream input = response.body()) {
            ...
            if (reference.etag() != null) {
                final String etag = response.headers().firstValue("etag").map(String::trim).orElseThrow(
            ...
            final String metadataHash = response.headers().firstValue("x-amz-meta-nereus-sha256")
    ...
    private final class S3PayloadBackend implements PayloadObjectBackend {
        ...
        public byte[] read(final String objectIdentity) {
            final HttpResponse<InputStream> response = send("GET", remoteObjectKey(objectIdentity), EMPTY_SHA256,
                    HttpRequest.BodyPublishers.noBody(), Map.of(), HttpResponse.BodyHandlers.ofInputStream());
            if (response.statusCode() == 404) {
                closeQuietly(response.body());
                return null;
        ...
        public void putIfAbsent(final String objectIdentity, final byte[] payload, final long maxBytes) {
            ...
            try {
                response = send("PUT", remoteObjectKey(objectIdentity), payloadHash,
        ...
        private RemoteIdentity head(final String objectIdentity) {
            requireIdentity(objectIdentity);
            final HttpResponse<Void> response = send("HEAD", remoteObjectKey(objectIdentity), EMPTY_SHA256,
                    HttpRequest.BodyPublishers.noBody(), Map.of(), HttpResponse.BodyHandlers.discarding());
            if (!isSuccess(response.statusCode())) {
            ...
            final String etag = response.headers().firstValue("etag").map(String::trim).orElse(null);
    ...
    private static byte[] requireVersion(final HttpResponse<?> response, final byte[] expected,
                                         final String identity) {
        final String value = response.headers().firstValue("x-amz-version-id")
                .map(String::trim).filter(item -> !item.isBlank()).orElseThrow(
    ...
    private static void requireIdentity(final String identity) {
        ...
        if (identity.length() != 64 || !identity.equals(identity.toLowerCase(Locale.ROOT))
                || !identity.chars().allMatch(character -> (character >= '0' && character <= '9')
