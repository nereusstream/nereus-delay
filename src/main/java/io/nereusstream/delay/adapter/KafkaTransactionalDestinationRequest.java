...
public record KafkaTransactionalDestinationRequest(
    ...
    public static KafkaTransactionalDestinationRequest create(final String targetPhysicalTopic,
        ...
        final byte[] receiptKey = Bytes.sha256(KEY_DOMAIN, laneId, laneIncarnation, transactionIdentity,
                targetTopicUuid, Bytes.u32beBits(mapping.producer().target().partition()), receiptTopicUuid,
                Bytes.u32beBits(receiptResource.receiptPartition()));
        final byte[] receiptValue = Bytes.concat(VALUE_DOMAIN, Bytes.u32be(1),
                Bytes.u64be(mapping.sequenceId()), Bytes.lp32(mapping.publishAttemptId()),
                Bytes.lp32(mapping.preparedPublishHash()), targetTopicUuid,
                Bytes.u32beBits(mapping.producer().target().partition()), receiptTopicUuid,
                Bytes.u32beBits(receiptResource.receiptPartition()), laneId, laneIncarnation,
                transactionIdentity, mapping.mappingId());
        return new KafkaTransactionalDestinationRequest(targetPhysicalTopic, target, receiptPhysicalTopic,
                receiptResource, mapping, receiptKey, receiptValue);
    }
