...
public final class FilesystemPayloadObjectStore {
    ...
    public FilesystemPayloadObjectStore(final Path rootPath,
        ...
        this(rootPath, profile, tenantRoutingScope, trustSet, proofKeyVersion, Long.MAX_VALUE,
                proofSigningKey);
    }
    ...
    private static final class ObjectFileBackend implements PayloadObjectBackend {
        ...
        private ObjectFileBackend(final Path rootPath, final ProfileSemanticEnvelopeV1 profile) {
            ...
            if (!(profile.body() instanceof io.nereusstream.delay.protocol.ObjectStoreProfileSemanticV1 objectStore)) {
            ...
            final Path normalizedRoot = Objects.requireNonNull(rootPath, "rootPath").toAbsolutePath().normalize();
        ...
        public synchronized byte[] read(final String objectIdentity) {
            ...
            try {
                return LocalStatePathGuard.readRegularFileNoFollow(object, maxObjectBytes,
                        "payload Object Store object");
            } catch (IOException failure) {
                ...
                try {
                    ...
                    try {
                        ...
                        if (primaryFailure != null && cleanupFailure != primaryFailure) {
                            ...
                            throw new IllegalStateException("cannot clean payload Object Store temporary file",
                                    cleanupFailure);
                        }
                    }
                }
        ...
        private Path objectPath(final String objectIdentity) {
            ...
            return objectsRoot.resolve(objectIdentity.substring(0, 2))
                    .resolve(objectIdentity.substring(2, 4))
                    .resolve(objectIdentity + ".payload").normalize();
        }
        ...
        private static void compareExisting(final Path object, final byte[] expected) throws IOException {
            if (Files.isSymbolicLink(object)
                    || !Files.isRegularFile(object, LinkOption.NOFOLLOW_LINKS)) {
                throw new IOException("payload Object Store object is not a regular file: " + object);
            }
            final byte[] existing = LocalStatePathGuard.readRegularFileNoFollow(object, expected.length,
                    "payload Object Store object");
