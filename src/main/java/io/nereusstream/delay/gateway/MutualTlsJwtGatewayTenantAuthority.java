...
public final class MutualTlsJwtGatewayTenantAuthority implements GatewayTenantAuthority {
    ...
    public AuthenticatedTenantContext authenticate(final GatewayPeerContext peerContext) {
        ...
        final GatewayJwtIdentity identity = Objects.requireNonNull(verifier.verify(token, peerSession),
                "JWT verifier returned no identity");
        return new AuthenticatedTenantContext(identity.authenticatedTenantScopeHash(), identity.tenantRoutingScope(),
                identity.principalScopeHash());
    }
