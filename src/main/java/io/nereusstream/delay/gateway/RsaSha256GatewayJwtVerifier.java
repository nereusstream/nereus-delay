...
public final class RsaSha256GatewayJwtVerifier implements GatewayJwtVerifier {
    ...
    private static final byte[] PRINCIPAL_DOMAIN =
            Bytes.utf8("nereus-delay-gateway-principal-scope-v1\0");
     ...
     */
    public RsaSha256GatewayJwtVerifier(final PublicKey verificationKey, final String expectedIssuer,
                                      final String expectedAudience, final String expectedKeyId,
                                      final Clock clock, final long clockSkewSeconds,
    ...
    public RsaSha256GatewayJwtVerifier(final PublicKey verificationKey, final String expectedIssuer,
                                      final String expectedAudience, final Clock clock,
                                      final long clockSkewSeconds, final long maxLifetimeSeconds) {
        this(verificationKey, expectedIssuer, expectedAudience, null, clock, clockSkewSeconds,
                maxLifetimeSeconds);
    }
    ...
    public GatewayJwtIdentity verify(final String bearerToken, final SSLSession peerSession) {
        ...
        final byte[] principalScopeHash = Bytes.sha256(PRINCIPAL_DOMAIN,
                Bytes.lp32(Bytes.utf8(issuer)), Bytes.lp32(Bytes.utf8(tenant)),
                Bytes.lp32(Bytes.utf8(subject)), certificateFingerprint);
    ...
    private void validateTimes(final long issuedAt, final long notBefore, final long expiresAt) {
        if (issuedAt < 0 || notBefore < 0 || expiresAt < 0 || expiresAt <= issuedAt || notBefore > expiresAt
    ...
    private record TokenParts(String headerSegment, String payloadSegment, byte[] headerBytes, byte[] payloadBytes,
                              byte[] signature) {
