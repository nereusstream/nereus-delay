...
public final class InMemoryGatewayAdmissionController implements GatewayAdmissionController {
    ...
    public InMemoryGatewayAdmissionController(final int maxScheduleInFlight, final long maxScheduleBytes,
                                              final int maxRetryInFlight, final int maxControlInFlight) {
    ...
    public synchronized Decision reserve(final GatewayAdmissionRequestV1 request) {
        ...
        final boolean accepted = switch (request.operation()) {
            case SCHEDULE -> current.scheduleInFlight < maxScheduleInFlight
    ...
    private synchronized void release(final Digest32 tenant, final GatewayIngressOperationV1 operation,
                                      final long bytes) {
    ...
    private static final class Lease implements GatewayAdmissionLease {
        ...
        private Lease(final InMemoryGatewayAdmissionController owner, final Digest32 tenant,
                      final GatewayIngressOperationV1 operation, final long estimatedRequestBytes) {
            this.owner = owner;
            this.tenant = tenant;
