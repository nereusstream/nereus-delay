...
public final class PulsarClientArtifactWorkerSourceFactory {
    ...
    public static WorkerShardRuntime create(
        ...
        requireCurrentGuardProof(
                consumer, guard, barrier, topic, assignment.shardId().partition());

        final PulsarClientArtifactSourceRecordConsumer source = new PulsarClientArtifactSourceRecordConsumer(
                consumer, guard, assignment.shardId(), topic, receiveTimeout);
        try {
    ...
    private static void closeAfterFailure(
            final PulsarClientArtifactSourceRecordConsumer source, final Throwable failure) {
        try {
            source.close();
