...
public final class PulsarClientArtifactBindingSmoke {
    ...
    public static void main(final String[] arguments) {
        ...
        final GuardedSendSuccessEvidence successEvidence = new GuardedSendSuccessEvidence(
                22, 3, 4, 5, new TopicResourceGuardAttestation(guard, TOPIC, 0), 17, 19, 23, digest(31), digest(41));
        ...
        final PulsarClientArtifactSendTransport transport =
                new PulsarClientArtifactSendTransport(producer, CLUSTER, INCARNATION, TOPIC, CREATION_TIMESTAMP, 0);
        final PulsarSendRequest request = new PulsarSendRequest(
                CLUSTER,
                INCARNATION,
                TOPIC,
                CREATION_TIMESTAMP,
                0,
                com.nereusstream.delay.protocol.CommandId.random(new com.nereusstream.delay.protocol.ShardId(
                        com.nereusstream.delay.protocol.RouteIncarnation.random(), 0)),
                Bytes.utf8("pulsar-binding-frame"));
        final PulsarSendResult persisted =
                transport.send(request).toCompletableFuture().join();
        if (persisted.disposition() != PulsarSendResult.Disposition.PERSISTED
                || persisted.ledgerId() != 17
                || persisted.entryId() != 19
        ...
        final GuardedSendSuccessEvidence mismatchedEvidence = new GuardedSendSuccessEvidence(
                22, 3, 4, 5, new TopicResourceGuardAttestation(guard, TOPIC, 0), 18, 19, 23, digest(31), digest(41));
        final PulsarSendResult mismatched = new PulsarClientArtifactSendTransport(
                        producer(TOPIC, guardedMessageId(guard, mismatchedEvidence), null),
                        CLUSTER,
                        INCARNATION,
                        TOPIC,
                        CREATION_TIMESTAMP,
        ...
        if (mismatched.disposition() != PulsarSendResult.Disposition.UNKNOWN
                || mismatched.stableCode() != com.nereusstream.delay.protocol.StableCode.INTEGRITY_ERROR.wireValue()) {
            throw new IllegalStateException("P1 mismatched success evidence was accepted as persisted");
        }

        final byte[] replacement = digest(99);
        final PulsarSendResult mismatch = transport
                .send(new PulsarSendRequest(
                        CLUSTER, replacement, TOPIC, CREATION_TIMESTAMP, 0, request.commandId(), request.frame()))
                .toCompletableFuture()
                .join();
        ...
        final GuardedSendErrorEvidence errorEvidence =
                new GuardedSendErrorEvidence(22, 6, 7, 8, 26, digest(51), digest(61));
        ...
        final PulsarSendResult rejected =
                rejectingTransport.send(request).toCompletableFuture().join();
        ...
        System.out.println("P1 Delay binding smoke passed: persisted=" + persisted.disposition() + ", mismatch="
                + mismatch.disposition() + ", rejection=" + rejected.disposition());
    }

    @SuppressWarnings("unchecked")
    private static Producer<byte[]> producer(
            final String topic, final GuardedMessageId messageId, final Throwable failure) {
    ...
    private static GuardedMessageId guardedMessageId(
            final TopicResourceGuard guard, final GuardedSendSuccessEvidence evidence) {
        return (GuardedMessageId) Proxy.newProxyInstance(
                PulsarClientArtifactBindingSmoke.class.getClassLoader(),
                new Class<?>[] {GuardedMessageId.class, MessageIdAdv.class},
                (proxy, method, arguments) -> {
