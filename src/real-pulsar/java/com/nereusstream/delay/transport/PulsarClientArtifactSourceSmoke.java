...
public final class PulsarClientArtifactSourceSmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        ...
        try (PulsarClient client = PulsarClient.builder().serviceUrl(serviceUrl).build()) {
            ...
            final SourceAssignment recoveryAssignment = new SourceAssignment(
                    shard,
                    Bytes.sha256(Bytes.utf8("pulsar-recovery-assignment")),
                    1,
                    PulsarActivationBarrier.empty(
                            shard,
                            INCARNATION,
                            physicalTopic,
                            recoveryProof.connectionGeneration(),
                            recoveryProof.attestationDigest()));
            ...
            try (PulsarClientArtifactRecoverySourceCursor recovery = new PulsarClientArtifactRecoverySourceCursor(
                    recoveryNative, guard, recoveryAssignment, physicalTopic, java.time.Duration.ofMillis(250))) {
            ...
            final PulsarClientArtifactRecoverySourcePositioner.PositionedGuardProof positionedRecoveryProof =
                    PulsarClientArtifactRecoverySourcePositioner.seekAfter(
                            positionedRecoveryNative,
                            guard,
                            physicalTopic,
                            shard,
                            Optional.of(recoveredFirstPosition),
                            java.time.Duration.ofSeconds(5));
            final SourceAssignment positionedAssignment = new SourceAssignment(
                    shard,
                    Bytes.sha256(Bytes.utf8("pulsar-positioned-recovery-assignment")),
                    1,
                    PulsarActivationBarrier.empty(
                            shard,
                            INCARNATION,
            ...
            final GuardedConsumer<byte[]> firstNative =
                    PulsarClientArtifactSourceConsumerFactory.create(client, guard, physicalTopic, subscription);
            final PulsarClientArtifactSourceRecordConsumer firstSource = new PulsarClientArtifactSourceRecordConsumer(
                    firstNative, guard, shard, physicalTopic, java.time.Duration.ofMillis(250));
            ...
            final GuardedConsumer<byte[]> replayNative =
                    PulsarClientArtifactSourceConsumerFactory.create(client, guard, physicalTopic, subscription);
            final PulsarClientArtifactSourceRecordConsumer replaySource = new PulsarClientArtifactSourceRecordConsumer(
                    replayNative, guard, shard, physicalTopic, java.time.Duration.ofMillis(250));
            ...
            final GuardedConsumer<byte[]> afterAckNative =
                    PulsarClientArtifactSourceConsumerFactory.create(client, guard, physicalTopic, subscription);
            final PulsarClientArtifactSourceRecordConsumer afterAck = new PulsarClientArtifactSourceRecordConsumer(
                    afterAckNative, guard, shard, physicalTopic, java.time.Duration.ofMillis(250));
            try {
    ...
    private static void send(
        ...
        try {
            ...
            if (result.disposition() != PulsarSendResult.Disposition.PERSISTED) {
                throw new IllegalStateException(
                        "guarded Pulsar source producer did not persist: " + result.disposition());
            }
        } finally {
            transport.close();
        }
    ...
    private static long requireProof(final SourceReplayRecord entry) {
        if (entry.sourceConnectionGeneration() == null
                || entry.sourceConnectionGeneration() == 0
                || entry.guardAttestationDigest() == null
                || allZero(entry.guardAttestationDigest())) {
    ...
    private static void requirePosition(
        ...
        if (!position.shardId().equals(shard)
                || !Arrays.equals(position.brokerResourceIncarnation(), incarnation)
                || !position.physicalTopic().equals(physicalTopic)
                || position.ledgerId() < 0
                || position.entryId() < 0
                || position.brokerEntryTimestampEpochMs() < 0) {
