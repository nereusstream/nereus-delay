...
public final class PulsarClientArtifactMutationWorkerSmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        ...
        try {
            ...
            try (PulsarClient client = PulsarClient.builder().serviceUrl(serviceUrl).build()) {
                final PulsarSourcePositionPair positions = appendMutations(client, guard, physicalTopic, shard,
                        recoveryMutation.mutation(), activeMutation.mutation());
                ...
                final PulsarClientArtifactRecoverySourcePositioner.PositionedGuardProof proof =
                        PulsarClientArtifactRecoverySourcePositioner.seekAfter(recoveryNative, guard, physicalTopic,
                                shard, Optional.empty(), Duration.ofSeconds(5));
                final SourceAssignment sourceAssignment = new SourceAssignment(shard,
                        Bytes.sha256(Bytes.utf8("pulsar-mutation-worker-assignment")), 1,
                ...
                try {
                    final WorkerAssignmentAuthority assignmentAuthority = oxia == null
                            ...
                            : new OxiaSyncWorkerAssignmentBackend(oxia,
                                    "nereus-delay/pulsar-mutation-worker-placement/" + UUID.randomUUID());
                    final WorkerAssignment accepted = publishAssignment(assignmentAuthority, sourceAssignment,
                            oxia != null);
                    ...
                    final OwnerLease lease = authority.acquire(assignment, "pulsar-mutation-worker",
                            oxia == null ? Bytes.sha256(Bytes.utf8("pulsar-mutation-worker-session"))
                                    : oxia.sessionIdentity(), System.currentTimeMillis(), LEASE_DURATION_MS)
                            .orElseThrow();
                    ...
                    try {
                        ...
                        try (SharedRocksDbResources resources = new SharedRocksDbResources(storeConfig);
                            ...
                            try (PulsarClientArtifactRecoverySourceCursor recovery =
                                         new PulsarClientArtifactRecoverySourceCursor(recoveryNative, guard, assignment,
                                ...
                                final OwnerRecoveryCoordinator coordinator = new OwnerRecoveryCoordinator(ownedShard,
                                        authority, assignment, SourceReplaySuccessor.strictPulsarBatchMember(), cursor,
                                ...
                                requireSystemMutation(delayShard, recoveryMutation.mutation(), positions.recovery(),
                                        "recovery");
                                ...
                                final WorkerShardRuntime runtime = PulsarClientArtifactWorkerSourceFactory.create(
                                        recoveryNative, guard, physicalTopic, Duration.ofMillis(250), assignment,
                                ...
                                try {
                                    ...
                                    requireSystemMutation(delayShard, activeMutation.mutation(), positions.active(),
                                            "active");
                                    ...
                                    final var drain = runtime.drain(
                                            new io.nereusstream.delay.ownership.OwnerDrainCoordinator.DrainRequest(
                                                    System.currentTimeMillis() + 30_000, 0, checkpointPath,
                                    ...
                                    if (drain.pendingCheckpointTask() != null || drain.finalCheckpointPath() == null
                                        ...
                                        throw new IllegalStateException(
                                                "Pulsar mutation Worker drain did not publish the final checkpoint and release the lease");
                                    }
                                    runtimeDrained = true;
                                    System.out.println("Pulsar mutation Worker vertical smoke passed: recovery TIME_FENCE "
                                            + "ledger/entry=" + positions.recovery().ledgerId() + "/"
                                            + positions.recovery().entryId() + ", active Store apply TIME_FENCE "
                                            + "ledger/entry=" + positions.active().ledgerId() + "/"
    ...
    private static PulsarSourcePositionPair appendMutations(final PulsarClient client,
                                                              final TopicResourceGuard guard,
                                                              final String physicalTopic, final ShardId shard,
                                                              final SystemMutation recovery,
                                                              final SystemMutation active) throws Exception {
        ...
        try (PulsarClientArtifactShardLogMutationAppender appender =
                     new PulsarClientArtifactShardLogMutationAppender(
                             PulsarClientArtifactProducerFactory.create(client, CLUSTER, INCARNATION,
                                     physicalTopic, CREATION_TIMESTAMP, "nereus-delay-mutation-worker-producer"),
                             proofConsumer, shard, CLUSTER, INCARNATION, physicalTopic, CREATION_TIMESTAMP,
            ...
            return new PulsarSourcePositionPair(requirePersisted(first, "recovery"),
                    requirePersisted(second, "active"));
        } finally {
            closeNative(proofConsumer);
        }
    }

    private static PulsarSourcePosition requirePersisted(final ShardLogMutationAppender.AppendOutcome outcome,
                                                          final String phase) {
        if (outcome.disposition() != ShardLogMutationAppender.AppendDisposition.PERSISTED
            ...
            throw new IllegalStateException("Pulsar " + phase + " mutation append was not persisted: "
                    + outcome.disposition());
        }
        return position;
    }

    private static void requireRecoveryOutcome(final OwnerRecoveryTurn turn, final SystemMutation mutation,
        ...
        if (!Arrays.equals(result.mutationId(), mutation.systemMutationId())
                ...
                || result.mutationType() != mutation.type() || !samePosition(outcome.position(), expectedPosition)) {
            throw new IllegalStateException("Pulsar recovery returned a different System Mutation");
        }
    }

    private static Iterator<SourceReplayEntry> firstOnly(
            final PulsarClientArtifactRecoverySourceCursor cursor) {
        return new Iterator<>() {
    ...
    private static io.nereusstream.delay.ownership.SourceApplyCoordinator.TurnResult runUntilApplied(
        ...
        do {
            ...
            if (result.status()
                ...
                throw new IllegalStateException("Pulsar mutation Worker source turn failed: " + result.status(),
                        result.failure());
            }
    ...
    private static void requireSystemMutation(final DelayShard shard, final SystemMutation mutation,
                                              final PulsarSourcePosition expectedPosition, final String phase) {
        final SystemMutationResult result = shard.getSystemMutationResult(mutation.systemMutationId());
        if (result == null || !Arrays.equals(result.mutationHash(), mutation.mutationHash())
                || result.mutationType() != mutation.type() || result.applyStatus() != ApplyStatus.APPLIED
                || result.stableCode() != StableCode.OK
                || !samePosition(result.appliedSourcePosition(), expectedPosition)) {
            throw new IllegalStateException("Pulsar " + phase + " mutation Store result was not APPLIED/OK: "
                    + result);
        }
    }
    ...
    private static boolean samePosition(final SourcePosition actual, final PulsarSourcePosition expected,
                                        final boolean allowLater) {
        return samePosition(actual, expected) || allowLater && actual instanceof PulsarSourcePosition observed
                && observed.compareTo(expected) > 0;
    }
    ...
    private static MutationFixture timeFence(final ShardId shard, final String identity, final KeyPair keyPair) {
        ...
        final TrustedUtcIntervalEvidence evidence = new TrustedUtcIntervalEvidence(now, now + 1_000,
                TrustedUtcIntervalEvidence.Source.CERTIFIED_HOST_CLOCK,
                Bytes.utf8("pulsar-mutation-worker-clock-" + identity), 1, 2, 3,
                Bytes.sha256(Bytes.utf8("pulsar-mutation-worker-evidence-" + identity)), 0, null);
        ...
        final byte[] body = io.nereusstream.delay.protocol.CanonicalProtobuf.message(output -> {
            io.nereusstream.delay.protocol.CanonicalProtobuf.bytes(output, 1,
                    new ShardSubjectV1(shard).canonicalBytes());
            io.nereusstream.delay.protocol.CanonicalProtobuf.uint32(output, 2,
                    SystemMutationType.TIME_FENCE.wireValue());
            io.nereusstream.delay.protocol.CanonicalProtobuf.int64(output, 3, retryUntil);
            io.nereusstream.delay.protocol.CanonicalProtobuf.int64(output, 10, closeThrough);
            io.nereusstream.delay.protocol.CanonicalProtobuf.uint32Bits(output, 11, keyVersion);
            io.nereusstream.delay.protocol.CanonicalProtobuf.bytes(output, 12, proofId);
            io.nereusstream.delay.protocol.CanonicalProtobuf.bytes(output, 13, evidence.canonicalBytes());
        });
        return new MutationFixture(SystemMutation.signed(shard, SystemMutationType.TIME_FENCE, retryUntil, proofId,
                body, io.nereusstream.delay.protocol.AuthorIdentity.fence(
                        Bytes.utf8("pulsar-mutation-worker-fence"), keyVersion).canonicalBytes(), keyVersion,
                keyPair.getPrivate()));
    }
    ...
    private static OxiaSyncOwnerLeaseBackend.ClientHandle connectOxiaIfConfigured() {
        ...
        return OxiaSyncOwnerLeaseBackend.connectUnchecked(endpoint, namespace,
                "nereus-delay-pulsar-mutation-worker-" + UUID.randomUUID(), Duration.ofSeconds(15),
    ...
    private static WorkerAssignment publishAssignment(final WorkerAssignmentAuthority authority,
        ...
        final WorkerPlacementPolicy.WorkerCandidate candidate = new WorkerPlacementPolicy.WorkerCandidate(
                "pulsar-mutation-worker", capacity(1), CapacityVectorV1.empty(), 0, 16, 0, 16,
        ...
        final WorkerAssignmentCoordinator.PlacementResult result = coordinator.place(sourceAssignment,
                Bytes.sha256(Bytes.utf8("pulsar-mutation-worker-capacity-envelope")), 1, List.of(candidate),
    ...
    private static void createTopic(final HttpClient client, final String adminUrl, final String topic)
        ...
        for (int attempt = 0; attempt < 40; attempt++) {
            ...
            if (response.statusCode() != 409 && response.statusCode() != 412
                ...
                throw new IllegalStateException("create topic failed with HTTP " + response.statusCode()
                        + ": " + response.body());
            }
    ...
    private static void deleteTopicIfPresent(final HttpClient client, final String adminUrl, final String topic) {
        try {
            ...
            if (response.statusCode() >= 300 && response.statusCode() != 404) {
                System.err.println("Pulsar mutation Worker cleanup could not delete topic: "
                        + response.statusCode());
            }
