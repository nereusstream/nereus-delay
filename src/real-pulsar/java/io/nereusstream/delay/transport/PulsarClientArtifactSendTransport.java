...
public final class PulsarClientArtifactSendTransport implements
        PinnedPulsarCommandIngress.PulsarSendTransport,
        io.nereusstream.delay.adapter.PinnedPulsarNativeSubmissionAdapter.PulsarNativeSendTransport {
    ...
    public PulsarClientArtifactSendTransport(final Producer<byte[]> producer,
        ...
        this.resourceIncarnation = Objects.requireNonNull(resourceIncarnation, "resourceIncarnation").clone();
    ...
    public CompletionStage<PulsarSendResult> send(final PulsarSendRequest request) {
        ...
        if (!matches(request.authenticatedClusterId(), request.resourceIncarnation(), request.physicalTopic(),
                request.physicalTopicCreationTimestamp(), request.partition())) {
            return CompletableFuture.completedFuture(PulsarSendResult.unknown(
                    io.nereusstream.delay.protocol.StableCode.RESOURCE_INCARNATION_MISMATCH.wireValue(), null));
        }
    ...
    private CompletionStage<PulsarSendResult> sendFrame(final byte[] frame) {
        try {
            return producer.newMessage().value(frame).sendAsync().handle((messageId, failure) ->
                    failure == null ? success(messageId) : failure(failure));
    ...
    private PulsarSendResult success(final MessageId messageId) {
        ...
        return new PulsarSendResult(PulsarSendResult.Disposition.PERSISTED, authenticatedClusterId,
                ...
                guarded.brokerEntryTimestamp(), io.nereusstream.delay.protocol.StableCode.OK.wireValue(),
                encodeRequestEvidence(evidence), encodeResponseEvidence(evidence));
    }
    ...
    private PulsarSendResult failure(final Throwable failure) {
        ...
        if (guardFailure != null && guardFailure.definitelyNotPersisted()
            ...
            return PulsarSendResult.definitelyNotPersisted(
                    io.nereusstream.delay.protocol.StableCode.BROKER_DEFINITIVE_NOT_PERSISTED.wireValue(),
                    encodeErrorEvidence(guardFailure.responseEvidence().get()));
        }
        final byte[] evidence = guardFailure == null || guardFailure.responseEvidence().isEmpty()
                ? null : encodeErrorEvidence(guardFailure.responseEvidence().get());
        return PulsarSendResult.unknown(
                io.nereusstream.delay.protocol.StableCode.ENQUEUE_RESULT_UNCERTAIN.wireValue(), evidence);
    }

    private boolean matches(final String cluster, final byte[] incarnation, final String topic,
                            final long creationTimestamp, final int requestPartition) {
        return authenticatedClusterId.equals(cluster) && Arrays.equals(resourceIncarnation, incarnation)
                && physicalTopic.equals(topic) && physicalTopicCreationTimestamp == creationTimestamp
