...
public final class PulsarClientArtifactMutationSmoke {
        ...
        try {
            ...
            try (PulsarClient client = PulsarClient.builder().serviceUrl(serviceUrl).build()) {
                ...
                try (PulsarClientArtifactShardLogMutationAppender appender =
                             new PulsarClientArtifactShardLogMutationAppender(
                                     PulsarClientArtifactProducerFactory.create(client, CLUSTER, INCARNATION,
                                             physicalTopic, CREATION_TIMESTAMP,
                                             "nereus-delay-mutation-producer"), appendProofConsumer, shard, CLUSTER,
                                     INCARNATION, physicalTopic, CREATION_TIMESTAMP, Duration.ofSeconds(15))) {
                    final var outcome = appender.append(mutation);
                    if (outcome.disposition() != io.nereusstream.delay.ownership.ShardLogMutationAppender
                            .AppendDisposition.PERSISTED) {
                        throw new IllegalStateException("Pulsar mutation append was not persisted: "
                                + outcome.disposition());
                    }
                ...
                final SourceAssignment assignment = new SourceAssignment(shard,
                        Bytes.sha256(Bytes.utf8("pulsar-mutation-assignment")), 1,
                ...
                try (PulsarClientArtifactSourceRecordConsumer source =
                             new PulsarClientArtifactSourceRecordConsumer(activeNative, guard, shard, physicalTopic,
                    ...
                    if (!(polled.entry() instanceof SourceReplayMutation replayed)
                        ...
                        throw new IllegalStateException("Pulsar active source did not expose the exact System Mutation");
                    }
                    requireReplayPosition(appendedPosition, replayed.position(), "active source");
                    final SourceAcknowledgement.AcknowledgementResult ack = polled.acknowledgement()
                            .acknowledge(polled.entry(), null);
                    if (ack.disposition() != SourceAcknowledgement.Disposition.ACKED) {
                        throw new IllegalStateException("Pulsar mutation ACK was not durable: " + ack.disposition(),
                                ack.failure());
                    }
                }
            }
    ...
    private static void requireReplayPosition(final PulsarSourcePosition appended,
                                              final SourcePosition replayedPosition, final String phase) {
        if (!(replayedPosition instanceof PulsarSourcePosition replayed)
            ...
            throw new IllegalStateException("Pulsar " + phase + " position changed: appended=" + appended
                    + ", replayed=" + replayedPosition);
        }
    }

    private static SystemMutation timeFence(final ShardId shard) throws Exception {
        final TrustedUtcIntervalEvidence evidence = new TrustedUtcIntervalEvidence(3_000, 3_001,
                TrustedUtcIntervalEvidence.Source.CERTIFIED_HOST_CLOCK, Bytes.utf8("pulsar-clock"), 1, 2, 3,
                Bytes.sha256(Bytes.utf8("pulsar-mutation-evidence")), 0, null);
        ...
        final byte[] body = io.nereusstream.delay.protocol.CanonicalProtobuf.message(output -> {
            ...
            io.nereusstream.delay.protocol.CanonicalProtobuf.int64(output, 3, 9_000);
        ...
        return SystemMutation.signed(shard, SystemMutationType.TIME_FENCE, 9_000, proofId, body,
                AuthorIdentity.fence(Bytes.utf8("pulsar-mutation-fence"), keyVersion).canonicalBytes(), keyVersion,
                keyPair.getPrivate());
    }
