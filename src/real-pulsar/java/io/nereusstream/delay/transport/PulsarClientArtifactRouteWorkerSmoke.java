...
public final class PulsarClientArtifactRouteWorkerSmoke {
     ...
     */
    private static void runMultiShard(final String serviceUrl, final String adminUrl,
                                      final String topicPrefix, final String oxiaEndpoint) throws Exception {
        ...
        try {
            ...
            final String assignmentPrefix = "nereus-delay/pulsar-route-worker-multi-assignment/"
                    + UUID.randomUUID();
            ...
            try (PulsarClient client = PulsarClient.builder().serviceUrl(serviceUrl).build()) {
                ...
                for (int partition = 0; partition < shardCount; partition++) {
                    ...
                    try {
                        nativeConsumer = PulsarClientArtifactSourceConsumerFactory.create(client, guard,
                                physicalTopic, "nereus-delay-route-source-multi-" + partition + "-"
                                        + UUID.randomUUID());
                        final PulsarClientArtifactSourceRecordConsumer source =
                                new PulsarClientArtifactSourceRecordConsumer(nativeConsumer, guard,
                                        new ShardId(routeIncarnation, partition), physicalTopic, RECEIVE_TIMEOUT);
                        ...
                        send(client, guard, physicalTopic, beforeRoute,
                                "route-multi-before-producer-" + partition, partition);
                        final SourceRecordConsumer.PolledSourceRecord first = poll(source);
                        final PulsarSourcePosition firstPosition = requireCommand(first, beforeRoute,
                                "before Route partition " + partition);
                        final PulsarClientArtifactRecoverySourcePositioner.PositionedGuardProof barrierProof =
                                PulsarClientArtifactRecoverySourcePositioner.awaitStableProof(nativeConsumer, guard,
                                        physicalTopic, partition, Duration.ofSeconds(5));
                        probes.add(new RouteShardProbe(shard, guard, physicalTopic, beforeRoute, afterRoute, first,
                                firstPosition, barrierProof, nativeConsumer));
                ...
                try (OxiaRouteAuthoritySession publisherSession = OxiaRouteAuthoritySession.connect(
                             oxiaEndpoint, namespace, "nereus-delay-pulsar-route-multi-publisher-"
                                     + UUID.randomUUID(), Duration.ofSeconds(15), routePrefix);
                     OxiaRouteAuthoritySession providerSession = OxiaRouteAuthoritySession.connect(
                             oxiaEndpoint, namespace, "nereus-delay-pulsar-route-multi-provider-"
                                     + UUID.randomUUID(), Duration.ofSeconds(15), routePrefix);
                     OxiaSyncOwnerLeaseBackend.ClientHandle assignmentHandle =
                             OxiaSyncOwnerLeaseBackend.connectUnchecked(oxiaEndpoint, namespace,
                    ...
                    for (RouteShardProbe probe : probes) {
                        ...
                        if (!assignedWorkers.add(accepted.workerId())) {
                            throw new IllegalStateException("Pulsar multi-shard placement reused a Worker identity: "
                                    + accepted.workerId());
                        }
                        admissions.add(new MultiShardAdmission(probe, placement.publication(), accepted, lease));
                    }
                    if (assignedWorkers.size() != shardCount) {
                        throw new IllegalStateException("Pulsar multi-shard placement did not span two Worker identities");
                    }
                    ...
                    try {
                        ...
                        try {
                                ...
                                final OwnedDelayShard ownedShard = new OwnedDelayShard(delayShard, admission.lease(),
                                        new OwnerIdentityV1(bytes(16, 70 + index), bytes(16, 90 + index),
                                                admission.lease().ownerEpoch(), Bytes.sha256(Bytes.utf8(
                                                        "pulsar-route-worker-multi-fencing-" + index))));
                                recoverRouteRecord(admission.assignment(), ownerAuthority, ownedShard,
                                        probe.firstRecord().entry(), verificationKey, controlSnapshot, workClasses);
                                if (ownedShard.state() != ShardLifecycleState.ACTIVE_FOR_COMMANDS
                                        || !(ownedShard.lastCatchupPosition() instanceof PulsarSourcePosition recovered)
                                        || !recovered.equals(probe.firstPosition())) {
                                    throw new IllegalStateException("Pulsar multi-shard recovery did not apply partition "
                                            + shard.partition() + " pre-Route record");
                                }
                                requireAcked(probe.firstRecord().acknowledgement().acknowledge(
                                        probe.firstRecord().entry(), null));
                                send(client, probe.guard(), probe.physicalTopic(), probe.afterRoute(),
                                        "route-multi-after-producer-" + index, shard.partition());
                                if (probe.nativeConsumer().connectionGeneration()
                                        != probe.barrierProof().connectionGeneration()) {
                                    throw new IllegalStateException("Pulsar multi-shard source connection generation changed "
                                            + "before active apply: partition=" + shard.partition());
                                }
                                runtimes.add(PulsarClientArtifactWorkerSourceFactory.create(probe.nativeConsumer(),
                                        probe.guard(), probe.physicalTopic(), RECEIVE_TIMEOUT,
                                        admission.assignment().sourceAssignment(), workClasses, ownedShard, store,
                                        resources, ownerAuthority, verificationKey.getPublic()));
                            ...
                            final Map<Integer, io.nereusstream.delay.ownership.SourceApplyCoordinator.TurnResult>
                            ...
                            while (!pending.isEmpty() && System.nanoTime() < deadline) {
                                ...
                                if (turn.result().status()
                                    ...
                                    if (!(turn.result().entry() instanceof SourceReplayRecord record)
                                            || !record.command().equals(
                                                    probes.get(turn.shardId().partition()).afterRoute())
                                            || !(record.position() instanceof PulsarSourcePosition)) {
                                        throw new IllegalStateException("Pulsar multi-shard Worker applied an unexpected "
                                    ...
                                    throw new IllegalStateException("Pulsar multi-shard Worker source turn failed: shard="
                                            + turn.shardId() + ", status=" + turn.result().status(),
                                            turn.result().failure());
                                }
                            }
                            if (!pending.isEmpty()) {
                                throw new IllegalStateException("Pulsar multi-shard Worker source apply timed out: "
                                        + pending);
                            }
                            for (int index = 0; index < admissions.size(); index++) {
                                final MultiShardAdmission admission = admissions.get(index);
                                final ShardId shard = admission.probe().shard();
                                final io.nereusstream.delay.ownership.SourceApplyCoordinator.TurnResult turn =
                                        appliedTurns.get(index);
                                if (turn == null || !(turn.entry() instanceof SourceReplayRecord record)
                                        ...
                                        || !applied.physicalTopic().equals(admission.probe().physicalTopic())) {
                                    throw new IllegalStateException("Pulsar multi-shard Store did not persist partition "
                            ...
                            for (int index = 0; index < runtimes.size(); index++) {
                                ...
                                final byte[] checkpointId = Arrays.copyOf(Bytes.sha256(Bytes.utf8(
                                        "pulsar-route-worker-multi-final-checkpoint-" + shard.partition())), 16);
                                ...
                                if (drain.pendingCheckpointTask() != null || drain.finalCheckpointPath() == null
                                        ...
                                        || CheckpointFileInventory.collect(checkpointPath).isEmpty()
                                        || !ownerAuthority.current(shard).isEmpty()) {
                                    throw new IllegalStateException("Pulsar multi-shard drain did not complete partition "
                            ...
                            for (MultiShardAdmission admission : admissions) {
                                if (!assignmentAuthority.withdraw(admission.publication())) {
                                    throw new IllegalStateException("Pulsar multi-shard assignment was not withdrawn exactly: "
                            ...
                            System.out.println("Pulsar signed Route -> two guarded SUBSCRIBE barriers -> Oxia "
                                    + "multi-shard Assignment/Owner -> one Worker fleet -> RocksDB apply/ACK/checkpoint "
                                    ...
                                    .map(probe -> probe.firstPosition().ledgerId() + "/" + probe.firstPosition().entryId())
                                    .toList());
                        } finally {
    ...
    private static RouteSnapshotV1 multiRouteSnapshot(final String physicalTopicBase,
        ...
        final List<PulsarPhysicalPartitionIdentityV1> physicalPartitions = probes.stream()
                .map(probe -> new PulsarPhysicalPartitionIdentityV1(probe.shard().partition(),
                        probe.physicalTopic(), INCARNATION, CREATION_TIMESTAMP))
                .toList();
        ...
        final List<RoutePartitionPolicyV1> policies = probes.stream().map(probe -> {
            final BrokerResourceIdentityV1 broker = BrokerResourceIdentityV1.pulsar(
                    new PulsarBrokerResourceIdentityV1(CLUSTER, INCARNATION, probe.physicalTopic(),
                            CREATION_TIMESTAMP));
    ...
    private static RouteWorkerAssignmentCoordinator.PlacementRequest placementRequest(final long now,
        ...
        return new RouteWorkerAssignmentCoordinator.PlacementRequest(partition,
                Bytes.sha256(Bytes.utf8("pulsar-route-worker-assignment-" + partition + "-" + workerId)), 1,
                Bytes.sha256(Bytes.utf8("pulsar-route-worker-capacity-" + partition + "-" + workerId)), 1,
    ...
    private static void requireRouteAssignment(final WorkerAssignment assignment, final RouteSnapshotV1 snapshot,
        ...
        if (!assignment.routeBound() || !Arrays.equals(snapshot.snapshotDigest(), assignment.routeSnapshotDigest())
                ...
                || barrier.guardedSourceConnectionGeneration() != probe.barrierProof().connectionGeneration()
                || !Arrays.equals(barrier.resourceGuardAttestationDigest(), probe.barrierProof().attestationDigest())) {
            throw new IllegalStateException("Oxia multi-shard Worker assignment did not retain the signed Pulsar "
                    + "Route barrier");
        }
    }
    ...
    private static void send(final PulsarClient client, final TopicResourceGuard guard, final String physicalTopic,
                             final PreparedCommand command, final String producerName, final int partition)
            throws Exception {
    ...
    private static void createTopic(final HttpClient client, final String adminUrl, final String topicBase,
        ...
        for (int partition = 0; partition < partitions; partition++) {
            ...
            final String guardPath = adminUrl + "/admin/v2/persistent/public/default/" + physicalTopic
                    + "/resourceGuard";
