...
public final class PulsarClientArtifactShardLogMutationAppender implements ShardLogMutationAppender, AutoCloseable {
    ...
    public PulsarClientArtifactShardLogMutationAppender(final Producer<byte[]> producer,
        ...
        this.expectedGuard = new TopicResourceGuard(Objects.requireNonNull(authenticatedClusterId,
        ...
        if (!physicalTopic.equals(producer.getTopic()) || !physicalTopic.equals(sourceConsumer.getTopic())
    ...
    private AppendOutcome persisted(final MessageId messageId, final SourceProof sourceProof) {
        ...
        if (batched && rawBatchSize <= 0 || batchSize <= 0
        ...
        if (!expectedAttestation.equals(evidence.attestation())
                || evidence.ledgerId() != ledgerId || evidence.entryId() != entryId
                || evidence.brokerEntryTimestamp() != guarded.brokerEntryTimestamp()) {
            return AppendOutcome.unknown();
        }
        final PulsarSourcePosition position = new PulsarSourcePosition(shard,
                expectedGuard.resourceIncarnation(), physicalTopic, ledgerId, entryId, batchIndex, batchSize,
    ...
    private AppendOutcome failure(final Throwable failure) {
        ...
        return guardFailure != null && expectedGuard.equals(guardFailure.expectedGuard())
    ...
    private SourceProof sourceProof() {
        if (!expectedGuard.equals(sourceConsumer.resourceGuard())
    ...
    private record SourceProof(long connectionGeneration, byte[] digest) {
        ...
        public boolean equals(final Object other) {
            return other instanceof SourceProof that && connectionGeneration == that.connectionGeneration
