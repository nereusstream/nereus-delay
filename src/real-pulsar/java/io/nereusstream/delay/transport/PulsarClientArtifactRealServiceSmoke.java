...
public final class PulsarClientArtifactRealServiceSmoke {
    ...
    public static void main(final String[] arguments) throws Exception {
        ...
        try (PulsarClient client = PulsarClient.builder().serviceUrl(serviceUrl).build()) {
            final Producer<byte[]> oldProducer = PulsarClientArtifactProducerFactory.create(client, CLUSTER,
                    OLD_INCARNATION, physicalTopic, OLD_CREATION_TIMESTAMP, "nereus-delay-p1-real-smoke-old");
            ...
            final PulsarSendResult first = oldTransport.send(oldRequest).toCompletableFuture().join();
            ...
            final Producer<byte[]> replacementProducer = PulsarClientArtifactProducerFactory.create(client, CLUSTER,
                    NEW_INCARNATION, physicalTopic, NEW_CREATION_TIMESTAMP, "nereus-delay-p1-real-smoke-new");
            ...
            final PulsarSendResult replacement = replacementTransport.send(
                    request(NEW_INCARNATION, physicalTopic, NEW_CREATION_TIMESTAMP, 2))
    ...
    private static TopicResourceGuardException requireStaleProducerRejected(final PulsarClient client,
                                                                               final String physicalTopic)
            throws Exception {
        final TopicResourceGuard expected = new TopicResourceGuard(CLUSTER, OLD_INCARNATION,
                OLD_CREATION_TIMESTAMP);
        try {
            final Producer<byte[]> staleProducer = PulsarClientArtifactProducerFactory.create(client, CLUSTER,
                    OLD_INCARNATION, physicalTopic, OLD_CREATION_TIMESTAMP, "nereus-delay-p1-real-smoke-stale");
            staleProducer.close();
            ...
            if (guardFailure == null || !expected.equals(guardFailure.expectedGuard())
                    || !guardFailure.definitelyNotPersisted() || guardFailure.responseEvidence().isPresent()) {
                throw new IllegalStateException("stale P1 producer rejection was not the expected typed boundary",
                        failure);
            }
            return guardFailure;
        }
    ...
    private static PulsarSendRequest request(final byte[] incarnation, final String topic,
                                             final long creationTimestamp, final int sequence) {
        final ShardId shard = new ShardId(RouteIncarnation.random(), 0);
        return new PulsarSendRequest(CLUSTER, incarnation, topic, creationTimestamp, 0,
                CommandId.random(shard), Bytes.utf8("nereus-delay-p1-real-smoke-" + sequence));
    }
    ...
    private static void requirePersisted(final PulsarSendResult result, final String label) {
        if (result.disposition() != PulsarSendResult.Disposition.PERSISTED
                || result.ledgerId() < 0 || result.entryId() < 0
                || result.brokerEntryTimestampEpochMs() < 0
                || result.requestEvidenceBytes() == null || result.responseEvidenceBytes() == null) {
            throw new IllegalStateException(label + " was not an evidenced persistence: "
    ...
    private static void deleteTopic(final HttpClient client, final String adminUrl, final String topic)
        ...
        final HttpResponse<String> response = request(client,
                adminUrl + "/admin/v2/persistent/public/default/" + topic, "DELETE", "");
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
