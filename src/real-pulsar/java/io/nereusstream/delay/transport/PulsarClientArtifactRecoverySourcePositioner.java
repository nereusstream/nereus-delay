...
public final class PulsarClientArtifactRecoverySourcePositioner {
    ...
    public static PositionedGuardProof awaitStableProof(
        ...
        final long deadline = started > Long.MAX_VALUE - timeoutNanos
    ...
    private static PositionedGuardProof requireCurrentProof(final GuardedConsumer<byte[]> consumer,
                                                             final TopicResourceGuard expectedGuard,
                                                             final String physicalTopic,
                                                             final int partition) {
        ...
        return new PositionedGuardProof(generation, attestation,
                PulsarClientArtifactSourceRecordConsumer.attestationDigest(attestation));
    }

    private static MessageId toMessageId(final PulsarSourcePosition position,
                                         final TopicResourceGuard expectedGuard,
                                         final String physicalTopic,
                                         final ShardId expectedShard) {
        ...
        if (!expectedShard.equals(accepted.shardId())
                ...
                || accepted.ledgerId() < 0 || accepted.entryId() < 0) {
        ...
        if (accepted.entryKind() == PulsarSourcePosition.EntryKind.BATCH) {
            ...
            return new BatchMessageIdImpl(accepted.ledgerId(), accepted.entryId(), -1,
                    accepted.normalizedBatchIndex(), accepted.batchSize(), null);
        }
    ...
    public record PositionedGuardProof(long connectionGeneration,
