...
### 实际 Store grant 检查接线（2026-09-08）
...
### 首条 SHARD grant 的 Store 初始化接入（2026-09-08）
...
### Target 控制记录接入 Worker source 循环（2026-09-08）
...
### Target Command 保留结果重放接入（2026-09-08）
...
### Target 首次 Cancel 实际状态应用（2026-09-18）
...
### 首次 Reschedule 的 generation 原子转换（2026-09-18）
...
### Target reservation QUERY WorkClass（2026-09-22）
...
持久状态与 quota 不因查询/排队改变；具体预算和失败分类见 NDIP-3 quota §41。

### Target TIME_FENCE 认证入口（2026-09-22）
...
继续沿用现有 TIME_FENCE wire fields 和 ProofId 公式；Target reader 仅收紧有界输入，
不增加 bodyVersion/NV/CF。认证判定不能直接充当持久 fence，也不能提前终止 reservation。
首次结果重放必须先于新认证；正式 writer 仍需在同一 ReadView 核对 prior fence/root/source，
将单调水位、最后 proof、完整结果/source 和实际费用一次提交，见 quota §42。

### Target TIME_FENCE 持久 source writer（2026-09-22）
...
该增量落实前序认证入口留下的原子写入：复用既有 IngressFenceState，无新 key/NV/CF。
已提交水位现可被 Target replay/首次入口读取；reservation Query/Commit/Cancel 的完整
状态覆盖及有界 expiry materializer 仍是下一步，不能仅凭 writer 判定 reservation 生命周期
已贯通。最后 proof 属于最后接受的 fence；非递增 fence 不将其较低 closeThrough 当成当前水位。

### Target reservation 的 TIME_FENCE 有效状态（2026-09-22）
...
该只读覆盖使用同一 Store view 的已提交水位，不读取系统时钟，不伪造新的 mutation/source。
Query Snapshot 同时保留物理 reservation、实际 payloadPhase、readSource 和 fence watermark；
...
### Target reservation 有界发现与 GC 调度（2026-09-22）

Target reservation 到期物化已接入 GC WorkClass：每次任务最多发现并处理一条记录，
在读权限保护下使用持久 TIME_FENCE 固定本轮扫描 cutoff；候选扫描、完整点查与实际
计费共用有限读取预算。成功后推进进程内游标，扫到末尾重置，能在下一轮发现插入旧
游标之前的记录。CLOSED Target 显式延期并在下一轮重访，完整 Close 终态顺序尚待实现。
四个实际 Worker/Store 必要开发场景通过；source 不因 GC 推进，空扫描/提交前拒绝不写。
生产触发与 providers/factory、完整 Close、恢复/迁移和集中验证仍未完成，29切片范围不变。

GC 不读取墙钟来认定到期，不将扫描游标写进 Store，不将队列接纳或发现候选作为写入
...
Closure 接口本身仍是进程内权限响应。全 Target 关闭现在由下述持久 marker 产生；
binding/domain/legacy 历史权限及关闭计费物化仍待生产实现，保护必须持续到 read guard/
commit。当前实现未激活生产入口，也不提供对象删除权限。
