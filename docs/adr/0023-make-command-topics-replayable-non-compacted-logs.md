...
- SDK Command and system writers carry the Route's protected incarnation token in Producer metadata, and `PULSAR_RESOURCE_GUARD` checks every SEND before persistence. Auto-topic-creation is disabled and only the resource controller can create/delete or mutate incarnation properties.
