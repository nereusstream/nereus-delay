...
Every Broker that may own a V1 Command/fence/control writer, managed target, Pulsar Attempt Journal, DLQ Export, or `AUTO_FAST` target loads `PULSAR_RESOURCE_GUARD_V1`. A trusted deployment controller signs a cluster capability attestation containing the guard protocol/version, plugin binary digest, complete Broker set, and configuration generation. Missing, expired, or partial coverage prevents Route/Profile registration and writer/Lane/native activation.

Each Nereus Producer carries its expected cluster, Profile, physical topic/partition, resource token, principal scope, and guard protocol in Producer metadata. On every `SEND`, including automatic reconnect retransmission, the guard:

1. runs from `BrokerInterceptor.onPulsarCommand` before `handleSend`;
2. resolves the exact Producer by the command's `producerId`;
3. reads the actual persistent Topic's loaded ManagedLedger incarnation property;
4. compares every expected field against the actual Producer/Topic/principal;
5. throws a typed `InterceptException` with wire `ServerError.NotAllowedError` and stable `NEREUS_RESOURCE_GUARD_REJECTED_V1` detail on mismatch or inability to prove identity.

Pulsar converts that exception to `SEND_ERROR` before `topic.publishMessage`. Receiving the exact guard rejection on the correlated pending operation is definitive nonpublication. Losing/mis-correlating that response is still `UNKNOWN`. Every guarded V1 Producer channel fixes `batching=false` and one in-flight SEND so the error's sequence/pending operation is unambiguous; parallelism uses bounded channel slots. The post-success `producerCreated` callback cannot reject Producer creation, and `onMessagePublish` has no typed `InterceptException` response contract, so neither is an acceptable replacement for the pre-handle command guard.
...
Pulsar consumption uses a source-locked `PULSAR_SUBSCRIBE_RESOURCE_GUARD_V1`: every initial/reconnect SUBSCRIBE carries the expected token, creation identity, physical topic/partition, and protocol. The Broker validates those fields synchronously against the exact `PersistentTopic`/ManagedLedger before adding the Consumer or returning success. Each connection generation begins `UNCERTIFIED`, sends no FLOW, and releases no record until guarded success. Mismatch closes the consumer/Source Assignment or evidence Lane, and queued callbacks from an older generation cannot apply/ACK. Admin lookups and `consumerCreated` are not Broker-bound proof. This contract covers Command source, Attempt Journal, and every Pulsar evidence/DLQ reader. Kafka consumption obtains the equivalent boundary directly from pinned Fetch requests.
