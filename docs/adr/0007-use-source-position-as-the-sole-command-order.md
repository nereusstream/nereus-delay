...
- Every Command for a Delayed Message uses its original route and partition. A record delivered through another route or partition yields `INGRESS_ROUTE_MISMATCH`; V1 neither scans partitions nor reorders records to repair it.
...
- V1 does not infer caller intent or defer a Command that overtakes an unresolved prerequisite. Dependencies are resolved by awaiting the preceding applied result; deferred-operation tombstones require a separate design.
