...
Nereus Delay V1 makes Schedule capacity a durable shard-local decision. Tenant-wide hard limits are pre-divided into versioned Shard Quota Grants whose sum cannot exceed the tenant policy; a shard atomically updates its grant usage, Lane usage, message state, Command result, and Source Position in one RocksDB WriteBatch. V1 does not pretend that independent per-shard checks form an exact global counter, and it does not dynamically borrow hard capacity across shards.
...
- Reschedule in V1 cannot change payload or destination and therefore moves timing state without releasing and reacquiring message bytes. Replay consumes a fresh active-message slot but reuses already retained payload.
...
The control plane creates static `(tenantId, routeId, partitionId, grantVersion)` allocations for pending messages, pending bytes, reservations, and Lane cardinality. The deployment validator proves that their sums fit the tenant-wide policy. Grant growth is safe only through the serialized shrink-first transfer below; a reduction below current usage is grandfathered and blocks new admission without evicting existing messages. V1 has no opportunistic online borrowing outside that protocol.
...
V1 quota redistribution is a serialized, shrink-first Control Operation, not dynamic borrowing:
...
- At placement/Owner activation the Worker assigns the Protocol Registry's immutable per-shard `CapacityGrantV1(OUTCOME_RESERVE)`, bound to reserve-source version, grant ID and the complete zero-explicit `CapacityVectorV1` digest. It is embedded as a projection of the full `ShardCapacityEnvelopeV1`, stored in Oxia placement and `meta_cf`, and revalidated on every Owner/Store change. No online cross-DB observation or borrowing decides correctness, and a component grant is never added twice to the full envelope.
...
- Successful Publish Admission atomically charges the exact attempt's vector against its shard partition in the same WriteBatch as `PUBLISH_ADMISSION_V1`. Logical callback timeout does not release it. The charge is released or reduced only when the logged outcome/retirement state and its actual retained charge are durable and checkpoint-safe.
