...
-Nereus Delay does not implement key-range export/import or merge a Worker-wide checkpoint into a running database. If benchmarks justify a future `ShardBundle`, the bundle must explicitly become the physical ownership, checkpoint, restore, and migration unit rather than silently combining otherwise independent shards.
