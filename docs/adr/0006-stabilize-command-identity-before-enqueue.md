...
- `delayMessageId` independently protects message-entity identity. Any different first-seen initial Schedule cannot overwrite an active or retained message and yields `DELAY_MESSAGE_ID_CONFLICT`; Nereus Delay has no separate “identical Schedule under a new Command ID” exception.
...
-Nereus Delay does not allow Cancel or Reschedule to overtake an unresolved initial Schedule. Callers first retry or resolve that Schedule; deferred cancellation tombstones would require a separate future design.
