...
- Timing validation uses the record's Broker persistence time. Current Trusted UTC may prepare a signed `EXPIRE_GENERATION`, but expiration becomes authoritative only at that System Mutation's Source Position and replay validates the persisted interval evidence.
