...
NDL1/ source frames, preserves Kafka Topic UUID/offset/leader-epoch/
...
`nereus/delay-resource-guard`, based on the locked
...
evidence remain OPEN, so this audit does not promote P1 or production
status.
...
`nereus/delay-full-implementation@1bee5b45`. `SourceRecordConsumer` and
...
`nereus-delay-oxia-e2e-1786729940-65321` on host endpoint
...
`nereus-delay-oxia-e2e-1786732310-90387` at `127.0.0.1:16649`; eight
...
`MessagePrecondition`/self-routing-ID decode, Semantic Core control
...
composition for the frozen digest-only `GatewayAuditEvent`. The
...
coincidental shard co-location as transaction evidence.
...
the contract: `WorkClassEventLoop` uses the scheduler's atomic
...
prepares and signs the exact `EXPIRE_GENERATION` body before queue
...
`ActiveLaneState`, exact current scheduler Owner and Store Incarnation,
...
After `666f56a`, the Registry distinction between outer `AuthorIdentity` and
nested `OwnerIdentity` is executable rather than prose-only. Admission body
field 10, Claim precondition field 14 and Ready Certificate field 2 now parse
and retain bare `OwnerIdentity`; Claim records and canonical attempt ledgers
...
After `fe7d484`, this local seam covers all eight work classes, including
...
it does not prove the -required cross-record Owner Lease/Upload
...
gap: all eight classes must have a handler before work can be accepted,
...
`OwnerLeaseTest.catchupPinsTheAdapterSuccessorAndRejectsAKafkaGapBeforeApplyingIt`;
...
batch-index checks remain active. The full constructor still requires and
pins a positive batch size, so this compatibility repair does not weaken the
 source-assignment contract. Focused evidence is
...
outcome-reserve-record bounds. Nereus Delay permits one Message to retain multiple
...
`CompactCommandResult` must be at least the result Source Position's Broker
...
ownership boundary: before `PUBLISH_OUTCOME(UNKNOWN)` can materialize an
...
Canonical ledgers additionally bind the exact Lane incarnation; missing Lane
...
stream identity. `EvidenceCursor.sameIdentity` and `dominates` reject a
...
splice two physical Journals. `EvidenceCursorTest`
...
through `AcknowledgementSet` and the typed varint helper. Direct inspection
cannot bypass canonical field order, known ACK kinds, or wire-type validation;
`PayloadProofControlPayloadTest.laneAcknowledgementQueryRejectsMalformedAckWireType`
...
The local codec audit additionally verified that implemented key-version
...
`path_start_epoch_ms`, exposes typed path/start fields, and `SloSampleStart`
...
The Registry-shaped `ActiveLaneState` projection also preserves raw unsigned
...
The nested `ChargeVector` now preserves the complete raw `uint64` domain at
...
`RetryPolicyRef` decoder, so the high-bit version boundary is preserved on
...
`TARGET_PARTITION_HASH` digest. `PublishAdmissionBodyTest` covers the
...
references now also reuse `ProfileRef.decode`, so malformed semantic-hash
...
The runtime Claim seam now has a strict `DelayShard.claimForPublish` entrypoint.
...
`ScheduleBinding`, it additionally re-decodes the accepted Schedule/Prepare
...
`PreparedPublishDescriptor`; `Descriptor.value()` returns that same typed
...
`PayloadForPublish` owns the inline/object union and length/SHA-256 equality,
and `ClaimMaterialization` owns the complete 11-field canonical projection,
...
`PublishAttemptLedgerTest.baselineLedgerPreservesHighBitGenerationAndAttemptBits`,
...
The same raw-bit rule now covers `RetryDecision.completed_attempt_no` and
...
The `ExactResourceIdentity` retirement projection now applies the same
...
`LaneChannelResource` and `LocalStoreResource`. Each value owns its direct
...
semantic-hash preimages. The separate `CapacityVector` amount projection
...
`QuotaTransferPlanRef` likewise retains the complete nonzero tenant-policy
...
| 材料 | 责任边界 |
|---|---|
| [`Nereus Delay 设计.md`](<Nereus Delay 设计.md>) | 行为、状态、时序、配置和验收语义 |
| [`PROTOCOL-REGISTRY.md`](PROTOCOL-REGISTRY.md) | 唯一 numeric/byte registry：frame、field、enum、hash、code、key、manifest 和 golden vector |
...
| remote accepted/unknown 不冒充 failure；多 attempt obligation 可闭合定位 | 主设计 §11/§15；Registry 的 `GenerationRuntimeIndex`、`AttemptObligationRef`；ADR 0022 |
...
`CredentialEquivalenceAttestation`, `CredentialBinding`,
`CredentialBindingHead` and `CredentialBindingProtection`. Their digests,
...
CAS and durable reread remain external authority gates. `CredentialUseLease`
...
`ProfileCatalogScheduleResolver` uses that seam only as a local exact-profile
...
When a persisted binding is revisited for Commit, Reschedule or recovery,
...
digest locally. `PreparedControlOperation` now additionally enforces the
...
local codec boundary. `ControlTargetMutationBinding` then validates a
...
`ControlOperationAuthorization` additionally checks the authenticator's
actor/role/scope hash projection and the minimum role matrix before that
step; its explicit scope proof remains an external authorization input.
`ControlRegistrationBinding` binds all three local registration outcome
...
`ControlSystemMutationFactory` now centralizes the signed envelope and
...
`ControlRegistrationProjection` keeps the receipt and that initial CURRENT
...
| expiration | qualified `EXPIRE_GENERATION` apply | raw timer、unqualified Worker clock |
...
严格 的 Schedule/PrepareLarge/Cancel/Reschedule prepare 入口也已进入同一
`DelayClient` 契约；它们只构造 Registry-shaped canonical bytes，不做网络 I/O，
`EmbeddedDelayServiceTest.delayClientPreparesStrictCommandsWithoutIo` 覆盖其
body round-trip。legacy prepare 仍是兼容桥，不能伪装成 managed submission。
对应的 `enqueue`/`enqueueBatch` 在 pending admission 前调用 strict frame
validator；legacy body 得到 `INVALID_PREPARED_COMMAND` definitive branch，不消耗
Source Position 或 byte budget，且 batch 仍逐条保持输入顺序。
`DelayClient.awaitApplied` 在同一边界上补齐等待语义：先验证 pinned embedded
receipt，再对 PENDING 结果 drain 并 reread；不合法 receipt 直接返回
`RECEIPT_MISMATCH`，不会推进 source。
`DelayClient.prepareManagedSubmission` 现在把严格 frame 包装为不可变的
managed `PreparedSubmission`，其 `submit` bridge 在嵌入式路径先验证
nonzero physical attempt，再分配 Source Position 并投影为 managed
`SubmissionOutcomeMessage`。注入 `PreparedSubmissionAdapter` 时由该 adapter
...
`NativeCapabilitySnapshot` 和 pinned issuer key，在签名/expiry/target/partition/
...
adapter metadata/Delay Message ID 按共享 `TargetPartitionHash` 重算目标分区，
...
typed `EvidenceCursor`（旧构造器未提供 Journal identity 时仅保留 target-derived
...
blocker。
`PublishAttemptLedger` 现在保留兼容的 legacy/current bytes，并提供不改变线上
`PublishAdmission` 的可选 V3 local Journal projection：先记录 adapter 分配的
...
`PreparationFailure`，其 `StableError.stage` 固定为 `PREPARATION`，同时保持
...
没有 value schema、digest 或 source-ordered accounting transition。因此不能把
...
`CommitLargeSchedule` body. This is still a local pre-I/O binding check;
...
adapter reconstructs the original receipt after reopening a current-format `COMMITTED`,
`ABANDONED` or materialized `EXPIRED` value. A strict legacy decode/upgrade
...
obligation framing in both initial and current branches; the local prefix evidence
...
`RecoveryFloorRefTest` and `RecoveryCatalogTest.catalogComparesManifestMutationSequenceAsUnsigned`
...
The nested `ProtectionRef.protection_generation` and its `gc_cf/PROTECTION`
...
bits for nested `ProfileRef` versions and Kafka receipt-slot/Pulsar journal
...
比较 `SchedulerRound.owner` 与当前 Owner；Owner/Store 更换会重新进入
...
`ActiveLaneState` 的 direct read、bounded compatibility projection and
...
`DelayShard`；并已补齐 Registry-shaped `ActiveLaneState`、
`LaneQuotaUsageEntry/Map`、`ReadyCertificate`、`ActivationBarrier` 与
`EvidenceCursor` 的独立 canonical codec 与交叉校验，checkpoint manifest 也能
严格 round-trip 非空的 Kafka/Pulsar typed evidence-cursor 数组；
`RecoveryFloorRef` 已补齐 lineage/checkpoint/source/typed-cursor-array 的
canonical floor reference codec，`RecoveryCandidateRef` 与 `RecoveryPin`
也已补齐 candidate branch、lineage binding 和 session-identity digest 的
canonical value codecs；`EvidenceCursor` 现在还提供同 generation
...
Owner Lease/session CAS；`CheckpointResource` 与
`CheckpointUploadIntent`
...
protection 和同 lineage/checkpoint 的 active `RecoveryPin`；catalog/pin
...
checksummed `ACTIVE` pointer 前，都会 reread exact active `RecoveryPin`。pin
...
Recovery-Floor/retention gate。`LaneRecordEnvelope` 现在还提供 Registry
field-10 直接承载 typed `ActiveLaneState` 的构造、严格解码和
...
Typed `ActiveLaneState` 与 `LaneTerminalGuard` 现在还会解析
...
`CanonicalLaneTuple` 及其 Active/Terminal 回归测试只证明本地 canonical shape
...
Active state 还要求嵌套 `ReadyCertificate` 的 Lane ID/incarnation 与自身完全
...
协议边界也已开始按 Registry 收敛：`CanonicalScheduleIntent` 及其
`RetryPolicyRef`、`AdapterMetadata`、`KafkaMetadata`、
`CommittedPayloadDescriptor` 已提供严格 canonical value codec，覆盖
...
wire/value 校验；`ScheduleCommandBody` 与 `PrepareLargeScheduleBody` 现在也
按 Registry 写入 Client common fields 1–3，`CommandBodies.*` 只作为显式迁移
seam；`PreparedCommand`/`CommandCodec.*` 还会把这些 fields 与 outer
message/type/retry identity 逐项比较。`DelayShard` 已把五类 body 分成明确
的运行时边界：Cancel/Reschedule 直接进入原子状态迁移；Schedule/Prepare
 必须经过显式 `ScheduleResolver`，校验 tuple 派生 Lane、payload 投影，
并把 canonical body/tuple 写入 `ScheduleBinding` sidecar。缺少 resolver 时
固定返回 `ROUTE_SNAPSHOT_UNAVAILABLE`，不会降级到旧 body；旧
`ScheduleIntent`/`LargeScheduleIntent` 只服务于非 兼容命令。这个 resolver
仍是本地 authority seam，不等于 Profile/Policy/Oxia/真实 Adapter 已接入。
`ProfileBindingActivatePayload`/`ProfileNewBindingClosePayload` 和
`ProfileBindingControlState` 现在也提供了 source-ordered first-binding marker
投影；catalog-backed 路径在尚未应用首个 Profile activation marker 时就
...
CommitLargeSchedule 也有独立 canonical body 和嵌套
`CanonicalPayloadCommitProof` codec，校验 reservation/message identity、typed Object
Store Profile、tenant scope、optional etag presence、proof ID/signature 后，
通过统一 proof view 复用现有 reservation commit 状态机。Prepare 现在还用
required field 15 固定 exact `OBJECT_STORE ProfileRef`，并把它随 canonical
body 持久化到 `ScheduleBinding`；typed proof/receipt 必须匹配完整 ref，legacy
...
固定为仍在有效期内的同一 opaque handle 与同一签名 `CanonicalPayloadCommitProof`。
...
DLQ 分支和 backoff arithmetic 漂移；`RetryPolicyCatalog` 接入后，
...
保留 full-shape `RetryDecision` 的 policy/cause/domain，并在 source-ordered
状态变更前重算 exact policy ref、checked retry deadline 和
`RETRY_JITTER(MESSAGE_PUBLISH)`；错误 jitter 被拒绝，正确值才进入本地
timeline/state transition。旧的 opaque `UNKNOWN` placeholder 仍保留兼容路径。
Canonical Admission bytes 已随 attempt ledger 保留，并为该重算提供
trusted `firstAttemptAt`。新 source-applied Admission 现在同时写入 Current
...
至少以 message expiry 作为安全上界。旧 opaque ledger 仍只走 structural
...
对应的 `ControlReason`、trust-set activate/issuance-close payload branches
...
canonical `ControlReason` 与 `AcknowledgementSet` decoder，不再用只检查
字段长度/哈希长度的浅层校验接受未知 reason kind 或 malformed optional
entries；`PayloadProofControlPayloadTest` 覆盖该 fail-closed 路径。
...
`StableError.stage=ENQUEUE`，不能把其它 stage 当作 ingress 结果；公共
`StableError` 拒绝 `OK` 成功码。`ProtocolCodecTest.stableErrorPinsRegistryRetryabilityAndPreparedRefPresence`
...
`EmbeddedDelayService.enqueueOutcome` 对 queued 与 uncertain 两个需要
physical attempt 的分支也先执行同一 nonzero 16-byte 校验；null、长度错误或全零
输入固定映射为本地 `DEFINITELY_NOT_QUEUED(INVALID_PREPARED_COMMAND)`，不会让
`CanonicalCommandQueuedReceipt`/`EnqueueUncertain` 构造器异常穿透，也不会产生无 attempt
...
embedded conformance seam。
 ...
 managed submission 现在还在 Producer ownership 前强制执行
`CommandCodec.encodeFrame/decodeFrame`：`PinnedKafkaCommandIngress`、
`PinnedPulsarCommandIngress` 的 `enqueueOutcome` 和
`PreparedSubmission` 不再接受 legacy compatibility body；
`AdapterIngressTest.kafkaWireRejectsLegacyBodyBeforeTransportOwnership` 与
`PreparedCommandTest.managedPreparedSubmissionRejectsACompatibilityBody` 覆盖该
fail-closed 边界；同一边界由
`AdapterIngressTest.pulsarWireRejectsLegacyBodyBeforeTransportOwnership` 对
Pulsar 复核。`CanonicalCommandQueuedReceipt.PreparedCommandRef` 也只从
`encodeFrame` 派生 frame digest；`ProtocolCodecTest.commandQueuedReceiptRejectsCompatibilityCommandBody`
证明 legacy body 不能被标成 `ProtocolTuple`。旧 `enqueue()` 路径仍明确保留为
compatibility seam，不被计入 submission/receipt 证据。
...
identity，避免把无法成为合法 `BrokerResourceIdentity` 的文本交给 Producer；
...
`ProfileCatalogScheduleResolver` 从 exact Destination Profile/Delivery Capability
...
`actionAt=deliverAt`；`ProfileCatalogScheduleResolverTest`
...
`ChargeVector`，Evidence Resolution 的 cursor 必须是 typed canonical
`EvidenceCursor`。Apply 现在还要求 definitive Outcome/Resolution 的 transfer 与
...
`SHA-256("nereus-delay-evidence-resolution-logical-id\0" || PublishAttemptId || evidenceId)`；
...
`OwnerIdentity` 与外层 Owner 做 byte-equality fence；同 epoch 但 deployment、worker
...
only arbitrary pre-registry synthetic adapter bytes use the bounded zero-charge
...
Current ledgers: before a `PUBLISHING` WriteBatch, the retained body is compared
...
the Source Position or creating an attempt. Opaque ledgers remain a
...
`PUBLISH_ADMISSION` 必须使用 body 的 `PublishAttemptId`，
`CLAIM_RESULT` 必须使用 body 的 `ClaimId`，而 `EXPIRE_GENERATION`
必须使用 Registry §4.8 规定的
`SHA-256("nereus-delay-expiry-logical-id" || DelayMessageId || u32be(generation) || i64be(expireAt))`。
...
DLQ Export Result 的 canonical body 已校验 `ChargeVector`，而 configured
`DlqExportRecord` 现在持久化 policy-derived retained charge projection；
`NOT_CONFIGURED` terminal record 固定保留 all-zero projection，initial-format record
...
The same `DLQ_EXPORT_RESULT` parser now closes the nested retry boundary:
`RetryDecision` must use the exact canonical field sequence and its nested
`RetryPolicyRef` is decoded (not merely accepted as nonempty bytes), including
...
`RetryDecision` field sequence and first-attempt/deadline interval fence as
...
`PublishEvidence`/`ExternalDeliveryIdentity` 进一步把 Registry 的
`PublishEvidence` 公共字段、kind 对应 oneof 分支、verification-status 语义、
owner identity 和 domain-separated `evidence_id` 固定在一个共享 codec 中；
`PUBLISH_OUTCOME` 与 `DLQ_EXPORT_RESULT` 不再只接受任意非空 nested bytes。
`ChannelResourceIdentity`/`CredentialUseLease` 现在把 channel-bearing
...
其中 `OperatorAttestationEvidence` 的 verifier 槽位也已强制为
`ProfileKind.EVIDENCE_VERIFIER`，并由 `PublishEvidenceTest` 覆盖错误
Profile kind 的负向回归；Profile 注册、key activation 与签名 authority
仍属于外部 release gate。
`LaneTerminalGuard` 的两个 Profile 槽位也已在构造和 decode 路径强制为
`DESTINATION`、`DELIVERY_CAPABILITY`，由 `LaneTerminalGuardTest` 覆盖
双向错误 kind；typed Active/Terminal 路径还由
`CanonicalLaneTuple` 强制 tuple 的 Registry shape 和 Profile
id/version/semantic-hash byte projection。该 parser 不代替 resolver 或外部
authority，兼容层仍可保存 resolver 提供的 opaque bytes。

`ChannelResourceIdentity` now preserves the complete raw unsigned patterns for
its physical `channel_generation`, optional `evidence_generation`, and
`credential_binding_generation` fields, matching the raw `EvidenceCursor`
...
`ControlResultCodecTest`, `ChannelResourceIdentityTest`,
...
The shared decoder also parses the nested `ActivationBarrier` and every
repeated `EvidenceCursor`, rather than treating those fields as opaque
...
Admission body or the public `ReadyCertificate` wrapper can expose the
certificate; direct-parser rejection vectors are in `ReadyCertificateTest`.

`PublishAdmissionBody` 还把 `PreparedPublishDescriptor` 的 adapter kind、固定
adapter encoding version（并要求 immutable Destination Profile 也 pin 版本 `1`）、target resource、physical partition 与嵌套
`ChannelResourceIdentity` 做 exact equality，要求 `business_metadata` branch
...
`ClaimResultBody` 现在还要求 field 20 `ChargeVector transfer` 与
`ClaimPrecondition.claimed_charge` 做 canonical byte-equality。这样签名的
...
当前 assignment 接管路径现在还会显式 pin `SourceReplaySuccessor`：同一
...
兼容性 monotonic seam，不能作为 source-gap evidence。
...
`meta_cf/CONTROL_RESERVE` 以 `CapacityVector` 持久化 Broker system-writer
...
Owner Lease 的本地 CAS 投影现在还按 lifecycle graph 拒绝回退状态和
...
fail closed；这与 `OwnerIdentity`、Store runtime metadata 和 inflight key
...
上下文丢失前先占用一个无法用于 activation 的租约。本地证据为
...
CAS，证据为 `OxiaOwnerLeaseStoreTest.rejectsBackendAcquireResultThatSkipsAcquiringState`。
 assignment acceptance also rejects a non-null compatibility lease context
with assignment epoch `0`; only a positive exact assignment epoch may bind the
catch-up window。证据为 `OwnerLeaseTest.legacyZeroEpochLeaseContextCannotAuthorizeAssignment`。
...
`CompatibleControlSnapshot` 与调用方提供的完整 snapshot exact match；缺失或
...
timeline/Message/READY；Current Claim format 还保留 `TimelineWorkRef`，因此
...
重复当成完成时间。`SloObservationOutboxTest` 覆盖 semantic-start-as-success
...
当前实现还把物理 value 边界收紧到 Registry 的 `TimelineWorkRef`：新的
...
runtime projection 存在时要求与 `GenerationRuntimeIndex.timeline` byte-identical。旧 `TimelineEntry` 只作为
...
v3 placeholder；它不是 canonical runtime value，下一次 typed mutation 必须替换，
...
gate。
在升级为 `HANDED_OFF` 前，`PublishEvidence` 还逐字段校验 `PULSAR_SEND_ACK` 的
target resource、physical partition、prepared hash 与 retained Admission 的
`ChannelResourceIdentity`/prepared hash 完全一致；外部目标、分区或 payload hash
...
`id_cf/_SCHEDULE_BINDING` 的 direct lookup 也会在读取 sidecar 前校验
...
回归覆盖拒绝、重复 identity 和释放路径。`WorkerRuntimeResourceProbe` 现在从 JVM、procfs、both cgroup generations
...
Kafka/Pulsar broker resource identities, Pulsar `EvidenceCursor` physical
...
`CanonicalCommandQueuedReceipt` additionally binds the `PreparedCommandRef` shard to
...
证明之后 reread 当前 `RecoveryPin`。活动 pin 若保护待删 checkpoint 的
...
`CompatibleControlSnapshot` 占用，严格绑定 shard subject、ProtocolTuple/Profile/
initial grant 集合及 digest；打开/恢复时逐项解码、校验 digest 和 shard identity，
并由 `CompatibleControlSnapshotTest`、
...
`RecoveryCandidateRef` lineage/base，key 2 持久完整 typed `RecoveryFloorRef`，
key 3 持久与 Floor generation 绑定的 raw `uint64` catalog generation，key 4 持久
带 digest 的 `RecoveryInstallState` install/open phase。Shard Store 在 open、
...
`RecoveryInstallStateTest` 和 `ShardStoreTest.catalogBoundRestoreRejectsPinDriftBeforeActivePublication`
...
`CompatibleControlSnapshot.snapshotDigest` 与 manifest 的
`controlStateDigest` 做 exact 比较；不一致会在 install 前 fail closed。
缺少 key 10 的旧本地 DB 仍保留为兼容 seam，但不能据此证明
...
`CompatibleControlSnapshot` 的 shard/digest 与 manifest 一致；adapter 返回的
...
查询层也已补齐 `CheckpointSummary`/`CheckpointCatalogResult` 的
...
`CheckpointControlResult` 也已补齐 checkpoint-control typed result 的
...
枚举/presence 校验，`ControlTypedResult` 也会按 branch 调用对应 codec，拒绝
...
The local `DeliveryCapabilitySemantic` value codec now closes the Registry
...
remain release blockers.
The four Profile semantic body codecs and `ProfileSemanticEnvelope` now add
...
authority remain release evidence. The three `TrustedUtcIntervalEvidence` uint64 counters and its
...
paths.
`NativeCapabilitySnapshot` likewise preserves the complete nonzero raw
...
| Delay MinIO provider smoke slice | `nereus/delay-full-implementation@31ba5661` (`S3CompatibleMinioRealSmokeTest` plus `e2e/run-minio-real-e2e.sh`; the harness locks the local MinIO image tag/repository digest, creates only its own temporary bucket through curl SigV4, runs the real adapter with `--rerun-tasks`, and removes only its own container; the receipt proves one MinIO endpoint's immutable checkpoint upload/idempotent retry/download path, while generic S3/provider breadth, credential authority/renewal, deletion, chaos and release gates remain open) |
| Delay exact provider-version slice | `nereus/delay-full-implementation@2981a269` (implementation `b971cd3f` makes missing `x-amz-version-id` fail closed for the mandatory exact-version Object Store Profile; the versioned MinIO receipt records provider manifest version `780f1e1f-c7da-4dc1-ae4e-a7b9be4f801c`; complete version-aware deletion, retire/Floor/Pin authority, provider consistency and release gates remain open) |
| Delay exact manifest-version readback slice | `nereus/delay-full-implementation@d7f51441` (download signs and requests the catalog-bound manifest `versionId` and rejects a different response version; the real MinIO receipt records `ac201fe8-ba70-4bcb-a49c-a75a6657be55`; complete object-set deletion and GC authority remain open) |
| Delay exact checkpoint object-set deletion slice | `nereus/delay-full-implementation@3bfe030a` (the S3-compatible adapter preflights every manifest/file identity, captures provider versions, deletes files by exact `versionId` and the catalog-bound manifest last, and requires matching delete response version plus provider request ID; the locked MinIO receipt records `e223584d-2863-45a1-8471-9b378c0899c5`; `ALREADY_ABSENT` reconciliation, final prefix sweep and retire/Floor/Pin authority remain open) |
| Delay checkpoint delete retry-convergence slice | `nereus/delay-full-implementation@220fc98a` (404 presence probes now carry provider request/response evidence; partial response loss resumes from the remaining verified object set and a completely absent set returns `ALREADY_ABSENT`; final prefix sweep and retire/Floor/Pin authority remain open) |
| Delay checkpoint prefix sweep slice | `nereus/delay-full-implementation@c32a98f328400c71346b98188930a6efa80da7c9` (bounded one-page `ListObjectVersions` over the exact checkpoint prefix, secure/version-complete parsing, exact-version deletes and a final empty-prefix listing; the locked MinIO receipt records `f905db1e-1a7e-455c-bb32-5fa90bb7ed1f`; external REAPING/Floor/Pin/Owner authorization and lifecycle state transition remain open) |
| Delay checkpoint REAPING sweep coordination slice | `nereus/delay-full-implementation@b9fcd2aa846329ed13986b122d287375a441b2fd` (exact PENDING_UPLOAD -> REAPING CAS and successor reread precede the provider sweep; response loss retries the same REAPING identity/prefix; the locked MinIO receipt records `f5404da4-4944-4581-a75d-80dccdad92c3`; old-Owner/session authority, quiescence and external delete confirmation remain open) |
| Delay checkpoint REAPING quiescence proof slice | `nereus/delay-full-implementation@7b8b73885c5ec26dfc96c1b5b8a1a6ab8ec0d1d9` (immutable proof binds pending/reaping evidence, enforces the provider-lifetime plus trusted-clock-width horizon, and requires old-owner/provider closure horizons before the coordinator calls the provider; the locked MinIO receipt records `9c4dcab9-c03c-4860-81de-07e62302d30e`; external attestation issuers, Owner/session loss and delete confirmation remain open) |
| Delay checkpoint REAPING Owner proof slice | `nereus/delay-full-implementation@44cd3230709f5e87742cd94cd9a8b7bce314a184` (typed proof binds pending/Owner/Store/session lease identity, distinguishes explicit abandonment from a recorded lease no longer current, and requires trusted UTC after the upload deadline; the locked MinIO receipt records `ea89d80e-e63e-4980-b225-94b070d3c36b`; the issuer is local composition, not the production cross-record intent/Owner/catalog authority) |
| Delay Object Store provider ownership horizon slice | `nereus/delay-full-implementation@cc97c7654cb19f88c69045cd3c33a4d970a9fed3` (local tracker spans complete upload/download/delete/sweep operations, retains response-loss uncertainty through a bounded horizon, fences new operations and rechecks the credential-use lease before each HTTP send; the locked MinIO receipt records `1b904a10-2104-46eb-a6fd-0bd2afe24524`; remote provider execution/quiescence attestation remains external) |
| Delay checkpoint delete-confirmation composition slice | `nereus/delay-full-implementation@70e5f0da` (pure local `CheckpointDeleteConfirmationComposer` binds `CheckpointDeleteResult` to the exact durable checkpoint retire identity, requires confirmation earliest time at or after the observation latest time, and composes a signed `RESOURCE_DELETE_CONFIRMED`; provider-side deletion, retire/Floor/Pin/Owner authorization, Shard Log append and mutation apply remain external) |
| Delay delete-confirmation temporal evidence fence slice | `nereus/delay-full-implementation@a26c6816` (`ResourceDeleteConfirmedBody` and `ResourceDeleteConfirmedRecord` both require confirmation earliest time at or after the complete provider-observation interval, so manual/signed callers cannot bypass the composer; provider execution and lifecycle authorization remain external) |
| Delay source-ordered GC confirmation handoff slice | `nereus/delay-full-implementation@b225cef9` (typed `GcWorkClassExecutor.submitDeleteConfirmation` binds the exact retire record and reports `PERSISTED` only for a strictly later position in the same physical source; the executor does not append, apply, or authorize the lifecycle) |
...
| Kafka isolated K1/K2 implementation | `nereus/delay-guarded-producer@05849884ca81fad767fda058444d1e17c7f9cbf9` |
| Pulsar isolated P1 implementation | `nereus/delay-resource-guard@0a2536484cd3932801a98dc88ff112b2df88a1c7` (dedicated admin/ownership-checked Resource Controller guard endpoint; generic topic-properties mutation remains fail-closed) |
...
| `CapacityDimension` | PASS（1–66 连续、完整） |
| ADR file/index sequence | PASS（0001–0044） |
| `*` cross-document references | Prior frozen audit PASS（336 个 unique refs）；2026-08-13 delta 已登记 Gateway/Route wire types，Java-only implementation class names 不计入 Registry；完整生成器复跑仍是 release gate |
| RocksDB CF namespace | PASS（只出现七个 CF 的 registered tags） |
| `RETRY_JITTER` independent recomputation | PASS（`dd78e75…d339`，first64 `15958759676622330853`） |
...
barrier。兼容构造器仍明确不计入 source-assignment 证据。
...
证据为 `SloObservationOutboxStoreTest`。`SloObservationOutbox` 现在还按 closed
objective branch 校验 Final 的 unit 与 merge direction，错误语义不会进入 durable
outbox；`SloObservationOutboxTest.rejectsFinalUnitAndMergeDirectionThatDisagreeWithObjective`
覆盖该边界；同一 severity 的重复 Final 还按较新 `observationRevision` 选择
evidence，避免 revision 与 evidence 混配。`SloObservationOutboxTest.mergeUsesNewestEvidenceWhenOutcomeSeverityIsEqual`
覆盖该边界；两个重复 Final 若携带不同的 non-null due exclusion reason
则 fail closed，而不是静默保留旧 reason，`SloObservationOutboxTest.mergeRejectsConflictingDueExclusionReasons`
...
`SloSampleStart`，按 sample ID 确定性排序并折叠 byte-identical 重复，冲突 Start
...
typed fields：`commandApplied(...)` 使用 Registry `SourcePosition` canonical bytes、
...
The source-ordered `PUBLISH_ADMISSION` seam now accepts an immutable
...
Broker `deliverAt` 只用于 Broker visibility 语义； native wire 没有独立的
...
Typed `ActiveLaneState` now rejects a present field-22 READY key unless its
...
fence is covered by `ActiveLaneStateTest.readyKeyMustBeTheExactLaneVersionAndEligibilityProjection`
...
non-empty encoded key and a decodable `ReadyCertificate` whose key fields
...
`RecoveryPin` 现在在 value 构造和 decode 边界把显式 `ShardSubject` 与
`observedFloor.appliedSourcePosition` 的 Shard 绑定。一个带有 foreign-Shard
Recovery Floor 的 pin 不能先作为看似有效的中间值被序列化，之后才等 Catalog
authority 拒绝；`RecoveryPinTest.rejectsLineageGenerationAndDigestDrift` 覆盖了
...
absent, and it checks the persisted `CompatibleControlSnapshot` digest
...
`CompatibleControlSnapshot` is rejected before the upload coordinator can
...
sources call only typed `DelayShard.claimForPublish(...)` through
...
tests but did not satisfy the bounded-turn contract. The strict
...
public scheduler constructor requires explicit `OwnerIdentity`. Reflection
...
`OwnerIdentity`, validates the exact Shard and lease epoch, and the strict
...
`ProfileCatalogScheduleResolver` rejects nested Profile decorators and exposes
...
`ProfileCatalogScheduleResolverTest.decoratorCannotHideAnotherProfileCatalog`
...
choose a weaker authority than the durable Prepare. A Registry Prepare pins
the exact `PayloadProofTrustSetRef(version, semanticHash)` in its
`ScheduleBinding`; a later legacy Commit body is still checked against that
semantic and its source-ordered activation/issuance-close state. Only a legacy
Prepare without the binding can use the version-only compatibility verifier.
`DelayShardTest.registryPrepareCannotDowngradeTrustSetAuthorityWithLegacyCommitBody`
...
`EmbeddedDelayService` reloads the durable `ScheduleBinding` before adapter
...
state. The end-to-end regression applies a Prepare, closes and reopens the
...
well. `PrepareLargeSchedule` required field 15 carries the complete
`OBJECT_STORE ProfileRef`; catalog-backed apply resolves its immutable
...
Claim WriteBatch, `DelayShard` now reloads `ScheduleBinding`: ordinary
...
`ScheduleBinding` retains the exact §4.1 canonical tuple, and the shared
...
projections to agree. `CanonicalLaneTupleTest` covers Kafka and Pulsar plus
...
durable Prepare body from `ScheduleBinding`, resolves the exact pinned
...
`ScheduleBinding`, and unchanged reservation quota before a valid Prepare.
...
Profile. `ProfileCatalogScheduleResolver` now rejects those Destination
...
`ScheduleBinding` or reservation quota side effect. Code commit `8955510c`
...
`GatewayAdmissionRecord`. The record is keyed by authenticated tenant-scope
...
`nereus-delay-oxia-e2e-1786746636-41339` on host port `16651`, and the
...
the configured issuer signature; `NativePreparationEligibility` then binds
...
`prepareScheduleSubmission(..., SubmissionMode)` prevents a Direct SDK
...
for `VerifiedNativePreparationSnapshotCacheTest`, `ActivationBarrierTest`
...
Worker assignment to `RouteSnapshot.snapshotDigest` and adds
...
`37a17bef17202d5fd6e23282da5fd26d94865484`, Compose project
`nereus-delay-oxia-e2e-1786765353-47776`, port `16660`, and image
...
`nereus-delay-oxia-e2e-1786768622-85502`, port `16663`, and image
...
derived materialization from the accepted durable binding, current Message
...
`nereus-delay-oxia-e2e-1786769822-98671`, port `16664`, with
...
## 2026-08-15 automatic Claim materialization and Worker handoff audit

Delay commit `5dbd0874` makes the durable Claim projection derivable at the
production handoff boundary. `CanonicalLaneTuple.project` parses the exact
Registry tuple; `DelayShard.resolveClaimMaterialization` combines its
...
retains reservation/proof identity. Missing binding, timeline or commit
...
This closes durable Claim materialization and local handoff composition for
...
exact Claim and an externally-authorized `ChannelResourceIdentity`, derives
...
Commit `d413869b` moves the Claim handoff regression to a real binding and
...
apply, automatic Claim/Publish authority, multi-shard placement or the
release gates.
...
`ActivationBarrier.pulsar`, publishes the signed Route event/head through a
...
Delay `nereus/delay-full-implementation@a73faf3e836ada67931f709d46214dde7caf3ad0`;
...
`nereus/delay-full-implementation@ac72e43803806b9c309b62150c0aa54b43f8a3ea`,
Oxia `37a17bef17202d5fd6e232da5fd26d94865484`, Compose project
`nereus-delay-oxia-e2e-1786787138-90186`, and host port `16675`. The
...
Compose project `nereus-delay-oxia-e2e-1786789198-22565`, and port `16684`.
...
Delay commit `defce755` adds the next D6 local composition slice. A canonical
 Schedule/Prepare resolver tuple now creates a typed `ActiveLaneState`
...
inferred from arbitrary bytes. `ReadyCertificate` exposes the certificate's
...
`ActiveLaneState` with `OPEN` admission, `READY` runtime state and a
...
persisted Lane to be `OPEN`/`READY` with a decodable `ReadyCertificate`.
...
This is direct adapter evidence, not a full or Worker production PASS. At
...
load, crash cuts or release readiness.
...
load, crash/response-loss cuts, live Kafka/Pulsar publication or release
readiness.
...
Compose `nereus-delay-oxia-e2e-1786822655-96457`, and port `16675`. After
...
authority, production Worker transport or release readiness.
...
live Kafka/Pulsar publication or release readiness.
...
crash/response-loss resolution, load, live Kafka/Pulsar publication or
release readiness.
...
transparent reconnect/HA, multi-node placement and release readiness
remain open.
...
resolution, transparent reconnect/HA, load, multi-shard placement and
...
so `GatewayIdempotencyRecord.withAttempt` clears the prior uncertain
...
loss, LSO/retention-floor recovery, or release readiness.
...
recovery, multi-Broker failover, Attempt Journal completion recovery or a
release PASS.
...
complete D6 source-ACK/crash or release receipt.
...
multi-Broker failover, Attempt Journal completion recovery or a release
PASS.
...
recovery, multi-Broker failover, Attempt Journal recovery or a release PASS.
...
multi-Broker failover, coordinator recovery or a release PASS.
...
transaction, process/network chaos, or the release matrix.
...
quiescence, version-aware deletion, catalog transaction, chaos or release
readiness.
...
`RotateEquivalentSecretRequest` use exact version CAS with idempotent retries;
...
failures/errors in Compose project `nereus-delay-oxia-e2e-1786835835-39861`
on port `16693`.
...
deletion, chaos or release readiness.
...
rotation/quiescence, real S3/MinIO, deletion, chaos or release readiness.
...
S3/MinIO, provider consistency/deletion, chaos or release readiness.
...
consistency attestation, cross-record failover, chaos and release readiness
remain open.
...
`x-amz-version-id` remains byte-equal to the `CheckpointResource` identity;
...
provider consistency, credential rotation, chaos, failover or release
readiness.
...
failover and release readiness remain open.
...
Source Position and signs the canonical `RESOURCE_DELETE_CONFIRMED`
...
`ResourceDeleteConfirmedRecord`. A `RESOURCE_DELETE_CONFIRMED` body or
...
Floor projections; an active `RecoveryPin` is held in a separate canonical
...
and Catalog manifest together, while the active `RecoveryPin` is a sibling
...
session reconnection, production query routing, chaos evidence or release
readiness.
...
reconnection, production control-query routing, chaos evidence or release
readiness.
...
provider rotation/quiescence, chaos evidence or release readiness.
...
publication, provider deletion, source/evidence replay, chaos evidence or
release readiness.
...
provider/source evidence, Owner/session recovery, chaos evidence or release
...
source ordering, chaos evidence or release readiness.
...
automatic session recovery, source ordering, chaos evidence or release
readiness.
...
reconnect, multi-node failover, placement/source ownership, raw chaos or
release readiness.
...
recovery, provider evidence, source ordering, chaos or release readiness is
claimed.
...
multi-node failover, placement/source ownership, raw chaos or release
readiness.
...
placement/source ownership, raw chaos, failover or release readiness is
claimed.
...
failover, production ownership and release readiness are not claimed.
...
ownership, raw chaos, failover or release readiness.
...
crash/chaos, failover or release readiness.
...
Owner/Oxia, raw chaos, failover or release readiness.
...
raw chaos or release readiness.
...
failover, raw chaos or release readiness.
...
raw chaos, failover or release readiness.
...
delivery, failover, raw chaos or release readiness.
...
Gateway evidence-ordering gap. `GatewayIdempotencyRecord` verifies every
...
Broker failover, raw chaos or release readiness.
...
delivery, Broker failover, raw chaos or release readiness.
...
transport delivery, Broker failover, raw chaos or release readiness.
...
`GatewayOperationKind`, and rejects every other pairing. Native prepared
...
phase/digest union gap in `GatewayAuditEvent`: a `COMPLETED` event must
...
local lifecycle-shape gap in `GatewayIdempotencyRecord`. The durable
...
attempt checks. `GatewayIdempotencyRecord` now tracks whether a source-ordered
...
integration, Worker production wiring or release readiness.
...
Broker rollout/failover, source/ACK integration, Worker production wiring and
 release readiness remain open. Follow-up commit
...
Worker Commit, not a full release PASS. The source topic has one partition;
...
failover, raw chaos and release readiness remain open.
...
This is a bounded positive production-authority E2E, not release approval.
...
the release gate: the run still has one ZooKeeper and one BookKeeper, does
...
multi-shard placement, checkpoint publication, chaos or release approval.
...
checkpoint publication, chaos or release approval.
...
## 2026-08-16 Pulsar Worker destination response-loss with real Oxia audit

Delay commit
`b647176ed92491fd96514eed2b87098454078a79` corrects the focused response-loss
runner so the real-Oxia option reaches the Worker process. The source-bound
command was:
...
and Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`. The Pulsar Compose
project was `nereus-delay-pulsar-e2e-1786885612-9737` on `19787/19788`; the
Oxia project was `nereus-delay-pulsar-oxia-e2e-1786885612-9737` on `16657`.
...
This is bounded positive evidence for real-Oxia Worker destination response
loss and typed Pulsar evidence resolution. The normal source `ENQUEUED`
admission path ran; this receipt does not claim recovered `UNKNOWN` Publish
Admission execution, raw socket/process/Broker cuts, combined Gateway
multi-Broker failover, checkpoint publication, multi-shard placement or
...
confirmation mutation, raw chaos, multi-shard runtime or the release gate.
...
production multi-shard ownership, the complete chaos matrix or the release
...
multi-shard runtime or release evidence. Exact cleanup found no named
...
proof, production multi-shard ownership or release PASS. Exact cleanup
...
multi-shard ownership or release PASS. Exact cleanup found no resources or
temporary images for the two named projects.

## Final gate

设计审计通过不代表实现发布通过。实现只有在上述 artifact matrix 和主设计 §23.5 十项 release gate 全部完成后才可宣称 release-ready；缺少数值、binary、benchmark 或 chaos evidence 的状态是“实现证据未完成”，不是“设计可自行解释”。

## 2026-08-16 Kafka raw TCP endpoint-cut audit receipt

The source-bound implementation is Delay `79d4617c`. Its focused
`NEREUS_DELAY_KAFKA_BROKER_TCP_CUT_ONLY=1` branch composes a real K1
three-Broker KRaft cluster, real Oxia Worker authority and a local raw TCP
proxy. The proxy forwards Broker-1 before the cut, rejects one post-cut
connection, then forwards later connections from the same public endpoint to
Broker-2. This is an explicit endpoint-handoff harness, not a production
Kafka failover mechanism.

The live run used Kafka
`nereus/delay-guarded-producer@05849884ca81fad767fda058444d1e17c7f9cbf9`,
client SHA-256
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image `sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`,
Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`, and projects
`nereus-delay-kafka-e2e-1786892446-98685` /
`nereus-delay-kafka-oxia-e2e-1786892446-98685` on ports
`19277,19278,19279/16769`. The source partition and the actual
`groupId.hashCode() % __consumer_offsets.partitions` coordinator partition
`15` both converged to `leader=2, replicas=[2, 3, 1]` while Broker-1 stayed
alive.
...
This provides positive evidence for the raw endpoint-cut and explicit
endpoint-handoff branch. It does not satisfy the design's automatic
controller/coordinator failover, Broker crash, network partition, multi-shard,
full chaos or release gates. Exact cleanup found no containers, networks,
...
crash/chaos, broader network/controller fault cases or the release gate.
...
matrix, and it does not close multi-shard placement or release readiness.
...
Compose project `nereus-delay-oxia-e2e-1786894971-29820` and the selected
...
ACK, scheduler fairness, raw chaos and release readiness remain open. The
...
processes, the full raw crash/chaos matrix or release approval. Exact
...
`nereus/delay-resource-guard@0a2536484cd3932801a98dc88ff112b2df88a1c7`, and
...
multi-Broker failover, multiple Worker processes, the raw crash/chaos matrix or
 release approval. Exact post-run Docker checks found no containers,
...
chaos completeness or release approval. Exact project and image checks
...
The implementation keeps `PulsarSourceReactivation`'s equal-generation
...
bounded two-Broker/one-physical-source-partition audit PASS, not a release
...
Broker-process local; equality is still rejected by `PulsarSourceReactivation`
...
This is not a release PASS and does not close arbitrary packet/proxy/socket
...
This is not a release approval. It uses one ZooKeeper and one BookKeeper
...
destination egress under the cut, multi-shard chaos or release approval.
...
multi-shard fault coverage or release approval. Exact project and image
...
placement or release approval. Exact project resources and the P1 image
...
placement, checkpoint publication, full chaos or release approval.
...
crash recovery, multi-shard placement, checkpoint publication or release
...
not a release PASS: the source is one physical partition, the local
...
credential authority, checkpoint REAPING/GC, full chaos or release
...
This is not release approval: cross-record disaster authority, external
...
`nereus-delay-oxia-e2e-1786899760-96148` used Delay
...
`nereus-delay-oxia-e2e-1786899721-95680` then passed the previously failing
...
multi-node Oxia failover, provider rotation/quiescence, full chaos or
...
network-partition failover evidence, including the post-dedupe current-source
Kafka and Pulsar revalidation, but not the complete all-language,
...
not a reason to reinterpret the existing positive slices as approval.
...
resolution, external credential/provider authority or release readiness.
...
multi-process renewal ownership, provider upload after renewal or release
readiness; the locked MinIO base remained and no global Docker prune was used.

## 2026-08-17 Current-source Pulsar Worker Publish Admission response-loss audit
...
P1 image to
`sha256:819a2a34b91d34468ac6caa048ec5cbf959fb9ecb40dbfd649a9fabf067318de`,
and Oxia to `37a17bef17202d5fd6e23282da5fd26d94865484`. The isolated projects
were `nereus-delay-pulsar-e2e-1786901196-18866` and
`nereus-delay-pulsar-oxia-e2e-1786901196-18866` on Pulsar `29400/29401` and
Oxia `29410`.

Audit result: PASS for the bounded current-source real Pulsar/real Oxia
Publish Admission response-loss slice. The runner discarded only the local
append response after the real Shard Log mutation had been persisted. Exact
source replay recovered that same mutation and the Worker resumed the
`PUBLISHING` admission without creating a second mutation:
...
This is not raw socket loss or a process/Broker crash; it does not establish
multi-Broker failover, multi-shard placement, checkpoint REAPING, the full
chaos matrix or release readiness. Exact postchecks found no project
...
failover, multi-shard placement, REAPING, full chaos or release evidence.
...
REAPING, full chaos or release readiness. Exact postchecks found no project
...
`ProtocolActivationState` is shard-bound and persisted at `meta/FIXED` key
...
path preserves the managed baseline, rejects an unmarked well-framed tuple
...
packaging or a certified activation artifact. Release readiness remains `NOT_READY` until
...
chaos or release readiness. Exact postchecks found no project resources,
...
the complete Kafka Fetch/LSO/retention recovery matrix, or release
...
egress, arbitrary multi-shard chaos or release evidence. Exact postchecks
...
`ProtocolVersionActivatePayload` and a typed field-1 accessor on
...
multi-shard chaos completeness or release evidence. Exact postchecks found
...
the complete chaos matrix or release evidence. Exact postchecks found no
...
This is not disk ENOSPC, raw socket/coordinator chaos, multi-shard placement or
 release evidence. Exact postchecks found no project resources, dangling
...
`InitialRouteControlActivatePayload` now has a typed canonical Java codec,
...
therefore remains `PARTIAL`, and the release result remains `NOT READY`.
...
Overall remains NOT READY; no focused Worker receipt is promoted to a
release PASS.
...
Pulsar P1
`nereus/delay-resource-guard@0a2536484cd3932801a98dc88ff112b2df88a1c7`,
and Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`. Kafka client, K1/P1
distribution and K1/P1 image digests were respectively
...
The six bounded current-source receipts were:

| Cell | Runtime scope | Outcome |
|---|---|---|
| Kafka Broker process crash | Kafka `31200/31201/31202`, Oxia `31210`, Compose projects `nereus-delay-kafka-e2e-1786915435-2995` / `nereus-delay-kafka-oxia-e2e-1786915435-2995` | SIGKILL recovery through survivors, typed transactional receipt, exact payload readback and Broker-1 rejoin. |
| Kafka Worker ACK process crash | Kafka `31220/31221/31222`, Oxia `31230`, Compose projects `nereus-delay-kafka-e2e-1786915514-3894` / `nereus-delay-kafka-oxia-e2e-1786915514-3894` | Durable Store `WriteBatch` before SIGKILL/`commitSync`, fresh-JVM replay, dedupe, ACK and checkpoint. |
| Kafka Broker raw TCP cut | Kafka `31240/31241/31242`, Oxia `31250`, Compose projects `nereus-delay-kafka-e2e-1786915550-4401` / `nereus-delay-kafka-oxia-e2e-1786915550-4401` | Deterministic proxy rejection plus Broker-2 handoff, source apply/ACK/checkpoint. |
| Pulsar Worker process crash | broker/web `31260/31261`, Oxia `31270`, projects `nereus-delay-pulsar-e2e-1786915591-4999` / `nereus-delay-pulsar-oxia-e2e-1786915591-4999` | Fresh Worker reacquired real Oxia authority, replayed and ACKed, and published the final checkpoint. |
| Pulsar Publish Admission response loss | broker/web `31280/31281`, Oxia `31290`, projects `nereus-delay-pulsar-e2e-1786915626-5474` / `nereus-delay-pulsar-oxia-e2e-1786915626-5474` | Discarded append response recovered from exact source mutation; typed send ACK and payload readback passed. |
| Checkpoint REAPING | Oxia `31300`, MinIO `31301`, project `nereus-delay-oxia-minio-checkpoint-e2e-1786915650-5809` | Real Oxia authority plus real MinIO immutable-object checkpoint publication and REAPING passed. |

The Kafka TCP receipt required two narrowly scoped harness commits:
`cc193958` made the post-cut rejection observation deterministic and
`e2b6b2da` made the pre-cut relay observation deterministic. This addresses
bootstrap selection nondeterminism in the receipt boundary; it does not turn
the bounded six-cell set into a full chaos or release gate.

The final runner cleanup verified no related containers, networks, volumes or
temporary Kafka/Pulsar/per-run Oxia images. It retained only the locked Oxia
base and MinIO base image (MinIO digest
`sha256:14cea493d9a34af32f524e538b8346cf79f3321eff8e708c1e2960462bd8936e`,
local ID
`sha256:8f08aee614800a237906bd48114d733e5ac5bfac4ccdf731f141b0e880d7a253`)
among the related images; no global Docker prune was used. Current full
`check` and cross-repository contract audit also passed.

Audit conclusion: Gates 2, 3 and 10 are strengthened but remain `PARTIAL`;
the complete §23.3 matrix, GC/ENOSPC/fsync/SST and authority-drift cuts,
benchmark/capacity artifacts, soak, and authenticated activation-state /
cutover evidence remain open. Gates 5, 6, 7 and 9 remain `OPEN`, Gate 8
remains `PARTIAL`, and remains `NOT READY`.

## 2026-08-17 Current-source expanded bounded response-loss matrix: ten-cell PASS

The current-source bounded matrix runner was expanded from six to ten focused
cells and completed with `matrix_status=0`; all ten `.result` files contain
...
The source lock is Delay
`e53e536adefeac272301e25bceffd1d5714e7ba7`, with Kafka K1
`nereus/delay-guarded-producer@05849884ca81fad767fda058444d1e17c7f9cbf9`,
Pulsar P1
`nereus/delay-resource-guard@0a2536484cd3932801a98dc88ff112b2df88a1c7`,
and Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`. The exercised K1 client,
K1 image, P1 distribution and P1 image were locked respectively to
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`,
`sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`,
`373d8ac01bb82e6625a18690ed62a95719719acebf05145f8c2eefcfc23cd3f3`, and
`sha256:819a2a34b91d34468ac6caa048ec5cbf959fb9ecb40dbfd649a9fabf067318de`.

| Cell | Runtime scope | Current-source audit receipt |
|---|---|---|
| Kafka Broker process crash | Kafka `31200/31201/31202`, Oxia `31210`; `nereus-delay-kafka-e2e-1786916050-12560` / `nereus-delay-kafka-oxia-e2e-1786916050-12560` | Broker-1 SIGKILL recovery through Kafka survivors, real Oxia authority, typed transactional receipt, exact payload readback and Broker-1 rejoin. |
| Kafka Worker ACK process crash | Kafka `31220/31221/31222`, Oxia `31230`; `nereus-delay-kafka-e2e-1786916144-13584` / `nereus-delay-kafka-oxia-e2e-1786916144-13584` | Durable Store `WriteBatch` before SIGKILL and before `commitSync`; fresh-JVM replay, dedupe, ACK and final checkpoint. |
| Kafka Broker raw TCP cut | Kafka `31240/31241/31242`, Oxia `31250`; `nereus-delay-kafka-e2e-1786916180-14098` / `nereus-delay-kafka-oxia-e2e-1786916180-14098` | Explicit Broker-2 source/coordinator placement, deterministic raw proxy rejection of Broker-1, then fresh Worker source apply/ACK/checkpoint. |
| Pulsar Worker process crash | broker/web `31260/31261`, Oxia `31270`; `nereus-delay-pulsar-e2e-1786916218-14636` / `nereus-delay-pulsar-oxia-e2e-1786916218-14636` | Fresh Worker reopened the exact Store, reacquired real Oxia authority, replayed and ACKed the unACKed source record, and published the final checkpoint. |
| Pulsar Publish Admission response loss | broker/web `31280/31281`, Oxia `31290`; `nereus-delay-pulsar-e2e-1786916253-15121` / `nereus-delay-pulsar-oxia-e2e-1786916253-15121` | Discarded Shard Log append response recovered from the exact source mutation; `PUBLISHING` replay, typed `PULSAR_SEND_ACK` and payload readback passed. |
| Checkpoint REAPING | Oxia `31300`, MinIO `31301`; `nereus-delay-oxia-minio-checkpoint-e2e-1786916277-15440` | Real Oxia Intent/Catalog/Owner and MinIO immutable-object checkpoint publication plus REAPING passed. |
| Kafka Fetch response loss | Kafka `31320/31321/31322`; `nereus-delay-kafka-e2e-1786916314-15851` | `responseDiscardedAfterFetch=true`, replay `0`, second record `1`, Fetch LSO `2`, committed after replay `2`; read-committed Fetch v13 recovery passed. |
| Kafka retention floor | Kafka `31340/31341/31342`; `nereus-delay-kafka-e2e-1786916332-16100` | `oldOffset=0`, retention floor `20`, end `21`, stale offset rejected, floor fetch `20`, Fetch LSO `21`; guarded floor read passed. |
| Pulsar destination SEND response loss | broker/web `31360/31361`, Oxia `31370`; `nereus-delay-pulsar-e2e-1786916374-16590` / `nereus-delay-pulsar-oxia-e2e-1786916374-16590` | Real SEND persisted the exact payload after local response loss; typed `PULSAR_SEND_ACK` authority resolved `PUBLISHED` and guarded readback matched. |
| Pulsar source ACK response loss | broker/web `31380/31381`, Oxia `31390`; `nereus-delay-pulsar-e2e-1786916395-16893` / `nereus-delay-pulsar-oxia-e2e-1786916395-16893` | Real ACK acceptance preceded discarded response; the same source record was ACKed on the next bounded turn and final checkpoint completed. |

The added Kafka Fetch/retention and Pulsar destination/source-ACK cells are
fresh current-source evidence for response loss, LSO/floor handling and typed
broker evidence. They do not satisfy the complete §23.3 failure-cut contract:
GC pause, half-open behavior, ENOSPC, fsync error, SST corruption, leader or
controller failover, Oxia session expiry, Object Store 5xx/timeout/config
drift, target-isolation gates, and fresh-process evidence for every required
row remain unclosed.

The expanded run's exact cleanup removed its containers, networks, volumes and
per-run Kafka/Pulsar/Oxia image references. Post-run Docker inspection found
no related runtime resources; only the locked Oxia base
`nereus/oxia-o1:37a17bef1720` (local ID
`sha256:5aa715e4f19091931743e5af489af5f8d6ee15efcce6430a908c6f65cc6d6516`)
and locked MinIO base at digest
`sha256:14cea493d9a34af32f524e538b8346cf79f3321eff8e708c1e2960462bd8936e`
(local ID
`sha256:8f08aee614800a237906bd48114d733e5ac5bfac4ccdf731f141b0e880d7a253`)
remain among related images. No global prune or unrelated image deletion was
performed.

The current full Delay `check` and cross-repository contract audit both
passed at this source lock. This is a bounded ten-cell audit delta, not a
release certification: Gates 2, 3 and 10 remain `PARTIAL`, Gates 5, 6, 7 and
9 remain `OPEN`, Gate 8 remains `PARTIAL`, and remains `NOT READY` pending
the full chaos matrix, benchmark/capacity/SLO artifacts, certified soak,
authenticated activation-state/cutover proof and rollout runbooks.

## 2026-08-17 Current-source expanded broker-failover matrix: twelve-cell PASS

The bounded matrix added two existing current-source cuts after the ten-cell
response-loss run: Kafka Broker network partition and Pulsar two-Broker Worker
process crash/failover. The first combined attempt exposed a harness-only
resume mutation: survivor leader recovery had already converged, but Worker
`resume` attempted to create an existing topic through the disconnected
controller. `ea88bc48` changed resume to require existing topic metadata and
never create during recovery; normal `run`/`prepare` behavior is unchanged.
The final run at Delay
`ea88bc488dd7b209c33d1ab17cde613584430fca` completed with
`matrix_status=0`, all twelve result files equal to `0`, and artifact
directory
`/var/folders/vk/l_r0z80j1dj93fsrjx3zqv4r0000gn/T/nereus-delay-broker-chaos-final.XXXXXX.3emvLopbKS`.

Kafka K1, Pulsar P1 and Oxia remained locked to
`05849884ca81fad767fda058444d1e17c7f9cbf9`,
`0a2536484cd3932801a98dc88ff112b2df88a1c7`, and
`37a17bef17202d5fd6e23282da5fd26d94865484`; the K1 client, K1 image, P1
distribution, P1 image and MinIO digest were
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`,
`sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`,
`373d8ac01bb82e6625a18690ed62a95719719acebf05145f8c2eefcfc23cd3f3`,
`sha256:819a2a34b91d34468ac6caa048ec5cbf959fb9ecb40dbfd649a9fabf067318de`,
and `sha256:14cea493d9a34af32f524e538b8346cf79f3321eff8e708c1e2960462bd8936e`.

| Cell | Runtime scope | Current-source audit receipt |
|---|---|---|
| Kafka Broker process crash | Kafka `31200/31201/31202`, Oxia `31210`; `nereus-delay-kafka-e2e-1786917649-32636` / `nereus-delay-kafka-oxia-e2e-1786917649-32636` | Broker-1 SIGKILL recovery through Kafka survivors, real Oxia authority, typed transactional receipt, exact payload readback and Broker-1 rejoin. |
| Kafka Worker ACK process crash | Kafka `31220/31221/31222`, Oxia `31230`; `nereus-delay-kafka-e2e-1786917689-33194` / `nereus-delay-kafka-oxia-e2e-1786917689-33194` | Durable Store `WriteBatch` before SIGKILL and before `commitSync`; fresh-JVM replay, dedupe, ACK and final checkpoint. |
| Kafka Broker raw TCP cut | Kafka `31240/31241/31242`, Oxia `31250`; `nereus-delay-kafka-e2e-1786917725-33694` / `nereus-delay-kafka-oxia-e2e-1786917725-33694` | Explicit Broker-2 source/coordinator placement, deterministic raw proxy rejection of Broker-1, then fresh Worker source apply/ACK/checkpoint. |
| Kafka Broker network partition | Kafka `31400/31401/31402`, Oxia `31410`; `nereus-delay-kafka-e2e-1786917765-34248` / `nereus-delay-kafka-oxia-e2e-1786917765-34248` | Broker-1 remained alive but disconnected; survivor leaders converged to source `2`, destination `2`, receipt `3`; Worker apply/ACK/checkpoint completed and Broker-1 reconnected. |
| Pulsar Worker process crash | broker/web `31260/31261`, Oxia `31270`; `nereus-delay-pulsar-e2e-1786917835-35093` / `nereus-delay-pulsar-oxia-e2e-1786917835-35093` | Fresh Worker reopened the exact Store, reacquired real Oxia authority, replayed and ACKed the unACKed source record, and published the final checkpoint. |
| Pulsar two-Broker process crash | broker-1/web-1 `31420/31421`, broker-2/web-2 `31422/31423`, Oxia `31430`; `nereus-delay-pulsar-multi-e2e-1786917871-35566` / `nereus-delay-pulsar-multi-oxia-e2e-1786917871-35566` | Broker-1 SIGKILL recovery through Broker-2 with real Oxia authority and Broker-1 rejoin. |
| Pulsar Publish Admission response loss | broker/web `31280/31281`, Oxia `31290`; `nereus-delay-pulsar-e2e-1786917969-36638` / `nereus-delay-pulsar-oxia-e2e-1786917969-36638` | Discarded Shard Log append response recovered from the exact source mutation; `PUBLISHING` replay, typed `PULSAR_SEND_ACK` and payload readback passed. |
| Checkpoint REAPING | Oxia `31300`, MinIO `31301`; `nereus-delay-oxia-minio-checkpoint-e2e-1786917996-36985` | Real Oxia Intent/Catalog/Owner and MinIO immutable-object checkpoint publication plus REAPING passed. |
| Kafka Fetch response loss | Kafka `31320/31321/31322`; `nereus-delay-kafka-e2e-1786918014-37242` | `responseDiscardedAfterFetch=true`, replay `0`, second record `1`, Fetch LSO `2`, committed after replay `2`; read-committed Fetch v13 recovery passed. |
| Kafka retention floor | Kafka `31340/31341/31342`; `nereus-delay-kafka-e2e-1786918031-37471` | `oldOffset=0`, retention floor `20`, end `21`, stale offset rejected, floor fetch `20`, Fetch LSO `21`; guarded floor read passed. |
| Pulsar destination SEND response loss | broker/web `31360/31361`, Oxia `31370`; `nereus-delay-pulsar-e2e-1786918072-37958` / `nereus-delay-pulsar-oxia-e2e-1786918072-37958` | Real SEND persisted the exact payload after local response loss; typed `PULSAR_SEND_ACK` resolved `PUBLISHED` and guarded readback matched. |
| Pulsar source ACK response loss | broker/web `31380/31381`, Oxia `31390`; `nereus-delay-pulsar-e2e-1786918093-38247` / `nereus-delay-pulsar-oxia-e2e-1786918093-38247` | Real ACK acceptance preceded discarded response; same source record ACKed on the next bounded turn and final checkpoint completed. |

The final full Delay `check` and cross-repository contract audit both passed
at this lock. Exact cleanup left no related containers, networks, volumes or
per-run Kafka/Pulsar/Oxia image references. Only the locked Oxia base
`nereus/oxia-o1:37a17bef1720` (local ID
`sha256:5aa715e4f19091931743e5af489af5f8d6ee15efcce6430a908c6f65cc6d6516`)
...
This is a twelve-cell bounded audit delta, not release certification. The
remaining §23.3 gaps are long GC pause, half-open behavior, ENOSPC, fsync
error, SST corruption, controller/coordinator/storage cuts, Oxia session
...
and 9 remain `OPEN`, Gate 8 remains `PARTIAL`, and remains `NOT READY`.
...
protocol activation/cutover, benchmark/capacity/soak certification or
...
`nereus/delay-guarded-producer@05849884ca81fad767fda058444d1e17c7f9`, its
client SHA-256
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, and
image `sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`;
Pulsar P1
...
source-ordered credential refresh or release approval. Exact cleanup found
...
but are not a full §23.3 or release PASS. They do not cover controller,
...
5, 6, 7 and 9 remain `OPEN`; Gate 8 remains `PARTIAL`; release readiness remains `NOT READY`.
...
Gate 8 remains `PARTIAL`; release readiness remains `NOT READY`.
...
Gates 2/3/10 promotion, the remaining §23.3 cuts and release readiness
remain open: Gates 2, 3 and 10 are `PARTIAL`; Gates 5, 6, 7 and 9 are `OPEN`;
Gate 8 is `PARTIAL`; is `NOT READY`.
...
the remaining §23.3 matrix, multi-shard production, benchmark/soak and
release gate remain open. Gates 2, 3 and 10 stay `PARTIAL`; Gates 5, 6, 7 and
9 stay `OPEN`; Gate 8 stays `PARTIAL`; stays `NOT READY`.
...
§23.3 matrix, multi-shard production, benchmark/soak and release gate
remain open. Gates 2, 3 and 10 stay `PARTIAL`; Gates 5, 6, 7 and 9 stay `OPEN`;
Gate 8 stays `PARTIAL`; stays `NOT READY`.
...
and 9 remain `OPEN`; Gate 8 remains `PARTIAL`; release readiness remains `NOT READY`.
...
9 stay `OPEN`, Gate 8 stays `PARTIAL`, and stays `NOT READY`.
...
placement, target isolation, benchmark/soak or release certification.
Gates 2, 3 and 10 stay `PARTIAL`; Gates 5, 6, 7 and 9 stay `OPEN`; Gate 8
stays `PARTIAL`; stays `NOT READY`.
...
marker by constructing the existing `CompatibleControlSnapshot` for its
...
real Worker rollout. Gate 8 remains `PARTIAL`; release readiness remains `NOT READY`. The
...
matrix, benchmark/soak or release readiness. Exact cleanup left no project
...
complete §23.3 matrix, benchmark/soak or release readiness. Exact cleanup
...
failover/fault coverage, catalog placement/churn, benchmark/soak or release
readiness. Gates 5, 6 and 7 remain `OPEN`; release readiness remains `NOT READY`. Exact
...
The fail-closed release-candidate artifact is
...
gate machinery; release certification remains OPEN. Certified benchmark,
...
is `/tmp/nereus-delay-release-gate-20260817-r5/release-candidate-gate.json`;
...
it cannot satisfy `PASS_CERTIFIED` or release readiness. The exact Compose
...
`PulsarPhysicalPartitionIdentity` resources and two independently proved
...
evidence, but not release approval. The one-cycle harness profile does not
...
certification remains open. This receipt therefore cannot promote the gate
...
capacity certification or release readiness. The gate requires the strict
...
It is schema-valid `nereus-delay-bounded-chaos-matrix` evidence with
...
the gate remains fail-closed at `release_status=NOT_READY` pending the
...
It is `schema=nereus-delay-bounded-chaos-matrix` with
...
release certification: `release_certification=OPEN` and the gate remains
...
`nereus-delay-rc1-bounded-capacity-r1`: three fixed smoke/burst/sustained
...
`nereus-delay-rc1-production-chain-soak-r1`, covering four strictly serial
...
Regenerate `/private/tmp/nereus-delay-production-chain-soak-20260821-r2/`
...
14-cell chaos wrapper, certified stability, or release gate; the old
...
current-source wrapper are rerun, the certified chaos artifact and release
...
`63b72ee9944995a88b0cfe4505ede2051e4392f`, and remains fail-closed until
...
capacity, soak, activation/cutover, operations and gate still require
...
matrix is generated, and remains fail-closed pending the independent
...
activation/cutover, operations or the release gate.
...
r19 is a named 14-cell chaos PASS_CERTIFIED, not the complete current system. r20 is
...
For the complete candidate, source freezing and evidence documentation are
...
The external nereus-delay-evidence-manifest binds the candidate lock,
...
e2e/verify-evidence-manifest.sh compares the candidate-to-overlay diff
...
The release command itself is also full-scope: `e2e/run-release-gate.sh`
delegates to `e2e/run-full-release-gate.sh` and accepts only the ten exact
`nereus-delay-full-gate-input` artifacts with `scope=full`,
`complete=true`, `PASS_CERTIFIED`, exact candidate locks, and empty
exclusions/boundaries. This is a governance boundary; the currently missing
full- inputs remain release blockers until independently produced.
...
the nine other full- inputs are still required before release promotion.

## Current-source no-early evidence

The exact-source no-early receipt is
`/private/tmp/nereus-delay-no-early-20260821-a.bOg67w/no-early.json` with
...
the no-early gate; all other missing full- artifacts remain blockers.
...
the remaining full- gates. They cannot be supplied as a complete
...
```text
/private/tmp/nereus-delay-cross-20260821-r29/K_TO_P.log
sha256=02db290caafda6d4cc814f2e2397726c50dcd91a2a3f1e0d9f2b27cfcdd76f40
/private/tmp/nereus-delay-cross-20260821-r29/P_TO_K.log
...
does not promote the full `real-service` gate or release: activation and
...
artifact is `/private/tmp/nereus-delay-full-chaos-20260821-r44/full-chaos-matrix.json`:
...
release blockers. No documentation append may promote this to release PASS.

## 2026-08-21 full contract evidence boundary

The new \`e2e/run-full-contract-gate.sh\` runner is the implementation seam
...
\`nereus-delay-full-gate-input\` before the release validator can inspect
it.
...
\`e2e/run-full-patch-distribution-gate.sh\` binds the K1/P1 source locks,
...
## 2026-08-21 full capacity-envelope audit

`e2e/run-full-capacity-envelope-gate.sh` was exercised against base Delay
`9ab82d11c0b1b8bd60547d94ea695403d2c73b1c`, K1
`05849884ca81fad767fda058444d1e17c7f9cbf9`, P1
`0a2536484cd3932801a98dc88ff112b2df88a1c7` and Oxia
...
This remains functional production-chain evidence only. The full capacity
...
Broker throughput, Lane fairness, resource bounds, SLO envelope or release
PASS.
...
## 2026-08-22 current full- evidence boundary
...
## 2026-08-22 current full- audit result
...
## 2026-08-22 latest exact-source full- audit — f4b7e005
...
full- release PASS claim for this source lock.
...
`e2e/run-physical-capacity-matrix.sh`, including the §23.4 cardinality,
...
`05849884ca81fad767fda058444d1e17c7f9cbf9`, Pulsar
`0a2536484cd3932801a98dc88ff112b2df88a1c7` and Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`. The candidate receipt at
`/private/tmp/nereus-delay-release-candidate-6f9ab51c/release-candidate-gate.json`
is `release_status=PASS`, with source, cross-repository, Gradle `check` and all
ten full- gates passing.
...
The physical supporting receipts are the benchmark observation
`/private/tmp/nereus-delay-benchmark-observation-6f9ab51c-r2/capacity-observation.json`
(`c836707323db0298cf15b4121b5f0614a140875cd18f992c359d4dca5d5ca6e3`), capacity
observation
`/private/tmp/nereus-delay-capacity-observation-6f9ab51c/capacity-observation.json`
(`c86ff0cbca4101e286f94a81971579af8ea203566fd62577422cc7b41ef38b32`) and the
real matrix
...
The post-overlay final receipt is
`/private/tmp/nereus-delay-release-final-6f9ab51c/release-candidate-gate.json`.
This evidence certifies the locked feature sources and does not claim merge,
deployment or promotion into a target `main`.

Cleanup used exact generated-resource removal and no global Docker prune. The
pinned Oxia, MinIO and exact benchmark `eclipse-temurin@sha256:57865c22b954cf920cb05a610af81d577e89783282514ba071e99c7357f6c769`
images remain; generated Nereus resources do not. Eighty-eight unreferenced
