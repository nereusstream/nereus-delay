...
the locked Kafka K1 and Pulsar P1 client artifacts, including a real three-broker K1
smoke; these are source-bound integration evidence, not a claim that the K2/D3,
source-consumer or full Worker production gates are closed. The isolated Kafka and
...
| local monolith packages exist | transport registry, ownership permits, guarded Kafka/Pulsar bridges, strict production configuration seams and opt-in source-locked K1/P1 bindings | `delay-transport-spi` / `delay-client-kafka` / `delay-client-pulsar` extraction, K2/D3 production cuts and full Worker wiring remain pending |
...
| Kafka generic guarded Producer patch | Implemented in isolated upstream worktree plus opt-in Delay K1 binding (K2 open) | Kafka branch `nereus/delay-guarded-producer-v1@95d48e89e7e8a4e6d8718e44d424ffef8f17829f` from locked `trunk@c300006a7705c240642db6950b5a95fec982bfc5`; focused client/mock, real KRaft delete/recreate/leader-failover, Delay source-set compile and three-broker Docker produce/resource-incarnation E2E pass. Client SHA-256 and broker image ID are recorded above; K2 target-plus-receipt transaction, source integration and release gates remain open |
...
| Pinned Kafka/Pulsar command ingress outcome mapping | Implemented (transport SPI plus source-locked K1/P1 bindings; full source assignment pending) | `PinnedKafkaCommandIngress`, `PinnedPulsarCommandIngress`, `KafkaClientArtifactProduceTransport`, `PulsarClientArtifactSendTransport`, `PulsarClientArtifactProducerFactory`, `WireCommandIngressAdapter`, `WireIngressOutcomeSupport`, `KafkaIngressResource`, `PulsarIngressResource`, `KafkaProduceRequest`, `PulsarSendRequest`, `KafkaProduceResult`, `PulsarSendResult`, `AdapterIngressTest`, `AdapterRequestIdentityTest`; Kafka topic UUID/canonical topic and Pulsar resource token/physical creation identity are carried at the request boundary, real bindings validate exact guarded success evidence, and Kafka Docker E2E proves delete/recreate and broker failover. Authenticated production rejection classifiers, K2/D3 transaction/source assignment and Worker ACK integration remain release blockers |
