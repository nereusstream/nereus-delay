...
| Registry-shaped `ScheduleIntentV1` | Implemented (canonical value/body codec; command runtime adapter pending) | `ScheduleIntentV1`, `ScheduleCommandBodyV1`, `CommandBodies.scheduleV1/decodeScheduleV1`, `RetryPolicyRefV1`, `AdapterMetadataV1`, `KafkaMetadataV1`, `CommittedPayloadDescriptorV1`, `DeliveryMode.fromWire`, `ScheduleIntentV1Test`, `ClientCommandBodyV1Test`; exact field order/oneof/presence, common body fields 1–3, Prepare-without-payload form, Profile/Object Store kind checks, Kafka/Pulsar metadata UTF-8/NFC/order and reserved-prefix rejection, quota version and canonical-byte rejection are covered; migration of `PreparedCommand`/`DelayShard` from legacy `ScheduleIntent`, full historical retry-policy binding and production profile/adapter authority remain pending |
