...
| NDR1 receipt frame | Implemented (queued/applied payload subset) | `ReceiptFrame`, `ReceiptKind`, `CommandQueuedReceiptV1`, `CommandAppliedReceiptV1`; registry zero-payload vector, canonical PreparedCommandRef/ProtocolTuple, Kafka/Pulsar Source Position and SafeBrokerAck agreement, queued-to-applied digest/source fencing, apply-status/message-field consistency, query boundary/capability/physical-attempt/digest checks, and flags/length/kind/CRC/Base64url rejection tests; native/control/reservation payload schemas and real Broker response adapters remain pending |
