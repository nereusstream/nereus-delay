...
Control Operation request support values now close the Registry's operation
kind enum, acknowledgement kind/set, close/uncertain branches and quota
transfer-plan reference. These are reusable canonical value codecs; the full
request oneof's target-presence matrix and authenticated operation authority
are still pending.
...
| Control Operation support values | Implemented (canonical dependencies; request union pending) | `ControlOperationKindV1`, `AcknowledgementKindV1`, `AcknowledgementV1`, `AcknowledgementSetV1`, `ClosePolicyV1`, `UncertainResolutionKindV1`, `QuotaTransferPlanRefV1`, `ControlRequestSupportCodecTest`; exact wire enums, empty/sorted acknowledgement sets, fixed hash/scope fields, quota-plan identity/version/hash and canonical rejection vectors are covered; branch request codecs, target-presence matrix, actor authorization and Oxia registration remain pending |
