...
| Target publish side-effect outcome boundary | Implemented (transport SPI plus durable attempt subset) | `DestinationPublishAdapter`, `PinnedKafkaDestinationAdapter`, `PinnedPulsarDestinationAdapter`, `PublishAttemptLedger`, `DelayShard`, `DestinationAdapterTest`, `DelayShardTest`; physical adapter evidence journal, full admission gate and remaining outcome/evidence mutations pending |
...
| `PUBLISHING`/`UNCERTAIN` attempt ledger and obligation locator | Implemented (durable source-ordered subset) | `PublishAttemptLedger`, `AttemptObligationRef`, `PublishAdmissionBody`, `DelayShard`; source-ordered `PUBLISH_ADMISSION_V1` now validates the descriptor/Ready Certificate/Claim identity projection and persists the full admission bytes, PUBLISHING ledger, message transition, READY removal and System Mutation result in one batch; source-ordered `PUBLISH_OUTCOME_V1` UNKNOWN atomically migrates to UNCERTAIN, verified-success subset closes the ledger, and System Mutation results are deduped in the same batch; full Profile/Adapter/evidence/capacity validation, NOT_PUBLISHED/retry/evidence resolution, obligation-set projection and remaining source-ordered mutations remain pending |
