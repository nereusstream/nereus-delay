...
| Publish Admission timing/profile gate | Implemented (local semantic gate; publication authority pending) | `PublishAdmissionBody.requireTimingPolicy`, `DelayShard` optional `ProfileCatalog`, `PublishAdmissionBodyTest`; descriptor structure permits only `actionAt <= deliverAt`, catalogued shards validate exact Destination/Delivery Capability refs, ordinary managed timing, fixed certified Pulsar handoff lead/capability, target resource and explicit partition policy, while catalog-less compatibility shards fail closed to `actionAt=deliverAt`; Profile publication, Broker guard attestation and production Producer authority remain release blockers |
