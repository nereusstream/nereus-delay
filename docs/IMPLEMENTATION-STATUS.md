...
| Local immutable Profile/binding catalog seam | Implemented (exact local projection; authority pending) | `ProfileCatalog`, `InMemoryProfileCatalog`, `ProfileCatalogTest`; exact Profile semantic reference lookup, generation-1 binding/head/protection creation, checked equivalent rotation with response-loss idempotency, immutable generation retention, deprecation intent and semantic-reference collision rejection are covered; authenticated Profile publication, source-ordered activation markers, retained-generation quota and Oxia CAS remain external |
