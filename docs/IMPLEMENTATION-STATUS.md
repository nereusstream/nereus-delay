...
The resolver is a local authority seam only: policy/Profile activation,
...
overflow, and rejects possible-duplicate retry for FIFO ordering. This is a
canonical value/validation seam only; policy publication, source-position
activation and historical lookup remain external-authority blockers.
...
canonical repeated-field value encoding for `meta_cf` persistence. The trust
set semantic bytes/catalog lookup, authenticated source-ordered control
authority, and Recovery-Floor historical verifier retention remain external
blockers; the state projection does not invent those authorities.
...
| Registry-shaped `ScheduleIntentV1` | Implemented (canonical codec plus resolver-backed local apply; external authority pending) | `ScheduleIntentV1`, `ScheduleCommandBodyV1`, `CommandBodies.scheduleV1/decodeScheduleV1`, `PreparedCommand.scheduleV1`, `CommandCodec.encode/decode*V1`, `V1ScheduleResolver`, `V1CommandResolutionException`, `V1ScheduleBinding`, `KeyCodec.idV1ScheduleBinding`, `PayloadReference.fromDescriptor`, `DelayShard`, `ScheduleIntentV1Test`, `ClientCommandBodyV1Test`, `PreparedCommandV1Test`, `V1ScheduleBindingTest`, `PayloadReferenceTest`, `DelayShardTest`; exact field order/oneof/presence, common fields 1–3, outer/body message identity/type/retry binding, tuple-derived Lane, resolver-required fail-closed route snapshot, inline/object payload projection including absent optional etag, canonical body/tuple binding persistence and reopen checks are covered; source-position Profile/Retry activation, full historical binding and production profile/adapter authority remain pending |
| Registry-shaped `PrepareLargeScheduleV1` | Implemented (canonical codec plus resolver-backed reservation apply; external authority pending) | `PrepareLargeScheduleBodyV1`, `CommandBodies.prepareLargeV1/decodePrepareLargeV1`, `V1ScheduleResolver`, `V1ScheduleBinding`, `DelayShard`, `ClientCommandBodyV1Test`, `V1ScheduleBindingTest`, `DelayShardTest`; common fields 1–3, intent-without-payload, expected length/SHA-256, reservation TTL/trust-set reference, resolver-derived Lane and atomic binding sidecar persistence are covered; source-position policy/Profile/trust-set authority, Object Store handle/attestation and full reservation binding migration remain pending |
...
| Registry-shaped `RetryPolicySemanticV1` | Implemented (canonical value/hash codec; authority pending) | `RetryPolicySemanticV1`, `RetryPolicyRefV1`, `UncertainPolicyV1`, `DlqExportModeV1`, `RetryPolicySemanticV1Test`; fields 1/4–18, domain-separated semantic hash, typed ref projection, uncertain/DLQ presence rules, FIFO possible-duplicate guard and checked backoff budgets are covered; policy publication, source-position activation/lookup and command-body binding remain pending |
| Registry-shaped `PayloadProofTrustSetSemanticV1` | Implemented (canonical verifier-key/hash codec plus source-ordered marker projection; authority pending) | `PayloadProofVerifierKeyV1`, `PayloadProofTrustSetSemanticV1`, `PayloadProofTrustSetRefV1`, `PayloadProofTrustSet.fromSemantic`, `PayloadProofTrustSetControlState`, `PayloadProofTrustSetSemanticV1Test`, `PayloadProofTrustSetControlStateTest`; sorted/unique Ed25519 raw keys, validity bounds, semantic hash/ref, canonical round-trip/tamper rejection, source-time verification windows, strictly ordered activation markers, idempotent marker replay, first-seen issuance close versus historical verification, and canonical marker-state encoding are covered; semantic catalog resolution, authenticated source-ordered control apply, durable shard wiring and Recovery-Floor historical retention remain pending |
...
| Worker DB/checkpoint resource limits | Implemented (config-envelope plus runtime shard/DB slots and local placement scorer) | `ShardStoreConfig`, `SharedRocksDbResources`, `WorkerResourceEnvelope`, `WorkerCapacityAdmission`, `WorkerLoadVector`, `WorkerPlacementPolicy`, `WorkerResourceEnvelopeTest`, `WorkerCapacityAdmissionTest`, `WorkerPlacementPolicyTest`, `ShardStoreTest`; checked memory/FD/disk cross-bucket inequalities fail closed before resource creation, logical `maxOwnedShards` and physical `maxOpenShardDbs` slots are tracked independently (restore staging does not consume ownership), shared RocksDB resources refuse premature close, local Worker admission sums distinct shard `committed` vectors plus fixed/transition demand once with checked per-dimension arithmetic, and the local placement seam rejects over-capacity candidates before applying dominant-resource/load scoring with stale-telemetry penalty, minimum residence, hysteresis and movement cost; live JVM/cgroup/rlimit probes, WAL/SST/temp accounting, per-work-class reserves, cooperative assignment and Oxia placement capacity authority remain pending |
...
| Checkpoint file inventory and canonical manifest projection | Implemented (local CAS projection and bounded upload coordinator; external publication pending) | `CheckpointFileInventory`, `CheckpointManifest`, `CheckpointManifestJson`, `CheckpointResourceV1`, `CheckpointUploadStateV1`, `CheckpointUploadIntentV1`, `CheckpointUploadIntentStore`, `CheckpointUploadAdapter`, `CheckpointUploadRequest`, `CheckpointUploadCoordinator`, `CheckpointManifestTest`, `CheckpointResourceV1Test`, `CheckpointUploadIntentV1Test`, `CheckpointUploadIntentStoreTest`, `CheckpointUploadCoordinatorTest`; inventory streams SHA-256 over each file without loading an SST into heap and rejects symbolic links before restore/copy, while the manifest decoder enforces the closed field order/types, Kafka/Pulsar typed `EvidenceCursorV1` branches, strict cursor identity ordering and byte-identical canonical JSON round trip; the published-manifest Object Store identity and upload-intent state branches have closed canonical codecs, and the local coordinator verifies exact pending intent, deadline, shard/lineage/owner/store/parent identity, complete local file inventory, Worker upload slot and returned manifest length/SHA-256 before PENDING_UPLOAD -> PUBLISHED revision CAS; an exact PUBLISHED successor is reread after response loss without another adapter call, while real Oxia lease/session/catalog CAS, Object Store upload/attestation/publication and reaping/quiescence remain pending |
...
| Destination Lane isolation and bounded weighted DRR | Implemented (scheduler core plus local persistence projection) | `LaneScheduler`, `PersistentLaneScheduler`, `LaneSchedulerTest`, `SchedulerProjectionsV1Test`; lane-local work discovery remains bounded in-memory, while the semantic ring order and lane identity/version are now represented by the canonical scheduler projections |
| Persistent scheduler fairness counters | Implemented (local closed projection subset) | `PersistentLaneScheduler`, `SchedulerProjectionsV1`, `OwnerIdentityV1`, `LaneSchedulerTest`, `SchedulerProjectionsV1Test`; all five `meta_cf/SCHEDULER` values are written in one batch and decode with Registry digests/order/identity checks, but READY-key discovery/rebuild from authoritative `meta_cf/LANE` and full fenced activation remain release blockers |
| Worker-to-shard-to-lane bounded DRR | Implemented (core snapshot plus local placement scoring) | `WorkerScheduler`, `WorkerSchedulerTest`, `WorkerLoadVector`, `WorkerPlacementPolicy`, `WorkerPlacementPolicyTest`; outer DRR persists/restores deficit, cursor/round and blocked-shard isolation state, while the local placement seam hard-filters full committed capacity/DB slots and deterministically scores unequal observed load; durable outer scheduler projections, broker assignors, Oxia desired-placement plans and authoritative placement weights remain pending |
