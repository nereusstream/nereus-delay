...
| DUE/ORDERED/READY/EXPIRY timeline namespaces | Implemented (embedded core subset) | `DelayShard`, `MessageRecord`, `ReadyIndexValue`, `KeyCodec`, `DelayShardTest`; READY key/value, laneVersion fencing, retry eligibility for unordered definitive retry, atomic affected-lane updates, fenced rebuild/discovery, and replay-stable Claim Result timeline-key/semantic-digest checks are covered; full registry `TimelineWorkRefV1`/GenerationRuntimeIndex pending |
| `PUBLISHING`/`UNCERTAIN` attempt ledger and obligation locator | Implemented (durable source-ordered subset) | `PublishAttemptLedger`, `AttemptObligationRef`, `PublishAdmissionBody`, `PublishOutcomeBody`, `DelayShard`; source-ordered `PUBLISH_ADMISSION_V1` validates the descriptor/Ready Certificate/Claim identity projection and persists the full admission bytes, PUBLISHING ledger, message transition, READY removal and System Mutation result in one batch; source-ordered `PUBLISH_OUTCOME_V1` UNKNOWN atomically migrates to UNCERTAIN, verified-success closes the ledger, and definitive `NOT_PUBLISHED` retriable/permanent/lane-unavailable outcomes atomically requeue, terminalize, or block the lane; service-authored verified `EVIDENCE_RESOLUTION_V1` can close or requeue the exact UNCERTAIN ledger; System Mutation results are deduped in the same batch; full Profile/Adapter/evidence/capacity validation, cursor/evidence semantics, obligation-set projection and remaining source-ordered mutations remain pending |
| Terminal generation history | Implemented (Cancel/Reschedule/expiry/claim-result subset) | `TerminalGenerationRecord`, `DelayShard`, `DelayShardTest`; replay-stable permanent Claim Result terminalization is covered; publish/DLQ terminal obligations, open-obligation summaries and GC retention pending |
