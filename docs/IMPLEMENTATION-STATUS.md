...
| Active READY-discovery work-class admission | Implemented (local bounded discovery and live-certificate handler; Claim/Admission handoff pending) | `DueSchedulerWorkClassExecutor`, `OwnedDelayShard`, `PersistentLaneScheduler`, `DueSchedulerWorkClassExecutorTest`; exact Shard/canonical trusted-time/scan-budget identity and canonical-request-plus-max-scan byte charge are bound before `DUE_SCHEDULER` admission, rejection is free of Oxia/Store/scheduler mutation, execution rereads the strict Owner Lease/session and passes the complete evidence to a strict typed-ACTIVE scanner, which uses the earliest bound as inclusive due-through and requires a byte-equal bare `OwnerIdentityV1`, current Store Incarnation, non-predating evidence and `latest < certificate.validUntil`; rollback protects cursor/ring/queues on failure, and success returns only newly promoted due heads. Claim materialization, permit, remaining channel/evidence/credential live generations and Publish Admission remain separate release blockers together with production trusted-time, destination/Oxia authority and dynamic IO attribution |
