...
`awaitApplied` to the embedded Kafka source before it drains any queued records.
`EmbeddedDelayServiceTest.awaitAppliedRejectsForeignSourceBeforeDraining` proves a
foreign receipt is rejected with no pending-command side effect, while
