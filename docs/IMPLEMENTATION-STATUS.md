...
| System Mutation envelope, type registry, canonical hash/ID and Ed25519 signature | Implemented (admission/expiry/outcome/evidence subset) | `SystemMutationType`, `SystemMutation`, `SystemMutationBodyCodec`, `PublishAdmissionBody`, `PublishOutcomeBody`, `TrustedUtcIntervalEvidence`, `SystemMutationResult`, `AuthorIdentity`, `DelayShard`, `ProtocolCodecTest`, `PublishAdmissionBodyTest`, `DelayShardTest`; canonical owner/control/fence/service branches, body fields 1–3, required/optional operation fields, wire widths, bool bounds, canonical nested bytes, durable `dedupe_cf/SYSTEM_MUTATION`, explicit signature verification, source-ordered `EXPIRE_GENERATION_V1`, `PUBLISH_ADMISSION_V1` descriptor/Ready Certificate/Claim identity projection and decision-time subset, definitive `PUBLISH_OUTCOME_V1/NOT_PUBLISHED` disposition/retry-shape subset, and verified published/not-published `EVIDENCE_RESOLUTION_V1` transition subset are covered; full Profile/Adapter/evidence/capacity semantics, source-protected signing-key trust/ACL, and remaining mutation application remain pending |
...
| DUE/ORDERED/READY/EXPIRY timeline namespaces | Implemented (embedded core subset) | `DelayShard`, `MessageRecord`, `ReadyIndexValue`, `KeyCodec`, `DelayShardTest`; READY key/value, laneVersion fencing, retry eligibility for unordered definitive retry, atomic affected-lane updates and fenced rebuild/discovery are covered; full registry `TimelineWorkRefV1`/GenerationRuntimeIndex pending |
...
| Terminal generation history | Implemented (Cancel/Reschedule/expiry subset) | `TerminalGenerationRecord`, `DelayShard`, `DelayShardTest`; publish/DLQ terminal obligations and GC retention pending |
