...
still fail-closes the evidence attachment branches until their full result and
retry/charge projection is available.
...
the strict decoder, and rejects a `messageId` whose self-routing Shard differs
from the mutation subject before the body can be signed. Immutable
