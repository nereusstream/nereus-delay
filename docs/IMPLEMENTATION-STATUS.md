...
payload branch. This is currently a protocol/value boundary only. Existing
`PreparedCommand` ingress still uses the legacy `ScheduleIntent` adapter;
explicit `CommandBodies.scheduleV1`/`prepareLargeV1` now emit the Registry
common fields for migration tests. Command-body runtime migration, policy
semantic authority
activation and production profile/adapter validation remain release blockers.
...
| Registry-shaped `ScheduleIntentV1` | Implemented (canonical value/body codec; command runtime adapter pending) | `ScheduleIntentV1`, `ScheduleCommandBodyV1`, `CommandBodies.scheduleV1/decodeScheduleV1`, `PreparedCommand.scheduleV1`, `CommandCodec.encode/decode*V1`, `RetryPolicyRefV1`, `AdapterMetadataV1`, `KafkaMetadataV1`, `CommittedPayloadDescriptorV1`, `DeliveryMode.fromWire`, `ScheduleIntentV1Test`, `ClientCommandBodyV1Test`, `PreparedCommandV1Test`; exact field order/oneof/presence, common body fields 1–3, outer/body message identity/type/retry binding, Prepare-without-payload form, Profile/Object Store kind checks, Kafka/Pulsar metadata UTF-8/NFC/order and reserved-prefix rejection, quota version and canonical-byte rejection are covered; migration of `DelayShard` from legacy `ScheduleIntent`, full historical retry-policy binding and production profile/adapter authority remain pending |
| Registry-shaped `PrepareLargeScheduleV1` | Implemented (canonical body codec; command runtime adapter pending) | `PrepareLargeScheduleBodyV1`, `CommandBodies.prepareLargeV1/decodePrepareLargeV1`, `ClientCommandBodyV1Test`; common fields 1–3, intent-without-payload, expected length/SHA-256, reservation TTL and trust-set reference are canonical and strictly round-tripped; large-payload command runtime migration and Object Store/policy authority remain pending |
| Registry-shaped `CancelV1` / `RescheduleV1` | Implemented (canonical body/outer identity seam; runtime adapter pending) | `MessagePreconditionV1`, `CancelCommandBodyV1`, `RescheduleCommandBodyV1`, `CommandBodies.cancelV1/decodeCancelV1`, `CommandBodies.rescheduleV1/decodeRescheduleV1`, `PreparedCommand.cancelV1/rescheduleV1`, `CommandCodec.encode/decode*V1`, `RemainingClientCommandBodyV1Test`, `RemainingPreparedCommandV1Test`; independently present generation/state-version preconditions, common fields 1–3, outer/body identity/type/retry binding, canonical timing and strict field order are covered; `DelayShard` runtime migration from legacy fixed bodies remains pending |
