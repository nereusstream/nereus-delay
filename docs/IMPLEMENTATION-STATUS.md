...
| System Mutation envelope, type registry, canonical hash/ID and Ed25519 signature | Implemented (envelope/body-shape/expiry subset) | `SystemMutationType`, `SystemMutation`, `SystemMutationBodyCodec`, `TrustedUtcIntervalEvidence`, `SystemMutationResult`, `AuthorIdentity`, `DelayShard`, `ProtocolCodecTest`, `DelayShardTest`; canonical owner/control/fence/service branches, body fields 1–3, required/optional operation fields, wire widths, bool bounds, canonical nested bytes, durable `dedupe_cf/SYSTEM_MUTATION`, explicit signature verification and source-ordered `EXPIRE_GENERATION_V1` are covered; nested semantic cross-field rules, source-protected signing-key trust/ACL, and remaining mutation application remain pending |
...
| `PUBLISHING`/`UNCERTAIN` attempt ledger and obligation locator | Implemented (durable core subset) | `PublishAttemptLedger`, `AttemptObligationRef`, `DelayShard`; Admission removes timeline/expiry and persists PUBLISHING in one batch, source-ordered `PUBLISH_OUTCOME_V1` UNKNOWN atomically migrates to UNCERTAIN, verified-success subset closes the ledger, and the System Mutation result is deduped in the same batch; full `PublishAdmissionV1`/Claim/ReadyCertificate/Channel/ChargeVector body validation, NOT_PUBLISHED/retry/evidence resolution and remaining source-ordered mutations remain pending |
