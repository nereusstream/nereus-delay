...
| Query, control operations, DLQ and observability | In progress (wire unions plus bounded local receipt/barrier/DLQ bridge) | `MessageQuerySnapshot`, `ReservationQuerySnapshot`, `DlqExportRecord`, `DelayShard.queryMessageSnapshot`, `DelayShard.queryReservationSnapshot`, `DelayShard.getDlqExportRecord`, `BoundedLocalQueryProjector`, `EmbeddedDelayService.queuedReceiptV1/appliedReceiptV1/queryCommand/queryMessage`, all V1 Command/Message query view codecs, `DelayShardTest`, `DlqExportRecordTest`, `EmbeddedDelayServiceTest`, `ProtocolCodecTest`; Dead Letter terminalization now writes the deterministic `terminal_cf/DLQ_EXPORT` `NOT_CONFIGURED` record atomically and local query can consume it, but real DLQ target/export result/evidence state machine, production receipt/barrier routing, authorization-safe binding/evidence/retention lookup, control-operation query and observability remain release blockers |
