...
and checkpoint-manifest JSON. Their comparisons and Kafka successor also use
unsigned order, including the `0x7fff... -> 0x8000...` boundary; high-bit
receipt/evidence/manifest round-trips are covered by `ProtocolCodecTest`,
`SourceReplaySuccessorTest` and `CheckpointManifestTest`. Source Position
partition, leader-epoch and batch fields still use the local non-negative Java
`int` subset, and auxiliary uint64/time fields plus real Broker adapter proof
remain release blockers.
