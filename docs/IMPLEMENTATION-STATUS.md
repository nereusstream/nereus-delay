...
| Oxia credential Profile catalog, Head, Protection and bounded use lease | Implemented (single-record CAS; control/session/provider authority pending) | `OxiaSyncProfileCatalogBackend`, `OxiaSyncProfileCatalogBackendTest`, `OxiaRealProfileCatalogSmokeTest`; one canonical Oxia record per Profile version stores the exact semantic Profile, immutable generations, mutable Head and every Protection projection with a final digest. Generation-1 publication and `RotateEquivalentSecretRequestV1` use idempotent exact version CAS; `issueCredentialUseLease` compares the current Head/binding/fingerprint, checks attestation scope/age and lease TTL, raises the relevant managed-channel or Object Store protection horizon monotonically in the same CAS, and returns a lease bound to the resulting protection revision. Response loss is accepted only after an exact reread. The Dockerized real-service smoke passed against Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`; secret resolution, trust-set verification, actor/source ordering, retained-generation quota/GC, cross-record Owner/Route/session transactions, multi-node failover for this authority, provider credential rotation/quiescence, real Object Store and release gates remain pending |
