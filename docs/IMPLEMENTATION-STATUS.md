...
| NDR1 receipt frame | Implemented (queued payload core subset) | `ReceiptFrame`, `ReceiptKind`, `CommandQueuedReceiptV1`; registry zero-payload vector, canonical PreparedCommandRef/ProtocolTuple, Kafka/Pulsar Source Position and SafeBrokerAck agreement, query boundary/capability/physical-attempt/digest checks, and flags/length/kind/CRC/Base64url rejection tests; applied/native/control/reservation payload schemas and real Broker response adapters remain pending |
| Query response error branches | Implemented (bounded wire subset) | `PublicQueryErrorV1`, `CommandQueryResult`, `MessageQueryResult`, `CommandQueryResponseV1`, `MessageQueryResponseV1`, `ProtocolCodecTest`; fixed error branch tags, StableCode agreement, `SHARD_TRANSITIONING`-only retry-at presence and canonical round-trip/rejection checks; public Message/Command views, receipt/barrier routing and authorization/evidence/retention remain pending |
