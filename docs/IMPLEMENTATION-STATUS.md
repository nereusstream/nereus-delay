...
an aborted pair with no additional `read_committed` records, stale target
TopicId rejection after same-name delete/recreate without a target write, and
successful replacement-target commit. The exit check found no matching
...
not pin exact receipt-value/Fetch v13/LSO/contiguous-cursor or retention-floor
recovery evidence, and does not test independent target/receipt failover.
...
`sha256:3116a80efc9d4a9399ca225c1de4288abde253659fd6fad2292af7727a2e9505`.
The K2 portion proved atomic target-plus-receipt commit, abort, stale target
...
exact receipt-value/Fetch v13/LSO/contiguous replay and retention-floor
proofs, independent target/receipt failover, D2 source/ACK, D3, and Worker
...
| Pulsar v22 first-class resource guard | Implemented in isolated upstream worktree plus opt-in Delay P1 binding (D3/open broker cuts) | Pulsar branch `nereus/delay-resource-guard-v1@7eebd41d5b0917a0dfe5ea26ef3062a39f70a6d9` from locked `5.0.0-M1@8dae0236c0a0d405ed7f8303081080520fe91551`; focused common/broker, real in-process guarded SEND plus delete/recreate, affected-module checkstyle, Delay source-set compile and typed API/evidence smoke pass. Artifact SHA-256 values are recorded above; multi-broker unload/failover, Docker lifecycle, guarded source and full D3 transport remain open |
...
| Kafka transactional receipt mapping-before-send seam | Partial (local journal plus source-locked real target/receipt transaction; receipt-read and failure evidence open) | `KafkaReceiptResource`, `KafkaReceiptJournal`, `KafkaReceiptJournalTest`, `KafkaTransactionalDestinationRequest`, `KafkaTransactionalDestinationAdapter`, `KafkaTransactionalDestinationAdapterTest`, `KafkaClientArtifactTransactionalDestinationTransport`, `KafkaClientArtifactTransactionalSmoke`; mapping-before-send and canonical receipt values are locally bound, and the real three-broker smoke proves `read_committed` atomic counts, abort, and same-name target TopicId fencing. Exact receipt-value/Fetch v13/LSO/contiguous replay, EndTxn response-loss, independent target/receipt failover, ExclusiveWithFencing, retention/Floor proof, slot authority and Worker integration remain release blockers |
