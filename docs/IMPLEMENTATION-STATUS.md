...
| `PUBLISHING`/`UNCERTAIN` attempt ledger and obligation locator | Implemented (durable core subset) | `PublishAttemptLedger`, `AttemptObligationRef`, `DelayShard`; Admission removes timeline/expiry and persists PUBLISHING in one batch, unknown outcome atomically migrates to UNCERTAIN, verified success closes the ledger; full `PublishAdmissionV1`/Claim/ReadyCertificate/Channel/ChargeVector body validation, retry/evidence resolution and source-ordered System Mutation application remain pending |
