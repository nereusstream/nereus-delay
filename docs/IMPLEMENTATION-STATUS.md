...
bytes, canonical manifest bytes, evidence-cursor count and per-file object
identity lengths are checked before local file hashing or provider I/O; the raw
manifest byte cap is enforced before JSON parsing.
