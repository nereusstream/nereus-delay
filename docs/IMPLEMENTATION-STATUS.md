...
| Persistent scheduler fairness counters | Implemented (local closed projection plus owner-bound recovery first pass and rotating READY cursor) | `PersistentLaneScheduler`, `LaneScheduler`, `SchedulerProjectionsV1`, `OwnerIdentityV1`, `LaneSchedulerTest`, `SchedulerProjectionsV1Test`; all five `meta_cf/SCHEDULER` values are written in one batch, persisted successor order is restored without re-adding discarded lanes, the persisted `SchedulerRoundV1.owner` is compared with the current owner and an owner/store change restarts `recovery_first_pass`, and the first recovery rotation serves at most one item per eligible Lane until every currently discovered Lane has received an opportunity; fenced READY rebuild now performs a complete bounded scan from the READY namespace rather than consuming the rotating discovery cursor, so an over-bound READY set cannot be silently truncated (`LaneSchedulerTest.fencedRecoveryUsesCompleteReadyPassDespitePersistedDiscoveryCursor`); Lane incarnation/version and message-generation checks remain enforced; full Oxia-fenced activation and typed ActiveLaneState cutover remain release blockers |
