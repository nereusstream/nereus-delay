...
immutable object identity, issues response-loss-idempotent opaque handles,
enforces the profile's max-bytes/length/SHA-256 and immutable-if-absent rules,
and returns a cached, trust-set-verifiable `PayloadCommitProofV1`; it keeps
capability/payload bytes in memory and does not claim provider credentials,
remote immutability, Oxia authority or production availability evidence.
