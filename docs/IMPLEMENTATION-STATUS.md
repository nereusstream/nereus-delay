...
| Source assignment, typed Activation Barrier and Owner Lease | Implemented (local boundary subset) | `SourceAssignment`, `KafkaActivationBarrier`, `PulsarActivationBarrier`, barrier-gated `OwnedDelayShard`, `OwnerLeaseStore`, `OwnerLeaseTest`; catch-up now requires an explicit non-zero assignment identity/epoch bound to the typed barrier, every catch-up and post-activation apply record is checked against the typed physical source identity before monotonic replay, including empty Kafka/Pulsar barriers, and renewal keeps owner identity/token/epoch and monotonic expiry; pinned broker assignment/guard, Oxia adapter and activation CAS pending |
