...
payload branch. This is currently a protocol/value boundary for
Schedule/Prepare only. Existing Schedule/Prepare ingress still uses the legacy
`ScheduleIntent` and `LargeScheduleIntent` adapters; explicit
`CommandBodies.scheduleV1`/`prepareLargeV1` emit the Registry common fields for
migration tests. The Cancel/Reschedule V1 bodies are now applied by
`DelayShard` with their independently present generation/state-version
preconditions. Schedule/Prepare runtime resolution, policy semantic authority
