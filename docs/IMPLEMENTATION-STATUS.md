...
common fields 1–3, reservation identity and the existing signed
`PayloadCommitProof` projection. `DelayShard` validates the outer/body
identity before reusing the proof in the existing reservation commit state
machine; a dedicated nested `PayloadCommitProofV1` protobuf and external
Object Store/trust-set authority remain release blockers.
...
| Registry-shaped `CommitLargeScheduleV1` | Implemented (canonical body plus local proof/outer-identity apply; nested proof authority pending) | `CommitLargeScheduleBodyV1`, `CommandBodies.commitLargeV1/decodeCommitLargeV1`, `PreparedCommand.commitLargeV1`, `CommandCodec.encode/decode*V1`, `DelayShard`, `CommitLargeScheduleBodyV1Test`, `DelayShardTest`; common fields 1–3, reservation identity, strict canonical field order, outer/body identity fencing and reuse of the signed legacy proof in the reservation commit state machine are covered; dedicated `PayloadCommitProofV1`, Object Store attestation/ownership and source-position trust-set authority remain pending |
