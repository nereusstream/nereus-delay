...
succeeds. Per-DB dynamic RocksDB statistics and production write-time
attribution remain release gates. `WorkClass`, `WorkClassPolicy`,
...
lease/fence preemption and stale-class selection; `WorkClassSchedulerTest`
covers the caps and isolation. Production Worker event-loop wiring, reserve
token borrowing/reacquisition and write-time authority remain release gates.
