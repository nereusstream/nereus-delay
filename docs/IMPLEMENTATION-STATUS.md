...
After `3ba6fb6` and the corresponding design/status/audit synchronization, the full
...
| Active READY-discovery work-class admission | Implemented (local bounded discovery handler; Claim/Admission handoff pending) | `DueSchedulerWorkClassExecutor`, `OwnedDelayShard`, `PersistentLaneScheduler`, `DueSchedulerWorkClassExecutorTest`; exact Shard/canonical trusted-time/scan-budget identity and canonical-request-plus-max-scan byte charge are bound before `DUE_SCHEDULER` admission, rejection is free of Oxia/Store/scheduler mutation, execution rereads the strict Owner Lease/session and uses the evidence earliest bound as inclusive due-through, and persistent scanner rollback protects cursor/ring/queues on failure; success returns only newly promoted due heads, while Claim materialization, permit and Publish Admission remain separate release blockers together with production trusted-time, destination/Oxia authority and dynamic IO attribution |
