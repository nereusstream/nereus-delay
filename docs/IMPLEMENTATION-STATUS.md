...
| One Delay Shard = one RocksDB DB | Implemented | `ShardStore`, `ShardStoreTest`; existing DBs missing the `meta_cf` shard-identity marker now fail closed instead of being initialized as fresh stores |
