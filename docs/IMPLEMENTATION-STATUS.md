...
| Worker event-loop work-class queue/turn seam | Implemented (local scheduler/resource/handler composition; production authority pending) | `WorkClass`, `WorkClassPolicy`, `WorkClassTask`, `WorkClassScheduler`, `WorkClassEventLoop`, `WorkClassDispatcher`, `WorkClassSchedulerTest`, `WorkClassEventLoopTest`, `WorkClassDispatcherTest`; all eight V1 classes require positive weights and bounded queue/turn records, bytes and time, including the independent `CHECKPOINT` execution class; `LEASE_FENCE` gets the first bounded preemptive turn but carries a cross-poll preemption debt so a continuously queued fence class yields to a runnable ordinary class (`WorkClassSchedulerTest.continuousPreemptiveQueueYieldsAcrossSmallPolls`), and stale queued classes are selected after the configured maximum delay; `WorkClassEventLoop` acquires one exact resource lease per task immediately before queue removal, restores the scheduler snapshot and releases earlier leases on a later admission rejection, fences a second poll until the prior turn closes, and `runTurn` executes the bounded callback sequence outside the monitor while closing all leases on success or failure; `WorkClassDispatcher` rejects incomplete class coverage and routes selected tasks through those same boundaries; actual shard handlers, dynamic RocksDB WriteBatch admission and WriteBatch/IO authority remain release gates |
