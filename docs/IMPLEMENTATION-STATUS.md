...
| Lane Close materialization cursor | Implemented (local source-marker overlay and bounded cursor subset) | `LaneCloseMaterializationCursor`, `DelayShard`, `ShardQuota`, `LaneCloseMaterializationCursorTest`, `DelayShardTest.closeTransfersUnadmittedQuotaAndResumesBoundedMaterializationCursor`; Close marker transfers unadmitted message/reservation quota once in the marker WriteBatch, persists canonical `timeline/SYSTEM` kind-2 cursor state, freezes only generations with an empty admitted-obligation set as `DEAD_LETTER(LANE_CLOSED_BEFORE_ADMISSION)`, closes uncommitted reservations as `ABANDONED`, and resumes message then reservation scans after restart; closed-lane Cancel/Reschedule/Commit paths return the stable frozen outcomes before cursor completion; PUBLISHING/UNCERTAIN obligations are retained and full close-owned Claim tagging, admitted-outcome retirement, object-handle/quiescence GC, Recovery-Floor protection and production materializer scheduling remain release blockers |
