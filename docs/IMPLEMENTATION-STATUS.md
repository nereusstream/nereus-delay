...
| Claim/Prepared Publish materialization projection | Implemented (typed local canonical codec; external authority pending) | `PayloadForPublishV1`, `ClaimMaterializationV1`, `ClaimResultBody`, `PublishAdmissionBody`, `PublishAdmissionBody.Descriptor`, `ClaimRecord`, `PayloadForPublishV1Test`, `ClaimMaterializationV1Test`, `ClaimResultBodyTest`, `PublishAdmissionBodyTest`; inline/object payload union, exact length/SHA-256, Profile slot kinds, Broker resource and adapter-metadata branch agreement, complete uint32 partition/generation, nonnegative timing/order constraints, canonical bytes and `nereus-delay-claim-materialization-v1` digest are shared across Claim Result, Admission and Descriptor parsing; typed accessors do not perform live catalog/Object Store/Producer calls, so full materialization authority, adapter serialization/target-size certification and crash recovery remain blockers |
