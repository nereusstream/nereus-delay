...
Key validity windows and trust-set issuance-open/close markers are still
source-ordered authority inputs, not inferred from this value codec.
...
| Registry-shaped `PayloadProofTrustSetSemanticV1` | Implemented (canonical verifier-key/hash codec; source authority pending) | `PayloadProofVerifierKeyV1`, `PayloadProofTrustSetSemanticV1`, `PayloadProofTrustSetRefV1`, `PayloadProofTrustSet.fromSemantic`, `PayloadProofTrustSetSemanticV1Test`; sorted/unique Ed25519 raw keys, validity bounds, semantic hash/ref and canonical round-trip/tamper rejection are covered; source-ordered trust-set activation/issuance-close, historical retention and proof-time authorization remain pending |
