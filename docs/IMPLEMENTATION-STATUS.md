...
| System Mutation envelope, type registry, canonical hash/ID and Ed25519 signature | Implemented (outer envelope/body-shape subset) | `SystemMutationType`, `SystemMutation`, `SystemMutationBodyCodec`, `AuthorIdentity`, `ProtocolCodecTest`; canonical owner/control/fence/service branches, body fields 1–3, required/optional operation fields, wire widths, bool bounds and canonical nested bytes are covered; nested semantic cross-field rules, signing-key trust/ACL checks and source-ordered application remain pending |
