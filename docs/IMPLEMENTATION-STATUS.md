...
`UNKNOWN` placeholder remains a compatibility seam. The local attempt ledger
does not yet persist `firstAttemptAt`/`retryDeadline` as independent admission
facts, so full historical retry binding, DLQ-domain application and external
