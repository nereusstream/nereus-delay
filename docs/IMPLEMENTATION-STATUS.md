...
digest-only Gateway audit sink, an Oxia-backed tenant-scoped admission CAS
...
Fetch/source poll/ACK handoff smoke, a real-Kafka Worker
recovery/apply/ACK smoke, and receipt-bound payload upload
...
## 2026-08-15 guarded Kafka Worker recovery/apply vertical

Delay commit `c72cac90` adds the opt-in
`KafkaClientArtifactWorkerSmoke`. Against the locked Kafka K1 client it
creates a real guarded Kafka source, publishes one command before activation,
recovers that record through `KafkaClientArtifactRecoverySourceCursor` and
`OwnerRecoveryCoordinator`, then activates the `OwnedDelayShard` at the exact
exclusive barrier. A second guarded record is consumed by
`KafkaClientArtifactWorkerSourceFactory` and `WorkerShardRuntime`; the shared
RocksDB `WriteBatch` apply is completed before the native `commitSync`, and the
smoke checks the exact Kafka source position, committed group offset and
session-bound owner lease release on drain.

The deterministic authority used by this smoke is an in-memory owner-lease
backend wrapped by `OxiaOwnerLeaseStore`; it is intentionally not presented as
the network Oxia session/placement path. The evidence therefore closes the
real Kafka guarded Fetch v13 → recovery → RocksDB apply → ACK vertical for one
partition, while Route assignment publication, Oxia session authority,
placement, due/Lane/publish/checkpoint orchestration and Broker failure-injection
cuts remain release blockers.

The same isolated Docker run passed with Kafka source
...
`19195,19196,19197`. The source line was:
...
Delay worktree commit `ca9134ec8a1922e68f76f69ae0aa9bdd6e7180d5` adds the
opt-in Kafka K2 binding. `KafkaTransactionalDestinationAdapter` requires an
explicit Kafka Source Position and the exact prepared-publish hash, persists
the `KafkaReceiptJournal` mapping before transport ownership, and sends the
target record plus canonical receipt record through one
`GuardedTransactionalProducer` transaction. Invalid source/prepared evidence
is rejected before transport; registration or transport uncertainty remains
`UNKNOWN`; the generic no-hash overload does not invent evidence.

The isolated Kafka client commit
`8bd66fbb26eae1b0e4c5867e61f41900c3f5e318` adds the public
`GuardedTransactionalProducer` surface and requires an active transaction,
transaction-v2 capability and `maybeAddPartition` registration before a
guarded in-transaction send. It remains a generic Kafka client change and
does not import Delay types. The source-locked client jar used by the real
smoke was
`/Users/liusinan/apps/ideaproject/nereusstream/kafka-worktrees/nereus-delay-k1/clients/build/libs/kafka-clients-4.4.0-SNAPSHOT.jar`, SHA-256
`4b6362d10146568c7ef78629ad678e50f164a750fdbb362ba0899dc49b815656`.

The independent three-broker command passed on 2026-08-15:
...
It used Kafka source `8bd66fbb26eae1b0e4c5867e61f41900c3f5e318`, Compose
project `nereus-delay-kafka-e2e-1786739311-64581`, broker ports
`19173,19174,19175`, broker image
`sha256:3116a80efc9d4a9399ca225c1de4288abde253659fd6fad2292af7727a2e9505`,
and base image
`eclipse-temurin:21-jre@sha256:371da296b8cb74c7e53fbe7083d5374befc0011b493231d97d45fa789915e434`.
The K1 and K2 smoke sequence proved an atomic target-plus-receipt commit,
an aborted pair with no additional `read_committed` records, exact target
payload and canonical receipt key/value reads, stale target TopicId rejection
after same-name delete/recreate without a target write, and successful
replacement-target commit. Commit
`7b64de0f9648815df55893d3dad3673093deabb7` adds the exact-record assertion;
the exit check found no matching
container, network, volume or temporary image.

This is current opt-in K2 implementation and real-broker evidence, not the
full K2 completion gate. The run does not induce EndTxn response loss, does
not pin Fetch v13/LSO/contiguous-cursor or retention-floor recovery evidence,
and does not test independent target/receipt failover.
The K2 atomic-target-receipt profile therefore remains inactive. D2 source
assignment/ACK-after-sync, D3, Worker integration, Gateway production
authority and release/chaos gates remain open.
...
outcome, explicit uncertain retry CAS and the source gRPC proto are covered by
...
distributed quota/control reserve, production safe-audit completeness beyond
the digest-only single-record sink, Gateway
...
The follow-up test commit `8e0ed49b706dda2a6cb0d7d011c72d2a9270157b` adds
`OxiaRealGatewayAuditSinkSmokeTest`. On 2026-08-15, Oxia source
`37a17bef17202d5fd6e23282da5fd26d94865484` was built to `/tmp` and run in
standalone mode on `127.0.0.1:16648`. The selected real-service tests for
Owner Lease/ephemeral session, Control Operation/Target CAS, Recovery
Catalog/Floor and Upload Intent, and the Gateway audit sink all passed with
`NEREUS_DELAY_OXIA_ENDPOINT=127.0.0.1:16648`; the Gateway test also read back
one exact canonical event after two writes. This upgrades the single-record
Oxia evidence to a live-service check, but does not close Route activation
barriers/session reconnect, cross-record transactions, deployable Gateway
authentication, real Broker transports or production Worker wiring.
...
rotation evidence, live Oxia admission deployment, HA idempotency/
...
rotation deployment, live Oxia admission/HA deployment, HA/transactional
...
lost response as successful only after rereading the exact lease identity.
Lease release is idempotent, and record/lease digests plus canonical field
ordering reject malformed or drifted values. `InMemoryGatewayAdmissionController`
remains the local conformance implementation.

`OxiaGatewayAdmissionControllerTest` covers canonical tamper rejection,
separate pools, hard byte quota, expiry reclaim, tenant isolation and exact
reserve/release response-loss recovery. The full local
`GRADLE_USER_HOME=/Users/liusinan/.gradle ./gradlew check` passed in 21
actionable tasks; the five opt-in real-Oxia methods were skipped because no
endpoint was configured. At that code commit the durable admission
composition had no live-service evidence; quota-rate windows, deployment/HA
observability, cross-record Gateway transactions and production Gateway
wiring remained open.
...
both handles idempotently and reread one exact canonical admission record with
zero leases. The exit cleanup removed the matching Compose container/network.
This upgrades admission to live single-node Oxia evidence; quota-rate/load,
HA/session churn, cross-record transactions and production Gateway wiring
remain open.
...
## 2026-08-15 source-locked Kafka/Pulsar binding and Kafka Docker E2E slice

Delay worktree commit `3f76e836964d818360d5affc122515ccbac04717` adds explicit
`realKafka` and `realPulsar` source sets. The normal `main` source set has no
upstream Kafka/Pulsar dependency and the opt-in compile/run tasks require an
explicit artifact path; omission fails closed. `KafkaClientArtifactProduceTransport`
constructs the K1 `ProducerResourceGuard`, sends only through
`GuardedProducer.sendGuarded`, validates authenticated cluster/canonical topic/
TopicId/partition and response evidence, and maps missing broker time or
ambiguous failures to `UNKNOWN`. `PulsarClientArtifactProducerFactory` creates
only a BYTES producer with the P1 `TopicResourceGuard`; its send transport
requires the exact `GuardedMessageId`, projects ledger/entry/batch/timestamp and
preserves typed v22 rejection evidence. Evidence encoders contain identities,
positions, timestamps and digests, not request payloads or credentials.

The source-bound checks passed with independent Gradle state:

```text
GRADLE_USER_HOME=/tmp/nereus-delay-full-gradle ./gradlew check --no-daemon --console=plain
GRADLE_USER_HOME=/tmp/nereus-delay-full-gradle ./gradlew compileRealKafka \
  -PkafkaClientJar=/Users/liusinan/apps/ideaproject/nereusstream/kafka-worktrees/nereus-delay-k1/clients/build/libs/kafka-clients-4.4.0-SNAPSHOT.jar \
  --no-daemon --console=plain
GRADLE_USER_HOME=/tmp/nereus-delay-full-gradle ./gradlew runRealPulsarSmoke \
  -PpulsarClientClasspath=/Users/liusinan/apps/ideaproject/nereusstream/pulsar-worktrees/nereus-delay-p1/pulsar-client/build/libs/pulsar-client-original-5.0.0-M1.jar:/Users/liusinan/apps/ideaproject/nereusstream/pulsar-worktrees/nereus-delay-p1/pulsar-client-api/build/libs/pulsar-client-api-5.0.0-M1.jar:/Users/liusinan/apps/ideaproject/nereusstream/pulsar-worktrees/nereus-delay-p1/pulsar-common/build/libs/pulsar-common-5.0.0-M1.jar \
  --no-daemon --console=plain
```

The Pulsar binding smoke reported `persisted=PERSISTED`,
`mismatch=UNKNOWN`, and `rejection=DEFINITIVELY_NOT_PERSISTED`. The locked P1
artifact SHA-256 values were `4177b4e090351342936cb0343e17aefcb358de6de9848c9a6f6c3855a9f704c1`
for `pulsar-client-original`,
`ac7675b50f6a025427e0089dd44baf42fc72c32fe9e7a1c35f3aea245fdd24f8` for
`pulsar-client-api`, and
`b09f7b4dd3fd55775dd50912152e7118166d89fd1bc238635a785b15436164d6` for
`pulsar-common`.
...
and the K1 client jar SHA-256 was
`722b09de1a6d79eba867ceda4baac085af0a59897f9e003b58f167ac13e35c24`.
The first run proved three-replica produce plus delete/recreate old-TopicId
rejection and replacement acceptance; after broker 1 stopped, the second run
proved same-TopicId produce through brokers 2/3. Cleanup left no matching
container, network or image.

This closes an opt-in K1 writer and a P1 client-API/evidence binding, not the
K2 target-plus-receipt transaction, Pulsar unload/reconnect Docker cut, guarded
...
Delay commit `ca9134ec8a1922e68f76f69ae0aa9bdd6e7180d5` adds the source-bound
target-plus-receipt adapter and real Kafka binding. The adapter requires the
Kafka Source Position and exact prepared-publish hash, writes the
...
`LC_ALL=C LANG=C ./e2e/run-kafka-real-client-e2e.sh` passed against a
three-broker KRaft Compose project
`nereus-delay-kafka-e2e-1786739311-64581` on ports `19173,19174,19175`.
The broker image was
`sha256:3116a80efc9d4a9399ca225c1de4288abde253659fd6fad2292af7727a2e9505`.
The K2 portion proved atomic target-plus-receipt commit, abort, exact target
payload and canonical receipt key/value reads, stale target TopicId rejection
after same-name delete/recreate, and replacement commit. Commit
`7b64de0f9648815df55893d3dad3673093deabb7` adds the exact-record assertion;
cleanup found no matching Docker resources.

This is partial K2 evidence only. EndTxn response-loss classification,
Fetch v13/LSO/contiguous replay and retention-floor proofs, independent
target/receipt failover, D2 source/ACK, D3, and Worker
production wiring remain open, so the atomic-target-receipt profile stays
inactive.
...
The large-payload row also has a source-bound live vertical now: Delay
`ca9e9623`, three real Kafka brokers, Oxia `37a17bef`, mTLS/RS256 Gateway,
Worker source apply and versioned MinIO all passed through Gateway Prepare,
upload/attestation and Commit, including exact duplicate idempotency and final
Owner/Assignment release. This is one source partition and ends at Worker
...
evidence, Claim charge and deadline, and all Publish inputs: exact Claim
reservation, prepared descriptor, Ready Certificate, decision time and signing
key. No live Profile/Object Store/credential state, payload serialization,
certificate, Publish descriptor, Claim authority or Source Position is inferred
by this wrapper.

This closes the local source → scheduling → derived-or-explicit Claim command
graph composition only. It does not claim external Profile/Object
Store/credential authority, automatic Publish materialization, real Broker
Shard Log append/ACK, multi-shard orchestration, crash recovery, broker
...
## 2026-08-16 real Gateway mTLS/RS256 network and Oxia durability vertical

Delay commit `9a170837` adds the opt-in
`OxiaRealGatewayGrpcSmokeTest` and
`e2e/run-gateway-real-e2e.sh`. The harness generates an ephemeral test CA,
server certificate and client certificate, starts the checked-in
`GatewayGrpcServer.mutualTls` composition, and calls the generated Schedule
RPC over a real Netty gRPC channel. The server verifies the RS256 issuer,
audience, time, key id and `cnf.x5t#S256` binding against the client
certificate. Two identical authenticated requests then use real Oxia
admission, idempotency and digest-only audit records; the test requires one
semantic preparation, one physical attempt, a released admission lease, one
quiescent idempotency record and two deduplicated audit events. A mutated JWT
signature is rejected as `UNAUTHENTICATED` before preparation.

The source-locked run used Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`, image
`sha256:797acbc5eac0f34b0bbfbe984f53075226368a8721f14e43a17237195dd6907b`,
Compose project `nereus-delay-gateway-e2e-1786820415-72294`, Oxia port
`16668`, and Gateway port `22350`. The real run forced Gradle execution and
ended with `BUILD SUCCESSFUL` and `11 actionable tasks: 11 executed`:
...
Status boundary: this closes one deployable Gateway network/authentication
and durable-record composition. The semantic core and submission coordinator
inside this smoke are deterministic test doubles and the successful outcome
is the bounded local `SDK_BACKPRESSURE_NOT_SUBMITTED` branch; this is not a
live Kafka/Pulsar publish, certificate deployment/rotation, quota-rate/load,
admission HA/session-churn, HA idempotency, crash-cut or release PASS.

## 2026-08-16 Gateway server-restart idempotency revalidation

Delay commit `232ce29d` extends the Gateway real-service smoke so the first
`GatewayGrpcServer` is shut down after the authenticated request and a second
server is started on the same port with the same Oxia-backed admission,
prepared-byte idempotency and audit composition. The restarted network client
must receive byte-identical public outcome data, while the deterministic
Semantic-Core and submission counters remain at one preparation and one
physical attempt. The real Oxia scans still require one released admission
record, one quiescent idempotency record and two deduplicated audit records.

The revalidation used Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`, image
`sha256:616b90416dd736b00de0d71b5e6edfa47cd47b079661140b545963d7e312e65c`,
Compose project `nereus-delay-gateway-e2e-1786820937-77983`, Oxia port
`16669`, and Gateway port `22351`. The forced Gradle run ended with
`BUILD SUCCESSFUL` and `11 actionable tasks: 11 executed`:
...
Delay commit `cbe895e1` extends the real Gateway harness with a separate
certificate-deployment cut. The harness creates an old CA/server/client set
and an independent rotated CA/server/client set. The first server persists one
authenticated request using the old set. A replacement server on the same
port then trusts only the rotated CA and presents the rotated server
certificate. An old client configured to trust the rotated CA but presenting
the old client certificate must fail at the TLS boundary; a rotated client
with a JWT whose `cnf.x5t#S256` binds the new certificate must authenticate
and receive the byte-identical durable idempotency outcome without a second
Semantic-Core preparation or physical attempt.

The source-locked run used Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`, image
`sha256:054bb7d13cd9c3d7a6c4dd0b70d5820b6ece2115e840cf47ff8ea0e679a9248c`,
Compose project `nereus-delay-gateway-e2e-1786823102-1813`, Oxia port `16677`
and Gateway port `22356`. It ended with `BUILD SUCCESSFUL in 15s` and
`11 actionable tasks: 11 executed`:
...
This closes bounded same-port server replacement plus authenticated-channel
revalidation, not hot certificate reload, staged rollback, revocation/CRL or
OCSP behavior, multi-process Gateway HA, load, crash/response-loss handling,
live Kafka/Pulsar publication or release PASS.

## 2026-08-16 Gateway durable admission/idempotency recovery after Oxia session churn

Delay commit `f9fa48b7` adds `SessionBoundOxiaGatewayRecordClient`, which binds
Gateway admission, idempotency and audit wrappers to the exact ephemeral marker
owned by `OxiaSyncOwnerLeaseBackend.ClientHandle`. Each Oxia `get`/`put` checks
the marker before and after the operation. A missing or changed marker is
converted to a fail-closed `UNAVAILABLE`; no durable reread recreates an
ownership permit or treats an unknown CAS result as success. Recovery is an
explicit new three-client Gateway composition over the same durable key prefix.

Test commit `241068fd` adds the gated real network cut
`gatewayDurableRecordsRecoverAfterOxiaSessionChurn`. The old composition uses
two-second Oxia sessions and remains live while the harness stops Oxia for five
seconds. Direct admission and idempotency calls prove the expired sessions are
rejected, and an old mTLS Gateway request returns `UNAVAILABLE`. After Oxia
health returns, three new fifteen-second sessions rebuild the Gateway on the
same durable prefix; the exact public outcome is byte-identical, Semantic-Core
and submission counters remain one, and scans require one quiescent attempt,
zero admission leases and two digest-only audit records.

The source-locked run used Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`, image
`sha256:e36ced8f25cff4ea67e61a1dd392668d53b5ac79ffe992587d49548cf038a059`,
Compose project `nereus-delay-gateway-e2e-1786824181-13578`, Oxia port
`16678` and Gateway port `22357`. It ended with `BUILD SUCCESSFUL` and
`11 actionable tasks: 11 executed`:
...
Status boundary: this closes one single-node Oxia stop/start session-rotation
fail-closed and controlled-recomposition cut. It does not prove transparent
automatic session reconnect, multi-node Oxia failover, production Gateway HA,
...
rebuilds the committed descriptor from the persisted `PayloadReference` and
the binding's Object Store Profile, including reservation/proof identity; a
missing binding, timeline projection or committed proof fails closed.

`OwnedDelayShard`, `ClaimHandoffWorkClassExecutor`, `WorkerCommandRuntime` and
`WorkerShardRuntime` now expose a derived-materialization Claim overload. It
uses the same strict Owner/Store/READY identity fence before queue admission;
the existing external prerequisite gate, trusted-time evidence, Claim charge,
deadline and all Publish inputs remain explicit. This removes the prior local
requirement for a caller to duplicate the durable Claim projection without
promoting external Profile, credential, channel, serialization, Producer or
Publish authority.
...
## 2026-08-15 Kafka guarded Fetch to signed Route Worker assignment evidence
...
rereads the exact route-bound assignment through Oxia revision CAS, then uses
the accepted assignment to recover the pre-Route record into the real Worker
Store and ACKs it only after the RocksDB apply. It appends a second record and
starts the guarded Worker source exactly at the signed barrier offset before
applying and `commitSync` ACKing it. The smoke then drains the Worker,
publishes the final local checkpoint and proves the Oxia owner lease and
assignment are released.

The fresh full run used Delay `7e0abb87fff8db1c1d2d2f73ffdd44a0c6097112`, Kafka source
`05849884ca81fad767fda058444d1e17c7f9cbf9`, Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`, client SHA-256
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image `sha256:4ad4078ccea32586873ae089a66c2d7425a0c96051d2a2de47dbd284f016724f`,
Kafka Compose project `nereus-delay-kafka-e2e-1786785694-74566`, Oxia Compose
project `nereus-delay-kafka-oxia-e2e-1786785694-74566`, broker ports
`19730,19731,19732`, and Oxia port `16673`. The exact command was:

```bash
JAVA_TOOL_OPTIONS='-Dorg.slf4j.simpleLogger.defaultLogLevel=warn' \
NEREUS_DELAY_KAFKA_WITH_OXIA=1 NEREUS_DELAY_KAFKA_OXIA_PORT=16673 \
KAFKA_BROKER_1_PORT=19730 KAFKA_BROKER_2_PORT=19731 KAFKA_BROKER_3_PORT=19732 \
./e2e/run-kafka-real-client-e2e.sh
```

The new Route cut printed:
...
The same full run also passed the existing guarded source replay, signed
mutation Worker, ordinary Worker, K1/K2, and broker-1 failover/resume cuts;
the aggregate receipt was:
...
This closes a bounded one-topic/one-partition Fetch proof -> signed Route
barrier -> real Oxia assignment -> Worker Store apply/checkpoint/source ACK
cut. It does not establish
catalog-driven multi-shard placement, Route session reconnect under churn,
...
Delay commit `6912b940` adds a deliberately opt-in K2 harness gate immediately
before the transaction-v2 `EndTxn` call. The gate is only enabled by
`NEREUS_DELAY_KAFKA_K2_FAILOVER`; the normal transport path has no wait or
filesystem dependency. `NEREUS_DELAY_KAFKA_K2_FAILOVER_ONLY=1` runs only this
bounded transaction cut, so the normal K2 smoke remains responsible for abort,
...
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image `sha256:4ad4078ccea32586873ae089a66c2d7425a0c96051d2a2de47dbd284f016724f`,
Compose project `nereus-delay-kafka-e2e-1786790805-40581`, and broker ports
`19795,19796,19797`. The exact command was:
...
incarnation/Owner/Store identity, and delegates live Profile, credential,
channel and evidence preparation to an injected authority. The resulting
`LaneActivationPrerequisites` must contain a self-consistent Channel Resource,
Ready Certificate, evidence cursors and trusted UTC interval. The owned shard
rereads the context-bound Oxia catch-up lease, then atomically installs the
certificate-backed READY projection and READY index. Foreign Owner, Store,
Lane, tuple, barrier or evidence identity is rejected; an already READY Lane
accepts only the exact same certificate. The compatibility readiness setter
cannot activate a typed Lane without a certificate.

Kafka and Pulsar `WorkerSourceFactory.create` now have complete-graph
overloads taking the registry-bound `WorkerSchedulingRuntime`,
`WorkerCommandRuntime` and `WorkerCheckpointRuntime`. The existing source
guard, assignment, Store/resource and Worker identity fences remain active.
...
The real Oxia smoke tests in the full check were skipped by their normal
opt-in gates. This is local typed Lane activation and Worker graph evidence,
not a production due-to-Claim-to-Publish E2E or release PASS. Live external
...
one-shard Worker entrypoint with `WorkerShardRuntime.runDueClaimPublishTurn`.
After due discovery and one strict READY head are submitted, the entrypoint
observes the exact Claim task through bounded fair shared-command turns. A
successful Claim result is then passed to an injected typed
`PublishPreparationProvider`; when preparation is available, the exact Claim
reservation is carried into Publish Admission and its exact task is also run
through bounded fair turns. An empty preparation result leaves the successful
Claim reservation active for a later retry or explicit revoke. Provider
failure fences the Owner; a Publish `UNKNOWN` result retains the exact active
reservation until source-ordered resolution or explicit release.

The focused Claim regression now exercises this complete bounded local turn,
including the injected preparation and `UNKNOWN` Publish outcome. The
following checks passed:

```text
./gradlew test --tests io.nereusstream.delay.ownership.ClaimHandoffWorkClassExecutorTest
./gradlew test --tests io.nereusstream.delay.ownership.ClaimHandoffWorkClassExecutorTest --tests io.nereusstream.delay.ownership.PublishAdmissionWorkClassExecutorTest
./gradlew check
BUILD SUCCESSFUL in 1m 13s
21 actionable tasks: 4 executed, 17 up-to-date
```

This closes only bounded local due → Claim → caller-supplied Publish
preparation composition for one shard. It does not provide live
Profile/credential/Broker prerequisite authority, physical Broker append or
ACK, automatic preparation, response-loss/crash resolution, multi-shard
placement, remote Object Store checkpoint authority or release PASS.
...
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image `sha256:4ad4078ccea32586873ae089a66c2d7425a0c96051d2a2de47dbd284f016724f`,
Compose project `nereus-delay-kafka-e2e-1786795881-97477`, broker ports
`19835,19836,19837`, and Oxia `37a17bef17202d5fd6e232da5fd26d94865484` in
project `nereus-delay-kafka-oxia-e2e-1786795881-97477` on port `16686`.
...
The Pulsar run used the same Delay commit, P1
...
`20135,20136`, and the same Oxia commit in project
`nereus-delay-pulsar-oxia-e2e-1786796050-99359` on port `16687`. The harness
ended with:
...
These are current-source real transport/Worker receipts, not a production or
release PASS. Kafka proves its three-broker K1/K2 cuts; Pulsar proves the
single-standalone-broker restart cut provided by the checked-in harness, not
Pulsar multi-broker failover. Neither run supplies live Profile/credential/
Broker prerequisite authority for typed Lane activation, automatic
due-to-Claim-to-Publish execution, multi-shard placement, remote Object Store
checkpoint authority, crash/response-loss evidence or the release gates. Each
harness removed its temporary Compose resources after exit.

## 2026-08-15 current-source Kafka and Pulsar revalidation after bounded Worker composition

The bounded due-to-Claim-to-Publish composition was committed at Delay source
...
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image `sha256:4ad4078ccea32586873ae089a66c2d7425a0c96051d2a2de47dbd284f016724f`,
Compose project `nereus-delay-kafka-e2e-1786797371-14292`, broker ports
`19845,19846,19847`, and Oxia `37a17bef17202d5fd6e232da5fd26d94865484` in
project `nereus-delay-kafka-oxia-e2e-1786797371-14292` on port `16696`. The
final receipt was:
...
`20145,20146`, and Oxia `37a17bef17202d5fd6e232da5fd26d94865484` in project
`nereus-delay-pulsar-oxia-e2e-1786797371-14293` on port `16697`, with Oxia
image `sha256:b8e9f6e6497308be5e1c1cb937a6af96be10d8b258cb660696f605cdf0b495e3`.
...
`5305748b`; the checked-in real-client harnesses do not invoke the new
provider-driven `runDueClaimPublishTurn` path and therefore do not prove its
automatic live preparation or physical Publish append/ACK. They remain
current-source transport/Worker evidence only: Kafka covers the checked-in
three-broker K1/K2 cuts, while Pulsar covers one standalone-broker restart and
not multi-broker failover. Profile/credential/Broker prerequisite authority,
multi-shard placement, remote Object Store checkpoint authority,
crash/response-loss resolution and release PASS remain open. Harness cleanup
...
## 2026-08-15 bounded PUBLISHING-to-Outcome physical bridge

Delay commit `9a9c14827d01f94b36820e2e4381373725cec7fa` adds
`WorkerPhysicalPublishExecutor` and binds it as an optional physical graph in
`WorkerShardRuntime`. `prepareRequest(...)` decodes the canonical
`PUBLISH_ADMISSION` retained by a `PUBLISHING` ledger, checks the ledger,
descriptor, Claim, Lane, message, generation, attempt and prepared-hash
identities, and checks inline or Object Store payload bytes against the frozen
length/SHA-256 projection before constructing `DestinationPublishRequest`.

The bridge calls the injected `PhysicalPublishGate` before physical admission
and again through `BoundedDestinationPublishAdapter.PublishPreflight`
immediately before the destination delegate. Deferred gates leave the attempt
open without a target call; definitive pre-ownership rejection is handed off
without a target call. Observed adapter results, including `UNKNOWN`, go
through the injected `PublishOutcomeMutationFactory` and the existing
`OutcomeWorkClassExecutor`; the bridge never applies RocksDB state or creates a
Source Position. `WorkerShardRuntime` exposes both exact-request and
Admission/payload entrypoints.

Verification passed:

```text
./gradlew test --tests io.nereusstream.delay.ownership.WorkerPhysicalPublishExecutorTest --tests io.nereusstream.delay.ownership.WorkerShardFleetRuntimeTest --tests io.nereusstream.delay.ownership.ClaimHandoffWorkClassExecutorTest --no-daemon --console=plain
...
This closes only the common Worker physical handoff composition. The gate,
payload/Object Store authority, signed outcome factory (typed evidence,
retry-decision and exact charge transfer), live channel/certificate/credential
authority, and Kafka/Pulsar Shard Log/target ACK implementations remain
external. The current real-client harnesses were not changed or rerun and do
not invoke this executor; their latest real receipts remain source-qualified
...
## 2026-08-15 source-bound physical adapter invocation

Delay commit `5222c9bbb9f64e6a0fe58009ccf143ca8ec59636` adds a source-aware
`DestinationPublishAdapter.publish(request, sourcePosition, preparedHash)`
compatibility boundary. `BoundedDestinationPublishAdapter` carries that exact
...
Publish hash immediately before the guarded delegate. Focused verification
passed:

```text
./gradlew test --tests io.nereusstream.delay.ownership.WorkerPhysicalPublishExecutorTest --tests io.nereusstream.delay.adapter.KafkaTransactionalDestinationAdapterTest --no-daemon --console=plain
BUILD SUCCESSFUL in 6s
11 actionable tasks: 2 executed, 9 up-to-date
...
This proves the common source-bound invocation seam and local K2 adapter
composition only. The checked-in real-client harnesses were not changed or
...
The earlier source-applied entries remain historical receipts. Delay commit
`e5cae7b8e7d9988cc6dca516212d011d49fea5fa` binds the real Kafka Worker smoke
to the active-owner typed scheduling, Claim Handoff, Publish Admission and
preparation-provider graph; commit
`3c6e605a33cea2de85fce473af740b5e05fcf74e` adds the same binding to the real
Pulsar Worker smoke. Both now invoke
`runDueClaimPublishPhysicalTurn(...)`, source-apply the provider-driven Claim
and Admission, and then complete the source-bound physical Publish/Outcome
path.
...
Status boundary: this closes positive provider-driven due-to-Claim-to-physical-
...
## 2026-08-16 real Oxia authority provider-driven Pulsar vertical

The provider-driven P1 E2E was rerun with
`NEREUS_DELAY_PULSAR_WITH_OXIA=1`. Worker assignment publication,
session-bound ownership and the due-to-Claim-to-physical-Publish graph used
the real Oxia backend. Locks: Oxia
...
Status boundary: this closes positive real-Oxia authority evidence for the
...
`nereus-delay-kafka-e2e-1786815566-17636`, Kafka ports
...
Status boundary: this closes positive real-Oxia authority evidence for the
provider-driven Kafka Worker path across the three-broker K1/K2 run and
...
image `sha256:4ad4078ccea32586873ae089a66c2d7425a0c96051d2a2de47dbd284f016724f`,
and Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`. Compose project was
`nereus-delay-kafka-e2e-1786815918-21809`, Kafka ports
...
Status boundary: this closes positive real-Oxia accepted-Route Worker
evidence across a three-broker broker-1 failover cut. It remains a
...
topology was one ZooKeeper, one BookKeeper and two P1 Brokers; the internal
bridge listener and host-facing external listener were separate, and the P1
client used `listenerName=external` for owner lookup. The harness follows
Admin owner redirects and cleans its Docker resources on exit.
...
Status boundary: this closes positive real-Oxia same-topic Worker failover
evidence for the bounded two-Broker harness. It is not production HA
...
Implementation commit `376252bae0faf6f2d5120e223886b3af8a54e636` adds a
dedicated K2 response-loss-only harness mode. The test-only producer wrapper
delegates the real guarded `commitTransaction()` call, lets Kafka commit the
transaction, and then raises locally before returning the commit result. The
production transport is unchanged: its existing commit-uncertainty path opens
a fresh `read_committed` evidence consumer and returns `PUBLISHED` only after
the exact target/receipt pair and typed `KAFKA_TRANSACTIONAL_RECEIPT` evidence
are verified. `NEREUS_DELAY_KAFKA_K2_RESPONSE_LOSS=1` with
...
The run ended with `BUILD SUCCESSFUL in 8s` / `11 actionable tasks: 1
...
The smoke also required one committed target, one committed receipt, and exact
...
`S3CompatibleCheckpointObjectStoreAdapter`, a JDK HTTP S3/S3-compatible
provider seam for the existing checkpoint manifest/resource contract. Adapter
activation verifies that the supplied endpoint, region, bucket and non-secret
credential scope match the exact Object Store Profile digests. Each request is
SigV4 signed; checkpoint file objects use the same deterministic identity
function as the filesystem adapter, `If-None-Match: *`, the manifest is written
last, and every successful, precondition-conflict or ambiguous PUT is resolved
by bounded exact length/SHA-256 reread. Downloads validate the catalog-bound
resource, stream each object into a private bounded staging tree, re-inventory
the complete manifest and publish only through atomic rename.
...
It passed with `BUILD SUCCESSFUL` and covers Profile endpoint/credential drift,
SigV4/conditional headers, manifest response loss, exact restore and immutable
object conflict against a local raw HTTP provider fixture. This is adapter-level
evidence only: no real S3/MinIO provider, credential-use lease/rotation,
provider quiescence/consistency attestation, version-aware delete, multi-shard
RecoveryPin/catalog transaction, crash/chaos, benchmark or release gate is
closed by this slice.

## 2026-08-16 Object Store credential-use lease gate slice
...
they are not credential-authority evidence.
...
--console=plain --quiet` also passed. This closes only the local pre-provider
call gate and fingerprint/age recheck. Oxia Head/protection CAS, trust-set
signature verification, secret resolution, credential rotation, real
S3/MinIO conformance, provider quiescence, version-aware deletion, catalog
transaction, chaos and release gates remain open.

## 2026-08-16 Oxia credential Profile CAS authority slice
...
Store protection horizon in that same record CAS before returning a lease
bound to the resulting protection revision. A lost PUT response is accepted
only after the exact canonical record is reread.

The deterministic authority regression is:
...
This closes one Profile's single-record CAS authority and response-loss
reread. It does not close secret resolution, attestation trust-set or actor
authorization, source ordering, retained-generation quota/GC, cross-record
...
## 2026-08-16 Object Store adapter activation binding slice
...
Activation resolves the exact Object Store Profile, current Head, immutable
Binding and post-issuance Protection from the authority once, asks an injected
private material resolver for the credential strings/fingerprint, rejects any
fingerprint drift before lease issuance, and constructs the existing
lease-gated S3-compatible adapter. The adapter's Provider calls therefore use
the local gate and do not perform an Oxia read per call.

The focused composition regression is:
...
closes only the activation-time authority/material/fingerprint wiring. A real
secret manager, trust-set/actor authorization, automatic lease renewal,
multi-node activation failover, provider rotation/quiescence, real S3/MinIO,
deletion, chaos and release gates remain open.

## 2026-08-16 Credential attestation trust-set verification slice

Delay commit `f758d010b4d75f9c53d1f6e2cf01d573d655fd1c` adds
`CredentialAttestationTrustSet` and makes the Oxia Profile authority require
it during generation-1 publication, equivalent-secret rotation, canonical
record decode/reopen and bounded credential-use lease issuance. The trust set
is immutable and sorted by the exact verifier-version/verifier-id/signing-key
tuple; each entry validates canonical Ed25519 public-key bytes, a retained
verification window and a stable trust-set semantic digest. An attestation is
accepted only when its tuple is present, its verification interval fits the
retained key window and its Ed25519 signature verifies. This prevents a
previously persisted but untrusted binding from becoming activatable after
reopen.
...
its integration with the single-record Profile CAS authority. It does not
...
`RenewableS3CompatibleCheckpointObjectStoreAdapter` and an atomic replacement
operation to `ObjectStoreCredentialUseLeaseGate`. The wrapper renews
opportunistically before upload/download only when the current lease is within
the explicit renewal window. It then resolves the exact Profile, checks that
the current Head and immutable Binding are byte-identical to the activated
generation, resolves private material and its attested fingerprint, obtains a
fresh trusted-time evidence interval, issues a bounded protected lease and
atomically replaces the local gate. A changed Head/generation is rejected as a
quiescence boundary; the wrapper never silently changes credentials while the
provider adapter is live, and provider calls do not read Oxia unless renewal is
due.
...
It passed with `BUILD SUCCESSFUL`; the full `./gradlew check --no-daemon
--console=plain --quiet` also passed. The focused tests cover no authority
read outside the renewal window, lease/protection revision advancement inside
the window, and fail-closed rejection of Head rotation before Provider I/O.
...
`VerifiedCredentialMaterialCache`, an immutable-view private material cache
that implements the activation resolver seam. Installation validates the
exact Object Store Profile, binding generation/reference hash, credential
authorization scope, configured attestation trust set and resolved fingerprint
before replacing the cache snapshot. Reads are local exact-key lookups over
Profile/generation/binding digest/reference hash; a miss returns null and
cannot fall back to another generation or perform Oxia/Vault/Provider I/O.
Failed single installs and failed `replaceAll` batches leave the previous
snapshot unchanged.
...
Delay commit `7b8b73885c5ec26dfc96c1b5b8a1a6ab8ec0d1d9` adds
`CheckpointReapingQuiescenceProof` and `CheckpointReapingQuiescenceGuard`.
The proof binds the exact pending intent digest and reaping trusted-time
evidence, bounds every UTC interval, requires
`requestQuiescenceHorizon >= maximumProviderOwnershipLifetime +
maximumTrustedUtcIntervalWidth`, and carries opaque digests for the certified
old-owner local guard and provider-ownership horizon. The gate requires the
observed trusted interval to be after the reaping-start horizon and both
external closure horizons before `CheckpointReapingSweepCoordinator` invokes
the provider. The coordinator still keeps the REAPING successor reread and
same-prefix retry behavior.
...
It passed with `BUILD SUCCESSFUL`; coverage includes provider-horizon
rejection and the arithmetic bound. The full
`./gradlew check --no-daemon --console=plain --quiet` returned 0.

The locked MinIO rerun used container
`nereus-delay-minio-e2e-1786843920-34723`, endpoint
`http://127.0.0.1:59954`, bucket
`nereus-delay-checkpoints-1786843920-34723`, the locked image at repository
digest
`sha256:14cea493d9a34af32f524e538b8346cf79f3321eff8e708c1e2960462bd8936e`
and image ID
`sha256:8f08aee614800a237906bd48114d733e5ac5bfac4ccdf731f141b0e880d7a253`.
...
harness ended with `BUILD SUCCESSFUL` after the proof-gated coordinator sweep.

This closes only a local proof and ordering gate. The opaque old-owner and
...
`ClientHandle` constructor to `OxiaSyncControlOperationBackend`. Its private
`SessionBoundRecordClient` checks the exact connected Oxia session marker before
and after every control-operation record `get` and `put`, while preserving
expected CAS-race classification only when the session remains valid. If a
write commits and the marker changes before the response or exact reread, the
backend propagates the session fence and never guesses that register/advance
succeeded. The existing unbound constructor remains available only for the
explicit external/deterministic record surface.
...
were skipped because `NEREUS_DELAY_OXIA_ENDPOINT` was not configured; the full
`./gradlew check --no-daemon --console=plain --quiet` returned 0. This closes
...
Delay commit `50435a1364d2e8f7d823cc05faa18e4766f5cbd6` adds a
`ClientHandle` constructor to `OxiaSyncControlTargetRegistrationBackend`. Its
private `SessionBoundRecordClient` verifies the exact connected Oxia session
marker before and after every registration-record `get` and `put`, including
`find` and mutation validation reads. A committed `IfRecordDoesNotExist` write
whose marker changes before the response or exact reread is propagated as a
session fence and is never reported as `RECORDED` or `ALREADY_RECORDED`.
...
`ClientHandle` constructor to `OxiaSyncProfileCatalogBackend`. Its private
`SessionBoundRecordClient` verifies the exact connected Oxia session marker
before and after every Profile catalog `get` and `put`, covering immutable
generation publication, equivalent-secret rotation, protection-before-lease
issuance, Head/binding resolution and response-loss rereads. If a Profile
record commits and the marker changes before the response or exact reread,
the backend propagates the session fence and does not report publication,
...
the full `./gradlew check --no-daemon --console=plain --quiet` returned 0.
This closes only the single-record Profile authority's Oxia I/O session
...
canonical `/publication` record that carries PUBLISHED Upload Intent and
Recovery Catalog manifest state is now wrapped by a private
`SessionBoundRecordClient`; every publication-record `get`, version-CAS `put`
and delete-capable record operation checks the exact connected Oxia session
marker before and after the client call. The existing explicit
`SyncOxiaClient`/session-identity constructors remain compatibility seams, and
the sibling ephemeral Recovery Pin remains separately session-identity-bound.
...
proves that a fake CAS can be durable while the fenced caller still receives a
...
`OxiaSyncCheckpointUploadIntentBackend`. Its independent `/intent` record
surface now uses a private `SessionBoundRecordClient` that checks the exact
connected Oxia session marker before and after every upload-intent `get` and
version-CAS `put`. A PENDING_UPLOAD, PUBLISHED or REAPING successor whose CAS
commits before marker loss is not reported as success when the response or
exact reread is fenced. The existing unbound `SyncOxiaClient` constructor
remains an explicit compatibility/test seam.
...
methods in `OxiaRealRecoveryAuthoritySmokeTest` were skipped because
`NEREUS_DELAY_OXIA_ENDPOINT` was not configured; the full
`./gradlew check --no-daemon --console=plain --quiet` returned 0. This closes
...
existing `OxiaSyncWorkerAssignmentBackend(ClientHandle, ...)` path. The
assignment record now uses a private `SessionBoundRecordClient` around every
get, version-CAS put and exact-version delete, so the connected Oxia marker is
checked both before and after the record call. A desired assignment that is
committed before marker loss is not returned as a successful publication when
the response or exact reread is fenced; the unbound constructor remains an
...
before and after every durable owner-epoch read/CAS and every ephemeral lease
read/CAS/delete. A lease that commits before marker loss is therefore not
returned as a successful acquire, renewal, transition or release when the
response or exact reread is fenced; the unbound constructor remains an
explicit deterministic/external seam.
...
`OxiaRouteAuthoritySession` record/watch surface. The session now checks the
exact ephemeral marker before and after synchronous `get`, `put`, notification
registration and range-scan creation. A private `SessionBoundIterable` also
checks before and after each lazy range iterator `hasNext`, `next` and
`remove`, so a Route cache cannot keep consuming authority data after marker
loss. A signed Route head that commits before marker loss is not reported as a
successful publication when the head response is fenced.
...
construction path in `CheckpointPublicationCoordinator`. A coordinator now
rejects a split intent/catalog pair when either supplied authority implements
`CheckpointAtomicPublicationAuthority`; the same object must own both sides of
the combined Upload Intent and Recovery Catalog record. This prevents a
miswired atomic catalog from accepting an independent `/intent` value and
failing only after provider I/O.
...
Delay commit `de203e4dc14de32746ce73da75381843152af922` adds the missing
session fence to the replacement notification path in
`OxiaRouteAuthoritySession.reconnectNotifications`. The connected Route
marker is checked before creating/replacing the notification client and again
immediately before and after registering the replacement callback. If the
marker changes after notification registration has committed, the replacement
client is closed, the previous client reference is restored and the operation
fails closed.

The focused receipt command is:
...
authority methods and one real Route-worker method were skipped because
`NEREUS_DELAY_OXIA_ENDPOINT` was not configured; the full local check remains
...
state left by a notification registration that commits and is then fenced by
the Route session. `OxiaSignedRouteSnapshotProvider.start()` now returns
immediately only for a healthy started cache; a started cache in a non-healthy
state reuses the explicit `refresh()` recovery path. That path reconnects the
Route session, replaces the notification client and rebuilds the signed Route
cache before reporting success.
...
first forces the post-registration marker fence, then calls `start()` again
and requires replacement notification registration plus `HEALTHY` cache state.
...
Delay commit `22780082d24e2011d44ead6ca62c38251a03633b` closes the remaining
initial-recovery subscription gap. When `OxiaSignedRouteSnapshotProvider` has
not completed `start()` because the first Route replay was unavailable or
gapped, a later `refresh()` now rebuilds the authority cache and establishes
the initial notification callback before returning. The provider marks the
subscription as started before callback registration so a response-loss fence
on that registration routes the next retry through notification replacement.

`OxiaSignedRouteSnapshotProviderTest.refreshAfterAnInitialRouteGapRestoresTheNotificationStream`
forces a missing Route event, repairs the event/head pair, and requires the
recovered revision plus exactly one initial notification registration. The
...
Route key prefix before `OxiaRouteAuthoritySession.connect()` creates either
the authority or notification Oxia client. The validated canonical value is
then passed into the session constructor, so an invalid trailing-slash,
blank, non-NFC or otherwise malformed prefix fails at the caller boundary
without entering external client creation and constructor cleanup.
...
Delay commit `e0d5bc9761fea57103518819165d54eb60662b99` makes each durable
Gateway physical attempt obey the Registry temporal and retry-shape rules:
`uncertaintyAtEpochMs` and `ownershipNotAfterEpochMs` are strictly after the
start, ownership expiry is no later than uncertainty, the first attempt has
no retry identity, and every later attempt carries a retry ID/hash pair.

The deterministic idempotency suite passed 11 tests with zero
failures/skips/errors, including zero/equal time-bound and malformed retry
...
`e2e/docker-compose.large-payload.yml` harness, plus the source-set smoke
`KafkaClientArtifactLargePayloadGatewaySmoke`. The live run creates a
three-node KRaft Kafka cluster with one replicated source topic partition,
publishes the source-ordered trust activation and pre-route record, verifies
guarded Fetch evidence, publishes and refreshes the signed Route through real
Oxia, accepts Worker Assignment and Owner Lease through real Oxia, and exposes
the Gateway over mTLS plus RS256/JWT tenant binding.

The source-bound command passed on 2026-08-16:
...
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image `sha256:4ad4078ccea32586873ae089a66c2d7425a0c96051d2a2de47dbd284f016724f`,
Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`, and Delay
`ca9e9623b082f1c7df575e7316fb806226860ee5`. It used the locked MinIO image
`quay.io/minio/minio:RELEASE.2025-09-07T16-13-09Z` at repository digest
`sha256:14cea493d9a34af32f524e538b8346cf79f3321eff8e708c1e2960462bd8936e`
and image ID
`sha256:8f08aee614800a237906bd48114d733e5ac5bfac4ccdf731f141b0e880d7a253`.
...
```text
Kafka + Oxia Route/Assignment/Owner + Gateway mTLS/JWT + Worker + MinIO large-payload authority E2E passed: activationOffset=0, barrierOffset=2, prepareOffset=KafkaSourcePosition[... offset=2 ...], commitOffset=KafkaSourcePosition[... offset=3 ...], providerVersion=5777ce46-f28c-4a68-ab76-71241ab5fd43, exactGatewayIdempotency=true
BUILD SUCCESSFUL in 11s
```

The positive path is now real through Gateway Prepare, Worker `RESERVED`,
receipt-bound upload-handle issuance, a 1 MiB + 4 KiB payload upload to
versioned MinIO, provider-issued immutable attestation, Gateway Commit, Worker
`COMMITTED`/`SCHEDULED`, exact Object Store proof/readback, byte-identical
duplicate Prepare without an extra Kafka record, final local checkpoint and
exact Oxia Owner/Assignment release. The current receipt proves the complete
large-payload control/object path up to Worker Commit; it does not promote the
separate Kafka/Pulsar destination egress receipts into one combined Gateway
destination chain.

The boundary remains explicit: the source topic is one partition, the harness
uses `InMemoryPayloadProofTrustSetCatalog` for local semantic resolution while
...
`accdc7074bfd38aed2cfd7c696a8c3ff62a972ba` adds the checked-in
`PulsarClientArtifactLargePayloadGatewaySmoke`, the
`runRealPulsarLargePayloadGatewaySmoke` Gradle task, and the isolated
`e2e/run-pulsar-large-payload-gateway-e2e.sh` composition. The source-bound
command was:
...
This is a bounded positive production-authority receipt across real Pulsar,
real Oxia Route/Assignment/Owner, Gateway mTLS/JWT, Worker source apply and
ACK, versioned MinIO payload upload/attestation/readback, due -> Claim ->
Publish Admission -> `PUBLISHING`, typed `PULSAR_SEND_ACK`, source
`PUBLISH_OUTCOME`, source apply to `PUBLISHED`, exact 1 MiB + 4 KiB
(`1,052,672` byte) destination readback, exact duplicate Prepare bytes and
final local checkpoint/Owner release. The receipt's source record count of 6
detects an extra Gateway idempotency append.

The boundaries remain explicit: the combined harness uses one physical source
partition, one ZooKeeper and one BookKeeper, and does not include the separate
...
image ID
`sha256:819a2a34b91d34468ac6caa048ec5cbf959fb9ecb40dbfd649a9fabf067318de`,
and Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`. The isolated Pulsar
Compose project was `nereus-delay-pulsar-e2e-1786885612-9737` on broker/web
ports `19787/19788`; the isolated Oxia project was
`nereus-delay-pulsar-oxia-e2e-1786885612-9737` on port `16657`.
...
This closes a bounded real-P1 Worker destination response-loss slice with
real Oxia Assignment/Owner authority, exact persisted destination payload,
typed `PULSAR_SEND_ACK`, source-applied `PUBLISHED` Outcome and final local
checkpoint. Exact-name post-run checks found no containers, images, volumes or
networks for either isolated Compose project. The focused run still used the
...
response, and leaves the later `PUBLISH_OUTCOME` append untouched. The Worker
must therefore retain the exact mutation as `UNKNOWN`, find the same bytes by
source replay, and continue the physical publish without appending a retry.
...
`f832e20478b7baa808e22f577028d26f7ae2fab8ddc0870d869a06e40dbd8394`, and
`94a865b5d858ea62ec980bdad70316c3cba576a7ce37009a20f4acae89f2d8e8`, P1
image ID `sha256:819a2a34b91d34468ac6caa048ec5cbf959fb9ecb40dbfd649a9fabf067318de`,
and Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`. Pulsar Compose project
`nereus-delay-pulsar-e2e-1786886923-27929` used broker/web ports `19679/19680`;
the Oxia project was `nereus-delay-pulsar-oxia-e2e-1786886923-27929` on port
`16657`.
...
This closes the bounded real-Broker/real-Oxia recovered `UNKNOWN` Publish
...
Delay implementation commit
`8f1116abad2bd77e2f384c04411dabaeb70b4f72` adds the focused
`KafkaClientArtifactFetchResponseLossSmoke`, the
`runRealKafkaFetchResponseLossSmoke` task and the
`NEREUS_DELAY_KAFKA_FETCH_RESPONSE_LOSS_ONLY=1` E2E mode. The source-bound
command was:
...
`1609dbd2794c5034d165769608767d5f8a01ea63293019cc0341e00d88ee1ed3`, broker
image ID
`sha256:b0fcef7eb6f8350af6c22d333de889155acf4b1ec157887266568fc78beada0e`,
and Delay `8f1116abad2bd77e2f384c04411dabaeb70b4f72`. The isolated Compose
project was `nereus-delay-kafka-e2e-1786879840-36136` on broker ports
`19228,19229,19230`; its dynamic source topic ended in
`fetch-response-loss-88f0cd99-c41b-4d1d-8c41-914aaab160f9`.
...
This closes a bounded real-Broker Fetch fault slice: the first
`read_committed` Fetch v13 response and exact two records were received, the
client discarded that response before source ACK, a fresh consumer in the same
group replayed offset 0, the second record reached offset 1, the Fetch LSO was
2, and `commitSync` advanced the group to 2. It is controlled client-side
response loss after a real Broker Fetch, not raw socket packet loss.
...
The Worker JVM reaches a gate after its local RocksDB WriteBatch is durable but
before the guarded Kafka `commitSync` invocation, records its PID, and is then
sent `SIGKILL`. A fresh JVM reopens the same per-run local Store root, reacquires
the real Oxia lease, replays the exact source record and completes ACK and final
checkpoint.
...
`sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`,
Oxia `37a17bef17202d5fd6e23282da5fd26d94865484`, Kafka Compose project
`nereus-delay-kafka-e2e-1786890684-77735` on ports `19327,19328,19329`, and
Oxia project `nereus-delay-kafka-oxia-e2e-1786890684-77735` on port `16719`.
The temporary Oxia image ID was
`sha256:6b8082d3b205230306c243b332a02c1c9d3ecd9c4286ae22b90743a0fc80d26c`.
The source topic was `nereus-delay-worker-ack-crash-live-20260816`.
...
The focused real three-Broker KRaft run used:
...
`nereus-delay-kafka-e2e-1786896942-56285` on effective broker ports
`19377,19378,19379`; the isolated Oxia project was
`nereus-delay-kafka-oxia-e2e-1786896942-56285` at `127.0.0.1:16761`. The
run-created Oxia image ID was
`sha256:e2f8da0d01bf3e7139e3f8cc5e6f2da32c22102f90ca1738ab97e86cad708fce`.
The source topic was `nereus-delay-broker-network-partition-20260817`.
...
This current-source rerun revalidates the bounded Docker-network partition
boundary: `kafka-1` remains running while its Compose network membership is
removed, survivor leaders are checked through the real Admin API, the same
guarded Worker source resumes through `kafka-2,kafka-3` under real Oxia
Assignment/Owner authority, and `kafka-1` is reconnected and made ready again.
...
The isolated Kafka project was
`nereus-delay-kafka-e2e-1786897339-61592` on ports
`19461,19462,19463`, with Broker-1's raw backend on `19561`; the isolated
Oxia project was `nereus-delay-kafka-oxia-e2e-1786897339-61592` at
`127.0.0.1:16762`. The run-created Oxia image ID was
`sha256:fbe2e98e015a7e733184ae8436ffb517ed9e42ed7724ee27252bcb8746c04d03`.
The source topic was `nereus-delay-worker-broker-tcp-cut-live-20260817` and
the fixed Worker group was
`nereus-delay-worker-broker-tcp-cut-live-20260817-group`.

The authoritative placement, proxy and Worker receipts were:
...
The runner also required the pre-cut forwarding, cut acknowledgement,
post-cut rejection and post-cut handoff marker files. This current-source run
closes the bounded raw TCP endpoint-cut plus explicit endpoint-handoff/source-
...
The isolated P1 project was
`nereus-delay-pulsar-multi-e2e-1786898570-79450` on broker/web ports
`22080,22081,22082,22083`; the Oxia project was
`nereus-delay-pulsar-multi-oxia-e2e-1786898570-79450` at
`127.0.0.1:16765`. The Oxia build image ID was
`sha256:00fccb88d365ae52bae39ba1461091f005dc96914fc000d9031ab93d499ded48`.
The current-source output was:
...
This refreshes the bounded current-source two-Broker P1 failover receipt:
Broker-1 was stopped after guarded preparation, a fresh Worker used the
multi-endpoint service URL to reconnect through Broker-2 under real Oxia
Assignment/Owner authority, completed guarded SUBSCRIBE, RocksDB apply,
physical PUBLISH, typed PULSAR_SEND_ACK readback, source ACK and final
...
## 2026-08-17 Current-source Object Store credential renewal authority receipt

The current-source Oxia+MinIO runner now includes the real renewable Object
Store lease test. It locks Delay to
...
`nereus-delay-oxia-minio-checkpoint-e2e-1786900763-13173`, with Oxia at
`26320`, MinIO at `27320`, and bucket
`nereus-delay-checkpoints-1786900763-13173`. The run-created Oxia image
`sha256:2e1f77971b6ef7acd879a0931bddd99aa23ed0f4c9e9420e3e4bef3605d01bda`
was removed by the runner's exact image cleanup. The three real-service
tests each passed with zero failures and zero errors:
...
The renewal test constructs the lease-gated adapter from the real Oxia
Profile Head, immutable binding, protection and session-bound lease; it
renews inside the explicit window, rereads the extended protection horizon,
then rejects renewal after the real Head rotates to generation two. The
adjacent test proves real MinIO upload/download; the renewal test itself does
not claim external secret-manager resolution or a provider upload after
...
## 2026-08-17 Current-source Pulsar Gateway + Broker network-partition large-payload receipt
...
`30452`. The current-source receipt was:
...
This closes the bounded current-source combined Gateway + real Oxia + two real
P1 Broker + Worker + real MinIO network-partition chain for one physical source
partition. The harness disconnects only Broker-1's exact Docker Compose network
...
`sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`,
and Oxia to `37a17bef17202d5fd6e23282da5fd26d94865484`. The isolated Kafka
project was `nereus-delay-kafka-e2e-1786910072-33614` on
`30770,30771,30772`; the Oxia project was
`nereus-delay-kafka-oxia-e2e-1786910072-33614` on `30780`.
...
This closes the bounded current-source Docker-network partition and survivor
...
## 2026-08-17 Current-source Kafka Fetch response-loss and LSO receipt
...
broker image to
`sha256:eb968fa8ea2fcc6c89dca3a9fbfcb4945af3909b574c3896947ffec85a2862e6`.
The isolated project was `nereus-delay-kafka-e2e-1786910277-37004` on
`30790,30791,30792`.
...
The run completed with `BUILD SUCCESSFUL in 56s` and printed:
...
The current-source rerun uses the same Delay/K1/client/image source locks as
the Fetch receipt above. The isolated project was
...
The run completed with `BUILD SUCCESSFUL in 1m 4s` and printed:
