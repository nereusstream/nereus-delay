...
`CommandBodies`/`PreparedCommand` ingress still uses the legacy
`ScheduleIntent` adapter, so command-body migration, policy semantic authority
...
| Registry-shaped `ScheduleIntentV1` | Implemented (canonical value codec; command ingress adapter pending) | `ScheduleIntentV1`, `RetryPolicyRefV1`, `AdapterMetadataV1`, `KafkaMetadataV1`, `CommittedPayloadDescriptorV1`, `DeliveryMode.fromWire`, `ScheduleIntentV1Test`; exact field order/oneof/presence, Prepare-without-payload form, Profile/Object Store kind checks, Kafka/Pulsar metadata UTF-8/NFC/order and reserved-prefix rejection, quota version and canonical-byte rejection are covered; migration of `CommandBodies`/`PreparedCommand` from legacy `ScheduleIntent`, full retry-policy semantic binding and production profile/adapter authority remain pending |
