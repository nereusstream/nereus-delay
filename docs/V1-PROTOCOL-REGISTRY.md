...
`RetryDecisionV1` exact fields: 1 `RetryDecisionKindV1 kind`; 2 `RetryPolicyRefV1 policy`; 3 `uint32 completed_attempt_no`; 4 `int64 first_attempt_at`; 5 `int64 retry_deadline`; optional 6 `int64 next_retry_at`; 7 `uint32 jitter_algorithm_version` (1 for `RETRY_JITTER_V1`); 8 `StableCodeV1 cause`; 9 `RetryDomainV1 retry_domain`. Field 3 is the complete unsigned `uint32` domain (`0xffffffff` is valid on the wire; values above it are rejected); local successor/comparison logic must not use signed ordering or wrap at the all-ones value. `SCHEDULED` and `LANE_WAIT` require field 6; `NONE`, `EXHAUSTED`, and `UNCERTAIN_HOLD` forbid it. Business Outcome/Resolution requires `MESSAGE_PUBLISH`; DLQ result requires `DLQ_EXPORT`. Apply recomputes the domain-separated V1 jitter/deadline from prior state and the accompanying trusted interval; mismatch is an integrity failure.
