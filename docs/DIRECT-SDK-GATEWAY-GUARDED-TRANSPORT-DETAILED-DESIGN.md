# Nereus Delay 双入口与 Guarded Transport 代码级详细设计

状态：Accepted implementation blueprint
Spec revision：`DESIGN-BASELINE-2026-08-25`
日期：2026-08-13
适用范围：Direct Java SDK、可选 Delay Gateway、Kafka/Pulsar Command Transport、现有 Worker

本文把“共享语义内核 + Direct SDK/Gateway 双入口 + Kafka/Pulsar 通用受保护发送”落实到类、模块、线程、协议、失败分类和测试切片。它是主设计的代码级实现蓝图，不改变 [`PROTOCOL-REGISTRY.md`](PROTOCOL-REGISTRY.md) 已冻结的 NDL1/NDR1 字段、Command Hash、Source Position、状态机和 RocksDB key；如果实现需要新增公共 wire 字段，必须先在 Registry 中登记，不能以本文中的 Java 草图绕过 Registry。
...
- [ADR 0024](adr/0024-use-versioned-self-routing-identities-and-a-fixed-hash.md)：继续使用现有 self-routing ID；
...
- 不把循环任务字段加入 。循环任务需要新的 Registry/状态机版本，属于后续 capability。
...
| 仓库 | 基线 | 用途 |
| --- | --- | --- |
| `nereus-delay` | `origin/main@2dfc3289ffdbe9cf9d7f4d0de1d701493d1b49a6` | 当前 protocol/client/adapter/worker 代码；本地 D1/D5 implementation branch 在该基线之上 |
...
| `PreparedSubmission` | MANAGED/AUTO_FAST 冻结分支 | 原样复用，I/O 后禁止重选 |
...
| `PinnedKafkaCommandIngress` | Kafka result -> `EnqueueOutcomeMessage` | 保留投影；替换 nested test SPI 为生产 transport 实现 |
| `PinnedPulsarCommandIngress` | Pulsar result -> `EnqueueOutcomeMessage` | 保留投影；接入 guarded receipt/error |
...
| `DelayClient` | 当前公共 facade | 收敛为严格 API；legacy/embedded overload 留在 conformance artifact |
...
:delay-api
  ...
  （包括 SubmissionMode、authenticated intent/error value API）
...
:delay-route-spi
  RouteSnapshot / RouteSnapshotProvider / RouteSnapshotCacheView
...
public record RouteSnapshot(
        ...
        IngressRouteResource ingress,
        RoutingHashVersion routingHashVersion,
        ProtocolTuple protocolTuple,
        long controlVersion,
        List<RoutePartitionPolicy> partitions,
        ...
        IngressCredentialBindingRef credentialBinding,
...
public sealed interface IngressRouteResource
        permits KafkaIngressRouteResource, PulsarIngressRouteResource {
    String authenticatedClusterId();
    int partitionCount();
}

public record KafkaIngressRouteResource(
        String authenticatedClusterId,
        String canonicalPhysicalTopic,
        UUID nativeTopicUuid,
        int partitionCount) implements IngressRouteResource {
}

public record PulsarIngressRouteResource(
        String authenticatedClusterId,
        String physicalTopicBase,
        List<PulsarPhysicalPartitionIdentity> partitions)
        implements IngressRouteResource {
    @Override public int partitionCount() { return partitions.size(); }
}

public record RoutePartitionPolicy(
        int partition,
        ActivationBarrier activationBarrier,
        QuotaGrantRef quotaGrant,
        long brokerGuardAttestationGeneration,
        Digest32 brokerGuardAttestationDigest) {
}
```

`PulsarPhysicalPartitionIdentity` 包含 physical topic、`resourceIncarnation[32]`、service-owned creation timestamp 和 partition。`RouteSnapshot` 不包含 plaintext credential、mutable secret reference 或 caller-supplied tenant ID。
...
public interface RouteSnapshotProvider {
    RouteSnapshot activeForNewSchedule(AuthenticatedTenantContext context,
                                         RouteSelectionHint hint);

    RouteSnapshot exact(RouteIncarnation incarnation,
...
`NativePreparationSnapshot` 是 immutable、已验签的本地投影，包含 exact Destination/
Capability Profile envelope、`NativeCapabilitySnapshot`、Pulsar target/partition 和 issuer
...
Destination/Capability/Profile 与 `NativeCapabilitySnapshot`，验证 issuer
Ed25519 signature，然后只暴露 immutable local views。共享
`NativePreparationEligibility` 在 Semantic Core 中再次绑定 authenticated
...
`prepareScheduleSubmission(..., SubmissionMode)` 与 Gateway 的
`SubmissionMode` 请求共用这个入口。这里的 issuer/catalog/Oxia refresh 仍是
...
4. partition = ROUTING_HASH(route, tenantRoutingScope, routingKey)
...
路由算法里。现有 `PreparedCommand.schedule(ShardId, ...)` 保留为低层/测试入口；生产
...
public interface DelaySemanticCore {
    PreparedSubmission prepareSchedule(AuthenticatedTenantContext tenant,
    ...
    PreparedSubmission prepareManaged(AuthenticatedTenantContext tenant,
...
按 `SubmissionMode` 冻结 branch。Direct SDK 与 Gateway 都只能传 `MANAGED/AUTO_FAST` enum；
...
```java
RouteSnapshot snapshot = routeProvider.activeForNewSchedule(tenant, hint);
snapshotVerifier.requireUsable(snapshot, trustedTime);
PreparedIdentity identity = identityPlanner.newSchedule(snapshot, intent);
PreparedCommand command = commandFactory.schedule(identity, intent, retryUntil);
preparedValidator.requireExact(command, snapshot);
Optional<NativePreparationSnapshot> candidate = submissionMode == SubmissionMode.AUTO_FAST
...
再把其 exact `PreparedSubmission` 交给 coordinator。Semantic Core 本身不引用
...
PreparedSubmission
  ...
  -> map to SubmissionOutcomeMessage
...
`DelayClient` 的生产 artifact 只暴露 Registry-shaped 方法。以下内容移到 conformance/legacy artifact：
...
```text
delay-gateway-api/src/main/proto/nereus/delay/gateway/delay_gateway.proto
...
message GatewayRouteSelector {
  uint32 ingress_adapter_kind = 1;       // AdapterKind
...
message GatewayScheduleRequest {
  ...
  bytes schedule_intent = 3;          // exact canonical CanonicalScheduleIntent
  int64 retry_until_epoch_ms = 4;
  uint32 submission_mode = 5;         // exact SubmissionMode
}

message GatewayPrepareLargeScheduleRequest {
  bytes idempotency_key = 1;
  GatewayRouteSelector route = 2;
  bytes schedule_intent = 3;          // forPrepare form; no payload branch
...
message GatewayIssuePayloadUploadHandleRequest {
  ...
  uint32 upload_handle_kind = 2;          // UploadHandleKind
}
...
message GatewayGetCommandResultRequest {
  oneof locator {
    bytes command_queued_receipt = 1;
...
message GatewayGetMessageRequest {
  oneof locator {
    ...
    bytes command_queued_receipt = 2;  // receipt with MessageSubject
  }
}
...
message GatewaySubmissionOutcome {
  oneof result {
    ...
    bytes preparation_error = 2;      // exact StableError, no prepared ref
  }
}
...
`StableError(stage=PREPARATION)`，且不得伪造或泄露已有 key 所绑定的 prepared/native ref。
...
`DelayMessageId` and canonical `MessagePrecondition`, while the shared
...
`OxiaGatewayAuditSink`. It stores each canonical `GatewayAuditEvent` under
...
```text
/nereus-delay/gateway-idempotency/
...
```java
public record GatewayIdempotencyRecord(
        int version,
        ...
        GatewayIdempotencyPhase phase,
        List<GatewayPhysicalAttempt> attempts,
...
public record GatewayPhysicalAttempt(
        int attemptNo,
        PhysicalEnqueueAttemptId physicalAttemptId,
        GatewayPhysicalAttemptState state,
        byte[] outcomeBytes,
...
`GatewayPhysicalAttempt` 至少保存递增 `attemptNo`、16-byte
...
的同名内部 state 分别绑定
`NativeDeliveryReceipt`/native-definite/native-uncertain。record、attempt、aggregate 始终保持
...
`SafeBrokerAck`；因此 observation 绝不能伪造 NDR1 queued receipt，也不 CAS 修改 idempotency
...
public record KafkaCommandTransportKey(
    ...
    @Override public AdapterKind kind() { return AdapterKind.KAFKA; }
}
...
public record PulsarCommandTransportKey(
        ...
        int partition,
        CredentialBindingKey credentialBinding) implements CommandTransportKey {
    @Override public AdapterKind kind() { return AdapterKind.PULSAR; }
}
...
public record ManagedRouteAuthority(RouteSnapshot historicalRoute)
        implements SubmissionRouteAuthority {
}

public record NativeTargetAuthority(NativePreparedDelivery prepared)
        implements SubmissionRouteAuthority {
}
...
把 `RouteSnapshot` 引入 `delay-transport-spi`，否则会违反 §3.2 的依赖方向。
...
authority，不使用 nullable `RouteSnapshot`。
...
实施基线：`/Users/liusinan/apps/ideaproject/nereusstream/kafka` 的 `trunk@c300006a77`。实施分支建议：`nereus/delay-guarded-producer`。
...
- transaction 的 Produce v11 上限与 guarded v13 冲突，在 API 层直接拒绝，不做事务降级。
...
`KafkaGuardedRequestEvidence` / `KafkaGuardedResponseEvidence` canonical bytes，并把 request
...
SenderTest
  ...
  requestResponseAndChildBatchDigestsMatchActual3Bytes
...
实施基线：`/Users/liusinan/apps/ideaproject/nereusstream/pulsar` 的 `5.0.0-M1@8dae0236c0`。从该基线 `checkout -b nereus/delay-resource-guard`，不基于当前无关功能分支。
...
因此 production path 使用 Pulsar 通用 first-class `TopicResourceGuard`。旧 plugin-only 方式可作为审计/兼容实验，不可满足 release gate。
...
- factory 固定 `batchingEnabled(false)`；
- factory 固定 chunking disabled，并证明 `maxCommandBytes` 小于 exact Broker max-message bound；
- factory 固定 `maxPendingMessages(1)`/nonblocking local admission；SDK 外层按 channel slots
...
`ledgerId=-1/entryId=-1`；它不足以构造 Nereus `SafeBrokerAck`，因此 guarded transport 保持
...
`PulsarGuardedRequestEvidence` / `PulsarGuardedResponseEvidence`，并分别交给
...
选择 `SSL`；两种模式都必须验证 endpoint/principal。Factory 从 `RouteSnapshot` 创建
...
presence matrix 与 Kafka 相同；Pulsar production success 另外固定
...
Worker-facing SPI, decodes NDL1/ bytes, and carries the exact authenticated
...
project `nereus-delay-oxia-e2e-1786729940-65321` on
...
`nereus-delay-oxia-e2e-1786732310-90387` on `127.0.0.1:16649`; eight
...
Pulsar 固定 batching false/one unresolved SEND，不作为可调开关。Kafka v13 minimum、auto-create false、transactional.id absent 也不是可降级配置。
...
same invalid input -> same StableError
...
- managed uncertain 后 internal query 即使得到 exact CommandHash + applied Source Position，也只
  返回独立 applied/rejected observation；缺少原 `SafeBrokerAck` 时 aggregate 仍 uncertain，
...
Route/resource value types, UUIDv7 identity seam, `ROUTING_HASH`
...
PreparedIdentityPlanner
RouteSnapshot + verifier + local provider
AutoFastSubmissionPlanner + trusted NativePreparationSnapshotProvider
...
`nereus/delay-guarded-producer` implements the generic public guarded
...
`nereus/delay-resource-guard` is implemented in the earlier writer commits
...
the exact projection from a signed Route partition `ActivationBarrier` to a
...
本地 conformance slice 已提供 `GatewayScheduleRequest`、canonical body/key
...
still controls release status; this slice cannot imply release-ready.
...
`RouteSnapshot.snapshotDigest`; `RouteWorkerAssignmentCoordinator` obtains
...
`nereus-delay-oxia-e2e-1786765353-47776` on port `16660`, and image
...
bytes or derives the Claim materialization from the accepted durable
...
The Kafka source set now treats the Shard Log as one ordered union of
Client Command and System Mutation frames. `SystemMutationIdentity` parses
...
### 2026-08-15 automatic Claim materialization implementation note

`CanonicalLaneTuple.project` is the public immutable tuple projection used by
the local Claim materializer. `DelayShard.resolveClaimMaterialization`
requires a current scheduled Message and its accepted Schedule or
...
`CommittedPayloadDescriptor` from the durable `PayloadReference` and the
...
externally-authorized `ChannelResourceIdentity` and derives the canonical
`PreparedPublishDescriptor` projection. The derivation carries the Claim
...
`ActivationBarrier.kafka`, signs a one-partition `RouteSnapshot`, publishes
...
`ActivationBarrier.pulsar`. The smoke recovers the pre-Route record into the
...
`nereus-delay-oxia-e2e-1786789198-22565`, port `16684`, and image
...
- Route activation barriers now have an exact signed-snapshot-to-source-assignment
  ...
  remain open；
- 当前 self-routing ID、tenant authority、Worker 状态机不改；
...
Delay commit `defce755` makes a Schedule/Prepare Lane typed when the first
projection contains the exact canonical Lane tuple. The persisted
`ActiveLaneState` carries the immutable Destination/Capability Profile refs,
...
`ReadyCertificate` now exposes its exact activation barrier and evidence
...
transport E2E, crash/response-loss evidence, or release PASS.
...
were persisted by typed activation, rereads each `ActiveLaneState`, and
...
`ReadyCertificate`, Channel Resource, Owner/Store/Lane identities, activation
...
`SHA-256("nereus-delay-kafka-receipt-wire-record\\0" || LP32(key) ||
LP32(value))`.
...
multi-broker equivalence, or satisfy the release gates.
...
Owner/Assignment, Object Store, checkpoint/quiescence and release gates.
...
runtime/milestone PASS, or release readiness. Oxia Route Worker authority
...
Worker/K2 evidence, not a release or milestone PASS: Claim, readiness,
...
or the §23.5 release gates. Pulsar ran with
...
Compose `nereus-delay-oxia-e2e-1786822655-96457`, and host port `16675`
...
multi-node placement or Gateway HA, live Kafka/Pulsar publication, or
release readiness.
...
load, multi-shard placement or the release gates.
...
`GatewayIdempotencyRecord.withAttempt` now drops the prior uncertain
...
response-loss, LSO/retention-floor, or release evidence.
...
placement, checkpoint REAPING or release activation.
...
crash recovery, multi-Broker failover, Attempt Journal recovery or release
readiness.
...
Download requires the full exact `ProfileRef`, bucket and canonical manifest
...
credential failover, or release readiness.
...
`ProfileRef` as the gate; immediately before `upload` or `download` starts
provider I/O, the adapter rechecks the exact `CredentialBinding`,
`CredentialBindingProtection` and `CredentialUseLease` projection, the
...
immutable credential generations, the current `CredentialBindingHead` and
matching `CredentialBindingProtection` values are wrapped in a versioned
record with a final digest. Generation-1 publication and
`RotateEquivalentSecretRequest` compare the current record version and Head
...
the same record CAS. The returned `CredentialUseLease` carries the resulting
...
`nereus-delay-oxia-e2e-1786835835-39861`, port `16693`). This remains a
...
rereads and validates its `CredentialBindingProtection`, then atomically
...
catalog-bound `CheckpointResource.immutableVersion()` as canonical UTF-8,
...
The mandatory Object Store Profile bit
...
fallback for a profile that could permit it, while Profile construction
...
pending intent digest, exact `OwnerIdentity`, source Store Incarnation and
...
`RESOURCE_DELETE_CONFIRMED` parser rejects a confirmation interval whose
...
`RecoveryPin` is encoded at a sibling recovery-pin key with Oxia's
...
validates key, value size, canonical `RecoveryPin` bytes, non-null Oxia
...
canonical `/publication` record and places `RecoveryPin` at a sibling
...
`GatewayPhysicalAttempt` couples evidence presence to the lifecycle state;
`GatewayIdempotencyRecord` additionally checks source order, unique physical
...
`SHA-256("nereus-delay-gateway-retry-request\0" || gatewayKeyHash ||
...
`GatewayAuditEvent` phase/digest union fail closed. `COMPLETED` events must
...
`GatewayIdempotencyRecord` ACTIVE projection match the Registry lifecycle
...
`GatewayPhysicalAttempt` construction and strict decode with the Registry:
...
attempt after a persisted `QUEUED` attempt. The queued aggregate is sticky:
...
checkpoint publication and release gates remain open.
...
restore activation, multi-shard placement, raw chaos or release gates.
...
publication, the broader chaos matrix and release gates remain open.
...
apply/publish crash, production multi-shard runtime or release gate is
...
multi-shard production or release readiness. Exact Compose resource and
...
controller/coordinator leader failover, production multi-shard runtime or
...
failover, production multi-shard runtime or release readiness. Exact cleanup
...
behavior, multi-shard scheduling, the full chaos matrix and the release gate
...
`PulsarSourceReactivation` transition binds the route snapshot digest, the
...
of `RouteSnapshotCompatibility` or `SourceAssignment.sameIdentity`, and it
...
signed `RouteSnapshot` containing two partition-specific guarded Fetch/LSO
...
churn, raw chaos completeness or release approval.
...
one exact `PulsarActivationBarrier` to the single signed `RouteSnapshot`.
...
scheduler/placement churn, raw chaos completeness or release approval.
...
the cut, multi-shard chaos or the release gate.
...
fault coverage or release readiness.
...
controller/coordinator failover, production multi-shard chaos or release
readiness.
...
publication, chaos and release gates remain separate.
...
checkpoint publication and release readiness remain unclaimed.
...
multi-shard production placement, the full chaos matrix or release gates.
...
Complete evidence must freeze the candidate implementation and normative
...
The stable release command `e2e/run-release-gate.sh` now uses the adjacent
full- validator. It requires a separately frozen four-repository candidate
lock and ten exact-source `PASS_CERTIFIED` inputs with the common
`nereus-delay-full-gate-input` contract. `scope=full`,
`complete=true`, empty exclusions/boundaries, and complete per-gate coverage
...
the complete chaos matrix or release gates.
...
rotation/quiescence, full chaos, soak and release gates remain separate.
...
quiescence, chaos and release gates remain separate.
...
source-locked release artifacts must be completed before any -ready claim.
...
load, full crash/response-loss resolution or release evidence. The locked
...
placement, full chaos or release readiness. Exact project cleanup removed
...
process/Broker crash, multi-Broker, multi-shard, REAPING or release
...
`PulsarSourceReactivation`; it closes the equal-generation candidate and
...
BookKeeper/storage failover, multi-shard production placement, full chaos and
 release gates remain separate obligations.
...
The large-payload fixture keeps `PulsarSourceReactivation` strict: equal
...
full crash/chaos matrix or release readiness. Exact post-run checks found no
...
remaining §23.3 matrix, multi-shard production, chaos or release proof.
...
Pulsar multi-Broker failover, multi-shard placement, crash/chaos and
...
failover, multi-shard placement, the complete chaos matrix or release proof.
...
`InitialRouteControlActivatePayload` into `DelayShard`. The apply path uses
the existing `CompatibleControlSnapshot` projection and `meta/FIXED` key 10;
...
external Oxia/Worker rollout authority; those remain explicit Gate 8 and
...
This is a platform-observation receipt, not a capacity or release PASS. It
...
coordinator/storage failover, the complete chaos matrix, benchmark/soak and
...
target isolation, full chaos or release readiness. The Gateway runner now
...
This source-locked PASS advances the crash/response-loss/recovery evidence boundary across Kafka, Pulsar, Oxia/MinIO Checkpoint REAPING and Gateway session expiry. It does not certify catalog-driven placement, target isolation, controller/coordinator/storage/provider failover, all large-payload fault cuts, benchmark/soak, activation/cutover or release readiness. Matrix cleanup removed its exact project containers, networks, volumes, listeners and generated images; locked Oxia/MinIO images remain.
...
This receipt proves bounded local Store writes, payload readback, durable SLO merge, platform resource observation and persistent reopen across smoke/burst/sustained cases. It does not establish the required Broker/Lane/shard/compaction/restore/inline-object benchmark campaign, multi-Worker fairness, long-cycle soak or release readiness; those gates remain separate.
...
egress, placement churn/failover, benchmark/soak and release certification
...
canonical `ProtocolActivationState` at `meta/FIXED` key 14 / ValueEnvelope
...
Managed Command remains the compatibility baseline; a different tuple is
...
release gate; allowing the script to emit `NOT_READY` does not promote .
...
required; `PASS_BOUNDED` cannot satisfy the `PASS_CERTIFIED` gate.
...
controller/storage/provider failover, benchmark/soak and certification are
...
rerun is `/tmp/nereus-delay-release-gate-20260817-r5/release-candidate-gate.json`
...
long-cycle soak or release certification. The runner removed its exact
...
failover, certified soak or release readiness.
...
This is bounded current-source evidence, not a `PASS_CERTIFIED` chaos or
...
placement churn, disaster continuity, certified soak or the release gate.
...
policy” from release promotion. The release gate requires this schema,
...
`NOT_COVERED`. Full §23.3 and `PASS_CERTIFIED` release evidence remain open.
...
inline/object, upgrade/downgrade or longest-cycle soak evidence. The gate
...
§23.3 matrix or release gate is closed.
...
This is the complete bounded fault-coverage receipt for the focused
production authority paths. It does not promote the certified chaos wrapper,
capacity/soak, activation/cutover, operations or release gate; those
...
transport response-loss, multi-shard placement or release readiness.
...
placement or release readiness.
...
This is a gate-specific receipt, not a complete release result.
...
Same-adapter success must not be promoted to a complete release PASS.
...
complete release. Activation/cutover, full 19-cell chaos, capacity, soak,
...
is `NOT_READY`; this is the correct boundary until the remaining full-gate
inputs are independently produced.

## 2026-08-21 source-locked full contract runner

The Delay worktree now contains \`e2e/run-full-contract-gate.sh\`, which
turns the existing protocol/restore/DLQ/resource tests into independently
auditable full input receipts. The runner binds Delay, K1, P1 and Oxia
...
\`e2e/run-full-patch-distribution-gate.sh\`. It keeps K1 and P1 source
...
The full capacity seam is explicit in
`e2e/run-full-capacity-envelope-gate.sh`. It source-locks Delay, K1, P1 and
...
source-matching `nereus-delay-capacity-observation` file whose declared
...
`e2e/run-physical-capacity-matrix.sh` now provide the concrete §23.4
...
revisions recorded in `/private/tmp/nereus-delay-candidate-6209d824.json`.
...
The current candidate is Delay
`6f9ab51c392ea47dba46e0d6d67ff7f7d0aa0312`, K1
`05849884ca81fad767fda058444d1e17c7f9cbf9`, P1
`0a2536484cd3932801a98dc88ff112b2df88a1c7` and Oxia
`37a17bef17202d5fd6e23282da5fd26d94865484`. The candidate release receipt
`/private/tmp/nereus-delay-release-candidate-6f9ab51c/release-candidate-gate.json`
is `release_status=PASS`; all ten exact-source full- gates and Gradle `check`
passed.
...
The independent physical evidence includes
...
plus the benchmark and capacity observation receipts recorded in the release
ledger. The post-overlay final receipt is fixed at
`/private/tmp/nereus-delay-release-final-6f9ab51c/release-candidate-gate.json`.
The overlay changes documentation only; it is not a merge, deployment or
promotion claim for any target `main`.

Exact generated Docker cleanup completed without a global prune. Pinned Oxia,
MinIO and the exact benchmark `eclipse-temurin@sha256:57865c22b954cf920cb05a610af81d577e89783282514ba071e99c7357f6c769`
images were retained and generated Nereus resources were absent. Eighty-eight
unreferenced temporary directories were moved recoverably to
`/Users/liusinan/.Trash/nereus-delay-cleanup-20260822-full`; no `.git`, source
checkout or code directory was moved.
